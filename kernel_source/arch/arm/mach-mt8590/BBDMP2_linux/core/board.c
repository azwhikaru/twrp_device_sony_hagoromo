...
/* Include GPIO port number macros. */
#include "../power/bq24262_config.h"
...
extern U32 pmic_read_interface (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT);
					...
					/*     m      u */
#define	POWER_KEY_RELEASE_WAIT_NSEC	(3500ULL * 1000 * 1000)
...
void mt_power_off(void)
{
	...
	int reg_val=0;
#endif
	unsigned long long	clock_t0;
	unsigned long long	clock_t1;
	unsigned long long	clock_dt;
	unsigned long		push_cntr;
...
#endif
	/* Turn SYS_WAK_STAT=L, then CMIC power key output turns low (pushing) 2 seconds.  */
	mt_set_gpio_out(BQ24262_WMPORT_SYS_WAK_STAT_GPIO, BQ24262_WMPORT_SYS_WAK_STAT_BOOTING);
	clock_t0 = local_clock();
	mdelay(1000);
	clock_t1  = local_clock();
	clock_dt = clock_t1 - clock_t0;
	push_cntr = 0;
	while (clock_dt < POWER_KEY_RELEASE_WAIT_NSEC) {
		U32	key_ret;
		U32	key_val;
		/* implement same function of pmic_detect_powerkey(). */
		key_val = 0;
		key_ret = pmic_read_interface((U32)(CHRSTATUS),
			(&key_val),
	...
	printk("%s: CMIC power down sequence done. clock_dt=%llu, push_cntr=%lu\n", __func__, clock_dt, push_cntr);
	/* pull PWRBB low */
	rtc_bbpu_power_down();
...
struct msdc_hw msdc1_hw = {
    ...
    .clk_drv_sd_18_hm	= 4,		   /* sdr104 mode HM*/
    .cmd_drv_sd_18_hm	= 4,
    .dat_drv_sd_18_hm	= 3,
    .clk_drv_sd_18_fs	= 4,		   /* sdr104 mode FS*/
    .cmd_drv_sd_18_fs	= 4,
    .dat_drv_sd_18_fs	= 4,
...
struct msdc_hw msdc2_hw = {
	...
	struct msdc_hw msdc2_hw = {	    
	    ...
	    .dat0rddly		= 0,
			...
			.cmdrddly		= 0,
#ifdef CONFIG_SDIOAUTOK_SUPPORT
		.cmdrtactr_sdr50= 1,   // command response turn around counter, sdr 50 mode
		.wdatcrctactr_sdr50= 1,	 // write data crc turn around counter, sdr 50 mode
		.intdatlatcksel_sdr50= 0,   // internal data latch CK select, sdr 50 mode
		.cmdrtactr_sdr200= 3,   // command response turn around counter, sdr 200 mode
		.wdatcrctactr_sdr200= 2,   // write data crc turn around counter, sdr 200 mode
		.intdatlatcksel_sdr200= 0,	// internal data latch CK select, sdr 200 mode
#endif			
	    .host_function	= MSDC_SDIO,
	    .boot			= 0,
	    .request_sdio_eirq = mtk_wcn_cmb_sdio_request_eirq,
...
struct msdc_hw msdc2_hw = {
	...
	struct msdc_hw msdc3_hw = {
	    ...
	    .cmd_drv        = 2,
	    .dat_drv        = 2,
	    .clk_drv_sd_18		= 3,		   /* HS200 mode */
	    .cmd_drv_sd_18		= 4,
	    .dat_drv_sd_18		= 3,
#ifdef CONFIG_ARCH_MT8590_ICX
	    .clk_drv_sd_18_hm	= 3,		   /* HS200 mode HM*/
	    .cmd_drv_sd_18_hm	= 3,
	    .dat_drv_sd_18_hm	= 2,
	    .clk_drv_sd_18_fs	= 3,		   /* HS200 mode FS*/
	    .cmd_drv_sd_18_fs	= 3,
	    .dat_drv_sd_18_fs	= 3,
#endif /*CONFIG_ARCH_MT8590_ICX*/
        //.rst_drv        = 6,
