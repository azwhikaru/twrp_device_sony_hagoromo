...
static int tpd_registration(struct i2c_client *client)
{
	...
	TPD_DEBUG("##mt_get_gpio_pull_enable(0x%x)=%d\n", GPIO_CTP_EINT_PIN,
...
/* interrupt registration */
#ifndef MT6589
	TPD_DEBUG("Before register mt_eint_print_status\n");
	...
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	mt_eint_dump_status(CUST_EINT_TOUCH_PANEL_NUM);
#endif

	TPD_DEBUG("-------------------------------  \n");
	mt_eint_print_status();
	TPD_DEBUG("After register mt_eint_print_status  \n");
#else
...
static int tpd_event_handler(void *unused)
{
	...
	do {
		...
		GTP_DEBUG("tpd_event_handler start read coordinates ");
