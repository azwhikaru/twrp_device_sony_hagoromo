...
struct cg_clk {
	...
	/* used for some special clk cg
	 * for example, ethernet related cg
	 */
	struct pll *pll;
#if CLKMGR_8127
	const char *name;
...
static struct pll_ops hadds2_pll_ops;
...
static struct pll plls[NR_PLLS] = {
	...
	[TRGPLL] = {
		.name = __stringify(TRGPLL),
		.type = PLL_TYPE_SDM,
		.feat = 0x0,
		.en_mask = 0x00000001,
		.base_addr = TRGPLL_CON0,
		.pwr_addr = TRGPLL_PWR_CON0,
		.ops = &sdm_pll_ops,
	},
	[ETHPLL] = {
		.name = __stringify(ETHPLL),
		.type = PLL_TYPE_SDM,
		.feat = 0x0,
		.en_mask = 0x00000001,
		.base_addr = ETHPLL_CON0,
		.pwr_addr = ETHPLL_PWR_CON0,
		.ops = &sdm_pll_ops,
	},
	[VDECPLL] = {
	...
	[HADDS2PLL] = {
		.name = __stringify(HADDS2PLL),
		.type = PLL_TYPE_SDM,
		.feat = 0x0,
		.en_mask = 0x00000001,
		.base_addr = HADDS2PLL_CON0,
		.pwr_addr = HADDS2PLL_PWR_CON0,
		.ops = &hadds2_pll_ops,
	},
	[AUD2PLL] = {
	...
	[TVD2PLL] = {
		.name = __stringify(TVD2PLL),
		.type = PLL_TYPE_SDM,
		.feat = 0x0,
		.en_mask = 0x00000001,
		.base_addr = TVD2PLL_CON0,
		.pwr_addr = TVD2PLL_PWR_CON0,
		.ops = &sdm_pll_ops,
	},
};
...
static void sdm_pll_fsel_op(struct pll *pll, unsigned int value)
{
	...
	if (pll->base_addr == UNIVPLL_CON0) {
		...
		ctrl_value |= value & UNIV_SDM_PLL_N_INFO_MASK;
				...
				(pll->base_addr == TRGPLL_CON0) ||
				(pll->base_addr == ETHPLL_CON0) ||
				(pll->base_addr == VDECPLL_CON0) ||
				(pll->base_addr == HADDS2PLL_CON0) ||
...
static unsigned int sdm_pll_get_postdiv_op(struct pll *pll)
{
	...
	if (pll->base_addr == ARMPLL_CON0) {
		con0 = clk_readl(pll->base_addr + 4);
		posdiv = (con0 & BITMASK(26 : 24)) >> 24;
	} else {
		con0 = clk_readl(pll->base_addr);
		posdiv = (con0 & BITMASK(6 : 4)) >> 4;
	}
...
static void sdm_pll_set_postdiv_op(struct pll *pll, unsigned int postdiv)
{
	...
	unsigned int con0_value = clk_readl(con0_addr);

	if (pll->base_addr == ARMPLL_CON0)
		clk_writel(con0_addr + 4, (con0_value & ~BITMASK(26 : 24)) | BITS(26 : 24, postdiv));
	else
		clk_writel(con0_addr, (con0_value & ~BITMASK(6 : 4)) | BITS(6 : 4, postdiv));
}
...
static unsigned int sdm_pll_get_pcw_op(struct pll *pll)
{
	...
	reg_value = clk_readl(con1_addr);               /* xPLL_CON1 */
...
static void sdm_pll_set_pcw_op(struct pll *pll, unsigned int pcw)
{
	...
	pll_en = clk_readl(pll->base_addr) & BIT(0);    /* xPLL_CON0[0] */

	reg_value = clk_readl(con1_addr);
	reg_value &= ~SDM_PLL_N_INFO_MASK;
	reg_value |= pcw & SDM_PLL_N_INFO_MASK;         /* xPLL_CON1[20:0] = pcw */

	if (pll_en)
		reg_value |= SDM_PLL_N_INFO_CHG;            /* xPLL_CON1[31] = 1 */
...
static void hadds2_pll_enable_op(struct pll *pll)
{
	pr_err("enable hadds2pll.\n");

	/* not used currently, return directly */
#if 1
	return;
#else
	DRV_WriteReg32(0xF0005170, 0x0000f81f);

	DRV_WriteReg32(0xF000623c, 0x0000ff16);
	DRV_WriteReg32(0xF000623c, 0x0000ff1e);
	DRV_WriteReg32(0xF000623c, 0x0000ff0e);
	DRV_WriteReg32(0xF000623c, 0x0000ff0c);
	DRV_WriteReg32(0xF000623c, 0x0000fe0c);
	DRV_WriteReg32(0xF000623c, 0x0000fc0c);
	DRV_WriteReg32(0xF000623c, 0x0000f80c);
	DRV_WriteReg32(0xF000623c, 0x0000f00c);
	DRV_WriteReg32(0xF000623c, 0x0000f00d);
	
	DRV_WriteReg32(0xF000629c, 0x0000ff16);
	DRV_WriteReg32(0xF000629c, 0x0000ff1e);
	DRV_WriteReg32(0xF000629c, 0x0000ff0e);
	DRV_WriteReg32(0xF000629c, 0x0000ff0c);
	DRV_WriteReg32(0xF000629c, 0x0000fe0c);
	DRV_WriteReg32(0xF000629c, 0x0000fc0c);
	DRV_WriteReg32(0xF000629c, 0x0000f80c);
	DRV_WriteReg32(0xF000629c, 0x0000f00c);
	DRV_WriteReg32(0xF000629c, 0x0000f00d);

	DRV_WriteReg32(0xFc000100, 0x00000000);
	DRV_WriteReg32(0xFc000110, 0x00000000);
	DRV_WriteReg32(0xFc000104, 0x00000000);
	DRV_WriteReg32(0xFc00001c, 0xfb5aab4f);
	DRV_WriteReg32(0xFc000020, 0x00000000);
	DRV_WriteReg32(0xFc000024, 0x00000001);
#endif
}

static void hadds2_pll_disable_op(struct pll *pll)
{
	unsigned int reg_val = 0;

	pr_err("disable hadds2pll.\n");
	reg_val = clk_readl(HADDS2PLL_CON0); 	
	reg_val &= ~(0x1<<0);
	clk_writel(HADDS2PLL_CON0, reg_val); 	

	reg_val = clk_readl(HADDS2PLL_PWR_CON0); 	
	reg_val |= (0x1<<1);
	clk_writel(HADDS2PLL_PWR_CON0, reg_val); 	
...
static struct pll_ops hadds2_pll_ops = {
	.get_state      = pll_get_state_op,
	.enable         = hadds2_pll_enable_op,
	.disable        = hadds2_pll_disable_op,
	.fsel = sdm_pll_fsel_op,
	.dump_regs = sdm_pll_dump_regs_op,
	.vco_calc = sdm_pll_vco_calc_op,
	.hp_enable = sdm_pll_hp_enable_op,
...
#ifdef SYS_LOG
	if (sys->cnt_en)
		clk_info("[%s]: Start. sys->name=%s, sys->cnt=%d, sys->state=%d\n",
					__func__, sys->name, sys->cnt, sys->state);
	else
...
#ifdef SYS_LOG
	if (sys->cnt_en)
		clk_info("[%s]: End. sys->name=%s, sys->cnt=%d, sys->state=%d\n",
					__func__, sys->name, sys->cnt, sys->state);
	else
...
#ifdef SYS_LOG
	if (sys->cnt_en)
		clk_info("[%s]: Start. sys->name=%s, sys->cnt=%d, sys->state=%d, force_off=%d\n",
			__func__, sys->name, sys->cnt, sys->state, force_off);
	else
		clk_info("[%s]: Start. sys->name=%s, sys->state=%d, force_off=%d\n",
			__func__, sys->name, sys->state, force_off);
#endif

	if (sys->cnt_en && force_off) {
		pr_warn("[%s]: force off:%s!! cnt will be clear!\n", __func__, sys->name);
		sys->cnt = 1;
	}
...
#ifdef SYS_LOG
	clk_info("[%s]: End. sys->name=%s, sys->cnt+%d, sys->state=%d, force_off=%d\n",
		__func__, sys->name, sys->cnt, sys->state, force_off);
#endif
	return err;
}
...
unsigned int subsys_get_cnt(int id)
{
	struct subsys *sys = id_to_sys(id);
...
unsigned int subsys_get_cnt_en(int id)
{
	struct subsys *sys = id_to_sys(id);
...
static struct clkmux muxs[NR_MUXS] = {
	...
	[MT_MUX_EMMC_HCLK] = {
		.name = __stringify(MT_MUX_EMMC_HCLK),
		.base_addr = CLK_CFG_6,
		.sel_mask = 0x03000000,
		.pdn_mask = 0x80000000,
		.offset = 24,
	...
	[MT_MUX_INTDIR] = {
		.name = __stringify(MT_MUX_INTDIR),
		.base_addr = CLK_CFG_12,
		.sel_mask = 0x00000300,
		.pdn_mask = 0x00008000,
		.offset = 8,
	...
	[MT_MUX_HDMIRX_BIST] = {
		.name = __stringify(MT_MUX_HDMIRX_BIST),
		.base_addr = CLK_CFG_12,
		.sel_mask = 0x00000007,
		.pdn_mask = 0x00000080,
		.offset = 0,
	...
	[MT_MUX_MS_CARD] = {
		.name = __stringify(MT_MUX_MS_CARD),
		.base_addr = CLK_CFG_13,
		.sel_mask = 0x00030000,
		.pdn_mask = 0x00800000,
		.offset = 16,
	...
	/* not used */
	[MT_MUX_HOST_SPI] = {
		.name = __stringify(MT_MUX_HOST_SPI),
		.base_addr = CLK_CFG_13,
		.sel_mask = 0x00000300,
		.pdn_mask = 0x00008000,
		.offset = 8,
	...
	[MT_MUX_SPI1] = {
		.name = __stringify(MT_MUX_SPI1),
		.base_addr = CLK_CFG_14,
		.sel_mask = 0x07000000,
		.pdn_mask = 0x80000000,
		.offset = 24,
		.nr_inputs = 8,
	...
	[MT_MUX_MSDC30_3] = {
		.name = __stringify(MT_MUX_MSDC30_3),
		.base_addr = CLK_CFG_14,
		.sel_mask = 0x00000700,
		.pdn_mask = 0x00008000,
		.offset = 8,
	...
	[MT_MUX_HDMIRX_26M_24M] = {
		.name = __stringify(MT_MUX_HDMIRX_26M_24M),
		.base_addr = CLK_CFG_14,
		.sel_mask = 0x00000001,
		.pdn_mask = 0x00000080,
		.offset = 0,
...
#if defined(CONFIG_CLKMGR_BRINGUP) || defined(CONFIG_MT8590_FPGA)
	return 0;
#endif
...
/* CG_AUDIO0, CG_AUDIO4, CG_AUDIO5,
 * CG_TOP_PWR2, CG_PWR3, CG_PWR4, CG_ETH,
 * CG_HIF, CG_CLKCFG1 is not checcked
 * because of no any information
 */
static struct cg_grp grps[NR_GRPS] = {
	[CG_PERI0] = {
	...
	[CG_BDP0] = {
		.name = __stringify(CG_BDP0),
		.set_addr = BDP_DISPSYS_CG_CLR0,
		.clr_addr = BDP_DISPSYS_CG_SET0,
		.sta_addr = BDP_DISPSYS_CG_CON0,
		.ops = &general_cg_grp_ops,
		.sys = &syss[SYS_BDP],
	},
	[CG_BDP1] = {
		.name = __stringify(CG_BDP1),
		.set_addr = BDP_DISPSYS_CG_CLR1,
		.clr_addr = BDP_DISPSYS_CG_SET1,
		.sta_addr = BDP_DISPSYS_CG_CON1,
		.ops = &general_cg_grp_ops,
		.sys = &syss[SYS_BDP],
	},
	[CG_AUDIO] = {
	...
	[CG_HIFSYS_CLKCFG1] = {
		.name = __stringify(CG_HIFSYS_CLKCFG1),
		.sta_addr = HIFSYS_SYSCTL_CLKCFG1,
		.ops = &general_cg_grp_ops,
	},
	[CG_ETHDMASYS_CLKCFG1] = {
		.name = __stringify(CG_ETHDMASYS_CLKCFG1),
		.sta_addr = ETHDMASYS_SYSCTL_CLKCFG1,
		.ops = &general_cg_grp_ops,
	},
};
...
static struct cg_clk_ops bdpsys_cg_clk_ops;
static struct cg_clk_ops audsys_cg_clk_ops; /* @audio sys */
static struct cg_clk_ops ethifsys_cg_clk_ops;
...
static struct cg_clk clks[NR_CLKS] = {
	...
	[MT_CG_PERI_MSDC30_3] = {
		.name = "MT_CG_PERI_MSDC30_3",
		.grp = &grps[CG_PERI0],
		.mask = BIT(16),
		.ops = &general_cg_clk_ops,
		.mux = &muxs[MT_MUX_MSDC30_3],
	},
	[MT_CG_PERI_MSDC30_4] = {
		.name = "MT_CG_PERI_MSDC30_4",
		.grp = &grps[CG_PERI0],
		.mask = BIT(17),
		.ops = &general_cg_clk_ops,
		.mux = &muxs[MT_MUX_EMMC_HCLK],
	},
	[MT_CG_PERI_NLI] = {
		.name = "MT_CG_PERI_NLI",
		.grp = &grps[CG_PERI0],
	...
	[MT_CG_PERI_FLASH] = {
		.name = "MT_CG_PERI_FLASH",
		.grp = &grps[CG_PERI1],
		.mask = BIT(5),
		.ops = &general_cg_clk_ops,
	},
	[MT_CG_PERI_HOST89_INT] = {
		.name = "MT_CG_PERI_HOST89_INT",
		.grp = &grps[CG_PERI1],
		.mask = BIT(6),
		.ops = &general_cg_clk_ops,
	},
	[MT_CG_PERI_HOST89_SPI] = {
		.name = "MT_CG_PERI_HOST89_SPI",
		.grp = &grps[CG_PERI1],
		.mask = BIT(7),
		.ops = &general_cg_clk_ops,
	},
	[MT_CG_PERI_HOST89_DVD] = {
		.name = "MT_CG_PERI_HOST89_DVD",
		.grp = &grps[CG_PERI1],
		.mask = BIT(8),
		.ops = &general_cg_clk_ops,
	},
	[MT_CG_PERI_SPI1] = {
		.name = "MT_CG_PERI_SPI1",
		.grp = &grps[CG_PERI1],
		.mask = BIT(9),
		.ops = &general_cg_clk_ops,
		.mux = &muxs[MT_MUX_SPI1],
	},
	[MT_CG_PERI_SPI2] = {
		.name = "MT_CG_PERI_SPI2",
		.grp = &grps[CG_PERI1],
		.mask = BIT(10),
		.ops = &general_cg_clk_ops,
		.mux = &muxs[MT_MUX_SPI2],
	},
	[MT_CG_PERI_FCI] = {
		.name = "MT_CG_PERI_FCI",
		.grp = &grps[CG_PERI1],
    ...
    [MT_CG_INFRA_QAXI_CM4] = {
		.name = "MT_CG_INFRA_QAXI_CM4",
		.grp = &grps[CG_INFRA],
		.mask = BIT(2),
		.ops = &general_cg_clk_ops,
	},
    [MT_CG_INFRA_AUD_SPLIN_BCK] = {
		.name = "MT_CG_INFRA_AUD_SPLIN_BCK",
		.grp = &grps[CG_INFRA],
		.mask = BIT(4),
		.ops = &general_cg_clk_ops,
	},
    ...
    [MT_CG_INFRA_RAMBUFIF] = {
		.name = "MT_CG_INFRA_RAMBUFIF",
		.grp = &grps[CG_INFRA],
		.mask = BIT(14),
		.ops = &general_cg_clk_ops,
	},
	...
	[MT_CG_DISP1_TVE_FMM_CLOCK] = {
		.name = "MT_CG_DISP1_TVE_FMM_CLOCK",
		.grp = &grps[CG_DISP1],
		.mask = BIT(14),
		.ops = &general_cg_clk_ops,
	},
	...
	[MT_CG_AUDIO_MULTI_IN] = {
		.name = "MT_CG_AUDIO_MULTI_IN",
		.grp = &grps[CG_AUDIO4],
		.mask = BIT(19),
		.ops = &audsys_cg_clk_ops,
	},
	...
	[MT_CG_BDP_BRIDGE_BACLK] = {
		.name = "MT_CG_BDP_BRIDGE_BACLK",
		.grp = &grps[CG_BDP0],
		.mask = BIT(0),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_BRIDGE_DRAM] = {
		.name = "MT_CG_BRIDGE_DRAM",
		.grp = &grps[CG_BDP0],
		.mask = BIT(1),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_LARB_DRAM] = {
		.name = "MT_CG_LARB_DRAM",
		.grp = &grps[CG_BDP0],
		.mask = BIT(2),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_WR_CHANNEL_VDI_PXL] = {
		.name = "MT_CG_WR_CHANNEL_VDI_PXL",
		.grp = &grps[CG_BDP0],
		.mask = BIT(3),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_WR_CHANNEL_VDI_DRAM] = {
		.name = "MT_CG_WR_CHANNEL_VDI_DRAM",
		.grp = &grps[CG_BDP0],
		.mask = BIT(4),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_WR_CHANNEL_BCLK] = {
		.name = "MT_CG_WR_CHANNEL_BCLK",
		.grp = &grps[CG_BDP0],
		.mask = BIT(5),
		.ops = &bdpsys_cg_clk_ops,
	},
	...
	[MT_CG_FMT_MAST_27_CK] = {
		.name = "MT_CG_FMT_MAST_27_CK",
		.grp = &grps[CG_BDP0],
		.mask = BIT(8),
		.ops = &bdpsys_cg_clk_ops,
	},
	...
	[MT_CG_DISPFMT_27M] = {
		.name = "MT_CG_DISPFMT_27M",
		.grp = &grps[CG_BDP0],
		.mask = BIT(17),
		.ops = &bdpsys_cg_clk_ops,
		.mux = &muxs[MT_MUX_DI],
	},
	[MT_CG_DISPFMT_27M_VDOUT] = {
		.name = "MT_CG_DISPFMT_27M_VDOUT",
		.grp = &grps[CG_BDP0],
		.mask = BIT(18),
		.ops = &bdpsys_cg_clk_ops,
		.mux = &muxs[MT_MUX_DI],
	},
	[MT_CG_DISPFMT_27_74_74] = {
		.name = "MT_CG_DISPFMT_27_74_74",
		.grp = &grps[CG_BDP0],
		.mask = BIT(19),
		.ops = &bdpsys_cg_clk_ops,
		.mux = &muxs[MT_MUX_DI],
	...
	[MT_CG_DISPFMT_2FS] = {
		.name = "MT_CG_DISPFMT_2FS",
		.grp = &grps[CG_BDP0],
		.mask = BIT(20),
		.ops = &bdpsys_cg_clk_ops,
		.mux = &muxs[MT_MUX_DI],
	},
	[MT_CG_DISPFMT_2FS_2FS74_148] = {
		.name = "MT_CG_DISPFMT_2FS_2FS74_148",
		.grp = &grps[CG_BDP0],
		.mask = BIT(21),
		.ops = &bdpsys_cg_clk_ops,
		.mux = &muxs[MT_MUX_DI],
	...
	[MT_CG_WR_CHANNEL_DI_PXL] = {
		.name = "MT_CG_WR_CHANNEL_DI_PXL",
		.grp = &grps[CG_BDP0],
		.mask = BIT(26),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_WR_CHANNEL_DI_DRAM] = {
		.name = "MT_CG_WR_CHANNEL_DI_DRAM",
		.grp = &grps[CG_BDP0],
		.mask = BIT(27),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_WR_CHANNEL_DI_BCLK] = {
		.name = "MT_CG_WR_CHANNEL_DI_BCLK",
		.grp = &grps[CG_BDP0],
		.mask = BIT(28),
		.ops = &bdpsys_cg_clk_ops,
	},
	...
	[MT_CG_RX_DDCSCL_N] = {
		.name = "MT_CG_RX_DDCSCL_N",
		.grp = &grps[CG_BDP1],
		.mask = BIT(5),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_RX_DDCSCL] = {
		.name = "MT_CG_RX_DDCSCL",
		.grp = &grps[CG_BDP1],
		.mask = BIT(6),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_RX_VCOCLK] = {
		.name = "MT_CG_RX_VCOCLK",
		.grp = &grps[CG_BDP1],
		.mask = BIT(7),
		.ops = &bdpsys_cg_clk_ops,
	},
	...
	[MT_CG_RX_PLLCLK] = {
		.name = "MT_CG_RX_PLLCLK",
		.grp = &grps[CG_BDP1],
		.mask = BIT(11),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_BRIDGE_RT_BCLK] = {
		.name = "MT_CG_BRIDGE_RT_BCLK",
		.grp = &grps[CG_BDP1],
		.mask = BIT(12),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_BRIDGE_RT_DRAM] = {
		.name = "MT_CG_BRIDGE_RT_DRAM",
		.grp = &grps[CG_BDP1],
		.mask = BIT(13),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_LARB_RT_DRAM] = {
		.name = "MT_CG_LARB_RT_DRAM",
		.grp = &grps[CG_BDP1],
		.mask = BIT(14),
		.ops = &bdpsys_cg_clk_ops,
	},
	[MT_CG_TMDS_SYN_CLK] = {
		.name = "MT_CG_TMDS_SYN_CLK",
		.grp = &grps[CG_BDP1],
		.mask = BIT(15),
		.ops = &bdpsys_cg_clk_ops,
	},
	...
	[MT_CG_USB0_HOST_PHY_CLK] = {
		.name = "MT_CG_USB0_HOST_PHY_CLK",
		.grp = &grps[CG_HIFSYS_CLKCFG1],
		.mask = BIT(21),
		.ops = &ethifsys_cg_clk_ops,
		.pll = &plls[ETHPLL],
	},
	[MT_CG_USB1_HOST_PHY_CLK] = {
		.name = "MT_CG_USB1_HOST_PHY_CLK",
		.grp = &grps[CG_HIFSYS_CLKCFG1],
		.mask = BIT(22),
		.ops = &ethifsys_cg_clk_ops,
		.pll = &plls[ETHPLL],
	...
	[MT_CG_HSDMA_CLK] = {
		.name = "MT_CG_HSDMA_CLK",
		.grp = &grps[CG_ETHDMASYS_CLKCFG1],
		.mask = BIT(5),
		.ops = &ethifsys_cg_clk_ops,
		//.mux = &muxs[MT_MUX_ETHIF],
	},
...
/*
 * bdp's clk gate is special:
 * every set of registers consist of three registers:
 * CONx - status and set register(1 - disable, 0 - enable)
 * SETx - set register(1 - enable)
 * CLRx - set register(1 - disable)
 */
static int bdpsys_clk_enable_op(struct cg_clk *clk)
{
#ifdef CLK_LOG
	clk_info("[%s]: clk->grp->name=%s, clk->mask=0x%x\n", __func__, clk->grp->name, clk->mask);
#endif
...
static int bdpsys_clk_disable_op(struct cg_clk *clk)
{
	unsigned int sta_val = 0;
#ifdef CLK_LOG
	clk_info("[%s]: clk->grp->name=%s, clk->mask=0x%x\n", __func__, clk->grp->name, clk->mask);
#endif
...
static struct cg_clk_ops bdpsys_cg_clk_ops = {
	.get_state = general_clk_get_state_op,
	.check_validity = general_clk_check_validity_op,
	.enable = bdpsys_clk_enable_op,
	.disable = bdpsys_clk_disable_op,
};
...
static int ethifsys_clk_enable_op(struct cg_clk *clk)
{
#ifdef CLK_LOG
	clk_info("[%s]: clk->grp->name=%s, clk->mask=0x%x\n", __func__, clk->grp->name, clk->mask);
#endif
...
static int ethifsys_clk_disable_op(struct cg_clk *clk)
{
#ifdef CLK_LOG
	clk_info("[%s]: clk->grp->name=%s, clk->mask=0x%x\n", __func__, clk->grp->name, clk->mask);
#endif
...
static struct cg_clk_ops ethifsys_cg_clk_ops = {
	.get_state = general_clk_get_state_op,
	.check_validity = general_clk_check_validity_op,
	.enable = ethifsys_clk_enable_op,
	.disable = ethifsys_clk_disable_op,
};
...
#if DISABLE_BUG_ON_CNT
#if !defined(CONFIG_ICX_SILENT_LOG)
	WARN_ON(!clk->cnt);
#endif
	if (!clk->cnt)
		return 0;
...
static int is_skip_without_display(enum cg_clk_id id)
{
#ifndef CONFIG_MTK_FB
        if ((id >= MT_CG_DISP0_SMI_COMMON && id <= MT_CG_IMAGE_VENC) ||
            (id >= MT_CG_VDEC0_VDEC && id <= MT_CG_MFG_G3D) ||
            (id == MT_CG_HDMI_MON))
			return true;
#endif
	return false;
...
static void cg_all_force_on(void)
{
	int i;
	//for (i = 0; i < NR_CLKS; i++) {
	for (i = 0; i < MT_CG_HDMI_MON + 1; i++) {
		struct cg_clk *clk = id_to_clk(i);

		if(is_skip_without_display(i))
			continue;
...
static void mt_subsys_init(void)
{
	...
	for (i = 0; i < NR_SYSS; i++) {
		...
		if (sys->state != sys->default_sta) {
			clk_info("[%s]%s, sys->cnt:%d, change state: (%u->%u)\n", __func__,
					sys->name, sys->cnt, sys->state, sys->default_sta);
			if (sys->default_sta == PWR_DOWN) {
				sys->cnt = sys->cnt_en ? 1 : 0;
...
static void mt_muxs_init(void)
{
	...
	/* off some unused or alone mux initially
	 */
	clk_setl(CLK_CFG_2, 0x00008000);
	clk_setl(CLK_CFG_5, 0x80000080);
	clk_setl(CLK_CFG_7, 0x00000080);
	clk_setl(CLK_CFG_12, 0x80808080);
	clk_setl(CLK_CFG_13, 0x00008000);
	clk_setl(CLK_CFG_15, 0x00808000);
...
static void init_pll_by_clk(int pllid, int8_t *pll_init, int clkstate, int sysstate)
{
	struct pll *pll;
...
static void init_mux_pll_by_clk(struct cg_clk *clk, int8_t *mux_init, int8_t *pll_init)
{
	...
	/* some cg is directly from plls
	 */
	if (clk->pll) {
		int id = id_from_pll(clk->pll);
		init_pll_by_clk(id, pll_init, clk->state, PWR_DOWN);
	}

	/* most cg has a parent mux
	 */
	if (clk->mux) {
...
static void mt_clk_mux_pll_post_init(void)
{
	...
	for (i = 0; i < NR_PLLS; i++) {
		...
		if ((pll_init[i] || p->cnt > 0) &&
			strcmp(p->name, "HADDS2PLL") && 
			strcmp(p->name, "AUD2PLL")) {
			p->ops->enable(p);
			p->state = PWR_ON;
		} else {
			p->ops->disable(p);
...
static void  hdmirx_dig_pdn(void)
{
...
static void hdmirx_analog_pdn(void)
{
	spm_write(0xF0209150, 0xC0000000);
	spm_write(0xF0209154, 0x44E00E42);
	spm_write(0xF0209158, 0x0B005010);
	spm_write(0xF020915C, 0x00000000);
	spm_write(0xF0209160, 0x00000100);
	spm_write(0xF0209164, 0x00000002);
	spm_write(0xF0209168, 0x00000009);
	spm_write(0xF020916C, 0x00000000);
	spm_write(0xF0209170, 0x00000000);
	spm_write(0xF0209174, 0x00000000);
	spm_write(0xF0209178, 0x00000000);
	spm_write(0xF020917C, 0x00000000);
}


static void hdmirx_internal_pdn(void)
{
	hdmirx_dig_pdn();
	hdmirx_analog_pdn();  
        printk("[HDMIRX]%s\n", __func__);
...
int mt_clkmgr_bringup_init(void)
{
	...
	spm_mtcmos_ctrl_ethsys(STA_POWER_DOWN);
	spm_mtcmos_ctrl_hifsys(STA_POWER_DOWN);
	...
	spm_mtcmos_ctrl_ethsys(STA_POWER_ON);
	spm_mtcmos_ctrl_hifsys(STA_POWER_ON);
#endif
...
static int subsys_test_show(struct seq_file *s, void *v)
{
	int i;
	int state;
	unsigned int value = 0, sta, sta_s, cnt;
	const char *name;
	...
	for (i = 0; i < NR_SYSS; i++) {
		...
		if (subsys_get_cnt_en(i)) {
			cnt = subsys_get_cnt(i);
			seq_printf(s, "[%d][%-8s]=[0x%08x], state(%u), cnt(%u)\n", i, name, value, state, cnt);
		} else
...
static int subsys_stat_show(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < NR_SYSS; i++) {
		sys = id_to_sys(i);
		if (sys->cnt_en)
			seq_printf(s, "[%d][%-7s]state=%u, cnt=%u", i, sys->name,
						sys->state, sys->cnt);
		else
			seq_printf(s, "[%d][%-7s]state=%u", i, sys->name,
						sys->state);
...
static void clkmgr_early_suspend(struct early_suspend *h)
{
	...
	clk_info("[%s]: Dump Top MUX register, CLK_CFG_4=0x%x, CLK_CFG_5=0x%x, CLK_CFG_6=0x%x, CLK_CFG_7=0x%x\n",
		__func__, clk_readl(CLK_CFG_4), clk_readl(CLK_CFG_5), clk_readl(CLK_CFG_6), clk_readl(CLK_CFG_7));

	clk_info("[%s]: Dump Top MUX register, CLK_CFG_12=0x%x, CLK_CFG_13=0x%x, CLK_CFG_14=0x%x, CLK_CFG_15=0x%x\n",
		__func__, clk_readl(CLK_CFG_12), clk_readl(CLK_CFG_13), clk_readl(CLK_CFG_14), clk_readl(CLK_CFG_15));
...
static int mt_clkmgr_debug_bringup_init(void)
{
	...
	/*clk debug*/
	spm_write(0xF0209000, spm_read(0xF0209000) & ~(0xf<<27));
...
{
	/*
	 * mark these avoiding for dead spinlock
