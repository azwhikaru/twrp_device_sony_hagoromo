...
static struct musb_hdrc_config musb11_config_mt65xx = {
	.multipoint     = true,
	.dyn_fifo       = true,
	.soft_con       = true,
	.dma            = true,
...
static struct musb_hdrc_platform_data usb11_data = {
#if 0
...
static struct platform_device mtk_vsw_dev = {
    .name = "vsw",
    .id   = 0,
};
...
static struct resource mt_spi0_resources[] =
{
	[0]={
		.start = IO_VIRT_TO_PHYS(SPI0_BASE),
		.end = IO_VIRT_TO_PHYS(SPI0_BASE) + 0x0028,
		.flags = IORESOURCE_MEM,
	},
	[1]={
		.start = MT6582_SPI0_IRQ_ID,
		.flags = IORESOURCE_IRQ,
	},
};

static struct resource mt_spi1_resources[] =
{
	[0]={
...
static struct resource mt_spi2_resources[] =
{
	[0]={
		.start = IO_VIRT_TO_PHYS(SPI2_BASE),
		.end = IO_VIRT_TO_PHYS(SPI2_BASE) + 0x0028,
		.flags = IORESOURCE_MEM,
	},
	[1]={
		.start = MT6582_SPI2_IRQ_ID,
		.flags = IORESOURCE_IRQ,
	},
};
...
static struct platform_device mt_spi_device[] = {
	{
		.name = "mt-spi",
		.id             = 0,
		.num_resources = ARRAY_SIZE(mt_spi0_resources),
		.resource=mt_spi0_resources
	},

	{
		.name = "mt-spi",
		.id 			= 1,
		.num_resources = ARRAY_SIZE(mt_spi1_resources),
		.resource=mt_spi1_resources
	},

	{
		.name = "mt-spi",
		.id 			= 2,
		.num_resources = ARRAY_SIZE(mt_spi2_resources),
		.resource=mt_spi2_resources
	},
};
...
static struct platform_device carddet_dev = {
    .name = "icx_carddet",
    .id   = 0,
};
...
static struct platform_device icx_key_pdev = {
    .name = "icx_key",
    .id   = 0,
};
...
/* CONFIG_SND_SOC_MT8127 */

#elif defined(CONFIG_SND_SOC_MT8590)
...
static struct platform_device mt8590_audio_device = {
	.name = "mt8590-audio",
	.id = PLATFORM_DEVID_NONE,
};

static struct platform_device mt8590_lp_audio_device = {
	.name = "mt8590-lp-audio",
	.id = PLATFORM_DEVID_NONE,
};

static struct platform_device mt8590_dai_device = {
	.name = "mt8590-dai",
	.id = PLATFORM_DEVID_NONE,
};
...
static struct platform_device pcm4202_device = {
	.name = "pcm4202",
	.id = PLATFORM_DEVID_NONE,
};

static struct platform_device pcm1861_device = {
	.name = "pcm1861",
	.id = PLATFORM_DEVID_NONE,
};

static struct platform_device pcm5102a_device = {
	.name = "pcm5102a",
	.id = PLATFORM_DEVID_NONE,
};
#ifdef CONFIG_SND_SOC_ICX_BOARD
static struct platform_device icx_machine_device = {
        .name = "icx-machine",
        .id = PLATFORM_DEVID_NONE,
};
...
struct platform_device dac_device = {
    .name   = "mt-dac",
    .id     = -1,
};
...
#if defined(CONFIG_ENABLE_CARDDET)
    retval = platform_device_register(&carddet_dev);
    if (retval != 0){
        return retval;
    }
#endif /*CONFIG_ENABLE_CARDDET*/
...
#if defined(CONFIG_ENABLE_ICX_KEY)
    retval = platform_device_register(&icx_key_pdev);
    if (retval != 0){
        return retval;
    }
#endif /* CONFIG_ENABLE_ICX_KEY */
...
#if defined(CONFIG_MTK_FB_NO_MEM) && (CONFIG_MTK_FB_NO_MEM)
	printk(KERN_ALERT"FB memory is not reserved in LK(%d)\n", use_bl_fb);
...
#ifdef CONFIG_MTK_HDMI_SUPPORT
	retval = platform_device_register(&mtk_vsw_dev);
	if (retval != 0){
		return retval;
	}
#endif
...
//    spi_register_board_info(spi_board_devs, ARRAY_SIZE(spi_board_devs));
	for (i = 0; i < ARRAY_SIZE(mt_spi_device); i++){
		retval = platform_device_register(&mt_spi_device[i]);
		if (retval != 0){
			return retval;
		}
	}
...
#ifndef CONFIG_MT8590_FPGA
  retval = platform_device_register(&dac_device);
    printk("[%s]: dac_device, retval=%d \n!", __func__, retval);
  if(retval != 0)
  {
     printk("****[dac_driver] Unable to device register(%d)\n", retval);
	 return retval;
  }
#endif
...
#ifndef CONFIG_MT8590_FPGA  /* Before USB driver register */
   retval = platform_device_register(&battery_device);
   if (retval) {
	   printk("[battery_driver] Unable to device register, retval:%d\n", retval);
        return retval;
	}
#endif

#if defined(CONFIG_USB11_MTK_HDRC)
	printk(KERN_ERR "mt11_device_usb register\n");
	retval = platform_device_register(&mt_device_usb11);
	if (retval != 0){
	printk("mt11_device_usb register fail\n");
        return retval;
	}
#endif
	
#if defined(CONFIG_USB_MTK_HDRC)
	printk(KERN_ERR "mt_device_usb register\n");
