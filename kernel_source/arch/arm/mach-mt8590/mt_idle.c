...
int idle_switch[NR_TYPES] = {
	1,  /* soidle switch */
#ifdef CONFIG_ICX_NO_ENTER_DEEPIDLE
	0,  /* dpidle switch */
#else
	1,  /* dpidle switch */
#endif
...
/* two notified points need be demostrated:
 * 1. emmc5.0 controller's hclk is ignored;
 * 2. audio0's hdmi_audio clk is ignored.
 */
static unsigned int dpidle_condition_mask[NR_GRPS] = {
	0x00000000, /* AUDIO - not used */
	0x00000000, /* MFG - not used */
	0x00000000, /* TOPCK */
	0x47F5EFFD, /* PERI0 */
	0x00000FFC, /* PERI1 */
	0x0100E090, /* INFRA */
	...
	0x00000000, /* AUDIO5 */
	0x00000000, /* TOP_PWR2 */
	0x07600000, /* HIFSYS_CLKCFG1 */
	0x200249E0, /* ETHDMASYS_CLKCFG1 */
};
...
struct dpidle_get_status {
	int		(*key_is_active)(void);
};

static struct dpidle_get_status dpidle_get_status_tbl = {
	.key_is_active = NULL,
};

void register_dpidle_key_status_func(int (*func)(void))
{
	dpidle_get_status_tbl.key_is_active = func;
}
EXPORT_SYMBOL(register_dpidle_key_status_func);
#endif
...
static bool dpidle_can_enter(bool mode)
{
	int reason = NR_REASONS;
	...
	/* check connsys power status at ULP mode
	*/
	if ((mode && idle_readl(SPM_PWR_STATUS) & 0x2)) {
		reason = BY_OTH;
		goto out;
	}

#ifdef CONFIG_ARCH_MT8590_ICX
	if (dpidle_get_status_tbl.key_is_active) {
		if (dpidle_get_status_tbl.key_is_active()) {
			reason = BY_OTH;
			goto out;
		}
...
static unsigned int slidle_condition_mask[NR_GRPS] = {
	...
	0x00000000, /* AUDIO5 */
	0x00000000, /* TOP_PWR2 */
	0x00000000, /* HIFSYS_CLKCFG1 */
	0x00000000, /* ETHDMASYS_CLKCFG1 */
};
/* */
...
static bool dpidle_mode = 0;

static inline int dpidle_handler(int cpu)
{
	int ret = 0;
	if (idle_switch[IDLE_TYPE_DP]) {
		if (dpidle_can_enter(dpidle_mode)) {
			spm_go_to_dpidle(dpidle_cpu_pdn, 0, dpidle_mode);
			ret = 1;
		}
	}
				...
				struct kobj_attribute *attr, char *buf)
{
	...
	p += sprintf(p, "dpidle_mode on/off: echo mode 1/0 > /sys/power/dpidle_state\n");
				...
				struct kobj_attribute *attr, const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%s %d", cmd, &param) == 2) {
		if (!strcmp(cmd, "dpidle"))
			...
			dpidle_mode = !!param;
			#ifdef CONFIG_CHARGER_BQ24262_WMPORT
			{
				void bq24262_wmport_set_ulp_mode(bool ulp_mode);
				bq24262_wmport_set_ulp_mode(dpidle_mode?true:false);
			}
			#endif
		}
		else if (!strcmp(cmd, "time"))
