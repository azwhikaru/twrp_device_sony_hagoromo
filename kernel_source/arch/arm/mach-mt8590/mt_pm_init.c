...
unsigned int mt_get_bus_freq(void)
{
    ...
    unsigned int clk_cfg_0, dcm_cfg, postdiv, bus_clk;
    unsigned int output_freq = 0;
    
    clk_cfg_0 = DRV_Reg32(CLK_CFG_0);
    dcm_cfg = DRV_Reg32(DCM_CFG);
    
    mainpll_con0 = DRV_Reg32(MAINPLL_CON0);
    mainpll_con1 = DRV_Reg32(MAINPLL_CON1);
    
    main_diff = ((mainpll_con1 & 0x1FFFFF) >> 14) - 0x4D;
    postdiv = (mainpll_con0 >> 4) & 0x7;
    
    output_freq = (2002 + (main_diff * 26)) >> postdiv; // Mhz
    ...
    else if ((clk_cfg_0 & 0x7) == 6) // DMPLL_CK = DMPLL = 400MHz
    {
        bus_clk = (400 * 1000);
    }
    else if ((clk_cfg_0 & 0x7) == 7) // DMPLL_D2 = DMPLL / 2
    {
        bus_clk = (400 * 1000) / 2 ;
    }
    else // CLKSQ
    ...
    if (dcm_cfg & (0x1 << 7)) {
	    if (dcm_cfg & (0x1 << 4))
			bus_clk /= 1;
	    else if (dcm_cfg & (0x1 << 3))
			bus_clk /= 2;
	    else if (dcm_cfg & (0x1 << 2))
			bus_clk /= 4;
	    else if (dcm_cfg & (0x1 << 1))
			bus_clk /= 8;
	    else if (dcm_cfg & (0x1 << 0))
			bus_clk /= 16;
	    else
			bus_clk /= 32;
    }
...
static int __init mt_power_management_init(void)
{
    ...
    #if !defined (CONFIG_MT8590_FPGA_CA7)
