...
#define MAGIC_KEY_WARM_SLEEP		0x7761726D
#define MAGIC_KEY_COLD_SLEEP		0x636F6C64
static int slp_mode = MAGIC_KEY_WARM_SLEEP;
...
static int slp_suspend_ops_enter(suspend_state_t state)
{
    ...
    /* if colde sleep feature is enabled and
     * magic key meets, then let system go into
     * cold sleep.
     */
    if (slp_mode == MAGIC_KEY_COLD_SLEEP) {
#ifdef SPM_COLD_SLEEP_ENABLED
	slp_notice("cold sleep enter.\n");
	spm_go_to_cold_sleep();
#else
	slp_err("Not Support cold-sleep!!\n");
	return -EINVAL;
#endif
    } else {
	slp_notice("warm sleep enter.\n");
...
module_param(slp_mode, int, 0644);
