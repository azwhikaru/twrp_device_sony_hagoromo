 ...
 * regmon.c
 *
 * Copyright 2011-2015 Sony Corporation.
 *
...
//	entry->proc_fops = &target_operations;
...
//	entry->proc_fops = &value_operations;
...
)
{
	...
	for(index=0; index<customer_count; index++){
		...
		if(file->f_dentry->d_inode->i_ino==customer_info[index]->target_node)
			break;
	}
...
)
{
	...
	for(index=0; index<customer_count; index++){
		...
		if(file->f_dentry->d_inode->i_ino==customer_info[index]->value_node)
			break;
	}
