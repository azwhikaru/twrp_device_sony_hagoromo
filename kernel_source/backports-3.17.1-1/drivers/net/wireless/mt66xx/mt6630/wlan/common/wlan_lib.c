...
VOID wlanSecurityFrameTxDone(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (GET_BSS_INFO_BY_INDEX(prAdapter, prCmdInfo->ucBssIndex)->eNetworkType ==
		...
		if (prSta) {
			...
			if (prCmdInfo->ucBssIndex >= (HW_BSSID_NUM + 1)) {
				ASSERT(FALSE);
				return;
			}
...
TX_PWR_T TxPwrPerCh[56];

char g_buffer[10000];

BOOLEAN wlanExtractSingleSku(
	UINT_8 ucSingleNum
)
{
	...
	int bufferSize = 0;
	int chanNum = 0;
	unsigned int txPwr[16];
	BOOLEAN fgFileExist = FALSE ;

	UINT_8 charBuf[100];

	printk("%s\n",__FUNCTION__);
	
	kalMemZero(charBuf, sizeof(charBuf));
    
	sprintf(charBuf, "/etc/MT6630_SingleSKU_%03d.dat",ucSingleNum);
    
	fd  = filp_open(charBuf,O_RDONLY,0);

	
#if 0
	fd  = filp_open(SINGLE_SKU_TABLE_FILE_NAME,O_RDONLY,0);
#endif
	
	if(IS_ERR(fd)) {
		printk("[SingleSKU] unable to open file %s \n",charBuf);
		fgFileExist = FALSE ;
		goto free_resource;
	}else {
		fgFileExist = TRUE ;
		printk("[SingleSKU(%x)] open file %s \n",(UINT_32)fd,charBuf);

	}
    if ((fd->f_op == NULL) || (fd->f_op->read == NULL)) {
        printk("[SingleSKU] file can not be read!!\n");
        goto free_resource;
    }	

	/* seek to the end and return file size */
	if (fd->f_op->llseek){
		bufferSize = fd->f_op->llseek(fd,0, SEEK_END);
		printk("[SingleSKU] seek to the end size(%d)\n",bufferSize);
	}else{
		printk("[SingleSKU] can not seek to the end size(%d)\n",bufferSize);
	}
	/* allocate memory to contain the whole file: */
	buffer = g_buffer;//kalMemAlloc(bufferSize, VIR_MEM_TYPE);	
	//buffer = (char*)kmalloc(sizeof(char)*bufferSize,GFP_KERNEL);

	/* seek to begin */
	fd->f_op->llseek(fd,0, SEEK_SET);
	/* copy file to buffer */
	retval = fd->f_op->read(fd,
			buffer,
			bufferSize,
			&fd->f_pos);

	printk("[SingleSKU] readfile size(%d)\n",retval);
	...
	memset(buffer,NULL,sizeof(char)*bufferSize);

	/* copy the file into the buffer: */
	retval = fread(buffer, 1, bufferSize, pFile);
#endif 
#if 1
		...
		for (readline = ptr = buffer, index = 0; (ptr = strchr(readline, '\n')) != NULL;
			...
			*ptr = '\0'; 	
			...
			if (!strncmp(readline, "ch", 2)) {
				token = strsep(&readline, " ");
				channel = simple_strtol(token + 2, 0, 10);
				TxPwrPerCh[chanNum].Channel = channel;
				...
				if (channel <= 14) {
					for (i = 0; i < SINGLE_SKU_TABLE_CCK_LENGTH; i++) {
						...
						if (token == NULL)
							break;
						txPwr[i] = simple_strtol(token, 0, 10) * 2;
						printk("%d ",txPwr[i]);
					}
				}
				TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4Cck = txPwr[0];
				...
				if(channel <= 14){
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_BPSK   = txPwr[0];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_QPSK   = txPwr[2];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_16QAM  = txPwr[4];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_48Mbps = txPwr[6];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_54Mbps = txPwr[7];
				}else{
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_BPSK    = txPwr[0];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_QPSK    = txPwr[2];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_16QAM   = txPwr[4];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_48Mbps  = txPwr[6];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_54Mbps  = txPwr[7];
				}
				/* HT20 */
				...
				if(channel <= 14){
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_BPSK  = txPwr[0];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_QPSK  = txPwr[2];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_16QAM = txPwr[4];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_MCS5  = txPwr[5];				
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_MCS6  = txPwr[6];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_MCS7  = txPwr[7];
				}else{
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_BPSK   = txPwr[0];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_QPSK   = txPwr[2];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_16QAM  = txPwr[4];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_MCS5   = txPwr[5];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_MCS6   = txPwr[6];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_MCS7   = txPwr[7];					
				}
				/* HT40 */
				for (i = 0; i < SINGLE_SKU_TABLE_HT_LENGTH; i++) {
					token = strsep(&readline, " ");
				...
				if(channel <= 14){
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_BPSK  = txPwr[0];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_QPSK  = txPwr[2];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_16QAM = txPwr[4];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_MCS5  = txPwr[5];				
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_MCS6  = txPwr[6];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_MCS7  = txPwr[7];
				}else{
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_BPSK   = txPwr[0];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_QPSK   = txPwr[2];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_16QAM  = txPwr[4];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_MCS5   = txPwr[5];				
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_MCS6   = txPwr[6];
					TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_MCS7   = txPwr[7];				
				}
...
free_resource:
	for(chanNum = 0 ; chanNum < 56 ; chanNum++){
		printk( "CH %3d 2G CCK(%2d) OFDM(%2d %2d %2d %2d %2d) HT20(%2d %2d %2d %2d %2d %2d) HT40(%2d %2d %2d %2d %2d %2d) ",
			TxPwrPerCh[chanNum].Channel,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4Cck,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_BPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_QPSK, 
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_16QAM, 
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_48Mbps,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4OFDM_54Mbps,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_BPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_QPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_16QAM,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_MCS5,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_MCS6,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT20_MCS7,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_BPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_QPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_16QAM,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_MCS5,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_MCS6,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr2G4HT40_MCS7
		);

		printk("5G OFDM(%2d %2d %2d %2d %2d) HT20(%2d %2d %2d %2d %2d %2d) HT40(%2d %2d %2d %2d %2d %2d)\n",
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_BPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_QPSK, 
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_16QAM, 
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_48Mbps,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GOFDM_54Mbps,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_BPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_QPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_16QAM,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_MCS5,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_MCS6,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT20_MCS7,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_BPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_QPSK,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_16QAM,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_MCS5,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_MCS6,
			TxPwrPerCh[chanNum].TxPwr.cTxPwr5GHT40_MCS7
		);
...
#endif
	//kfree(buffer);	
	if (fgFileExist == FALSE ) {
		printk("[Warning][SingleSKU] need MT6630L_SingleSKU_000.dat , use default power setting   \n");
		return FALSE;
	}else {
		filp_close(fd,NULL);
		printk("[SingleSKU] free buffer and close file\n");
		return TRUE;
	}
}
...
#if CFG_BENTEN_SUPPORT

		if (wlanExtractSingleSku(prRegInfo->ucSingleSKUNumber)) {

			/*set channel power to fw*/
			nicUpdateTxPowerV2(prAdapter, NULL);
		}
#endif
...
VOID wlanInitFeatureOption(IN P_ADAPTER_T prAdapter)
{
	...
	prWifiVar->ucApVht = (UINT_8) wlanCfgGetUint32(prAdapter, "ApVHT", FEATURE_DISABLED);
	...
	prWifiVar->ucApBandwidth = (UINT_8) wlanCfgGetUint32(prAdapter, "ApBw", MAX_BW_INVALID);
	...
	prWifiVar->u4MaxTxDeQLimit = (UINT_32) wlanCfgGetUint32(prAdapter, "MaxTxDeQLimit", 0xa);
