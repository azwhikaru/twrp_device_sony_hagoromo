...
VOID bssDetermineApBssInfoPhyTypeSet(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgIsPureAp, OUT P_BSS_INFO_T prBssInfo)
{
	...
	if (fgIsPureAp) {
		...
		if (IS_FEATURE_FORCE_ENABLED(ucVhtOption)) {
			/*double check nvram support vht or not*/
			ucVhtOption = (prAdapter->prGlueInfo->rRegInfo.ucFeatureEnable == NVRAM_FEATURE_DISABLE_VHT)? FEATURE_DISABLED : FEATURE_ENABLED;
			DBGLOG(BSS,INFO,("NVRAM set VHT feature as (%d) in apmode",ucVhtOption));
		}
#endif
	}
