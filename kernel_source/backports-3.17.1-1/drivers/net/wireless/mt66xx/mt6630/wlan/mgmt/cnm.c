...
BOOLEAN cnmBss40mBwPermitted(P_ADAPTER_T prAdapter, UINT_8 ucBssIndex)
{
	...
	if (cnmGetCustomerMaxBw(prAdapter, ucBssIndex) == MAX_BW_20MHZ) {
		return FALSE;	
	}
...
UINT_8 cnmGetCustomerMaxBw(P_ADAPTER_T prAdapter, UINT_8 ucBssIndex)
{
	...
	if (IS_BSS_AIS(prBssInfo)) {
		/* STA mode */
		/*todo: 6630 only ap mode in sony*/
	} else if (IS_BSS_P2P(prBssInfo)) {
		/* AP mode */
		if (p2pFuncIsAPMode(prAdapter->rWifiVar.prP2PConnSettings)) {
			
			/*1.check channel list*/
			for (i=0; i< MAX_BW20_NUM;i++) {

				if (g_arChannelListSupportOnlyBw20[prRegInfo->ucCountryListIndex].ucBw20Channel[i] == prBssInfo->ucPrimaryChannel) {
					ucMaxBandwidth = MAX_BW_20MHZ;
					DBGLOG(CNM,INFO,("check channel list fix bw 20 on channel[%d]",prBssInfo->ucPrimaryChannel));
				}
			}
#if 0
			if((prRegInfo->ucCountryListIndex == 12) || (prRegInfo->ucCountryListIndex == 18) ) {
                    
				ucMaxBandwidth = MAX_BW_20MHZ;
            } 
#endif

			/*2.check nvram featrure enable*/
			if((prRegInfo->ucFeatureEnable == NVRAM_FEATURE_2G_ONLY_HT20) && (prAdapter->rWifiVar.aprP2pRoleFsmInfo[0]->rChnlReqInfo.eBand == BAND_2G4)){

				ucMaxBandwidth = MAX_BW_20MHZ;
				DBGLOG(CNM,INFO,("check nvram fix bw20 on 2.4g"));
			}

			
			if((prRegInfo->ucFeatureEnable == NVRAM_FEATURE_5G_ONLY_HT20) && (prAdapter->rWifiVar.aprP2pRoleFsmInfo[0]->rChnlReqInfo.eBand == BAND_5G)){
			
				ucMaxBandwidth = MAX_BW_20MHZ;
				DBGLOG(CNM,INFO,("check nvram fix bw20 on 5g"));
			}
...
UINT_8 cnmGetBssMaxBw(P_ADAPTER_T prAdapter, UINT_8 ucBssIndex)
{
	...
	if (IS_BSS_AIS(prBssInfo)) {
		...
		/* AP mode */
		if (p2pFuncIsAPMode(prAdapter->rWifiVar.prP2PConnSettings)) {
		    /*follow wifi.cfg value if it assigned*/
		    if(prAdapter->rWifiVar.ucApBandwidth != MAX_BW_INVALID) {
			    ucMaxBandwidth = prAdapter->rWifiVar.ucApBandwidth;
			}
			/*follow value from hostapd*/
			else {
                ucMaxBandwidth = prAdapter->rWifiVar.aprP2pRoleFsmInfo[0]->rConnReqInfo.rChannelInfo.ucBandwidth;
            }    
		}
		/* P2P mode */
		else {
