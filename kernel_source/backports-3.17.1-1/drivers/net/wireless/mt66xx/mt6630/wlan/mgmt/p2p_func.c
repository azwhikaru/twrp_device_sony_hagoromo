	       ...
	       IN P_P2P_CONNECTION_REQ_INFO_T prP2pConnReqInfo, IN P_P2P_CHNL_REQ_INFO_T prP2pChnlReqInfo)
{
	do {
		...
		prAdapter->rWifiVar.prP2pDevFsmInfo->rScanReqInfo.fgIsGOInitialDone = TRUE;
		DBGLOG(P2P, INFO, ("p2pFuncStartGO:NFC Done[%d]\n", prAdapter->rWifiVar.prP2pDevFsmInfo->rScanReqInfo.fgIsGOInitialDone));
        ...
        kalP2PIndicateSoftAPEnabled(prAdapter, 1);
...
VOID p2pFuncSetChannel(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx, IN P_RF_CHANNEL_INFO_T prRfChannelInfo)
{
	...
	do {
		...
		prP2pConnReqInfo->rChannelInfo.ucBandwidth = prRfChannelInfo->ucBandwidth;
		prP2pConnReqInfo->rChannelInfo.eSCO = prRfChannelInfo->eSCO;
