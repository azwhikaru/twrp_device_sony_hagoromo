...
VOID scanReportBss2Cfg80211(IN P_ADAPTER_T prAdapter, IN ENUM_BSS_TYPE_T eBSSType, IN P_BSS_DESC_T SpecificprBssDesc)
{
	...
	if (SpecificprBssDesc) {
        ...
        LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
            ...
            if (prAdapter->fgIsP2PRegistered && prAdapter->rWifiVar.prP2pDevFsmInfo) {
                if (prAdapter->rWifiVar.prP2PConnSettings->fgIsApMode == 1 ||
                    prAdapter->rWifiVar.prP2pDevFsmInfo->rScanReqInfo.fgIsGOInitialDone == 0 ) {
