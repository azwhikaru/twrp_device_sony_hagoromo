...
VOID
nicCmdEventQueryLTESafeChn (IN P_ADAPTER_T  prAdapter,IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	UINT_8 ucIdx = 0;
    UINT_32 u4QueryInfoLen;
    ...
    if (prCmdInfo->fgIsOid) {
        ...
        if(prEvent->u4Flags & BIT(0)) {
			for(ucIdx = 0; ucIdx < (NL80211_TESTMODE_AVAILABLE_CHAN_NUM-1); ucIdx++) {
				...
				DBGLOG(P2P, INFO,("[Auto Channel]LTE safe channels [%d]=[%x]\n",ucIdx,(UINT_32)prLteSafeChnInfo->rLteSafeChnList.au4SafeChannelBitmask[ucIdx]));
			}
        }
