...
#if CFG_SUPPORT_WAPI
		rStatus = kalIoctl(prGlueInfo,
				   wlanoidSetWapiAssocInfo, (PVOID)sme->ie, sme->ie_len, FALSE, FALSE, FALSE, &u4BufLen);
...
int mtk_cfg80211_testmode_cmd(IN struct wiphy *wiphy,
	struct wireless_dev *wdev, IN void *data, IN int len)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
...
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
        ...
        UINT_32 AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_NUM];
    ...
    */
        ...
        skb = cfg80211_testmode_alloc_reply_skb(wiphy, sizeof(AcsChnReport) +sizeof(UINT_8)+ 1);
        if(!skb) {
        ...
        if (fgIsReady == FALSE)
            ...
            if (prGlueInfo->prAdapter->rWifiVar.prP2PConnSettings->fgIsApMode == 1) {
                P_PARAM_GET_CHN_LOAD  prGetChnLoad;
