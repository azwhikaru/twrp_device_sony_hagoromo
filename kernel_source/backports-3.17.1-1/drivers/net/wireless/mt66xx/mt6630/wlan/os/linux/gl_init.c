...
*/
	...
	extern void register_get_6630_TSF_for_media_synchronization_handler(pf_get_latched_tsf handler);
#endif
...
/* load EFUSE overriding part */
		...
		printk("[%s] offset(%x) ucFeatureEnable(%x) ucCountryListIndex(%x)\n",
		    __FUNCTION__,
		    OFFSET_OF(WIFI_CFG_PARAM_STRUCT, ucFeatureEnable),
		...
		printk("[%s] Offset(%x) ucSingleSKUNumber(%x)\n",
		    __FUNCTION__,
		    OFFSET_OF(WIFI_CFG_PARAM_STRUCT, ucSingleSKUNumber),
...
//	aucDebugModule[DBG_P2P_IDX] = (DBG_CLASS_ERROR | DBG_CLASS_WARN | DBG_CLASS_TRACE | DBG_CLASS_INFO | DBG_CLASS_STATE);
#endif /* DBG */
...
WLAN_STATUS
wlanoidQueryTsf(IN P_ADAPTER_T prAdapter,
		      ...
		      IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	/*query FW TSF , then report by ptr*/
	rCmdSwCtrl.u4Id = LOW_TSF_POSITION;
	rCmdSwCtrl.u4Data = 0;
	...
	*pu4QueryInfoLen = sizeof(UINT_32);
	...
	return wlanSendSetQueryCmd(prAdapter,
				   ...
				   swTsfQuery,
				   swTsfQueryTimeout,
				   sizeof(rCmdSwCtrl), &rCmdSwCtrl, pvQueryBuffer, u4QueryBufferLen);
...
WLAN_STATUS
wlanoidQueryTsfHigh(IN P_ADAPTER_T prAdapter,
		      IN PVOID pvQueryBuffer,
		      IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	/*query FW TSF , then report by ptr*/
	rCmdSwCtrl.u4Id = HIGH_TSF_POSITION;
	rCmdSwCtrl.u4Data = 0;
...
int
get_latched_tsf_6630(unsigned int *ptr)
{
	...
	if (!prGlueInfo)
		return 0;
	/*Query Low TSF 4bytes*/
	rStatus = kalIoctl(prGlueInfo,
			   wlanoidQueryTsf,
			   &u4Tsf, sizeof(u4Tsf), TRUE, TRUE, TRUE, &u4BufLen);

	*(ptr+2) = prGlueInfo->prAdapter->au4TsfLatch[0];

	/*Query High TSF 4bytes*/
	rStatus = kalIoctl(prGlueInfo,
			   wlanoidQueryTsfHigh,
			   &u4Tsf, sizeof(u4Tsf), TRUE, TRUE, TRUE, &u4BufLen);

	
	*(ptr+3) = prGlueInfo->prAdapter->au4TsfLatch[1];

#if 0
	/*Debugging TSF result*/
	printk("TSF (driver) Tsf=[%d][%d]\n",prGlueInfo->prAdapter->au4TsfLatch[1],prGlueInfo->prAdapter->au4TsfLatch[0]);
#endif
...
bailout:
		...
		prGlueInfo->main_thread = kthread_run(tx_thread, prGlueInfo->prDevHandler, "6630_tx_thread");
#if CFG_SUPPORT_MULTITHREAD
		prGlueInfo->hif_thread = kthread_run(hif_thread, prGlueInfo->prDevHandler, "6630_hif_thread");
		prGlueInfo->rx_thread = kthread_run(rx_thread, prGlueInfo->prDevHandler, "6630_rx_thread");
#endif
		/* [ALPS02591336] From SONY's request, increase thread priority for performance */
		struct sched_param scp = { .sched_priority = 1 };
		sched_setscheduler(prGlueInfo->main_thread, SCHED_RR, &scp);
#if CFG_SUPPORT_MULTITHREAD
		sched_setscheduler(prGlueInfo->hif_thread, SCHED_RR, &scp);
		sched_setscheduler(prGlueInfo->rx_thread, SCHED_RR, &scp);
#endif
		DBGLOG(INIT, INFO, ("Set sched = %d, pri = %d\n", SCHED_RR, scp.sched_priority));
		...
		register_set_p2p_mode_handler_6630(set_p2p_mode_handler, prGlueInfo->prDevHandler);
#endif

#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
	   register_get_6630_TSF_for_media_synchronization_handler(get_latched_tsf_6630);
#endif
...
#if (CFG_ENABLE_WIFI_DIRECT && MTK_WCN_HIF_SDIO && CFG_SUPPORT_MTK_ANDROID_KK)
	register_set_p2p_mode_handler_6630(NULL, NULL);
#endif

#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
	register_get_6630_TSF_for_media_synchronization_handler(NULL);
#endif
...
#if (MTK_WCN_HIF_SDIO && CFG_SUPPORT_MTK_ANDROID_KK)
	register_set_dbg_level_handler_6630(set_dbg_level_handler);
#endif
...
EXPORT_SYMBOL(mtk_wcn_wlan_6630_init);
EXPORT_SYMBOL(mtk_wcn_wlan_6630_exit);
static int __init init6630Wlan(void)
{
	return initWlan();
}

static VOID __exit exit6630Wlan(void)
{
	exitWlan();
	return;
}

module_init(init6630Wlan);
module_exit(exit6630Wlan);
