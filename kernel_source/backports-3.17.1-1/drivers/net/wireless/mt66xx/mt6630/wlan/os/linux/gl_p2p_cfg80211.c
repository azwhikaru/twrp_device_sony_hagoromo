...
mtk_p2p_cfg80211func_channel_format_switch(IN struct ieee80211_channel *channel,
					   ...
					   OUT P_RF_CHANNEL_INFO_T prRfChnlInfo, OUT P_ENUM_CHNL_EXT_T prChnlSco,
...
int mtk_p2p_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, ("Requesting channel numbers:%d.\n", request->n_channels));
        ...
        if (prGlueInfo->prAdapter->rWifiVar.prP2PConnSettings->fgIsApMode == 1 ||
            prGlueInfo->prAdapter->rWifiVar.prP2pDevFsmInfo->rScanReqInfo.fgIsGOInitialDone == 0 ) {
            P_PARAM_GET_CHN_LOAD  prGetChnLoad;

            DBGLOG(P2P, TRACE, ("AP: Preceding Scan for ACS\n"));

            prGetChnLoad = (P_PARAM_GET_CHN_LOAD)&(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);
            kalMemZero(prGetChnLoad, sizeof(PARAM_GET_CHN_LOAD));

            prGetChnLoad->u4ScanChnNum = request->n_channels;
            for (u4Idx = 0; u4Idx < request->n_channels; u4Idx++) {
                prChannel = request->channels[u4Idx];
                prGetChnLoad->rScanChnLoad[u4Idx].u2Freq = prChannel->center_freq;
                prGetChnLoad->rScanChnLoad[u4Idx].ucChannel = nicFreq2ChannelNum(prChannel->center_freq * 1000);
            }
	...
	do {
		...
		*cookie = prGlueP2pInfo->u8Cookie++;
		...
		mtk_p2p_cfg80211func_channel_format_switch(chan,
							   NL80211_CHAN_NO_HT,
							   &prMsgChnlReq->rChannelInfo, &prMsgChnlReq->eChnlSco, NULL);
	...
	u64 *cookie)
{
	...
	do {
		if ((wiphy == NULL) || (params->buf == NULL) || (params->len == 0) ||
		    /* (dev == NULL) || */
		    (cookie == NULL)) {
			break;
		}
		...
		*cookie = prGlueP2pInfo->u8Cookie++;
		...
		if (params->offchan) {
			...
			mtk_p2p_cfg80211func_channel_format_switch(params->chan,
								   NL80211_CHAN_NO_HT,
								   &prMsgTxReq->rChannelInfo, &prMsgTxReq->eChnlExt, NULL);
		} else {
...
int mtk_p2p_cfg80211_connect(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	do {
		...
		mtk_p2p_cfg80211func_channel_format_switch(sme->channel,
							   NL80211_CHAN_NO_HT,
							   &prConnReqMsg->rChannelInfo, &prConnReqMsg->eChnlSco, NULL);
				 ...
				 struct cfg80211_chan_def *chandef)
{
	...
	do {
		...
		mtk_p2p_cfg80211func_channel_format_switch(chandef->chan, chandef->width, &rRfChnlInfo, NULL, chandef);
...
mtk_p2p_cfg80211func_channel_format_switch(IN struct ieee80211_channel *channel,
					   IN enum nl80211_channel_type channel_type,
					   OUT P_RF_CHANNEL_INFO_T prRfChnlInfo, OUT P_ENUM_CHNL_EXT_T prChnlSco, IN struct cfg80211_chan_def *chandef)
{
	BOOLEAN fgIsValid = FALSE;

	do {
		if ((channel == NULL) && (chandef == NULL))
			break;

		if (prRfChnlInfo) {
            if(chandef) {
                DBGLOG(P2P, INFO, ("Apply channel info from upper layer\n"));

    		    /*<1>channel number*/
        		prRfChnlInfo->ucChannelNum = nicFreq2ChannelNum(chandef->chan->center_freq * 1000);
                ...
                case NL80211_CHAN_WIDTH_20:
                    prRfChnlInfo->ucBandwidth = MAX_BW_20MHZ;
                    break;
                case NL80211_CHAN_WIDTH_40:
                    prRfChnlInfo->ucBandwidth = MAX_BW_40MHZ;
                    break;
                case NL80211_CHAN_WIDTH_80:
                    prRfChnlInfo->ucBandwidth = MAX_BW_80MHZ;
                    break;
                case NL80211_CHAN_WIDTH_160:
                    prRfChnlInfo->ucBandwidth = MAX_BW_160MHZ;
                    break;
                case NL80211_CHAN_WIDTH_80P80:
                    prRfChnlInfo->ucBandwidth = MAX_BW_80_80_MHZ;
                    break;
                default:
                    prRfChnlInfo->ucBandwidth = MAX_BW_20MHZ;
                    break;
                ...
                case NL80211_CHAN_WIDTH_20:
                    ...
                    break;
                case NL80211_CHAN_WIDTH_40:
                    prRfChnlInfo->eSCO = ((chandef->chan->center_freq)>(chandef->center_freq1))?(CHNL_EXT_SCB):(CHNL_EXT_SCA);
                    break;
                case NL80211_CHAN_WIDTH_80:
                    if(CAL_PRIMARY_CH_OFFSET(chandef->chan->center_freq, chandef->center_freq1) % 2 == 0)
...
int mtk_p2p_cfg80211_testmode_cmd(IN struct wiphy *wiphy, struct wireless_dev *wdev, IN void *data, IN int len)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
...
void kalP2PIndicateSoftAPEnabled(P_ADAPTER_T prAdapter, UINT_32 u4Reserved)
{
...
#define NLA_PUT_U32(skb, attrtype, value) \
	...
	P_GL_P2P_INFO_T prP2PInfo = prAdapter->prGlueInfo->prP2PInfo;
    struct sk_buff *skb;
    INT_32 i4Status = -EINVAL;

    ASSERT(prP2PInfo->prWdev->wiphy);

    DBGLOG(REQ, INFO, ("%s for vendor command\r\n", __func__));

    skb = cfg80211_vendor_event_alloc(prP2PInfo->prWdev->wiphy, sizeof(u4Reserved), VENDOR_EVENT_ENABLE_SOFTAP_DONE, GFP_KERNEL);
    if (!skb) {
        DBGLOG(REQ, ERROR, ("%s allocate skb failed\n", __func__));
        return -ENOMEM;
    }

    NLA_PUT_U32(skb, VENDOR_EVENT_ENABLE_SOFTAP_DONE, u4Reserved);

    cfg80211_vendor_event(skb, GFP_KERNEL);
    DBGLOG(P2P, INFO, ("%s for vendor command done\r\n", __func__));
    return 0;
