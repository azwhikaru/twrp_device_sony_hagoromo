...
VOID kalP2PIndicateScanDone(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucRoleIndex, IN BOOLEAN fgIsAbort)
{
	...
	do {
		...
		if (prScanRequest != NULL) {
            ...
            //for AP mode, there are three stages prerrequiste to establish
            // Stage 1:MTK ACS scan
            // Stage 2:Go scan
            // Stage 3:Start Go
            // Only report BSS before BSS established
            if (prGlueInfo->prAdapter->rWifiVar.prP2PConnSettings->fgIsApMode == 0 ||
		      ...
		      IN UINT_8 ucRoleIndex, IN P_STA_RECORD_T prCliStaRec, IN BOOLEAN fgIsNew)
{
	...
	do {
		if ((prGlueInfo == NULL) || (prCliStaRec == NULL) || (ucRoleIndex >= KAL_P2P_NUM))
			break;
