...
int priv_driver_get_sta_statistics(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	UINT_32 u4BufLen = 0;
	...
	if (i4Argc >= i4ArgNum) {
	    wlanHwAddrToBin(apcArgv[1], &rQueryStaStatistics.aucMacAddr);
    ...
    u2LinkSpeed = rQueryStaStatistics.u2LinkSpeed/2;

	i4BytesWritten = snprintf(pcCommand, i4TotalLen, "Rssi:%d LinkSpeed:%u TxFailCnt:%u\n ", rRssi, u2LinkSpeed, rQueryStaStatistics.u4TxFailCount);
