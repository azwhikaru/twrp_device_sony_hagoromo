...
#if CFG_SUPPORT_BENTEN_TXPWR_V2
...
)
{
    ...
    sprintf(charBuf, "/etc/MT6625L_SingleSKU_%03d.dat",ucSingleNum);
	...
	if(IS_ERR(fd)) {
		 ...
		 {
			 printk("[SingleSKU(%d)] open file %s \n",(UINT_32)fd,charBuf);
			 fgFileExist = TRUE ;
		 }
	}
    else {
...
#define NVRAM_5G_TX_BANDEDGE_OFDM40_OFFSET 12
    ...
    /* 11. update Tx Power Offset For 5G */
    DBGLOG(INIT, INFO, ("CMD_ID_SET_5G_PWR_OFFSET \n"));
    CMD_5G_PWR_OFFSET_T rCmd5GChOffset;
    int i = 0;
    for(i = 0 ; i < 13 ; i++)
        rCmd5GChOffset.cTxPowerOffset5G[i] = prRegInfo->auc5GChOffset[i];
