...
extern unsigned int  g_u4AddrOffset;
extern UINT8 __iomem *pEmibaseaddr2;
extern UINT8 g_start;
...
*******************************************************************************
*/
#define MAX_STA_REC_LINK_SPEED_PEER	20
UINT_32 g_arStaRecLinkSpeed[MAX_STA_REC_LINK_SPEED_PEER] = { 0x00 };
    ...
    )
{
    ...
    *pu4QueryInfoLen = u4BssidListExLen;
    ...
    else if (prAdapter->rWifiVar.rConnSettings.fgIsScanReqIssued)
    ...
    *pu4SetInfoLen = sizeof(PARAM_CUSTOM_MCR_RW_STRUC_T);
    ...
    /* 0x9000 - 0x9EFF reserved for FW */
        ...
        if (prMcrWrInfo->u4McrOffset == 0x11111113){
            //DbgPrint("Enter test mode\n");
            extern int glInputReportKey(void);
            glInputReportKey();
            return WLAN_STATUS_SUCCESS;
        }
#endif 
...
#define CMD_WOWLAN_NORMAL   "NORMAL"
#define CMD_WOWLAN_MAGIC    "MAGIC"
    ...
    )
{
	...
	head = pvSetBuffer + kalStrLen(CMD_WOWLAN) + 1;
	kalMemSet(&rCmdWowlanParam, 0, sizeof(CMD_WOWLAN_PARAM_T));

	if (!kalStrnCmp(head, CMD_WOWLAN_NORMAL, kalStrLen(CMD_WOWLAN_NORMAL))) {
        rCmdWowlanParam.ucCmd = PM_WOWLAN_REQ_STOP;
    }
	else if (!kalStrnCmp(head, CMD_WOWLAN_MAGIC, kalStrLen(CMD_WOWLAN_MAGIC))) {
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to get RSSI from each or specific STA rec.
*	, then copy strings to upper layer, without querying FW
*
* \param[in]  prAdapter        Pointer to the Adapter structure.
* \param[out] pvQueryBuffer    A pointer to the buffer that holds the result of
*                              the query.
* \param[in]  u4QueryBufferLen N.A.
* \param[out] pu4QueryInfoLen  N.A.
*
* \retval WLAN_STATUS_SUCCESS
* \retval WLAN_STATUS_INVALID_DATA
*/
	...
	)
{
	...
	KAL_CMD_GET_CORE_T *prCmdContent;
	STA_RECORD_T *prStaRec;
	UINT8 ucMacZero[6];
	UINT8 i;
	PCHAR pucChar[50] = { 0x00 };
	PCHAR pucStr = NULL;
	...
	if (prCmdContent == NULL) {
		DBGLOG(REQ, WARN, ("Invalid data\n"));
		return WLAN_STATUS_INVALID_DATA;
	}

	pucStr = prCmdContent->pucStr;

	if (kalMemCmp(prCmdContent->aucPeerMac, ucMacZero, MAC_ADDR_LEN) == 0) {
		for (i = 0; i < CFG_STA_REC_NUM; i++) {
			prStaRec = &prAdapter->arStaRec[i];

			if (prStaRec->fgIsInUse &&
				prStaRec->ucNetTypeIndex == prCmdContent->ucNetTypeIndex) {
				sprintf (pucChar, ""MACSTR": %d dbm\n", 
					MAC2STR(prStaRec->aucMacAddr), RCPI_TO_dBm(prStaRec->ucRCPI));
				DBGLOG(INIT, INFO, ("%s", pucChar));

				kalMemCopy (pucStr, pucChar, strlen (pucChar) );
				*prCmdContent->pu4StrLen += strlen (pucChar);
				pucStr += strlen (pucChar);
			}
		}
	} else {
		prStaRec = cnmGetStaRecByAddress(prAdapter, prCmdContent->ucNetTypeIndex, prCmdContent->aucPeerMac);
		if (prStaRec != NULL) {
			sprintf (pucChar, ""MACSTR": %d dbm\n", 
				MAC2STR(prStaRec->aucMacAddr), RCPI_TO_dBm(prStaRec->ucRCPI));
			DBGLOG(INIT, INFO, ("%s", pucChar));
...
get_latched_link_speed()
{
	...
	//Hbyte:Sta record index
	//Lbyte : link speed 
	for (i = 0 ; i < MAX_STA_REC_LINK_SPEED_PEER ; i++) {
		g_arStaRecLinkSpeed[i] = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset+24+(i*4)));
	}
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to get link speed from each or specific STA rec.
*	, then copy strings to upper layer, without querying FW
*
* \param[in]  prAdapter        Pointer to the Adapter structure.
* \param[out] pvQueryBuffer    A pointer to the buffer that holds the result of
	...
	)
{
	...
	if (kalMemCmp(prCmdContent->aucPeerMac, ucMacZero, MAC_ADDR_LEN) == 0) {
		for (i = 0; i < CFG_STA_REC_NUM; i++) {
			...
			if (prStaRec->fgIsInUse) {
				for ( j = 0; j < MAX_STA_REC_LINK_SPEED_PEER; j++) {
					if(((UINT8)(g_arStaRecLinkSpeed[j] >> 16))
						== prStaRec->ucIndex){
						sprintf (pucChar, ""MACSTR": %d Mbps\n", 
							MAC2STR(prStaRec->aucMacAddr), (g_arStaRecLinkSpeed[j] & 0x0000ffff));
						DBGLOG(INIT, INFO, ("%s", pucChar));
