...
typedef enum _ENUM_CMD_ID_T {
    ...
    CMD_ID_SET_WOWLAN = 0x41,           /* 0x41 (Set) */ //CFG_SUPPORT_WOWLAN
    ...
    CMD_ID_SET_TX_PWR_V2    = 0xF4,  /*0xf4 (Set)*/
#endif
...
typedef enum _ENUM_EVENT_ID_T {
	...
	EVENT_ID_TDLS = 0x80,
    EVENT_ID_WOWLAN_NOTIFY = 0x2d,      /* 0x2d (Unsolicited) */
...
typedef struct _CMD_TX_PWR_V2_BY_CHANNEL_T {
	CMD_TX_PWR_T 			rTxPwrV2ByChannel;
	//AC_PWR_SETTING_STRUCT	rTxAcPwr;
	UINT_8					ucChannel;
}CMD_TX_PWR_V2_BY_CHANNEL_T, *P_CMD_TX_PWR_V2_BY_CHANNEL_T;

typedef struct _CMD_TX_PWR_V2_T {
	UINT_8		ucChannelNum;
	UINT_8 		aucPwrSetting[3000];  /*RLM_TX_PWR_BY_ALL_CHANNEL_T*/
}CMD_TX_PWR_V2_T,*P_CMD_TX_PWR_V2_T;

typedef struct _TX_PWR_T {
...
typedef UINT_8                                  PARAM_MAC_ADDR[6];

#define WOWLAN_ADDR_COUNT_MAX                   4
#define WOWLAN_DUPLICATE_MAC_ADDR_NO            16
