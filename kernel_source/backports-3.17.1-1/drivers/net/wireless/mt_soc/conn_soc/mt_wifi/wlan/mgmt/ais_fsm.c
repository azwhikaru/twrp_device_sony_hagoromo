    ...
    )
{
    ...
    prAisFsmInfo->fgIsDfsJoinProcess = FALSE;
    prAisFsmInfo->fgInformToDisableAp = FALSE;
	prAisFsmInfo->ucApDfsNewChan = 0;
    prAisFsmInfo->ucDfsChannelSwitchCount = 0;
    ...
    cnmTimerInitTimer(prAdapter,
            &prAisFsmInfo->rDfsJoinTimeoutTimer,
            (PFN_MGMT_TIMEOUT_FUNC)aisFsmRunEventDfsJoinTimeout,
            (UINT_32)NULL);
    ...
    )
{
    ...
    cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rDfsJoinTimeoutTimer);
    ...
    )
{
    ...
    do {
        ...
        case AIS_STATE_LOOKING_FOR:
            ...
            else if( (prAisFsmInfo->fgIsScanChanSepified == TRUE) && (prAisFsmInfo->ucNumScanChannelList > 0)) {
                UINT_32 u4Idx = 0;
                P_RF_CHANNEL_INFO_T prDomainInfo = (P_RF_CHANNEL_INFO_T)prAisFsmInfo->arScanChannelList;

                prScanReqMsg->eScanChannel      = SCAN_CHANNEL_SPECIFIED;

                if (prAisFsmInfo->ucNumScanChannelList > MAXIMUM_OPERATION_CHANNEL_LIST) {
                    prAisFsmInfo->ucNumScanChannelList = MAXIMUM_OPERATION_CHANNEL_LIST;
                }


                for (u4Idx = 0; u4Idx < prAisFsmInfo->ucNumScanChannelList; u4Idx++) {
                    prScanReqMsg->arChnlInfoList[u4Idx].ucChannelNum = prDomainInfo->ucChannelNum;
                    prScanReqMsg->arChnlInfoList[u4Idx].eBand = prDomainInfo->eBand;
                    prDomainInfo++;
                }

                prScanReqMsg->ucChannelListNum = prAisFsmInfo->ucNumScanChannelList;
    ...
    )
{
	...
	#if CFG_SUPPORT_ROAMING
					//Remove roaming function due to customer request
    ...
    )
{
	...
	UINT_8 uTimeout = AIS_DELAY_TIME_OF_DISCONNECT_SEC;
#endif
    ...
    if (!fgDelayIndication) {
        ...
        if (prAisFsmInfo->fgIsDfsJoinProcess == TRUE){
            uTimeout = prAisFsmInfo->ucDfsChannelSwitchCount;
        }

		cnmTimerStartTimer(prAdapter,
                &prAisFsmInfo->rIndicationOfDisconnectTimer,
                SEC_TO_MSEC(uTimeout));
#else
        cnmTimerStartTimer(prAdapter,
                &prAisFsmInfo->rIndicationOfDisconnectTimer,
                SEC_TO_MSEC(prConnSettings->ucDelayTimeOfDisconnectEvent));

#endif	/* CONFIG_MTK_BENTEN_SUPPORT */
    }
    ...
    )
{
    ...
    DEBUGFUNC("aisFsmRunEventDfsJoinTimeout()");

    prAisFsmInfo = &(prAdapter->rWifiVar.rAisFsmInfo);

	if (prAisFsmInfo->fgIsDfsJoinProcess == FALSE) {
    ...
    if (CHECK_FOR_TIMEOUT(rCurrentTime,
                          prAisFsmInfo->rDfsStartTime,
                          MSEC_TO_SYSTIME(60 * 1000))) {
        printk("DFS join channel=%d expired\n",  prAisFsmInfo->ucApDfsNewChan);
        prAisFsmInfo->fgIsDfsJoinProcess = FALSE;
        prAisFsmInfo->fgInformToDisableAp = FALSE;
		prAisFsmInfo->ucApDfsNewChan = 0;
        prAisFsmInfo->ucDfsChannelSwitchCount = 0;
    ...
    switch(prAisFsmInfo->eCurrentState) {
    case AIS_STATE_JOIN:
        DBGLOG(AIS, INFO, ("DFS_JOIN: AIS_STATE_JOIN\n"));
		cnmTimerStartTimer(prAdapter,
			&prAisFsmInfo->rDfsJoinTimeoutTimer,
			3 * MSEC_PER_SEC);
    ...
    case AIS_STATE_NORMAL_TR:
        DBGLOG(AIS, INFO, ("DFS_JOIN: AIS_STATE_NORMAL_TR\n"));
		prAisFsmInfo->fgIsDfsJoinProcess = FALSE;
        prAisFsmInfo->fgInformToDisableAp = FALSE;
		prAisFsmInfo->ucApDfsNewChan = 0;
        prAisFsmInfo->ucDfsChannelSwitchCount = 0;
    ...
    default:
        DBGLOG(AIS, INFO, ("DFS_JOIN: AIS STATE=%d\n", prAisFsmInfo->eCurrentState));
		cnmTimerStartTimer(prAdapter,
			&prAisFsmInfo->rDfsJoinTimeoutTimer,
			3 * MSEC_PER_SEC);
        break;
    ...
    )
{
    ...
    if(prAisFsmInfo->eCurrentState == AIS_STATE_DISCONNECTING) {
        if(rTxDoneStatus != TX_RESULT_DROPPED_IN_DRIVER) {
           if (prAisFsmInfo->fgIsDfsJoinProcess == TRUE){
               aisFsmStateAbort(prAdapter, DISCONNECT_REASON_CODE_NEW_CONNECTION, TRUE);
           } else {
