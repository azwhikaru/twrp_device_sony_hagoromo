...
********************************************************************************
*/
#if CFG_ENABLE_SONY_CHANNEL_TABLE
    ...
    )
{
    ...
    if ((eNetTypeIdx == NETWORK_TYPE_AIS_INDEX)&&(prAisFsmInfo->prTargetBssDesc  != NULL)){
		if((prRegInfo->ucFeatureEnable == 1) && (prAisFsmInfo->prTargetBssDesc->eBand == BAND_2G4)){
#if CFG_ENABLE_SONY_CHANNEL_TABLE_DEBUG
           Channel = prAisFsmInfo->prTargetBssDesc->ucChannelNum;
		   DBGLOG(CNM, INFO, ("@@@ Channel #0 HT20\n",	Channel));
#endif
			DBGLOG(CNM, INFO, ("STA #0 Band(%d) FeatureEn(%d) BW40Permit(%d))\n",
...
#if CFG_ENABLE_SONY_CHANNEL_TABLE
		else if((prRegInfo->ucFeatureEnable == 1) && (prAisFsmInfo->prTargetBssDesc->eBand == BAND_5G)){

			Channel = prAisFsmInfo->prTargetBssDesc->ucChannelNum;
		#if CFG_ENABLE_SONY_CHANNEL_TABLE_DEBUG
			DBGLOG(CNM, INFO, ("@@@ Channel #1 = (%d) table3 reserveded=%d  table4 reserveded=%d \n",
			Channel,prDomainInfo->rSubBand[3].ucReserved, prDomainInfo->rSubBand[4].ucReserved));
		#endif
			...
			else if ((Channel == 149)|| (Channel == 153) || (Channel == 157) || (Channel == 161) || (Channel == 165))
			{
               // HT20
			...
			{
         #if CFG_ENABLE_SONY_CHANNEL_TABLE_DEBUG
					DBGLOG(CNM, INFO, ("@@@ Channel #2 =%d  HT40\n", Channel));
         #endif
			}
...
#endif
		else if((prRegInfo->ucFeatureEnable == 2) && (prAisFsmInfo->prTargetBssDesc->eBand == BAND_5G)){

		    #if 1
			DBGLOG(CNM, INFO, ("STA #1 Band(%d) FeatureEn(%d) BW40Permit(%d))\n",
			prAisFsmInfo->prTargetBssDesc->eBand,
			prRegInfo->ucFeatureEnable,
			FALSE));
			#endif
	...
	#if CFG_ENABLE_SONY_CHANNEL_TABLE
	    prBssInfo = &prAdapter->rWifiVar.arBssInfo[eNetTypeIdx];
		if (prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT)
		{
		   Channel = prBssInfo->ucPrimaryChannel;
		   if((prRegInfo->ucFeatureEnable == 1) && (prBssInfo->eBand == BAND_2G4)){
           #if CFG_ENABLE_SONY_CHANNEL_TABLE_DEBUG
			   DBGLOG(CNM, INFO, ("@@@ Channel #0=%d HT20\n",	Channel));
           #endif
				DBGLOG(CNM, INFO, ("AP #0 Band(%d) FeatureEn(%d) BW40Permit(%d))\n",
				prBssInfo->eBand,
				prRegInfo->ucFeatureEnable,
				FALSE));
			...
			else if((prRegInfo->ucFeatureEnable == 1) && (prBssInfo->eBand == BAND_5G)){
			...
			else if((prRegInfo->ucFeatureEnable == 2) && (prBssInfo->eBand == BAND_5G)){

			    #if 1
				DBGLOG(CNM, INFO, ("AP #1 Band(%d) FeatureEn(%d) BW40Permit(%d))\n",
				prBssInfo->eBand,
				prRegInfo->ucFeatureEnable,
				FALSE));
				#endif
            ...
            else if(prRegInfo->ucFeatureEnable == 3){

				DBGLOG(CNM, INFO, ("AP with HT20 Band(%d) FeatureEn(%d) BW40Permit(%d))\n",
				prBssInfo->eBand,
				prRegInfo->ucFeatureEnable,
				FALSE));
