        ...
        )
{
    ...
    if(prChannelSwitchIE->ucChannelSwitchMode == 1){
        ...
        if (prAisFsmInfo->fgInformToDisableAp == FALSE){
            if (prAdapter->fgIsP2PRegistered &&  prAdapter->rWifiVar.prP2pFsmInfo) {
                if (prAdapter->rWifiVar.prP2pFsmInfo->fgIsApMode == 1) {
                    kalIndicateRadarDetect(prAdapter, prChannelSwitchIE->ucNewChannelNum);
                }
            }
            prAisFsmInfo->fgInformToDisableAp = TRUE;
        }
#endif
	//Send Deauth or switch channel immediatly
#if 0
        if (prChannelSwitchIE->ucChannelSwitchCount >= 2)
            return;
#endif

        //Disconnect in DFS channel
        if ((prChannelSwitchIE->ucNewChannelNum >= 52) && (prChannelSwitchIE->ucNewChannelNum <= 144)) {
            P_MSG_AIS_ABORT_T prAisAbortMsg;

			if(prAisFsmInfo->fgIsDfsJoinProcess == TRUE) {
				printk("[5G DFS] Already in DFS Join process... \n");
				return;
			}
            ...
            mboxSendMsg(prAdapter,
                    ...
                    MSG_SEND_METHOD_BUF);
			//Add timer to check the new connection
			GET_CURRENT_SYSTIME(&prAisFsmInfo->rDfsStartTime);
			prAisFsmInfo->fgIsDfsJoinProcess = TRUE;
			prAisFsmInfo->ucApDfsNewChan = prChannelSwitchIE->ucNewChannelNum;
            prAisFsmInfo->ucDfsChannelSwitchCount = prChannelSwitchIE->ucChannelSwitchCount;
			printk("ucApDfsNewChan=0x%08x(%08p) \n", prAisFsmInfo->ucApDfsNewChan, &prAisFsmInfo->ucApDfsNewChan);
            printk("ucDfsChannelSwitchCount=0x%08x(%08p) \n", prAisFsmInfo->ucDfsChannelSwitchCount, &prAisFsmInfo->ucDfsChannelSwitchCount);
			prAisBssInfo = &(prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX]);
			COPY_MAC_ADDR(prAisFsmInfo->aucDfsBSSID, prAisBssInfo->aucBSSID);
