    	...
    	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
			...
			if (prAdapter->fgIsP2PRegistered &&  prAdapter->rWifiVar.prP2pFsmInfo) {
				if (prAdapter->rWifiVar.prP2pFsmInfo->fgIsApMode == 1 ||
					...
					prGetChnLoad=(P_PARAM_GET_CHN_LOAD)&(prAdapter->rWifiVar.rChnLoadInfo);
					for (u4Idx = 0; u4Idx < prGetChnLoad->u4ScanChnNum; u4Idx++) {
						if(prGetChnLoad->rScanChnLoad[u4Idx].ucChannel == prBssDesc->ucChannelNum)
							prGetChnLoad->rScanChnLoad[u4Idx].u2APNum++;
						else 
							continue;
					}
...
#endif
#if (CONFIG_MTK_BENTEN_SUPPORT == 0)
...
//						prBssDesc->u2RawLength=0;
...
#if CFG_AUTO_CHANNEL_SEL_SUPPORT
		{
			...
			for (ucIdx = 0; ucIdx < 11; ucIdx++) {
				...
				DBGLOG(SCN, TRACE, ("Report Channel[%d] AP number=%d\n", ucIdx + 1, prChnLoad->u2APNum));
			}
			...
			if (prAdapter->rWifiVar.prP2pFsmInfo->fgIsApMode == 1 ||
				...
				for (u4Idx = 0; u4Idx < prGetChnLoad->u4ScanChnNum; u4Idx++) {
					...
					DBGLOG(SCN, TRACE, ("Report Scan Channel[%d] AP number=%d\n", 
						prScanChnLoad->ucChannel, prScanChnLoad->u2APNum));
				}
			}
#endif
