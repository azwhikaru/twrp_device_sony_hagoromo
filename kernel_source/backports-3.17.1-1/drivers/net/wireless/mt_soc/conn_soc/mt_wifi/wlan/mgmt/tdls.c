...
static void TdlsCmdInfoDisplay(GLUE_INFO_T *prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen, UINT_8 *prOutBuf, PUINT_32 pu4OutBufLen);
...
TdlsTestTdlsFrameSend(ADAPTER_T *prAdapter, VOID *pvSetBuffer, UINT_32 u4SetBufferLen, UINT_32 *pu4SetInfoLen)
{
	...
	TdlsexCfg80211TdlsMgmt(prWdev->wiphy, NULL, prCmd->arRspAddr, prCmd->ucFmeType, 1, 0, 0, 0, 
				NULL,	/* open/none */
				0);
...
*/
/*----------------------------------------------------------------------------*/
static void TdlsCmdInfoDisplay(GLUE_INFO_T *prGlueInfo, PUINT_8 prInBuf, UINT_32 u4InBufLen, PUINT_8 prOutBuf, PUINT_32 pu4OutBufLen)
{
	WLAN_STATUS rStatus;
	TDLS_CMD_CORE_T rCmd;
	...
	if (prOutBuf != NULL && pu4OutBufLen != NULL ) {
		rCmd.Content.rCmdInfoDisplay.fgIsRead = TRUE;
		rCmd.Content.rCmdInfoDisplay.pucStr = prOutBuf;
		rCmd.Content.rCmdInfoDisplay.pu4StrLen = pu4OutBufLen;
	} else {
		rCmd.Content.rCmdInfoDisplay.fgIsRead = FALSE;
	}
	...
	/* command to do this */
	rStatus = kalIoctl(prGlueInfo, TdlsInfoDisplay, &rCmd, sizeof(rCmd), FALSE, FALSE, FALSE, FALSE, &u4InBufLen);

	/*
	* DBGLOG(TDLS, INFO, ("<tdls_cmd> %s: prOutBuf:%s pu4OutBufLen:%d\n",
	*		    __func__, prOutBuf, *pu4OutBufLen));
	*/
...
*
* \param[in] prAdapter		Pointer to the Adapter structure
* \param[in] prLink		A pointer to the TDLS link
* \param[in] u4HisIdx		The index of the link
* \param[out] prInfoDisplay	If the call is successful, returns the number of
*	bytes read from the set buffer. If the call failed due to invalid length of
*	the set buffer, returns the amount of storage needed.
*/
/*----------------------------------------------------------------------------*/
...
TdlsInfoPrint(
	...
	P_TDLS_INFO_LINK_T					prLink,
	UINT32								u4HisIdx,
	TDLS_CMD_CORE_INFO_DISPLAY_T		*prInfoDisplay
	)
{
	PCHAR pucChar[200] = { 0x00 };
	PCHAR pucStr = NULL;
	BOOLEAN fgIsRead = FALSE;

	if (prStaRec == NULL) {
		DBGLOG(TDLS, TRACE, ("prStaRec == NULL\n"));
		return;
	}

	if (prLink == NULL) {
		DBGLOG(TDLS, TRACE, ("prLink == NULL\n"));
		return;
	}

	if (prInfoDisplay != NULL) {
		fgIsRead = prInfoDisplay->fgIsRead;
		pucStr = prInfoDisplay->pucStr;
	}

	sprintf (pucChar, "\t\t%d. "MACSTR": jiffies start(%lu %ums) jiffies end(%lu %ums) "
	...
	DBGLOG(TDLS, TRACE, ("%s", pucChar));
	if (fgIsRead == TRUE) {
		kalMemCopy (pucStr, pucChar, strlen (pucChar) );
		*prInfoDisplay->pu4StrLen += strlen (pucChar);
		pucStr += strlen (pucChar);
	}
	...
	sprintf (pucChar, "\t\t\t State %d, PM %d, Cap 0x%x RSSI :%d\n",
		prStaRec->ucStaState, prStaRec->fgIsInPS, prStaRec->u2CapInfo, RCPI_TO_dBm(prStaRec->ucRCPI));
	DBGLOG(TDLS, TRACE, ("%s", pucChar));
	if (fgIsRead == TRUE) {
		kalMemCopy (pucStr, pucChar, strlen (pucChar) );
		*prInfoDisplay->pu4StrLen += strlen (pucChar);
	...
	sprintf (pucChar, "\t\t\t SetupDisable %d, ChSwDisable %d\n",
	...
	)
{
	...
	TDLS_CMD_CORE_INFO_DISPLAY_T *prInfoDisplay = NULL;;
	...
	/* display specified link information */
	if (prCmdContent != NULL) {
		if (kalMemCmp(prCmdContent->aucPeerMac, ucMacZero, 6) != 0) {
			prLink = TdlsGetLinkByAddress(prGlueInfo, prCmdContent->aucPeerMac);
			ucNetTypeIndex = prCmdContent->ucNetTypeIndex;
			if (prLink != NULL)
				fgIsListAll = FALSE;
		}

		prInfoDisplay = &(prCmdContent->Content.rCmdInfoDisplay);
	}

	if (fgIsListAll == FALSE) {
		prStaRec = cnmGetStaRecByAddress(prAdapter,
			ucNetTypeIndex, prLink->aucPeerMac);

		TdlsInfoPrint(prStaRec, prLink, 0, prInfoDisplay);
	}
	else {
		for(u4HisIdx = 0;
			...
			u4HisIdx++)
		{
			...
			if (EQUAL_MAC_ADDR(prLink->aucPeerMac,ucMacZero) || prLink->fgIsInUse == FALSE)
				continue; /* skip all zero */

			prStaRec = cnmGetStaRecByAddress(prAdapter,
				ucNetTypeIndex, prLink->aucPeerMac);

			TdlsInfoPrint(prStaRec, prLink, u4HisIdx, prInfoDisplay);
		}
	...
	{
		DBGLOG(TDLS, TRACE, ("Clear all TDLS link history\n"));
	...
	)
{
	...
	prLink = TdlsGetLinkByAddress(prGlueInfo, pucPeerMac);
	...
	if (prLink == NULL) {
		...
		DBGLOG(TDLS, INFO, ("<tdls_evt> %s: Line:%d\n", __FUNCTION__, __LINE__));
		prLink = TdlsLinkAlloc(prGlueInfo);
	}

	if (prLink == NULL) {
		/* shall not be here */
		DBGLOG(TDLS, ERROR,
			("<tdls_cmd> %s: alloc prLink fail!\n", __FUNCTION__));
		return;
	} else {
		DBGLOG(TDLS, INFO,
			("<tdls_evt> %s: record history for "MACSTR" %d; test "MACSTR"\n",
			__FUNCTION__, MAC2STR(prLink->aucPeerMac), 
			prGlueInfo->rTdlsLink.u4LinkIdx, MAC2STR(&prLink->aucPeerMac)));
	}
	...
	if (fgIsTearDown == TRUE)
	{
		...
		prLink->fgIsInUse = FALSE;
		kalMemZero(prLink->aucPeerMac, MAC_ADDR_LEN);

		DBGLOG(TDLS, INFO, ("<tdls_cfg> %s: Clear the link by TearDown frame\n",
			__FUNCTION__));
	...
	else
	{
		...
		DBGLOG(TDLS, INFO, ("<tdls_cfg> %s: prGlueInfo->rTdlsLink.u4LinkIdx:%d\n",
			__FUNCTION__, prGlueInfo->rTdlsLink.u4LinkIdx));
		...
		DBGLOG(TDLS, INFO,
			("<tdls_evt> %s: Check addr "MACSTR"; test "MACSTR"\n",
			__FUNCTION__, MAC2STR(prLink->aucPeerMac), MAC2STR(&prLink->aucPeerMac)));
			    ...
			    UINT8 *pucPeerMac, TDLS_EVENT_HOST_SUBID_SPECIFIC_FRAME eFmeStatus, VOID *pInfo)
{
	...
	DBGLOG(TDLS, INFO,
		...
		__FUNCTION__, MAC2STR(pucPeerMac), *(UINT32 *)pInfo,
		eFmeStatus));
	...
	if (prLink == NULL) {
		/* error! can not find the link entry */
		DBGLOG(TDLS, INFO,
			("<tdls_evt> %s Line:%d: cannot find the same entry!!!\n",
			__FUNCTION__, __LINE__));
		return;
	}
...
*/
/*----------------------------------------------------------------------------*/
int TdlsexCfg80211TdlsOper(struct wiphy *wiphy, struct net_device *dev, const u8 *peer, enum nl80211_tdls_operation oper)
{
	ADAPTER_T *prAdapter;
	GLUE_INFO_T *prGlueInfo;
	WLAN_STATUS rStatus;
...
VOID TdlsexCmd(P_GLUE_INFO_T prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	case TDLS_CMD_INFO:
		...
		TdlsCmdInfoDisplay(prGlueInfo, prInBuf, u4InBufLen, NULL, NULL);

		DBGLOG(TDLS, INFO, ("<tdls_cmd> %s: prInBuf:%s u4InBufLen:%d\n",
					__func__, prInBuf, u4InBufLen));
...
/*----------------------------------------------------------------------------*/
/*! \brief  This routine is called to send a command to TDLS module and Get.
*
* \param[in] prGlueInfo		Pointer to the Adapter structure
* \param[in/out] prInBuf		A pointer to the command string buffer and for the output
* \param[in/out] u4InBufLen	The length of the buffer and for the output
* \param[out] None
*
* \retval None
*/
/*----------------------------------------------------------------------------*/
VOID TdlsexCmdGet(P_GLUE_INFO_T prGlueInfo, PUINT_8 prInBuf, UINT_32 u4InBufLen, PUINT_8 prOutBuf, PUINT_32 pu4OutBufLen)
{
	UINT_32 u4Subcmd;
	...
	case TDLS_CMD_INFO:
		/* display all TDLS information */
		TdlsCmdInfoDisplay(prGlueInfo, prInBuf, u4InBufLen, prOutBuf, pu4OutBufLen);
...
TDLS_STATUS TdlsexLinkCtrl(ADAPTER_T *prAdapter, VOID *pvSetBuffer, UINT_32 u4SetBufferLen, UINT_32 *pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(TDLS_CMD_LINK_T);
	...
	if (prCmd->fgIsEnabled == TRUE) {
		...
		prStaRec->ucRCPI = prBssInfo->prStaRecOfAP->ucRCPI;
	...
	*pu4SetInfoLen = sizeof(TDLS_MGMT_TX_INFO);
	...
	if (prStaRec != NULL) {
		...
		if (prMgmtTxInfo->ucActionCode == TDLS_FRM_ACTION_TEARDOWN) {
			...
			if(TdlsGetLinkByAddress(prGlueInfo, prMgmtTxInfo->aucPeer) == NULL
			   && prMgmtTxInfo->u2StatusCode == TDLS_REASON_CODE_MTK_DIS_BY_US_DUE_TO_RX_TEARDOWN) {
				DBGLOG(TDLS, ERROR,
					("<tdls_evt> %s: bypass teardown throw back\n",
					__func__));
				return TDLS_STATUS_SUCCESS;
			} else {
...
/* ASSERT(prStaRec->ucPhyTypeSet & PHY_TYPE_SET_802_11N); */
...
#if 1		/* TdlsexPeerAdd() will be called before we receive setup rsp in TdlsexRxFrameHandle() */
	...
	if (kalMemCmp(prGlueInfo->aucTdlsHtPeerMac, prCmd->aucPeerMac, 6) == 0) {
		...
		prStaRec->fgSupMcs32 = (prStaRec->rTdlsHtCap.rSupMcsSet.aucRxMcsBitmask[32 / 8] & BIT(0)) ? TRUE : FALSE;
		prStaRec->ucMcsSet = prStaRec->rTdlsHtCap.rSupMcsSet.aucRxMcsBitmask[0];
		prStaRec->u2HtCapInfo = prStaRec->rTdlsHtCap.u2HtCapInfo;
		prStaRec->ucAmpduParam = prStaRec->rTdlsHtCap.ucAmpduParam;
		prStaRec->u2HtExtendedCap = prStaRec->rTdlsHtCap.u2HtExtendedCap;
		prStaRec->u4TxBeamformingCap = prStaRec->rTdlsHtCap.u4TxBeamformingCap;
		prStaRec->ucAselCap = prStaRec->rTdlsHtCap.ucAselCap;
...
BOOLEAN TdlsexRxFrameDrop(GLUE_INFO_T *prGlueInfo, UINT_8 *pPkt)
{
	...
	if (ucActionId == TDLS_FRM_ACTION_TEARDOWN) {
		 ...
		 */
		cfg80211_tdls_oper_request(prGlueInfo->prDevHandler,
					   pPkt + 6, NL80211_TDLS_TEARDOWN,
			                   TDLS_REASON_CODE_MTK_DIS_BY_US_DUE_TO_RX_TEARDOWN , GFP_ATOMIC);
	}
...
/*----------------------------------------------------------------------------*/
/*!
* @brief his routine is handle deauth from TDLS peer
*
* \param[in] prAdapter                 Pointer to the Adapter structure
...
*/
/*----------------------------------------------------------------------------*/
VOID TdlsexRxEventDeauth(ADAPTER_T *prAdapter, P_STA_RECORD_T prStaRec)
{
       UINT16 u2ReasonCode = TDLS_REASON_CODE_MTK_DIS_BY_US_DUE_TO_RX_DEAUTH;

       /* sanity check */
       if (prStaRec == NULL)
               return;

       TdlsLinkHistoryRecord(prAdapter->prGlueInfo, TRUE, prStaRec->aucMacAddr, TRUE,
               u2ReasonCode, NULL);
       /* 16 Nov 21:49 2012 http://permalink.gmane.org/gmane.linux.kernel.wireless.general/99712 */
       cfg80211_tdls_oper_request(prAdapter->prGlueInfo->prDevHandler,
               prStaRec->aucMacAddr, NL80211_TDLS_TEARDOWN,
               u2ReasonCode, GFP_ATOMIC);
