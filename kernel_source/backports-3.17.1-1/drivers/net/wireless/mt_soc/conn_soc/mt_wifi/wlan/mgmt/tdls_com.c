...
UINT_32 TdlsFrameGeneralIeAppend(ADAPTER_T *prAdapter, STA_RECORD_T *prStaRec, UINT_16 u2StatusCode, UINT_8 *pPkt)
{
	...
	u2SupportedRateSet = rNonHTPhyAttributes[PHY_TYPE_ERP_INDEX].u2SupportedRateSet;

	rateGetDataRatesFromRateSet(u2SupportedRateSet,
				    u2SupportedRateSet, aucAllSupportedRates, &ucAllSupportedRatesLen);
...
#if HT_20_40_COEX_ALIGN_BSS
		fg40mAllowed = prBssInfo->fg40mBwAllowed;
#else
        fg40mAllowed = cnmBss40mBwPermitted(prAdapter, prBssInfo->ucNetTypeIndex);
#endif /*HT_20_40_COEX_ALIGN_BSS */
...
/* pPktTemp = pPkt; */
	...
	*pPkt = ucActionCode;
	...
	/* Fill elements */
	if (ucActionCode != TDLS_FRM_ACTION_TEARDOWN && ucActionCode != TDLS_FRM_ACTION_DISCOVERY_REQ) {
		/*
		   Capability
	...
	#if DISABLE_TDLS_CHANNEL_SWITCH
	        //SONY request to disable TDLS channel switch capability
	#else
...
/* if (prGlueInfo->rTdlsLink.fgIs2040Sup == TRUE) */ /* force to enable */
	if (ucActionCode != TDLS_FRM_ACTION_TEARDOWN &&
		...
		ucActionCode != TDLS_FRM_ACTION_CONFIRM) {
		/*
...
/* LR_TDLS_FME_FIELD_FILL(u4IeLen); */
	if (ucActionCode == TDLS_FRM_ACTION_CONFIRM &&
		...
		u4IeLen = rlmTdlsFrameFillHtOpIE (prAdapter, prBssInfo, pPkt);
		LR_TDLS_FME_FIELD_FILL(u4IeLen);
	}
...
/* if ((ucActionCode != TDLS_FRM_ACTION_TEARDOWN) && (pAppendIe != NULL)) */
	...
	if (ucActionCode != TDLS_FRM_ACTION_TEARDOWN && ucActionCode != TDLS_FRM_ACTION_DISCOVERY_REQ) {
		...
		u4IeLen = rlmDomainSupOperatingClassIeFill(pPkt, ucActionCode);
		LR_TDLS_FME_FIELD_FILL(u4IeLen);
	}
    ...
    )
{
    ...
    for (i = 0; i < TDLS_LINK_HISTORY_MAX; i++) {
		prLink = &prGlueInfo->rTdlsLink.rLinkHistory[i];

        if (prLink->fgIsInUse &&
            EQUAL_MAC_ADDR(prLink->aucPeerMac, pucPeerMacAddr)) {
            break;
        }
    }

    return (i < TDLS_LINK_HISTORY_MAX) ? prLink : NULL;
}
    ...
    )
{
    ...
    for (i = 0; i < TDLS_LINK_HISTORY_MAX; i++) {
        ...
        if (!prLink->fgIsInUse) {
            prLink->fgIsInUse = TRUE;
			prGlueInfo->rTdlsLink.u4LinkIdx = i;
			break;
     	}
    }
