...
/* fgIsTDLSlinkEnable = 0; */
	...
	prStaRec = cnmGetStaRecByAddress(prAdapter, (UINT_8) NETWORK_TYPE_AIS_INDEX, mac);
    ...
    )
{
    ...
    if (sme->bssid) {
        DBGLOG(INIT, TRACE, ("%s: bssid "MACSTR"\n", __func__, MAC2STR(sme->bssid)));
        COPY_MAC_ADDR(prGlueInfo->prAdapter->rWifiVar.rConnSettings.aucBSSID,
                      sme->bssid);
		prGlueInfo->prAdapter->rWifiVar.rConnSettings.fgIsConnByBssidIssued =
			TRUE;
    }
    else {
        DBGLOG(INIT, TRACE, ("%s: no bssid\n", __func__));
        prGlueInfo->prAdapter->rWifiVar.rConnSettings.fgIsConnByBssidIssued =
			FALSE;
    }
...
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
    ...
    UINT32 AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_NUM];
	...
	PARAM_PREFER_CHN_INFO_EXT rPreferChanExt[2];
...
*/
	...
	if (fgIsReady == FALSE)
		...
		//4 2. Get current domain channel list
            ...
            aucChannelList);
#endif		
		//4 3. Calculate drity score based on AP number							 
		if (prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.rScanChnLoad[0].u2Freq <= 2483){	
		...
		/*Enhance Calculation of Each Channel Direty Score for 2.4G or 5G*/
		if (prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo->fgIsApMode == 1) {
			P_PARAM_GET_CHN_LOAD  prGetChnLoad;
			P_PARAM_SCAN_CHN_LOAD_INFO prScanChnLoad, prOffsetChnLoad;
			UINT_32 u4Idx = 0, u4LeftIdx = 0, u4RightIdx = 0;
			UINT_16 u2FreqOffset, u2ChanWeighting;
		    //Give weighting to adacent channel
			prGetChnLoad=(P_PARAM_GET_CHN_LOAD)&(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);		
			for (u4Idx = 0; u4Idx < prGetChnLoad->u4ScanChnNum; u4Idx++) {
				prScanChnLoad = &prGetChnLoad->rScanChnLoad[u4Idx];
				prScanChnLoad->u2DirtyScore += prScanChnLoad->u2APNum * CHN_DIRTY_WEIGHT_UPPERBOUND;
				//Give weighting to adacent left channel
				if (u4Idx < CHN_DIRTY_WEIGHT_UPPERBOUND)
					u4LeftIdx = 0;
				else 
					u4LeftIdx = u4Idx - CHN_DIRTY_WEIGHT_UPPERBOUND + 1;
				
				while (u4LeftIdx < u4Idx) {
					prOffsetChnLoad = &prGetChnLoad->rScanChnLoad[u4LeftIdx];
					u2FreqOffset = prScanChnLoad->u2Freq - prOffsetChnLoad->u2Freq;
					// Assume use 20Mhz bandwidth
					if (u2FreqOffset < 20) {
						u2ChanWeighting = CHN_DIRTY_WEIGHT_UPPERBOUND - u2FreqOffset / 5;
						//DBGLOG(SCN, TRACE, ("        Left Channel Index=%d, Weighting=%d\n", u4LeftIdx, u2ChanWeighting)); 
						prOffsetChnLoad->u2DirtyScore += prScanChnLoad->u2APNum * u2ChanWeighting;
					}
					u4LeftIdx++;
				}
				
				//Give weighting to adacent right channel
				u4RightIdx = u4Idx + CHN_DIRTY_WEIGHT_UPPERBOUND - 1;
				if (u4RightIdx > (prGetChnLoad->u4ScanChnNum - 1))
					u4RightIdx = prGetChnLoad->u4ScanChnNum - 1;
				
				while (u4RightIdx > u4Idx) {
					prOffsetChnLoad = &prGetChnLoad->rScanChnLoad[u4RightIdx];
					u2FreqOffset = prOffsetChnLoad->u2Freq - prScanChnLoad->u2Freq;
					// Assume use 20Mhz bandwidth
					if (u2FreqOffset < 20) {
						u2ChanWeighting = CHN_DIRTY_WEIGHT_UPPERBOUND - u2FreqOffset / 5;
						//DBGLOG(SCN, TRACE, ("        Right Channel Index=%d, Weighting=%d\n", u4RightIdx, u2ChanWeighting));
						prOffsetChnLoad->u2DirtyScore += prScanChnLoad->u2APNum * u2ChanWeighting;
					}
					u4RightIdx--;
				}			
			...
			for (u4Idx = 0; u4Idx < prGetChnLoad->u4ScanChnNum; u4Idx++) {
				prScanChnLoad = &prGetChnLoad->rScanChnLoad[u4Idx];		
				DBGLOG(SCN, TRACE, ("[Auto Channel Ext]chn=%d num=%d score=%d\n", 
				prScanChnLoad->ucChannel, prScanChnLoad->u2APNum, prScanChnLoad->u2DirtyScore));
			}
		}

		//4 4. Find out the best channel to report
		//u4LteSafeChnBitMask_2_4G=rQueryLTEChn.rLteSafeChnList.au4SafeChannelBitmask[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1-1];			
		AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1-1]   = 0;
		AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34-1]	= 0;
		AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_100-1] = 0;
		AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_149-1] = 0;
		AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_184-1] = 0;
		...
		if (prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.rScanChnLoad[0].u2Freq <= 2483){
			...
			for(ucIdx=ucDefaultIdx;ucIdx< ucMax_24G_Chn_List;ucIdx++) {			
				...
				//Skip Passive channel
				fgIsPassiveCh = FALSE;
				for(ucSupChIdx = 0; ucSupChIdx < NumOfChannel; ucSupChIdx++) {
					if (prChnLoad->ucChannel== aucChannelList[ucSupChIdx].ucChannelNum) {
						if (aucChannelList[ucSupChIdx].fgIsPassiveCh) {
							DBGLOG(P2P, INFO,("[Auto Channel]Found Passive chn=%d\n", prChnLoad->ucChannel));
							fgIsPassiveCh = TRUE;
						}
						break;
					}
				...
				if (fgIsPassiveCh) {
					DBGLOG(P2P, INFO,("[Auto Channel]Skip Passive chn=%d\n", prChnLoad->ucChannel));
					continue;
				}					
...
#if 0			
//Enhance Auto Channel Selection verified
			{
				P_PARAM_GET_CHN_LOAD  prGetChnLoad;
				P_PARAM_SCAN_CHN_LOAD_INFO prScanChnLoad;
	
				prGetChnLoad=(P_PARAM_GET_CHN_LOAD)&(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);
				kalMemZero(rPreferChanExt, sizeof(rPreferChanExt));
				rPreferChanExt[0].u2APNumScore=0xFFFF;
				rPreferChanExt[1].u2APNumScore=0xFFFF;
				for (ucIdx = 0; ucIdx < prGetChnLoad->u4ScanChnNum; ucIdx++) {
					prScanChnLoad = &prGetChnLoad->rScanChnLoad[ucIdx]; 
					
					if (rPreferChanExt[0].u2APNumScore >= prScanChnLoad->u2DirtyScore) {
						//Save lower priority
						rPreferChanExt[1].u2APNumScore = rPreferChanExt[0].u2APNumScore;
						rPreferChanExt[1].ucChannel = rPreferChanExt[0].ucChannel;
						rPreferChanExt[1].u4Freq = rPreferChanExt[0].u4Freq;
						//Keep first priority
						rPreferChanExt[0].u2APNumScore = prScanChnLoad->u2DirtyScore;
						rPreferChanExt[0].ucChannel = prScanChnLoad->ucChannel;
						rPreferChanExt[0].u4Freq = prScanChnLoad->u2Freq;
					} else {
						//Keep second priority
						if (rPreferChanExt[1].u2APNumScore >= prScanChnLoad->u2DirtyScore || fgIsFistRecord == TRUE) {
							fgIsFistRecord=FALSE;
							rPreferChanExt[1].u2APNumScore = prScanChnLoad->u2DirtyScore;
							rPreferChanExt[1].ucChannel = prScanChnLoad->ucChannel;
							rPreferChanExt[1].u4Freq = prScanChnLoad->u2Freq;
	
						}
					}
				}
				DBGLOG(P2P, INFO,("[Auto Channel]Legacy BEST=CH[%d], SECOND=[%d]\n", rPreferChannels[0].ucChannel, rPreferChannels[1].ucChannel));
			}			
//Enhance Auto Channel Selection verified End
#endif            
			...
			//Enhance to support 5G Channel Selection
			P_PARAM_GET_CHN_LOAD  prGetChnLoad;
			P_PARAM_SCAN_CHN_LOAD_INFO prScanChnLoad;
			...
			for (ucIdx = 0; ucIdx < prGetChnLoad->u4ScanChnNum; ucIdx++) {
				...
				fgIsPassiveCh = FALSE;
				for(ucSupChIdx = 0; ucSupChIdx < NumOfChannel; ucSupChIdx++) {
					if (prScanChnLoad->ucChannel == aucChannelList[ucSupChIdx].ucChannelNum) {
						if (aucChannelList[ucSupChIdx].fgIsPassiveCh) {
							DBGLOG(P2P, INFO,("[Auto Channel]Found Passive chn=%d\n", prScanChnLoad->ucChannel));
							fgIsPassiveCh = TRUE;
						}
						break;
					}
				...
				if (rPreferChanExt[0].u2APNumScore >= prScanChnLoad->u2DirtyScore) {
					...
					rPreferChanExt[0].ucChannel = prScanChnLoad->ucChannel;
					rPreferChanExt[0].u4Freq = prScanChnLoad->u2Freq;
					if (rPreferChanExt[1].u2APNumScore == 0) {
						break;
					}
				}
				else {
			...
			DBGLOG(P2P, INFO,("[Auto Channel]Enhance BEST=CH[%d], SECOND=[%d]\n", rPreferChanExt[0].ucChannel, rPreferChanExt[1].ucChannel));
			//AcsChnRepot[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1-1]	=BITS((rQueryLTEChn.rLteSafeChnList.ucChannelLow-1),(rQueryLTEChn.rLteSafeChnList.ucChannelHigh-1));			
			AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34-1]= fgIsReady?BIT(31):0;
			if (rPreferChanExt[0].ucChannel >= 34 && rPreferChanExt[0].ucChannel < 100) {
				AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34-1] |= BIT((rPreferChanExt[0].ucChannel - 34)/2);
			} else if (rPreferChanExt[0].ucChannel >= 100 && rPreferChanExt[0].ucChannel < 149) {
				AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_100-1] = BIT((rPreferChanExt[0].ucChannel - 100)/2);
			} else if (rPreferChanExt[0].ucChannel >= 149 && rPreferChanExt[0].ucChannel < 184) {
				AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_149-1] = BIT((rPreferChanExt[0].ucChannel - 149)/2);
			} else if (rPreferChanExt[0].ucChannel >= 184 && rPreferChanExt[0].ucChannel <=196 ) {
				AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_184-1] = BIT((rPreferChanExt[0].ucChannel - 184)/2);
			}				
			//find better 5G Channel
		}
	}
	...
	NLA_PUT_U32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_100, 	AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_100-1]);
    ...
    DBGLOG(P2P, INFO,("[Auto Channel]Relpy AcsChanInfo[%x:%x:%x:%x:%x]\n",
		AcsChnReport[0],AcsChnReport[1],AcsChnReport[2],AcsChnReport[3], AcsChnReport[4]));
...
#endif
#if CFG_SOFTAP_DFS_SUPPORT	

int mtk_cfg80211_vendor_event_radar_detect(struct wiphy *wiphy, struct wireless_dev *wdev, UINT_32 num)
{
	struct sk_buff *skb;
	INT_32 i4Status = -EINVAL;
	...
	skb = cfg80211_vendor_event_alloc(wiphy, sizeof(num), VENDOR_EVENT_RADAR_DETECT, GFP_KERNEL);
	if (!skb) {
		DBGLOG(REQ, ERROR, ("%s allocate skb failed\n", __func__));
		return -ENOMEM;
	}
	/* scan_result = 2; */
	NLA_PUT_U32(skb, VENDOR_EVENT_RADAR_DETECT, num);
...
void kalIndicateRadarDetect(P_ADAPTER_T prAdapter, UINT_32 u4NewChan)
{
	P_GLUE_INFO_T prGlueInfo = prAdapter->prGlueInfo;
	P_BSS_INFO_T prP2pBssInfo = &(prGlueInfo->prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_P2P_INDEX]);

	mtk_cfg80211_vendor_event_radar_detect(priv_to_wiphy(prGlueInfo), NULL, u4NewChan);

	if (prAdapter->fgIsP2PRegistered && prAdapter->rWifiVar.prP2pFsmInfo &&
	    prP2pBssInfo->fgIsApStarted) {

        printk("Radar Detected in chan=%d, Target chan=%d\n", 
			prGlueInfo->prP2PInfo->wdev.preset_chandef.chan->hw_value, u4NewChan);

		prP2pBssInfo->ucApDfsNewChan = u4NewChan;
	    cfg80211_radar_event(prGlueInfo->prP2PInfo->wdev.wiphy, &prGlueInfo->prP2PInfo->wdev.preset_chandef,GFP_KERNEL);
	}	

}
UINT_32 mtk_priv_get_dfs_channel(P_ADAPTER_T prAdapter)
{
	P_GLUE_INFO_T prGlueInfo = prAdapter->prGlueInfo;
	P_AIS_FSM_INFO_T prAisFsmInfo;	
	UINT_32 u4DfsNewChan;

    prAisFsmInfo = &(prAdapter->rWifiVar.rAisFsmInfo);
	u4DfsNewChan = (UINT_32)prAisFsmInfo->ucApDfsNewChan;
	printk("ucApDfsNewChan=0x%08x(%08p) \n", prAisFsmInfo->ucApDfsNewChan, &prAisFsmInfo->ucApDfsNewChan);
	printk("priv dfs new channel=%d\n",  u4DfsNewChan);

    if (prAisFsmInfo->fgIsDfsJoinProcess == TRUE)
		return (u4DfsNewChan | BIT(31));
	else 	
        return u4DfsNewChan;
...
int
mtk_cfg80211_testmode_get_dfs_channel(
    IN struct wiphy *wiphy,
    IN void *data,
    IN int len,
    IN P_GLUE_INFO_T prGlueInfo)
{
#define NL80211_TESTMODE_AP_DFS_INVALID      0
#define NL80211_TESTMODE_AP_DFS_NEW_CHAN     1
...
#define NLA_PUT_U32(skb, attrtype, value) \
	...
	P_BSS_INFO_T prP2pBssInfo =
		&(prGlueInfo->prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_P2P_INDEX]);
	P_AIS_FSM_INFO_T prAisFsmInfo;	
	...
	if(!skb) {
 	...
 	if (prAdapter->fgIsP2PRegistered && prAdapter->rWifiVar.prP2pFsmInfo &&
	    prP2pBssInfo->fgIsApStarted) {
        printk("DFS: Radar chan=%d, Target chan=%d\n", 
		prGlueInfo->prP2PInfo->wdev.preset_chandef.chan->hw_value, prP2pBssInfo->ucApDfsNewChan);			  
 	} else {
 	    return -EINVAL;
 	}
    
    NLA_PUT_U8(skb, NL80211_TESTMODE_AP_DFS_INVALID, 0);
    prAisFsmInfo = &(prAdapter->rWifiVar.rAisFsmInfo);
    if (prAisFsmInfo->fgIsDfsJoinProcess == TRUE)
		NLA_PUT_U32(skb, NL80211_TESTMODE_AP_DFS_NEW_CHAN, prP2pBssInfo->ucApDfsNewChan | BIT(31));
	else 	
    NLA_PUT_U32(skb, NL80211_TESTMODE_AP_DFS_NEW_CHAN, prP2pBssInfo->ucApDfsNewChan);
