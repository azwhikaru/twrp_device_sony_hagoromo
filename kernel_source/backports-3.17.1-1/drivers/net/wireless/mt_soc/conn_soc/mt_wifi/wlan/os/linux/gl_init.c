...
#endif
#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
unsigned int  g_u4AddrOffset=0;
UINT8 __iomem *pEmibaseaddr2;
UINT8 g_start = 0;
...
static const struct nl80211_vendor_cmd_info mtk_wlan_vendor_events[] = {
	{
		.vendor_id = SONY_OUI,
		.subcmd = VENDOR_EVENT_RESERVE_0
	},
	{
		.vendor_id = SONY_OUI,
		.subcmd = VENDOR_EVENT_ENABLE_SOFTAP_DONE
	},
	{
		.vendor_id = SONY_OUI,
		.subcmd = VENDOR_EVENT_RADAR_DETECT
	},
};
#endif
/*******************************************************************************
...
#if CFG_SUPPORT_ALPS_BOX
extern int glRegisterInputDevice(void);
extern int glUnRegisterInputDevice(void);
#endif
    ...
    )
{
    ...
    if(kalCfgDataRead16(prGlueInfo,
        ...
        for (i = 0 ; i < sizeof(prRegInfo->auc5GChOffset) ; i += sizeof(UINT_16)) {
            kalCfgDataRead16(prGlueInfo,
                    OFFSET_OF(WIFI_CFG_PARAM_STRUCT, auc5GChOffset) + i,
                    (PUINT_16) (((PUINT_8)&(prRegInfo->auc5GChOffset)) + i));
#if 0
            printk("[%s] offsset(%x) auc5GChOffset[%d](%x) [%d](%x)\n",
                __FUNCTION__,
                OFFSET_OF(WIFI_CFG_PARAM_STRUCT, auc5GChOffset) + i,
                i,
                prRegInfo->auc5GChOffset[i],
                i+1,
                prRegInfo->auc5GChOffset[i+1]);
#endif
        }
        kalCfgDataRead16(prGlueInfo,
                OFFSET_OF(WIFI_CFG_PARAM_STRUCT, ucFeatureEnable),
                (PUINT_16)aucTmp);
        prRegInfo->ucFeatureEnable = aucTmp[0];
        prRegInfo->ucCountryListIndex = aucTmp[1];

    	DBGLOG(INIT, INFO, ("ucFeatureEnable(%x) ucCountryListIndex(%x)\n",
            prRegInfo->ucFeatureEnable,
            prRegInfo->ucCountryListIndex));


        kalCfgDataRead16(prGlueInfo,
                OFFSET_OF(WIFI_CFG_PARAM_STRUCT, ucSingleSKUNumber),
                (PUINT_16)aucTmp);
        prRegInfo->ucSingleSKUNumber = aucTmp[0];

    	DBGLOG(INIT, INFO, ("ucSingleSKUNumber(%x)\n",
            prRegInfo->ucSingleSKUNumber));
    ...
    )
{
    ...
    do {
        ...
        printk(KERN_ERR "Reday to call CFG80211 wiphy_register ...\n");
...
//    prDev = &((struct sdio_func *) pvData)->dev; //samp
    ...
    prWdev->wiphy->flags            = WIPHY_FLAG_SUPPORTS_FW_ROAM | WIPHY_FLAG_HAS_REMAIN_ON_CHANNEL;
    prWdev->wiphy->regulatory_flags = REGULATORY_COUNTRY_IE_IGNORE;
	...
	prWdev->wiphy->wowlan = &wlan_wowlan_support;
#endif
    ...
    prGlueInfo->prDevHandler = alloc_netdev_mq(sizeof(P_GLUE_INFO_T), NIC_INF_NAME, NET_NAME_PREDICTABLE, ether_setup, CFG_MAX_TXQ_NUM);
	...
	prGlueInfo->u4MagicNum = EVENT_MAGIC_NUM;
...
{
    DBGLOG(INIT, INFO, ("*********Skip wlan_early_suspend************\n"));
    //wlanEarlySuspend();
}

static void wlan_late_resume(struct early_suspend *h)
{
    DBGLOG(INIT, INFO, ("*********Skip wlan_late_resume************\n"));
    //wlanLateResume();
}
...
#if CFG_ENABLE_WIFI_COEXIST
extern void register_set_p2p_mode_handler_soc(set_p2p_mode handler, struct net_device *netdev);
extern void register_set_dbg_level_handler_soc(set_dbg_level handler);
#else
...
#endif
extern int init_mac80211_hwsim(void);
#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
typedef int (*pf_get_latched_tsf)(unsigned int *ptr);
extern void register_get_TSF_for_media_synchronization_handler(pf_get_latched_tsf handler);
#endif
...
)
{
	...
	if (u4SetBeand == 0) {
		printk(KERN_WARNING "SW SET DUAL BAND\n");
		prAdapter->aePreferBand[NETWORK_TYPE_AIS_INDEX] = BAND_NULL;
	}
	else if (u4SetBeand == 1) {
		printk(KERN_WARNING "SW SET 2.4G BAND\n");
		prAdapter->aePreferBand[NETWORK_TYPE_AIS_INDEX] = BAND_2G4;
	}
	else if (u4SetBeand == 2) {
		printk(KERN_WARNING "SW SET 5G BAND\n");
...
)
{
    ...
    switch(prAdapter->aePreferBand[NETWORK_TYPE_AIS_INDEX]){
        case BAND_NULL:
            *u4GetBeand = 0;
		    printk(KERN_WARNING "SW GET DUAL BAND\n");
            break;
        case BAND_2G4:
            *u4GetBeand = 1;
            printk(KERN_WARNING "SW GET 2G4 BAND\n");
            break;
		case BAND_5G:
            *u4GetBeand = 2;
            printk(KERN_WARNING "SW GET 5G BAND\n");
            break;
		default:
			*u4GetBeand = 0;
	}
...
)
{
	...
	printk(KERN_ERR "Set AP BSSID: ["MACSTR"]\n", MAC2STR(pucApBssid));
	COPY_MAC_ADDR(prAdapter->rApMacAddr, pucApBssid);

	//Example 0x0011 0x2233 0x4455 ==> 00:11:22:33:44:55

}
#endif
#if CFG_SCAN_CHAN_LIST_SUPPORT
void staSetScanChan(
	P_GLUE_INFO_T prGlueInfo,
	UINT_32 u4ChanListLen,
	UINT_32 *u4ChanList
)
{
	P_ADAPTER_T prAdapter = NULL;
	...
	if (u4ChanListLen > 0) {
		prAisFsmInfo->fgIsScanChanSepified = TRUE;

        for (u4Idx = 0; u4Idx < u4ChanListLen; u4Idx++) {
            /* Translate Freq from MHz to channel number. */
            prRfChannelInfo = &(prAisFsmInfo->arScanChannelList[u4Idx]);

            prRfChannelInfo->ucChannelNum = u4ChanList[u4Idx];
            DBGLOG(INIT, INFO, ("Scanning Channel:%d\n", prRfChannelInfo->ucChannelNum));
        ...
        prAisFsmInfo->ucNumScanChannelList = u4ChanListLen;

	} else {
		prAisFsmInfo->fgIsScanChanSepified = FALSE;
	}
...
int
get_latched_tsf(unsigned int *ptr)
{
	#if 1
  //extern phys_addr_t gConEmiPhyBase;
	//UINT8 __iomem *pEmibaseaddr2;
	...
	{
		g_start ++;
		pEmibaseaddr2 = ioremap_nocache(gConEmiPhyBase ,0x80000);
	}

	*ptr = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset));
	*(ptr +1) = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset+4));
	*(ptr +2) = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset+8));
	*(ptr +3) = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset+12));

	//printk("@@@ TSF #2 g_u4AddrOffset = 0x%x ptr=0x%x *(ptr +4)=0x%x *(ptr +8)=0x%x *(ptr +12)=0x%x\n",
	//	g_u4AddrOffset,*ptr,*(ptr +4),*(ptr +8),*(ptr +12) );
	#endif
    return 0;//(int)g_u4AddrOffset;
}
#endif
...
bailout:
        ...
        prWdev->wiphy->bands[IEEE80211_BAND_5GHZ] = &mtk_band_5ghz;
#if CFG_ENABLE_WIFI_COEXIST
        prGlueInfo->main_thread = kthread_run(tx_thread, prGlueInfo->prDevHandler, "soc_tx_thread");
#else
        prGlueInfo->main_thread = kthread_run(tx_thread, prGlueInfo->prDevHandler, "tx_thread");
#endif
...
#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
       /* get FW EMI variable offset */
        {
            WLAN_STATUS rStatus = WLAN_STATUS_FAILURE;
			CMD_SW_DBG_CTRL_T rCmdSwCtrl;
            //struct sockaddr MacAddr;
            UINT_32 u4SetInfoLen = 0;
			rCmdSwCtrl.u4Id = 0x70020000;
            rStatus = kalIoctl(prGlueInfo,
            ...
            if (rStatus != WLAN_STATUS_SUCCESS) {
				printk("@@@ Driver TSF fail 0x%lx\n", rStatus);
                //prGlueInfo->u4ReadyFlag = 0;
            } else {

               g_u4AddrOffset = rCmdSwCtrl.u4Data;
			   if (g_start == 0)
			   {
					g_start ++;
					pEmibaseaddr2 = ioremap_nocache(gConEmiPhyBase ,0x80000);
			   }
			   printk("@@@ Driver TSF #1 g_u4AddrOffset= 0x%x\n", g_u4AddrOffset);
...
#if CFG_SUPPORT_ALPS_BOX
        DBGLOG(INIT, TRACE, ("glRegisterInputDevice...\n"));
        glRegisterInputDevice();
#endif
...
#if CFG_ENABLE_WIFI_COEXIST
        register_set_p2p_mode_handler_soc(set_p2p_mode_handler, prGlueInfo->prDevHandler);
#else
        register_set_p2p_mode_handler(set_p2p_mode_handler);
#endif
#endif
#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
   register_get_TSF_for_media_synchronization_handler(get_latched_tsf);
#endif
...
#if CFG_ENABLE_WIFI_COEXIST
   register_set_p2p_mode_handler_soc(NULL, NULL);
#else
    register_set_p2p_mode_handler(NULL);
#endif
#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
    register_get_TSF_for_media_synchronization_handler(NULL);
#endif
...
#if CFG_SUPPORT_ALPS_BOX
    DBGLOG(INIT, TRACE, ("glUnRegisterInputDevice...\n"));
    glUnRegisterInputDevice();
#endif
...
#if CFG_ENABLE_WIFI_COEXIST
    printk("initWlan...Co-exist\n");
    register_set_dbg_level_handler_soc(set_dbg_level_handler);
#else
...
#if CFG_ENABLE_WIFI_COEXIST
    register_set_dbg_level_handler_soc(NULL);
#else
...
#else
#if CFG_ENABLE_WIFI_COEXIST
static VOID __exit exitSocWlan(void)
{
	exitWlan();
	return;
}

static int __init initSocWlan(void)
{
	return initWlan();
}

module_init(initSocWlan);
module_exit(exitSocWlan);
#else
