...
/*----------------------------------------------------------------------------*/
/*! \brief  This routine is called to get rssi.
*
* \param[in] prGlueInfo Pointer to the Adapter structure
...
*/
/*----------------------------------------------------------------------------*/
static void KalCmdGetRssi(GLUE_INFO_T *prGlueInfo, PUINT_8 prInBuf, UINT_32 u4InBufLen, PUINT_8 prOutBuf, PUINT_32 pu4OutBufLen)
{
	WLAN_STATUS rStatus;
	KAL_CMD_GET_CORE_T rCmd;
	...
	rCmd.pucStr = prOutBuf;
	rCmd.pu4StrLen = pu4OutBufLen;

	DBGLOG(INIT, INFO, ("%s: Command PeerMac=" MACSTR " in BSS%u\n",
			    __func__, MAC2STR(rCmd.aucPeerMac), rCmd.ucNetTypeIndex));

	/* command to do this */
	rStatus = kalIoctl(prGlueInfo, wlanoidGetRssi, &rCmd, sizeof(rCmd), FALSE, FALSE, FALSE, FALSE, &u4InBufLen);
			
	if (rStatus != WLAN_STATUS_SUCCESS) {
		DBGLOG(REQ, ERROR, ("%s kalIoctl fail:%x\n", __func__, rStatus));
		return;
		}
	}
...
*/
/*----------------------------------------------------------------------------*/
static void KalCmdGetLinkSpeed(GLUE_INFO_T *prGlueInfo, PUINT_8 prInBuf, UINT_32 u4InBufLen, PUINT_8 prOutBuf, PUINT_32 pu4OutBufLen)
{
	WLAN_STATUS rStatus;
	KAL_CMD_GET_CORE_T rCmd;
	...
	/* command to do this */
	rStatus = kalIoctl(prGlueInfo, wlanoidGetLinkSpeed, &rCmd, sizeof(rCmd), FALSE, FALSE, FALSE, FALSE, &u4InBufLen);
...
/*----------------------------------------------------------------------------*/
/*! \brief  This routine is called to get infomation.
*
* \param[in] prGlueInfo		Pointer to the Adapter structure
* \param[in/out] prInBuf		A pointer to the command string buffer and for the output
* \param[in/out] u4InBufLen	The length of the buffer and for the output
...
*/
/*----------------------------------------------------------------------------*/
VOID kalCmdGet(P_GLUE_INFO_T prGlueInfo, PUINT_8 prInBuf, UINT_32 u4InBufLen, PUINT_8 prOutBuf, PUINT_32 pu4OutBufLen)
{
	UINT_32 u4Subcmd;

	/* parse sub-command */
	u4Subcmd = CmdStringDecParse(prInBuf, &prInBuf, &u4InBufLen);
	DBGLOG(INIT, INFO, ("sub command = %u\n", (UINT32) u4Subcmd));

	/* handle different sub-command */
	switch (u4Subcmd) {
	case KAL_CMD_GET_RSSI:
		KalCmdGetRssi(prGlueInfo, prInBuf, u4InBufLen, prOutBuf, pu4OutBufLen);
		break;
	case KAL_CMD_GET_LINKSPEED:
		KalCmdGetLinkSpeed(prGlueInfo, prInBuf, u4InBufLen, prOutBuf, pu4OutBufLen);
		break;
	default:
		break;
	}
    ...
    )
{
    ...
    rlmDomainDumpChnlList(prGlueInfo->prAdapter,
                              eSpecificBand,
                              ucMaxChannelNum,
                              pucNumOfChannel,
                              paucChannelList, 
...
VOID
kalGetChannelListBuf(
    IN P_GLUE_INFO_T           prGlueInfo,
    IN ENUM_BAND_T             eSpecificBand,
    IN UINT_8                  ucMaxChannelNum,
    IN PUINT_8                 pucNumOfChannel,
    IN P_RF_CHANNEL_INFO_T     paucChannelList,
    IN PUINT_8                 pucDumpBuf,
    IN PUINT_32                pu4DumpBufLen
    )
{
