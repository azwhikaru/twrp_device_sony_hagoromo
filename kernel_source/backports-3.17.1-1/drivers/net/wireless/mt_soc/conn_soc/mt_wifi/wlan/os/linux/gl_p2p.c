    ...
    )
{
    ...
    prGlueInfo->prP2PInfo->wdev.wiphy->regulatory_flags = REGULATORY_CUSTOM_REG;
...
#endif
#ifdef CONFIG_PM
	prGlueInfo->prP2PInfo->wdev.wiphy->wowlan = &p2p_wowlan_support;
#endif
#if 0
    /* 2. Register WIPHY */
	...
	prGlueInfo->prP2PInfo->wdev.wiphy->vendor_events = mtk_p2p_vendor_events;
	prGlueInfo->prP2PInfo->wdev.wiphy->n_vendor_events = ARRAY_SIZE(mtk_p2p_vendor_events);
    ...
    *((P_GLUE_INFO_T *) wiphy_priv(prGlueInfo->prP2PInfo->wdev.wiphy)) = prGlueInfo;
    ...
    /* 3. allocate netdev */
    prGlueInfo->prP2PInfo->prDevHandler = alloc_netdev_mq(sizeof(P_GLUE_INFO_T), prDevName, NET_NAME_PREDICTABLE, ether_setup, CFG_MAX_TXQ_NUM);
    ...
    *((P_GLUE_INFO_T *) netdev_priv(prGlueInfo->prP2PInfo->prDevHandler)) = prGlueInfo;
	...
	//Overwrite AP MAC address for Benten platform
    if (fgIsApMode) {
		UINT_8 aucNullAddr[] = NULL_MAC_ADDR;
		if (UNEQUAL_MAC_ADDR(prAdapter->rApMacAddr, aucNullAddr)) {
			COPY_MAC_ADDR(rMacAddr, prAdapter->rApMacAddr);
			COPY_MAC_ADDR(prAdapter->rWifiVar.aucDeviceAddress, prAdapter->rApMacAddr);
			COPY_MAC_ADDR(prAdapter->rWifiVar.aucInterfaceAddress,  prAdapter->rApMacAddr);
		}
    }
#endif
