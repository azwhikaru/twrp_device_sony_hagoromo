    ...
    )
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo->fgIsApMode == 1 ||
		prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo->rScanReqInfo.fgIsGOInitialDone == 0 ) {
...
//    P_IE_SSID_T prSsidIE = (P_IE_SSID_T)NULL;
    ...
    do {
        ...
        i4Rslt = 0;
        prP2pBssInfo->fgIsApStarted = TRUE;
    ...
    )
{
    ...
    do {
        ...
        i4Rslt = 0;
        prP2pBssInfo->fgIsApStarted = FALSE;
#if CFG_SPM_WORKAROUND_FOR_HOTSPOT
        if (glIsChipNeedWakelock(prGlueInfo))
    ...
    do {
        ...
        case MAC_FRAME_PROBE_REQ:
            if (reg) {
                ...
                //Probe Request is handle in firmware
                DBGLOG(P2P, TRACE, ("Disable Open packet filer probe request\n"));
            }
            else {
                prGlueInfo->prP2PInfo->u4OsMgmtFrameFilter &= ~PARAM_PACKET_FILTER_PROBE_REQ;
...
#if CFG_SOFTAP_DFS_SUPPORT
            case 0x31:
                i4Status = mtk_cfg80211_testmode_get_dfs_channel(wiphy, data, len, prGlueInfo);
                break;
#endif
			default:
				i4Status = -EINVAL;
...
#define NLA_PUT_U32(skb, attrtype, value) \
    ...
    ASSERT(prP2PInfo->wdev.wiphy);

    DBGLOG(REQ, INFO, ("%s for vendor command\r\n", __func__));

    skb = cfg80211_vendor_event_alloc(prP2PInfo->wdev.wiphy, sizeof(u4Reserved), VENDOR_EVENT_ENABLE_SOFTAP_DONE, GFP_KERNEL);
    if (!skb) {
