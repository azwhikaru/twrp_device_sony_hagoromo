...
extern int mtk_wcn_wmt_msgcb_reg (ENUM_WMTDRV_TYPE_T eType,PF_WMT_CB pCb, CHIP_TYPE_F chipType);
extern int mtk_wcn_wmt_msgcb_unreg (ENUM_WMTDRV_TYPE_T eType, CHIP_TYPE_F chipType);

extern int
wifi_reset_start_soc(void);

extern int
wifi_reset_end_soc(void);
#else
extern int
...
#if CFG_ENABLE_WIFI_COEXIST
    mtk_wcn_wmt_msgcb_reg(WMTDRV_TYPE_WIFI, (PF_WMT_CB)glResetCallback, CHIP_TYPE_SOC);
#else
...
#if CFG_ENABLE_WIFI_COEXIST
    mtk_wcn_wmt_msgcb_unreg(WMTDRV_TYPE_WIFI, CHIP_TYPE_SOC);
#else
