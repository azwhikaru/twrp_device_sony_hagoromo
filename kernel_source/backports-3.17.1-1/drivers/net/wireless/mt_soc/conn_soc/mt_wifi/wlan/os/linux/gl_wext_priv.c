    ...
    )
{
    ...
    case PRIV_CMD_ACCESS_MCR:
        ...
        status = priv_set_ndis(prNetDev, prNdisReq, &u4BufLen);
        break;
    case PRIV_CMD_ACCESS_MCR_LINUX:
        //printk("addr=0x%08lx, data_h=0x%08lx, data_l=0x%08lx\n", pu4IntBuf[1], pu4IntBuf[2], pu4IntBuf[3]);
        prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];
        pu4IntBuf[2] = (pu4IntBuf[2] << 16) + pu4IntBuf[3];
    ...
    case PRIV_CMD_BAND_CONFIG:
        {
            extern void staSetScanBand(P_GLUE_INFO_T prGlueInfo, UINT_32 u4SetBeand);
            UINT_32 u4SetBand = pu4IntBuf[1];

            DBGLOG(INIT, INFO, ("CMD set_band=%u\n", (UINT32)pu4IntBuf[1]));
            staSetScanBand(prGlueInfo, u4SetBand);
    ...
    case PRIV_CMD_TEST_CMD_LINUX:
	{
	UINT_32 u4TestCmd  = pu4IntBuf[1];
	UINT_32 u4HighPart = pu4IntBuf[2];
	UINT_32 u4LowPart  = pu4IntBuf[3];

	pu4IntBuf[2] = (UINT_32)((u4HighPart << 16) | u4LowPart);
	printk("CMD=0x%08lx, data=0x%08lx\n", u4TestCmd, pu4IntBuf[2]);
        prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];
...
#endif
#if CONFIG_MTK_BENTEN_SUPPORT
    case PRIV_CMD_AP_BSSID:
    {
        extern void apSetBssid(P_GLUE_INFO_T prGlueInfo, PUINT_8 pucApBssid);

        PARAM_MAC_ADDRESS rApBssid;

		//Example: BSSID=00:11:22:33:44:55
		//             Command = 'iwpriv wlan0 set_ap_bssid 0x0011 0x2233 0x4455'
		rApBssid[0] = (UINT_8)(pu4IntBuf[1] >> 8);
        rApBssid[1] = (UINT_8)(pu4IntBuf[1] & 0xff);
        rApBssid[2] = (UINT_8)(pu4IntBuf[2] >> 8);
        rApBssid[3] = (UINT_8)(pu4IntBuf[2] & 0xff);
        rApBssid[4] = (UINT_8)(pu4IntBuf[3] >> 8);
        rApBssid[5] = (UINT_8)(pu4IntBuf[3] & 0xff);


        //printk(KERN_ERR "Command AP BSSID: ["MACSTR"]\n", MAC2STR(rApBssid));
		apSetBssid(prGlueInfo, rApBssid);
    }
        break;
#if CFG_SOFTAP_DFS_SUPPORT
    case PRIV_CMD_RADAR_DETECT:
    {
        extern void kalIndicateRadarDetect(P_ADAPTER_T prAdapter, UINT_32 u4NewChan);
        UINT_32 u4NewChan = pu4IntBuf[1];

        DBGLOG(INIT, INFO, ("RADAR Detected. Switch to Channel =%u\n", (UINT32)pu4IntBuf[1]));
		if (u4NewChan > 0)
            kalIndicateRadarDetect(prGlueInfo->prAdapter, u4NewChan);
    }
        break;
#endif
#endif
    ...
    )
{
    ...
    case PRIV_CMD_BAND_CONFIG:
	{
			extern void staGetScanBand(P_GLUE_INFO_T prGlueInfo, UINT_32 *u4SetBeand);
			UINT_32 u4GetBand = 0;

			staGetScanBand(prGlueInfo, &u4GetBand);
			DBGLOG(INIT, INFO, ("CMD get_band=%u\n", u4GetBand));
        	prIwReqData->mode = u4GetBand;
        	return status;
    	}
#if CFG_SOFTAP_DFS_SUPPORT		
    case PRIV_CMD_RADAR_DETECT:	
		{
			extern UINT_32 mtk_priv_get_dfs_channel(P_GLUE_INFO_T prGlueInfo);
			UINT_32 u4GetDfsChan = 0;
		
			u4GetDfsChan = mtk_priv_get_dfs_channel(prGlueInfo);
			DBGLOG(INIT, INFO, ("CMD u4GetDfsChan=%u\n", u4GetDfsChan));
        	prIwReqData->mode = u4GetDfsChan;
        	return status;
    	}	
#endif
    default:
    ...
    )
{
    ...
    case PRIV_CMD_SCAN_CHAN_LIST:
        {
		extern void staSetScanChan(P_GLUE_INFO_T prGlueInfo, UINT_32 u4ChanListLen, UINT_32 *u4ChanList);
        INT_32 *setting = prIwReqData->data.pointer;
        UINT_16 i;

        if (prIwReqData->data.length == 0) {
			printk("Specify Scan Chan Disable... \n");
        } else {
		    printk("Specify Scan Chan Enable... Number=%d\n", prIwReqData->data.length);
            for (i=0; i < prIwReqData->data.length; i++)
               printk("    Channel = %d\n", setting[i]);
    	}
	    staSetScanChan(prGlueInfo, prIwReqData->data.length, prIwReqData->data.pointer);
    	}
#endif
        break;
    default:
    ...
    )
{
    ...
    if(i4CmdFound == 0) {
        ...
        if (strnicmp(pcCommand, CMD_WOWLAN, strlen(CMD_WOWLAN)) == 0) {
            kalIoctl(prGlueInfo,
                        wlanoidSetWowlan,
                        (PVOID)pcCommand,
                        i4TotalLen,
...
{
	UINT_32			u4SubCmdFlags, u4SubcmdParse;
	P_GLUE_INFO_T	prGlueInfo;
	INT_32			Status = 0;
	UINT_8			*InBuf, OutBuf[1024] = { 0x00 };
	UINT_32			InBufLen, OutBufLen = 0;
	...
	u4SubcmdParse= CmdStringDecParse(prIwReqData->data.pointer, &InBuf, &InBufLen);

    switch (u4SubCmdFlags) {
    case PRIV_CMD_OTHER_GET:
		switch (u4SubcmdParse) {
			case PRIV_CMD_OTHER_GET_STAREC:
				kalCmdGet(prGlueInfo, InBuf, InBufLen, OutBuf, &OutBufLen);

				prIwReqData->data.length = OutBufLen;
				sprintf( pcExtra, "\n%s\n", OutBuf);

				break;
			case PRIV_CMD_OTHER_GET_TDLS:
				TdlsexCmdGet(prGlueInfo, InBuf, InBufLen, OutBuf, &OutBufLen);
	...
	case PRIV_CMD_GET_CH_CAP:
		{
            ...
            OutBufLen = 1024;
			kalGetChannelListBuf(prGlueInfo, BAND_NULL, ucMaxChannelNum, &NumOfChannel, aucChannelList, OutBuf, &OutBufLen);
            prIwReqData->data.length = strlen(OutBuf);
			//printk("outbuf len=%d\n", prIwReqData->data.length);
			sprintf( pcExtra, "\n%s", OutBuf);
