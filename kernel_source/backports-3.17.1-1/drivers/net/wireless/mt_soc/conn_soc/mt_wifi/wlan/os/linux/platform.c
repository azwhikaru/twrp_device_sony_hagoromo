...
//#ifdef  CONFIG_IPV6
	...
	if (prGlueInfo->u4MagicNum != EVENT_MAGIC_NUM) {
		DBGLOG(REQ, ERROR, ("prGlueInfo magic number is not match.\n"));
		return NOTIFY_DONE;
	}
...
static struct input_dev *g_wlan_input_dev = NULL;

int glRegisterInputDevice(void)
{
    int ret = 0;

    DBGLOG(INIT, INFO, ("%s\n", __FUNCTION__));

    g_wlan_input_dev = input_allocate_device();
    if (!g_wlan_input_dev) {
        ret = -ENOMEM;
        goto err_free_mem;
    }
    ...
    set_bit(EV_KEY, g_wlan_input_dev->evbit);

    /* Indicate that we generate *any* key event */
    //bitmap_fill(wlan_input_dev->keybit, KEY_MAX);
    __set_bit(KEY_POWER, g_wlan_input_dev->keybit);
    g_wlan_input_dev->name = WOWLAN_NAME;
    ret = input_register_device(g_wlan_input_dev);
    if (ret) 
        goto err_free_mem;

    DBGLOG(INIT, INFO, ("%s, register input device success!\n", __FUNCTION__));

    return ret;

err_free_mem:
    input_free_device(g_wlan_input_dev);
    return ret;
}

int glUnRegisterInputDevice(void)
{
    if (g_wlan_input_dev)
        input_unregister_device(g_wlan_input_dev);
    g_wlan_input_dev = NULL;

    return 0;
}

int glInputReportKey(void)
{
    DBGLOG(INIT, INFO, ("%s, send KEY_POWER to input device!\n", __FUNCTION__));

    input_report_key(g_wlan_input_dev, KEY_POWER, 1);
    input_sync(g_wlan_input_dev);

    input_report_key(g_wlan_input_dev, KEY_POWER, 0);
    input_sync(g_wlan_input_dev);

    return 0;
}
#endif /* CFG_SUPPORT_ALPS_BOX */
