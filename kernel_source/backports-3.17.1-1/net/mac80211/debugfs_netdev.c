...
static ssize_t ieee80211_if_parse_delay_tsf(
	struct ieee80211_sub_if_data *sdata, const char *buf, int buflen)
{
	struct ieee80211_local *local = sdata->local;
	u64 delay;
	...
	ret = kstrtoull(buf, 10, &delay);
	if (ret < 0)
		return ret;
	drv_delay_tsf(local, sdata, delay);
	wiphy_info(local->hw.wiphy, "debugfs delay tsf by %llu usec\n", delay);

	return buflen;
}
IEEE80211_IF_FILE_W(delay_tsf);
...
static void add_mesh_files(struct ieee80211_sub_if_data *sdata)
{
	...
	DEBUGFS_ADD_MODE(delay_tsf, 0200);
}
