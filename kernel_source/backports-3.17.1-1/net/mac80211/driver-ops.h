...
static inline void drv_set_sta_mesh_power_mode(struct ieee80211_local *local,
				struct ieee80211_sub_if_data *sdata,
				struct ieee80211_sta *sta,
				enum nl80211_mesh_power_mode  local_pm,
				enum nl80211_mesh_power_mode  peer_pm,
				int deep_sleep_listen_intval)
{
	might_sleep();
	...
	if (local_pm == NL80211_MESH_POWER_UNKNOWN)
		local_pm = NL80211_MESH_POWER_ACTIVE;
	if (peer_pm == NL80211_MESH_POWER_UNKNOWN)
		peer_pm = NL80211_MESH_POWER_ACTIVE;

	mps_dbg(sdata, "drv_set_sta_mesh_power_mode for %pM: "
			"local_pm %d, peer_pm %d, interval %d\n",
			sta->addr, local_pm, peer_pm,
			deep_sleep_listen_intval);

	trace_drv_set_sta_mesh_power_mode(local, sdata, sta, local_pm,
			peer_pm, deep_sleep_listen_intval);
	if (local->ops->set_sta_mesh_power_mode)
		local->ops->set_sta_mesh_power_mode(&local->hw, &sdata->vif,
				sta, local_pm, peer_pm,
				deep_sleep_listen_intval);
	trace_drv_return_void(local);
}

static inline void drv_set_nonpeer_mesh_power_mode(
				...
				enum nl80211_mesh_power_mode pm)
{
	...
	if (pm == NL80211_MESH_POWER_LIGHT_SLEEP)
		pm = NL80211_MESH_POWER_LIGHT_SLEEP;

	mps_dbg(sdata, "drv_set_nonpeer_mesh_power_mode: pm %d\n", pm);

	trace_drv_set_nonpeer_mesh_power_mode(local, sdata, pm);
	if (local->ops->set_nonpeer_mesh_power_mode)
		local->ops->set_nonpeer_mesh_power_mode(&local->hw,
				&sdata->vif, pm);
	trace_drv_return_void(local);
}

static inline void drv_set_mesh_beacon_filtering(
				struct ieee80211_local *local,
				struct ieee80211_sub_if_data *sdata,
				struct mesh_beacon_filtering_conf *conf)
{
	...
	trace_drv_set_mesh_beacon_filtering(local, sdata, conf);
	if (local->ops->set_mesh_beacon_filtering)
		local->ops->set_mesh_beacon_filtering(&local->hw,
				&sdata->vif, conf);
	trace_drv_return_void(local);
}

static inline void drv_enable_clock_sync(struct ieee80211_local *local,
				  struct ieee80211_sub_if_data *sdata,
				  bool enabled)
{
	might_sleep();
	...
	trace_drv_enable_clock_sync(local, sdata, enabled);
	if (local->ops->enable_clock_sync)
		local->ops->enable_clock_sync(&local->hw, &sdata->vif, enabled);
	trace_drv_return_void(local);

}

static inline void drv_delay_tsf(struct ieee80211_local *local,
				struct ieee80211_sub_if_data *sdata,
				u64 delay)
{
	might_sleep();
	...
	trace_drv_delay_tsf(local, sdata, delay);
	if (local->ops->delay_tsf)
		local->ops->delay_tsf(&local->hw, &sdata->vif, delay);
	trace_drv_return_void(local);
}
/* Sony extension ends */
