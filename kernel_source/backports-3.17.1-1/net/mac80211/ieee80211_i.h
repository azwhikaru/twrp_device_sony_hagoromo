...
struct ieee80211_mesh_sync_ops {
	...
	void (*adjust_tbtt_setter)(struct ieee80211_sub_if_data *sdata);
};

struct beacon_timing_ie {
	u8 ie_len;
	u8 *ie_char;
};

struct mbca_tbtt_adjust_setting {
	enum nl80211_tbtt_adjust_action action;
	/* new Beacon Timing IE to be used after adjusting (delaying) */
	struct beacon_timing_ie *bcn_timing_ie;
	/* TSF timer time at which delaying the timer is scheduled */
	u64 tsf;
	/* amount of delay to adjust */
	u64 delay;
	struct list_head list;
};
...
struct ieee80211_if_mesh {
	...
	struct mesh_beacon_filtering_conf  bcn_filter_conf;
	/* add end */
	/* mesh power save */
	...
	struct ps_data ps;
	/* add sony param */
	struct list_head sta_pm_change_list;
	spinlock_t sta_pm_change_lock;
	bool force_nonpeer_pm_active;
	struct timer_list mps_stop_force_nonpeer_pm_timer;
	/* add end */
	...
	/* treebuilding multicast */
	u8 mcast_addr[ETH_ALEN];

	/* add sony param - MBCA stuff */
	enum nl80211_mbca_executor mbca_executor;
	struct beacon_timing_ie *bcn_timing_ie;
	struct delayed_work mbca_work;
	struct list_head mbca_tbtt_adjust_list; /* struct mbca_tbtt_adjust_setting */
	size_t mbca_tbtt_adjust_list_len;
	/* add end */
