...
enum mesh_path_flags {
	...
	MESH_PATH_USER_REFRESHING = 	BIT(6),
	MESH_PATH_PROACTIVE = BIT(7),
};
 ...
 * @MESH_WORK_MBCA_ADJUST_TBTT:
 * @MESH_WORK_MBCA_ADJUST_TBTT_FINISHED:
 */
enum mesh_deferred_task_flags {
	...
	MESH_WORK_PS_SET_DRV_STA_PEER_PM,
	MESH_WORK_PS_START_FORCE_NONPEER_PM_ACTIVE,
	MESH_WORK_PS_STOP_FORCE_NONPEER_PM_ACTIVE,
	MESH_WORK_MBCA_ADJUST_TBTT,
	MESH_WORK_MBCA_ADJUST_TBTT_FINISHED,
	/* Sony extension ends */
};
...
/**
 * enum mesh_plink_broken_reason - plink broken reason.
 *
 * @PLINK_BROKEN_REASON_DISCONNECTED: plink transitions from PLINK_ESTAB to
 *  any other state.
 * @PLINK_BROKEN_REASON_WEEK_LINK: detect week link.
 */
enum mesh_plink_broken_reason {
	PLINK_BROKEN_REASON_DISCONNECTED = 0,
	PLINK_BROKEN_REASON_WEEK_LINK,
};
 ...
 * @plink_broken: for PERR substitute, used to send PREQ to retrieve broken path
 *
 *
...
struct mesh_path {
	...
	bool is_root;
	bool is_gate;
	bool plink_broken;	// for PERR substitue
};
...
int ieee80211_fill_mesh_addresses(struct ieee80211_hdr *hdr, __le16 *fc,
				  const u8 *da, const u8 *sa, bool unicast);
...
void ieee80211_mps_start_force_nonpeer_pm_active(struct ieee80211_sub_if_data *sdata);
void ieee80211_mps_stop_force_nonpeer_pm_active(struct ieee80211_sub_if_data *sdata);
...
struct mesh_path *mesh_path_mcast_lookup(struct mesh_local_bss *mbss, const u8 *dst,
					 u8 *mcast_tab, bool *more);
struct mesh_path *mesh_path_mcast_nexthop_lookup(struct mesh_local_bss *mbss,
...
static inline bool mesh_path_sel_is_user_handled(struct ieee80211_sub_if_data *sdata)
{
	return (sdata->u.mesh.mshcfg.dot11MeshUserPathSel != IEEE80211_PATHSEL_KERNEL);
}

static inline bool mesh_path_sel_is_root(struct ieee80211_sub_if_data *sdata)
{
	return sdata->u.mesh.mshcfg.dot11MeshUserPathSel == IEEE80211_PATHSEL_ROOT;
}
...
int mesh_mbca_get_tick_to_next_adjustment(
...
{}
static inline void mpp_path_flush_by_iface(struct ieee80211_sub_if_data *sdata)
{}
