...
static int mesh_plink_frame_tx(struct ieee80211_sub_if_data *sdata,
			       ...
			       u8 *da, u16 llid, u16 plid, u16 aid, u16 reason);
...
static u32 __mesh_plink_deactivate(struct sta_info *sta)
{
	...
	cfg80211_plink_broken(&sdata->wdev, sta->sta.addr,
			PLINK_BROKEN_REASON_DISCONNECTED, GFP_ATOMIC);
			       ...
			       u8 *da, u16 llid, u16 plid, u16 aid, u16 reason)
{
	...
	if (action != WLAN_SP_MESH_PEERING_CLOSE) {
		...
		if (action == WLAN_SP_MESH_PEERING_CONFIRM) {
			/* AID */
			mpl_dbg(sdata, "send in CONFIRM: AID 0x%04x\n", aid);
			pos = skb_put(skb, 2);
			put_unaligned_le16(aid, pos);
			pos += 2;
		}
...
u32 mesh_plink_open(struct sta_info *sta)
{
	...
	/* TODO: As AID should be in a small range of sequential values so that
	 * TIM element should not be large, llid, which is a random number,
	 * should not be used for AID
	 */
	sta->sta.aid = sta->llid;
	if (sta->plink_state != NL80211_PLINK_LISTEN &&
	    sta->plink_state != NL80211_PLINK_BLOCKED) {
		spin_unlock_bh(&sta->lock);
		return 0;
	...
	mesh_plink_frame_tx(sdata, WLAN_SP_MESH_PEERING_OPEN,
			    sta->sta.addr, sta->llid, 0, sta->sta.aid, 0);
	return changed;
}
			  ...
			  struct sta_info *sta, enum plink_event event)
{
	...
	case NL80211_PLINK_LISTEN:
		...
		case OPN_ACPT:
			...
			sta->llid = mesh_get_new_llid(sdata);
			/* TODO: llid should not be used for AID. see a
			 * comment in mesh_plink_open() for its reason */
			sta->sta.aid = sta->llid;
			 ...
			 struct ieee802_11_elems *elems)
{
	...
	if (event == OPN_ACPT) {
		...
		goto unlock_rcu;
	} else if (sta && event == CNF_ACPT) {
		u16 aid = *(u16*)(mgmt->u.action.u.self_prot.variable + 2);
		aid = le16_to_cpu(aid);
		if (aid > IEEE80211_MAX_AID) {
			mpl_dbg(sdata, "Mesh plink: AID is too large! %u\n",
					aid);
			goto unlock_rcu;
		}
		sta->peer_aid = aid;
		mpl_dbg(sdata, "CNF_ACPT: sta->peer_aid 0x%04x\n",
				sta->peer_aid);
