...
TRACE_EVENT(drv_set_sta_mesh_power_mode,
	TP_PROTO(struct ieee80211_local *local,
		struct ieee80211_sub_if_data *sdata,
		struct ieee80211_sta *sta,
	...
	TP_ARGS(local, sdata, sta, local_pm, peer_pm,
	...
	TP_fast_assign(
		...
		VIF_ASSIGN;
		STA_ASSIGN;
		__entry->local_pm = local_pm;
		__entry->peer_pm = peer_pm;
		__entry->intval = deep_sleep_listen_intval;
	),

	TP_printk(
		LOCAL_PR_FMT VIF_PR_FMT STA_PR_FMT
		" local_pm: %d" " peer_pm: %d" " intval %d",
		LOCAL_PR_ARG, VIF_PR_ARG, STA_PR_ARG,
		__entry->local_pm, __entry->peer_pm, __entry->intval
	)
);

TRACE_EVENT(drv_set_nonpeer_mesh_power_mode,
	TP_PROTO(struct ieee80211_local *local,
		struct ieee80211_sub_if_data *sdata,
		enum nl80211_mesh_power_mode pm),

	TP_ARGS(local, sdata, pm),
	...
	TP_printk(
		LOCAL_PR_FMT VIF_PR_FMT " pm: %d",
		LOCAL_PR_ARG, VIF_PR_ARG, __entry->pm
	)
);

TRACE_EVENT(drv_set_mesh_beacon_filtering,
	TP_PROTO(struct ieee80211_local *local,
		struct ieee80211_sub_if_data *sdata,
		const struct mesh_beacon_filtering_conf *conf),
...
TRACE_EVENT(drv_enable_clock_sync,
	TP_PROTO(struct ieee80211_local *local,
		struct ieee80211_sub_if_data *sdata,
		bool enabled),

	TP_ARGS(local, sdata, enabled),
	...
	TP_printk(
		LOCAL_PR_FMT VIF_PR_FMT " enabled: %d",
		LOCAL_PR_ARG, VIF_PR_ARG, __entry->enabled
	)

);

TRACE_EVENT(drv_delay_tsf,
	TP_PROTO(struct ieee80211_local *local,
		struct ieee80211_sub_if_data *sdata,
		u64 delay),

	TP_ARGS(local, sdata, delay),
	...
	TP_printk(
		LOCAL_PR_FMT VIF_PR_FMT " delay: %llu",
		LOCAL_PR_ARG, VIF_PR_ARG, __entry->delay
	)
);
/* Sony extension ends */
