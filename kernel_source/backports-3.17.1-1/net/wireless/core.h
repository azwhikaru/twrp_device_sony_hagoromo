...
cfg80211_rdev_free_wowlan(struct cfg80211_registered_device *rdev)
{
	...
	if (rdev->wiphy.wowlan_config->mesh) {
		struct cfg80211_wowlan_mesh *wowlan_mesh = rdev->wiphy.wowlan_config->mesh;
		for (i = 0; i < wowlan_mesh->n_patterns; i++)
			kfree(wowlan_mesh->patterns_80211[i].mask);
		kfree(wowlan_mesh->patterns_80211);
		kfree(wowlan_mesh);
	}
