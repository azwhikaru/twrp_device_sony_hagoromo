...
{
	int ret;
	trace_rdev_del_mpath(&rdev->wiphy, dev, dst, next_hop);
	ret = rdev->ops->del_mpath(&rdev->wiphy, dev, dst, next_hop);
	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}

static inline int rdev_del_mpp_path(struct cfg80211_registered_device *rdev,
				 struct net_device *dev, u8 *dst, u8 *next_hop)
{
	int ret;
//	trace_rdev_del_mpath(&rdev->wiphy, dev, dst, next_hop);
	ret = rdev->ops->del_mpp_path(&rdev->wiphy, dev, dst, next_hop);
//	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}
...
static inline int rdev_get_mpp_path(struct cfg80211_registered_device *rdev,
				 struct net_device *dev, u8 *dst, u8 *next_hop,
				 struct mpath_info *pinfo)
{
	int ret;
//	trace_rdev_get_mpath(&rdev->wiphy, dev, dst, next_hop);
	ret = rdev->ops->get_mpp_path(&rdev->wiphy, dev, dst, next_hop, pinfo);
//	trace_rdev_return_int_mpath_info(&rdev->wiphy, ret, pinfo);
	return ret;
...
static inline int rdev_dump_mpp_path(struct cfg80211_registered_device *rdev,
				  struct net_device *dev, int idx,
				  struct net_device **pathdev,
				  u8 *dst, u8 *next_hop,
				  struct mpath_info *pinfo, bool mbss)

{
	int ret;
	//trace_rdev_dump_mpath(&rdev->wiphy, dev, idx, dst, next_hop);
	ret = rdev->ops->dump_mpp_path(&rdev->wiphy, dev, idx, pathdev,
				    dst, next_hop, pinfo, mbss);
	//trace_rdev_return_int_mpath_info(&rdev->wiphy, ret, pinfo);
	return ret;
}
...
static inline int
rdev_update_mesh_sync_executor(struct cfg80211_registered_device *rdev,
			struct net_device *dev, u8 sync_executor)
{
	int ret;
	trace_rdev_update_mesh_sync_executor(&rdev->wiphy, dev, sync_executor);
	ret = rdev->ops->update_mesh_sync_executor(&rdev->wiphy, dev, sync_executor);
	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}
