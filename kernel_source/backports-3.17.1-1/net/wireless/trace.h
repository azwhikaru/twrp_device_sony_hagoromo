...
DEFINE_EVENT(mpath_evt, rdev_del_mpath,
	TP_PROTO(struct wiphy *wiphy, struct net_device *netdev, u8 *dst,
		 u8 *next_hop),
	TP_ARGS(wiphy, netdev, dst, next_hop)
);
...
TRACE_EVENT(rdev_update_mesh_sync_executor,
	TP_PROTO(struct wiphy *wiphy, struct net_device *netdev, u8 sync_executor),
	TP_ARGS(wiphy, netdev, sync_executor),
	TP_STRUCT__entry(
		WIPHY_ENTRY
		NETDEV_ENTRY
		__field(u8, sync_executor)
	),
	TP_fast_assign(
		WIPHY_ASSIGN;
		NETDEV_ASSIGN;
		__entry->sync_executor = sync_executor;
	),
	TP_printk(WIPHY_PR_FMT ", " NETDEV_PR_FMT ", sync_executor: %d",
		  WIPHY_PR_ARG, NETDEV_PR_ARG, __entry->sync_executor)
);
