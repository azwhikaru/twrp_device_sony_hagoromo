...
_mali_osk_errcode_t mali_group_upper_half_pp(void *data)
{
	...
	if (MALI200_REG_VAL_IRQ_MASK_NONE != irq_readout) {
		...
		MALI_DEBUG_PRINT(3, ("Mali %s IRQ: upper half read status 0x%08x \n",mali_pp_get_hw_core_desc(group->pp_core), irq_readout));
		...
		if (MALI200_REG_VAL_IRQ_END_OF_FRAME == irq_readout) {
			...
			if (MALI200_REG_VAL_IRQ_END_OF_FRAME != irq_readout) {
				MALI_DEBUG_PRINT(3, ("Mali %s IRQ: upper half read rawstatus 0x%08x \n",mali_pp_get_hw_core_desc(group->pp_core), irq_readout));
