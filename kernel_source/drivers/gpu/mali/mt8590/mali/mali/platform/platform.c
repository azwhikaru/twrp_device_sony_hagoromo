...
{
    MALI_GPU_RESOURCES_MALI450_MP1(
                    IO_VIRT_TO_PHYS(MALI_BASE),
                    MT_MFG_IRQ0_ID,
                    MT_MFG_IRQ1_ID,
                    MT_MFG_IRQ2_ID,
...
static u32 get_devinfo(void) ;
static u32 get_gpuinfo(void) ;
...
static u32 get_gpuinfo(void) {
    ...
    return get_devinfo_with_index(3);
}
...
static void update_dev_info(struct platform_device * device ) {
	...
	u32 pp;
#ifdef MTK_CONFIG_FF_PP
		MALI_DEBUG_PRINT(1,("update_dev_info of MP1 %s\n", __FUNCTION__));
		MALI_REASSIGN_RESOURCE(device, mali_gpu_resources_MP1);
		return; 
#endif
    ...
    if(pp == 0x1) {
		...
		MALI_REASSIGN_RESOURCE(device, mali_gpu_resources_MP2);
 		...
 		MALI_DEBUG_PRINT(1, ("Found devinfo of MP1 %s, %d\n", __FUNCTION__, pp));
		MALI_REASSIGN_RESOURCE(device, mali_gpu_resources_MP1);
    } else {
