...
static uint AUXADC_channels = 8;
module_param_named(auxadc_channels, AUXADC_channels, uint, 0644);
...
static ssize_t show_AUXADC_chanel(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	channels = AUXADC_channels;
	if (channels >= ADC_CHANNEL_MAX) {
		channels = ADC_CHANNEL_MAX;
	}
...
EXPORT_SYMBOL(IMM_IsAdcInitReady);
EXPORT_SYMBOL(IMM_get_adc_channel_num);
EXPORT_SYMBOL(IMM_GetOneChannelValue);
EXPORT_SYMBOL(IMM_GetOneChannelValue_Cali);
