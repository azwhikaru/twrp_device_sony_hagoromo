...
#define DEBUG_AUDDRV_BTCVSD_LV1	0x1
#define DEBUG_AUDDRV_BTCVSD_LV2	0x2
#define DEBUG_AUDDRV_BTCVSD_LV3	0x4
...
static kal_uint32 AudDrv_btcvsd_log_on = 0;
...
#define PRINTK_BTCVSD_LV1(format, args...)	\
do { \
	if (AudDrv_btcvsd_log_on&DEBUG_AUDDRV_BTCVSD_LV1) { \
		printk(format, ##args); } \
} while (0)

#define PRINTK_BTCVSD_LV2(format, args...)	\
do { \
	if (AudDrv_btcvsd_log_on&DEBUG_AUDDRV_BTCVSD_LV2) { \
		printk(format, ##args); } \
} while (0)

#define PRINTK_BTCVSD_LV3(format, args...)	\
do { \
	if (AudDrv_btcvsd_log_on&DEBUG_AUDDRV_BTCVSD_LV3) { \
		printk(format, ##args); } \
} while (0)
...
static void Disable_CVSD_Wakeup(void)
{
    ...
    *INFRA_MISC_REGISTER |= 0x00000800;
    PRINTK_BTCVSD_LV1("Disable_CVSD_Wakeup \n");
}
...
static void Enable_CVSD_Wakeup(void)
{
    volatile kal_uint32 *INFRA_MISC_REGISTER = (volatile kal_uint32 *)(INFRA_MISC);
    *INFRA_MISC_REGISTER &= ~(0x00000800);
    PRINTK_BTCVSD_LV1("Enable_CVSD_Wakeup \n");
}

static int AudDrv_btcvsd_Allocate_Buffer(struct file *fp, kal_uint8 isRX)
{
    PRINTK_BTCVSD_LV1("AudDrv_btcvsd_Allocate_Buffer(+) isRX=%d\n", isRX);
    ...
    if (isRX == 1)
    {
        ...
        if ((BT_CVSD_Mem.pucRXVirtBufAddr == NULL) && (BT_CVSD_Mem.pucRXPhysBufAddr == 0))
        {
            ...
            PRINTK_BTCVSD_LV2("BT_CVSD_Mem.pucRXVirtBufAddr = %p BT_CVSD_Mem.pucRXPhysBufAddr = 0x%x\n" ,
    ...
    else
    {
        ...
        if ((BT_CVSD_Mem.pucTXVirtBufAddr == NULL) && (BT_CVSD_Mem.pucTXPhysBufAddr == 0))
        {
            ...
            PRINTK_BTCVSD_LV2("BT_CVSD_Mem.pucTXVirtBufAddr = %p BT_CVSD_Mem.pucTXPhysBufAddr = 0x%x\n" ,
    ...
    PRINTK_BTCVSD_LV1("AudDrv_btcvsd_Allocate_Buffer(-) \n");
    return 0;
}

static int AudDrv_btcvsd_Free_Buffer(struct file *fp, kal_uint8 isRX)
{
    PRINTK_BTCVSD_LV1("AudDrv_btcvsd_Free_Buffer(+) isRX=%d\n", isRX);

    if (isRX == 1)
    {
        if ((BT_CVSD_Mem.pucRXVirtBufAddr != NULL) && (BT_CVSD_Mem.pucRXPhysBufAddr != 0))
        {
            PRINTK_BTCVSD_LV2("AudDrv_btcvsd_Free_Buffer dma_free_coherent pucRXVirtBufAddr = %p pucRXPhysBufAddr = %x", BT_CVSD_Mem.pucRXVirtBufAddr, BT_CVSD_Mem.pucRXPhysBufAddr);
            btsco.pRX =  NULL;
        ...
        {
            printk("AudDrv_btcvsd_Free_Buffer cannot dma_free_coherent pucRXVirtBufAddr = %p pucRXPhysBufAddr = %x", BT_CVSD_Mem.pucRXVirtBufAddr, BT_CVSD_Mem.pucRXPhysBufAddr);
            return -1;
        }
    }
    else
    {
        if ((BT_CVSD_Mem.pucTXVirtBufAddr != NULL) && (BT_CVSD_Mem.pucTXPhysBufAddr != 0))
        {
            PRINTK_BTCVSD_LV2("AudDrv_btcvsd_Free_Buffer dma_free_coherent pucTXVirtBufAddr = %p pucTXPhysBufAddr = %x", BT_CVSD_Mem.pucTXVirtBufAddr, BT_CVSD_Mem.pucTXPhysBufAddr);
            btsco.pTX =  NULL;
        ...
        {
            printk("AudDrv_btcvsd_Free_Buffer cannot dma_free_coherent pucTXVirtBufAddr = %p pucTXPhysBufAddr = %x", BT_CVSD_Mem.pucTXVirtBufAddr, BT_CVSD_Mem.pucTXPhysBufAddr);
            return -1;
        }
    }
    PRINTK_BTCVSD_LV1("AudDrv_btcvsd_Free_Buffer(-) \n");
    return 0;
}
...
static long AudDrv_btcvsd_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    PRINTK_BTCVSD_LV1("AudDrv_btcvsd_ioctl cmd = 0x%x arg = %lu\n", cmd, arg);
    ...
    switch (cmd)
    {
        ...
        {
            PRINTK_BTCVSD_LV1("AudDrv SET_BTCVSD_STATE \n");
            ...
            {
                btsco.uTXState = arg;
                PRINTK_BTCVSD_LV1("SET_BTCVSD_STATE set btsco.uTXState to 0x%lux \n", arg);
            }
            else //RX state
            {
                btsco.uRXState = arg;
                PRINTK_BTCVSD_LV1("SET_BTCVSD_STATE set btsco.uRXState to %lu \n", arg);
            }
            if (btsco.uTXState == BT_SCO_TXSTATE_IDLE && btsco.uRXState == BT_SCO_RXSTATE_IDLE)
            {
                PRINTK_BTCVSD_LV1("SET_BTCVSD_STATE disable BT IRQ disableBTirq = %d \n", disableBTirq);
                if (disableBTirq == 0)
                {
            ...
            else
            {
                if (disableBTirq == 1)
                {
                    PRINTK_BTCVSD_LV1("SET_BTCVSD_STATE enable BT IRQ disableBTirq = %d \n", disableBTirq);
...
static void AudDrv_BTCVSD_ReadFromBT(BT_SCO_PACKET_LEN uLen, kal_uint32 uPacketLength, kal_uint32 uPacketNumber, kal_uint32 uBlockSize, kal_uint32 uControl)
{
    ...
    PRINTK_BTCVSD_LV2("AudDrv_BTCVSD_ReadFromBT(+) btsco.pRX->iPacket_w=%d\n",btsco.pRX->iPacket_w);
    ...
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_ReadFromBT connsys_addr_rx=0x%x,ap_addr_rx=0x%x \n", connsys_addr_rx, ap_addr_rx);
    pSrc = (kal_uint8 *)ap_addr_rx;

    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_ReadFromBT()uPacketLength=%d,uPacketNumber=%d, btsco.uRXState=%d\n", uPacketLength, uPacketNumber,btsco.uRXState);
    AudDrv_BTCVSD_DataTransfer(BT_SCO_DIRECT_BT2ARM, pSrc, btsco.pRX->TempPacketBuf, uPacketLength, uPacketNumber, btsco.uRXState);
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_ReadFromBT()AudDrv_BTCVSD_DataTransfer DONE!!! uControl=0x%x,uLen=%d\n", uControl, uLen);
    ...
    for (i = 0; i < uBlockSize; i++)
    {
        ...
        if (packet_cnt == 30)
        {
            PRINTK_BTCVSD_LV1("AudDrv_BTCVSD_ReadFromBT()Test Packet Loss \n");
    ...
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_ReadFromBT(-) btsco.pRX->iPacket_w=%d\n",btsco.pRX->iPacket_w);
}
...
static void AudDrv_BTCVSD_WriteToBT(BT_SCO_PACKET_LEN uLen, kal_uint32 uPacketLength, kal_uint32 uPacketNumber, kal_uint32 uBlockSize)
{
    ...
    PRINTK_BTCVSD_LV2("AudDrv_BTCVSD_WriteToBT(+) btsco.pTX->iPacket_r=%d\n",btsco.pTX->iPacket_r);
    ...
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_WriteToBT connsys_addr_tx=0x%x,ap_addr_tx=0x%x \n", connsys_addr_tx, ap_addr_tx);
    ...
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_WriteToBT(-) btsco.pTX->iPacket_r=%d\n", btsco.pTX->iPacket_r);
}
...
static int AudDrv_BTCVSD_IRQ_handler(void)
{
    ...
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_IRQ_handler FILL PACKETBUF\n");
    ...
    PRINTK_BTCVSD_LV2("AudDrv_BTCVSD_IRQ_handler uControl = 0x%x, uPacketType = %d\n", uControl, uPacketType);
    ...
    PRINTK_BTCVSD_LV3("AudDrv_BTCVSD_IRQ_handler uPacketLength=%d, uPacketNumber=%d, uBufferCount_TX=%d, uBufferCount_TX=%d \n", uPacketLength, uPacketNumber, uBufferCount_TX, uBufferCount_RX);
    PRINTK_BTCVSD_LV2("btsco.uTXState=0x%x, btsco.uRXState=0x%x\n", btsco.uTXState, btsco.uRXState);
    ...
    else
    {
        if (btsco.pRX)
        {
            if (btsco.uRXState == BT_SCO_RXSTATE_RUNNING || btsco.uRXState == BT_SCO_RXSTATE_ENDING)
            {
                PRINTK_BTCVSD_LV2("AudDrv_BTCVSD_IRQ_handler pRX->fOverflow=%d, pRX->iPacket_w=%d, pRX->iPacket_r=%d, uBufferCount_RX=%d \n", btsco.pRX->fOverflow, btsco.pRX->iPacket_w, btsco.pRX->iPacket_r, uBufferCount_RX);
        ...
        if (btsco.pTX)
        {
            ...
            {
                PRINTK_BTCVSD_LV2("AudDrv_BTCVSD_IRQ_handler pTX->fUnderflow=%d, pTX->iPacket_w=%d, pTX->iPacket_r=%d, uBufferCount_TX=%d \n", btsco.pTX->fUnderflow, btsco.pTX->iPacket_w, btsco.pTX->iPacket_r, uBufferCount_TX);
    ...
    PRINTK_BTCVSD_LV2("writeToBT_cnt=%d, readFromBT_cnt=%d \n", writeToBT_cnt, readFromBT_cnt);
...
static ssize_t AudDrv_btcvsd_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        PRINTK_BTCVSD_LV3("AudDrv_btcvsd_write btsco.pTX->iPacket_w=%d, btsco.pTX->iPacket_r=%d\n",btsco.pTX->iPacket_w, btsco.pTX->iPacket_r);
        spin_lock_irqsave(&auddrv_BTCVSDTX_lock, flags);
        ...
        PRINTK_BTCVSD_LV2("AudDrv_btcvsd_write count=%d, copy_size=%d\n",count, copy_size);
        ...
        if (copy_size != 0)
        {
            ...
            if (BTSCOTX_WriteIdx + copy_size < btsco.pTX->u4BufferSize) // copy once
            {
                ...
                {
                    /*PRINTK_BTCVSD_LV3("mcmcpy btsco.pTX->PacketBuf+BTSCOTX_WriteIdx= %x data_w_ptr = %p copy_size = %x\n", btsco.pTX->PacketBuf+BTSCOTX_WriteIdx,data_w_ptr,copy_size);*/
                ...
                count -= copy_size;
                PRINTK_BTCVSD_LV3("AudDrv_btcvsd_write finish1, copy_size:%d, pTX->iPacket_w:%d, pTX->iPacket_r=%d, count=%d \r\n",  copy_size,btsco.pTX->iPacket_w,btsco.pTX->iPacket_r,count);
            }
            else  // copy twice
            {
                ...
                PRINTK_BTCVSD_LV3("AudDrv_btcvsd_write size_1=%d, size_2=%d \n",size_1,size_2);
                ...
                {
                    /*PRINTK_BTCVSD_LV3("mcmcpy btsco.pTX->PacketBuf+BTSCOTX_WriteIdx= %x data_w_ptr = %p size_1 = %x\n",  btsco.pTX->PacketBuf+BTSCOTX_WriteIdx,data_w_ptr,size_1);*/
                ...
                {
                    /*PRINTK_BTCVSD_LV3("mcmcpy btsco.pTX->PacketBuf+BTSCOTX_WriteIdx+size_1= %x data_w_ptr+size_1 = %p size_2 = %x\n", btsco.pTX->PacketBuf+BTSCOTX_WriteIdx+size_1,data_w_ptr+size_1,size_2);*/
                ...
                count -= copy_size;
                data_w_ptr += copy_size;
                PRINTK_BTCVSD_LV3("AudDrv_btcvsd_write finish2, copy size:%d, pTX->iPacket_w=%d,pTX->iPacket_r=%d, count:%d \r\n", copy_size,btsco.pTX->iPacket_w,btsco.pTX->iPacket_r,count );
            }
        }
        ...
        {
            kal_uint64 t1, t2;
            PRINTK_BTCVSD_LV2("AudDrv_btcvsd_write WAITING...btsco.pTX->iPacket_w=%d, count=%d \n",btsco.pTX->iPacket_w,count);
            t1 = sched_clock();
            ...
            t2 = sched_clock();
            PRINTK_BTCVSD_LV2("AudDrv_btcvsd_write WAKEUP...count=%d \n",count);
            t2 = t2 - t1; // in ns (10^9)
    ...
    PRINTK_BTCVSD_LV2("AudDrv_btcvsd_write written_size = %d, write_timeout_limit=%llu \n", written_size, write_timeout_limit);
    return written_size;
}
...
static ssize_t AudDrv_btcvsd_read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read btsco.pRX->iPacket_w=%d, btsco.pRX->iPacket_r=%d, count=%d\n",btsco.pRX->iPacket_w, btsco.pRX->iPacket_r,count);
        ...
        PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read read_size=%d, BTSCORX_ReadIdx_tmp=%d \n",read_size, BTSCORX_ReadIdx_tmp);
        PRINTK_BTCVSD_LV2("AudDrv_btcvsd_read finish0, read_count:%x, read_size:%x, u4DataRemained:%x, pRX->iPacket_r:0x%x, pRX->iPacket_w:%x \r\n", read_count,read_size,u4DataRemained,btsco.pRX->iPacket_r,btsco.pRX->iPacket_w);
        if (BTSCORX_ReadIdx_tmp + read_size < btsco.pRX->u4BufferSize) //copy once
        {
            PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read 1 copy_to_user target=0x%x, source=0x%x, read_size=%d \n",Read_Data_Ptr,((kal_uint8 *)btsco.pRX->PacketBuf+BTSCORX_ReadIdx_tmp),read_size);
            ...
            count -= read_size;
            PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read finish1, read_sizesize:%x, pRX->iPacket_r:0x%x, pRX->iPacket_w:%x, count:%x \r\n",  read_size,btsco.pRX->iPacket_r,btsco.pRX->iPacket_w,count );
        }
        else //copy twice
        {
            kal_uint32 size_1 = btsco.pRX->u4BufferSize - BTSCORX_ReadIdx_tmp;
            kal_uint32 size_2 = read_size - size_1;
            PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read 2-2 copy_to_user target=0x%x, source=0x%x, size_1=%d\n",Read_Data_Ptr,((kal_uint8 *)btsco.pRX->PacketBuf+BTSCORX_ReadIdx_tmp),size_1);
            if (copy_to_user((void __user *)Read_Data_Ptr, (void *)((kal_uint8 *)btsco.pRX->PacketBuf + BTSCORX_ReadIdx_tmp), size_1))
            {
                printk("AudDrv_btcvsd_read Fail 2 copy to user Read_Data_Ptr:%p, pRX->PacketBuf:%p, BTSCORX_ReadIdx_tmp:0x%x, read_size:%x", Read_Data_Ptr,btsco.pRX->PacketBuf, BTSCORX_ReadIdx_tmp, read_size);
                if (read_count == 0)
                {
                    return -1;
                }
            ...
            PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read 2-2 copy_to_user target=0x%x, source=0x%x,size_2=%d\n",(Read_Data_Ptr+size_1),((kal_uint8 *)btsco.pRX->PacketBuf+BTSCORX_ReadIdx_tmp+size_1),size_2);
            if (copy_to_user((void __user *)(Read_Data_Ptr + size_1), (void *)((kal_uint8 *)btsco.pRX->PacketBuf), size_2))
            {
                printk("AudDrv_btcvsd_read Fail 3 copy to user Read_Data_Ptr:%p, pRX->PacketBuf:%p, BTSCORX_ReadIdx_tmp:0x%x, read_size:%x", Read_Data_Ptr, btsco.pRX->PacketBuf, BTSCORX_ReadIdx_tmp, read_size);
                if (read_count == 0)
                {
                    return -1;
                }
            ...
            count -= read_size;
            Read_Data_Ptr += read_size;
            PRINTK_BTCVSD_LV3("AudDrv_btcvsd_read finish3, copy size_2:%x, pRX->iPacket_r:0x%x, pRX->iPacket_w:0x%x u4DataRemained:%x \r\n",   size_2,btsco.pRX->iPacket_r,btsco.pRX->iPacket_w);
        }
        ...
        if (count != 0)
        {
            ...
            PRINTK_BTCVSD_LV2("AudDrv_btcvsd_read WAITING... pRX->iPacket_r=%d, count=%d \n",btsco.pRX->iPacket_r,count);
            t1 = sched_clock();
            ...
            t2 = sched_clock();
            PRINTK_BTCVSD_LV2("AudDrv_btcvsd_read WAKEUP...count=%d \n",count);
            t2 = t2 - t1; // in ns (10^9)
    ...
    PRINTK_BTCVSD_LV2("AudDrv_btcvsd_read read_count = %d,read_timeout_limit=%llu \n", read_count, read_timeout_limit);
    return read_count;
}

static ssize_t AudDrv_btcvsd_debug_write(struct file *file, const char *buffer, unsigned long len, void *data)
{
    char token[256] = {'\0'};
    int log_level = 0;
    int ret;

    ret = sscanf(buffer, " %s %d\n", token, &log_level);
    if (ret < 0)
        return -EINVAL;

    if (strncmp(token, "EnableLog", 9) == 0) {
        if (log_level >= 0 && log_level <= 2) {
	    AudDrv_btcvsd_log_on = log_level;
        } else if (log_level == 3) {
            AudDrv_btcvsd_log_on = 0x4;
        } else {
            pr_err("%s() unsupported log_level(%d)\n", __func__, log_level);
        }
    } else
        pr_err("%s() unsupported command\n", __func__);
...
static const struct file_operations AudDrv_btcvsd_debug_fops = {
	.owner = THIS_MODULE,
	.write = AudDrv_btcvsd_debug_write,
};
...
static int AudDrv_btcvsd_mod_init(void)
{
    ...
    struct proc_dir_entry *btcvsd_dir;
    ...
    // register debug node
    btcvsd_dir = proc_mkdir("btcvsd", NULL);
    if (!btcvsd_dir) {
	pr_err("%s() error: to mkdir btcvsd\n", __func__);
	return -1;
    }
    entry = proc_create("debug", 00640, btcvsd_dir, &AudDrv_btcvsd_debug_fops);
    if (!entry) {
