...
#define WMT_DBG_PROCNAME "driver/wmt_dbg_c"
...
INT32 wmt_dbg_dsns_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
	if (WMTDSNS_FM_DISABLE <= par2 && WMTDSNS_MAX > par2) {
		...
		mtk_wcn_wmt_dsns_ctrl(par2, CHIP_TYPE_COMBO);
	} else {
...
INT32 wmt_dbg_hwver_get(INT32 par1, INT32 par2, INT32 par3)
{
	...
	mtk_wcn_wmt_hwver_get(CHIP_TYPE_COMBO);
	return 0;
}
...
INT32 wmt_dbg_func_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
	if (WMTDRV_TYPE_WMT > par2 || WMTDRV_TYPE_LPBK == par2) {
		if (0 == par3) {
			...
			mtk_wcn_wmt_func_off(par2, CHIP_TYPE_COMBO);
		} else {
			WMT_INFO_FUNC("function on test, type(%d)\n", par2);
			mtk_wcn_wmt_func_on(par2, CHIP_TYPE_COMBO);
		}
	} else {
...
INT32 wmt_dbg_ut_test(INT32 par1, INT32 par2, INT32 par3)
{
	...
	while ((i--) > 0) {
		...
		while ((j--) > 0) {
			...
			iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_BT, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### GPS On .... (%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_GPS, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### FM  On .... (%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_FM, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### WIFI On .... (%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_WIFI, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### ANT On .... (%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_ANT, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### BT  Off .... (%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_BT, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### GPS  Off ....(%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_GPS, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### FM  Off .... (%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_FM, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### WIFI  Off ....(%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_WIFI, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
			WMT_INFO_FUNC("#### ANT  Off ....(%d, %d)\n", i, j);
			iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_ANT, CHIP_TYPE_COMBO);
			if (iRet == MTK_WCN_BOOL_FALSE) {
				break;
			}
		}
