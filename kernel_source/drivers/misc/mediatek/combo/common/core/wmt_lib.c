...
INT32 wmt_lib_init(VOID)
{
	...
	osal_strncpy(pThraed->threadName, "mtk_wmtd_c", sizeof(pThraed->threadName));
...
static INT32 wmt_lib_ps_handler(MTKSTP_PSM_ACTION_T action)
{
	...
	if ((0 != mtk_wcn_stp_coredump_start_get(CHIP_TYPE_COMBO)) && (action == SLEEP)) {
...
MTK_WCN_BOOL wmt_lib_put_act_op(P_OSAL_OP pOp)
{
	...
	do {
		...
		if (0 != mtk_wcn_stp_coredump_start_get(CHIP_TYPE_COMBO)) {
