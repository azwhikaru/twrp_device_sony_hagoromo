...
//for mt6630, chage major number from 192 to 193
#define BT_DEV_MAJOR 193	/* never used number */
...
/* down(&wr_mtx); */
	 ...
	 */
	if (mtk_wcn_stp_is_rxqueue_empty(BT_TASK_INDX, CHIP_TYPE_COMBO)) {
		poll_wait(filp, &inq, wait);

		/* empty let select sleep */
		if ((!mtk_wcn_stp_is_rxqueue_empty(BT_TASK_INDX, CHIP_TYPE_COMBO)) || retflag) {
...
ssize_t BT_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (count > 0) {
		...
		written = mtk_wcn_stp_send_data(&o_buf[0], copy_size, BT_TASK_INDX, CHIP_TYPE_COMBO);
		if (0 == written) {
			retval = -ENOSPC;
...
ssize_t BT_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	retval = mtk_wcn_stp_receive_data(i_buf, count, BT_TASK_INDX, CHIP_TYPE_COMBO);
...
long BT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOC_BT_HWVER:
		...
		hw_ver_sym = mtk_wcn_wmt_hwver_get(CHIP_TYPE_COMBO);
	...
	case COMBO_IOCTL_FW_ASSERT:
		...
		bRet = mtk_wcn_wmt_assert(WMTDRV_TYPE_BT, arg, CHIP_TYPE_COMBO);
	...
	case COMBO_IOCTL_BT_IC_HW_VER:
		return mtk_wcn_wmt_ic_info_get(WMTCHIN_HWVER, CHIP_TYPE_COMBO);
		break;
	case COMBO_IOCTL_BT_IC_FW_VER:
		return mtk_wcn_wmt_ic_info_get(WMTCHIN_FWVER, CHIP_TYPE_COMBO);
		break;
	default:
		retval = -EFAULT;
...
static int BT_open(struct inode *inode, struct file *file)
{
	...
	/* turn on BT */
	if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_BT, CHIP_TYPE_COMBO)) {
		BT_WARN_FUNC("WMT turn on BT fail!\n");
		return -ENODEV;
	} else {
		retflag = 0;
		mtk_wcn_wmt_msgcb_reg(WMTDRV_TYPE_BT, bt_cdev_rst_cb, CHIP_TYPE_COMBO);
		BT_INFO_FUNC("WMT register BT rst cb!\n");
	}
#endif

	if (mtk_wcn_stp_is_ready(CHIP_TYPE_COMBO)) {
...
#endif
		mtk_wcn_stp_set_bluez(0, CHIP_TYPE_COMBO);
		...
		mtk_wcn_stp_register_event_cb(BT_TASK_INDX, BT_event_cb, CHIP_TYPE_COMBO);
...
static int BT_close(struct inode *inode, struct file *file)
{
	...
	if (current->pid == 1)
		return 0;
	retflag = 0;
	mtk_wcn_wmt_msgcb_unreg(WMTDRV_TYPE_BT, CHIP_TYPE_COMBO);
	mtk_wcn_stp_register_event_cb(BT_TASK_INDX, NULL, CHIP_TYPE_COMBO);

	if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_BT, CHIP_TYPE_COMBO)) {
...
static int BT_init(void)
{
	...
	//for mt6630, chage class from "stpbt" to "stpbt6630"
	stpbt_class = class_create(THIS_MODULE, "stpbt6630");
	if (IS_ERR(stpbt_class))
		goto error;
	//for mt6630, chage device from "stpbt" to "stpbt6630"
	stpbt_dev = device_create(stpbt_class, NULL, dev, NULL, "stpbt6630");
	if (IS_ERR(stpbt_dev))
		goto error;
#endif
...
static void BT_exit(void)
{
	dev_t dev = MKDEV(BT_major, 0);
	retflag = 0;
	mtk_wcn_stp_register_event_cb(BT_TASK_INDX, NULL, CHIP_TYPE_COMBO);	/* unregister event callback function */
#if REMOVE_MK_NODE
	device_destroy(stpbt_class, dev);
	class_destroy(stpbt_class);
	stpbt_class = NULL;
