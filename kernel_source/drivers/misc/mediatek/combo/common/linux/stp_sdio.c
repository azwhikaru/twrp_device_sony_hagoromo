...
UINT32 gStpSdioDbgLvl = STPSDIO_LOG_ERR;
#else
...
INT32 _stp_sdio_issue_fake_coredump (UINT8 *str)
{
	...
	mtk_wcn_stp_parser_data(&AssertStr[0], 4 + length + 1 + 2, CHIP_TYPE_COMBO);
	...
	mtk_wcn_stp_parser_data(&AssertStr[0], 4 + length + 2 + 2, CHIP_TYPE_COMBO);
	...
	while (!osal_thread_should_stop(&pInfo->tx_rx_thread)) {
		...
		if ((0 != pInfo->irq_pending) && (1 == pInfo->awake_flag)) {
			...
			if (chisr & RX_DONE) {
                ...
                mtk_wcn_stp_wmt_sdio_host_awake(CHIP_TYPE_COMBO);
			}
...
/* [COHEC_00006052] SW work-around solution:
   using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
		...
		if ((0 != pInfo->sleep_flag) && (0 == pInfo->wakeup_flag) && mtk_wcn_stp_is_ready(CHIP_TYPE_COMBO)
		    && (0 == pInfo->irq_pending)
		    && (0 == pInfo->firmware_info.tx_packet_num)
		    && (0 == pInfo->rx_pkt_len)
		    && !mtk_wcn_stp_coredump_start_get(CHIP_TYPE_COMBO)	/* f/w assert disable sdio sleep */
		    ) {
			/* pInfo->awake_flag = 0; */
			/* STPSDIO_INFO_FUNC("set firmware own! (sleeping)\n"); */
/* [COHEC_00006052] SW work-around solution:
		...
		if (1000 < while_loop_counter) {
			...
			STPSDIO_ERR_FUNC
			    ("sleeping check result. stp_is_ready(%d) irq_pending(%d) tx_packet_num(%d) rx_pkt_len(%d)\n",
			     mtk_wcn_stp_is_ready(CHIP_TYPE_COMBO), pInfo->irq_pending,
...
static VOID stp_sdio_rx_wkr(struct work_struct *work)
{
	...
	if (STP_SDIO_HDR_SIZE < bus_rxlen) {
        ...
        ret = mtk_wcn_stp_parser_data(bufp, bus_rxlen, CHIP_TYPE_COMBO);
#if STP_SDIO_DBG_SUPPORT && (STP_SDIO_TXDBG || STP_SDIO_TXPERFDBG)
...
using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
	...
	mtk_wcn_stp_register_if_tx(STP_SDIO_IF_TX, (MTK_WCN_STP_IF_TX)stp_sdio_tx, CHIP_TYPE_COMBO);
...
static INT32 stp_sdio_remove(const MTK_WCN_HIF_SDIO_CLTCTX clt_ctx)
{
	...
	mtk_wcn_stp_register_if_tx(STP_SDIO_IF_TX, NULL, CHIP_TYPE_COMBO);
...
static int stp_sdio_init(void)
{
	...
	ret = mtk_wcn_stp_wmt_sdio_op_reg(stp_sdio_own_ctrl, CHIP_TYPE_COMBO);
	if (ret) {
		STPSDIO_ERR_FUNC
...
static void stp_sdio_exit(void)
{
	...
	mtk_wcn_stp_register_if_tx(STP_SDIO_IF_TX, 0x0, CHIP_TYPE_COMBO);
