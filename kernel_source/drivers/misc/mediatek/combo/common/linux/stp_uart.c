...
/* //    spin_lock_init(&buf_lock); */
	...
	rd_idx = wr_idx = 0;
	stp_tty = tty;
	mtk_wcn_stp_register_if_tx(STP_UART_IF_TX, (MTK_WCN_STP_IF_TX)mtk_wcn_uart_tx, CHIP_TYPE_COMBO);
...
static void stp_uart_tty_close(struct tty_struct *tty)
{
	...
	mtk_wcn_stp_register_if_tx(STP_UART_IF_TX, NULL, CHIP_TYPE_COMBO);
...
/* read_lock(&g_stp_uart_rx_handling_lock); */
	...
	do {
		...
		mtk_wcn_stp_parser_data((UINT8 *) g_rx_data, how_much_get, CHIP_TYPE_COMBO);
		how_much_to_get = kfifo_len(g_stp_uart_rx_fifo);
...
static void stp_uart_rx_worker(struct work_struct *work)
{
	...
	while (kfifo_len(g_stp_uart_rx_fifo)) {
		...
		if (likely(read)) {
			mtk_wcn_stp_parser_data((UINT8 *) g_stp_uart_rx_buf, read, CHIP_TYPE_COMBO);
		}
	}
#else
...
static void stp_uart_tty_receive(struct tty_struct *tty, const u8 *data, char *flags, int count)
{
	...
	mtk_wcn_stp_parser_data((UINT8 *) data, (UINT32) count, CHIP_TYPE_COMBO);
...
static int mtk_wcn_stp_uart_init(void)
{
	...
	/*
	   mtk_wcn_stp_register_if_tx( mtk_wcn_uart_tx, CHIP_TYPE_COMBO);
	 */
...
static void mtk_wcn_stp_uart_exit(void)
{
	...
	mtk_wcn_stp_register_if_tx(STP_UART_IF_TX, NULL, CHIP_TYPE_COMBO);	/* unregister if_tx function */
