...
#define WIFI_DEV_FILE_NAME "wmtWifi6630"
...
VOID register_set_p2p_mode_handler_6630(set_p2p_mode handler, struct net_device *netdev) {
	WIFI_INFO_FUNC("(pid %d) register set p2p mode handler %p\n", current->pid, handler);
	pf_set_p2p_mode = handler;
	g_netdev = netdev;
}
EXPORT_SYMBOL(register_set_p2p_mode_handler_6630);
...
VOID register_set_dbg_level_handler_6630(set_dbg_level handler) {
	pf_set_dbg_level = handler;
}
EXPORT_SYMBOL(register_set_dbg_level_handler_6630);
 ...
 */
/*-----------------------------------------------------------------*/
INT32 wifi_reset_start_6630(VOID)
{
...
EXPORT_SYMBOL(wifi_reset_start_6630);
 ...
 */
/*-----------------------------------------------------------------*/
INT32 wifi_reset_end_6630(ENUM_RESET_STATUS_T status)
{
	struct net_device *netdev = NULL;
	PARAM_CUSTOM_P2P_SET_STRUC_T p2pmode;
	INT32 wait_cnt = 0;
	...
	if (status == RESET_FAIL) {
		...
		if (powered == 1) {
			...
			if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_WIFI, CHIP_TYPE_COMBO)) {
...
EXPORT_SYMBOL(wifi_reset_end_6630);
...
ssize_t WIFI_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
		...
		if (local[0] == '0') {
			...
			if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_WIFI, CHIP_TYPE_COMBO)) {
...
static int WIFI_init_6630(void)
{
...
#if REMOVE_MK_NODE
	wmtWifi_class = class_create(THIS_MODULE, WIFI_DEV_FILE_NAME);
	if (IS_ERR(wmtWifi_class))
		goto error;
	wmtWifi_dev = device_create(wmtWifi_class, NULL, dev, NULL, WIFI_DEV_FILE_NAME);
...
static void WIFI_exit_6630(void)
{
	dev_t dev = MKDEV(WIFI_major, 0);
	retflag = 0;
