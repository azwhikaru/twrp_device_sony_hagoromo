...
#define WMT_AEE_PROCNAME "driver/wmt_aee_c"
...
#define WMT_DRIVER_NAME "mtk_stp_wmt_combo"
...
static void wmt_combo_dev_early_suspend(struct early_suspend *h)
{
    osal_lock_sleepable_lock(&g_es_lr_lock);
    g_early_suspend_flag = 1;
    osal_unlock_sleepable_lock(&g_es_lr_lock);
    WMT_INFO_FUNC("@@@@@@@@@@wmt combo enter early suspend@@@@@@@@@@@@@@\n");
    if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_LPBK, CHIP_TYPE_COMBO)) {
...
static void wmt_combo_dev_late_resume(struct early_suspend *h)
{
    WMT_INFO_FUNC("@@@@@@@@@@wmt combo enter late resume@@@@@@@@@@@@@@\n");
    osal_lock_sleepable_lock(&g_es_lr_lock);
...
struct early_suspend wmt_combo_early_suspend_handler = {
    .suspend = wmt_combo_dev_early_suspend,
    .resume = wmt_combo_dev_late_resume,
};
#else
UINT32 g_early_suspend_flag = 0;
#endif
...
static INT32 wmt_pwr_on_thread (void *pvData)
{
...
#endif
	do {
		if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_LPBK, CHIP_TYPE_COMBO)) {
	...
	if (1 == g_early_suspend_flag)
	{
		...
		mtk_wcn_wmt_func_off(WMTDRV_TYPE_LPBK, CHIP_TYPE_COMBO);
	}
	osal_unlock_sleepable_lock(&g_es_lr_lock);
#endif	
...
#define REFRESH_TIME    300	/* sec */
	...
	if (query_cond) {
	    ...
	    bRet = mtk_wcn_wmt_therm_ctrl(WMTTHERM_ENABLE, CHIP_TYPE_COMBO);
		if (bRet == MTK_WCN_BOOL_TRUE)
		{
		    ctemp = mtk_wcn_wmt_therm_ctrl(WMTTHERM_READ, CHIP_TYPE_COMBO);
		    bRet = mtk_wcn_wmt_therm_ctrl(WMTTHERM_DISABLE, CHIP_TYPE_COMBO);
			if(bRet == MTK_WCN_BOOL_TRUE)
		    	wmt_lib_notify_stp_sleep();
			if (0 != (ctemp & 0x80))
			{
...
#define WMT_IOCTL_WMT_STP_ASSERT_CTRL   _IOW(WMT_IOC_MAGIC, 27, int)
	...
	case WMT_IOCTL_FUNC_ONOFF_CTRL:	/* test turn on/off func */
		...
		do {
			MTK_WCN_BOOL bRet = MTK_WCN_BOOL_FALSE;
			if (arg & 0x80000000) {
				bRet = mtk_wcn_wmt_func_on(arg & 0xF, CHIP_TYPE_COMBO);
			} else {
				bRet = mtk_wcn_wmt_func_off(arg & 0xF, CHIP_TYPE_COMBO);
			}
			iRet = (MTK_WCN_BOOL_FALSE == bRet) ? -EFAULT : 0;
		} while (0);
	...
	case WMT_IOCTL_LPBK_POWER_CTRL:
		...
		do {
			MTK_WCN_BOOL bRet = MTK_WCN_BOOL_FALSE;
			if (arg & 0x01) {
				bRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_LPBK, CHIP_TYPE_COMBO);
			} else {
				bRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_LPBK, CHIP_TYPE_COMBO);
			}
			iRet = (MTK_WCN_BOOL_FALSE == bRet) ? -EFAULT : 0;
		} while (0);
...
static int WMT_init(void)
{
...
#if REMOVE_MK_NODE
	wmt_class = class_create(THIS_MODULE, "stpwmtC");
	if (IS_ERR(wmt_class))
		goto error;
	wmt_dev = device_create(wmt_class, NULL, devID, NULL, "stpwmtC");
	if (IS_ERR(wmt_dev))
		goto error;
#endif
    ...
    register_early_suspend(&wmt_combo_early_suspend_handler);
...
#if CONSYS_EARLYSUSPEND_ENABLE
    unregister_early_suspend(&wmt_combo_early_suspend_handler);
