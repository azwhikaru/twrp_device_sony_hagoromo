...
VOID swTsfQuery(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	P_CMD_SW_DBG_CTRL_T prCmdSwCtrl = (P_CMD_SW_DBG_CTRL_T) (pucEventBuf);


	DBGLOG(SW4, INFO, ("swTsfCheck =%d\n",prCmdSwCtrl->u4Data));

	if (prCmdSwCtrl->u4Id == LOW_TSF_POSITION) {
		DBGLOG(SW4, INFO, ("TSF query low\n"));
		prAdapter->au4TsfLatch[0] = prCmdSwCtrl->u4Data;
	} else if (prCmdSwCtrl->u4Id == HIGH_TSF_POSITION) {
		DBGLOG(SW4, INFO, ("TSF query high\n"));
		prAdapter->au4TsfLatch[1] = prCmdSwCtrl->u4Data;
	} else {
		DBGLOG(SW4, INFO, ("swTsfQuery Error\n"));
	}
...
VOID swTsfQueryTimeout(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	P_CMD_SW_DBG_CTRL_T prCmdSwCtrl = (P_CMD_SW_DBG_CTRL_T) (pucEventBuf);


	DBGLOG(SW4, INFO, ("swTsfQueryTimeout =%d\n",prCmdSwCtrl->u4Data));

	
	prAdapter->au4TsfLatch[0] = 0;
	prAdapter->au4TsfLatch[1] = 0;
...
{
	ASSERT(prAdapter);
printk("TSF swCrDebugQueryTimeout\n");
