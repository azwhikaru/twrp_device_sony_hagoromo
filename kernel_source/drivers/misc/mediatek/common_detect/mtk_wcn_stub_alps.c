...
int mtk_wcn_wmt_soc_chipid_query(void)
{
	return gSocChipId;
}
...
{
	CMB_STUB_LOG_INFO("set Combo chipid (0x%x)\n",chipid);
	gComboChipId = chipid;
}

void mtk_wcn_wmt_set_soc_chipid(int chipid)
{
	CMB_STUB_LOG_INFO("set Soc chipid (0x%x)\n",chipid);
	gSocChipId = chipid;
}
...
EXPORT_SYMBOL(mtk_wcn_wmt_soc_chipid_query);
EXPORT_SYMBOL(mtk_wcn_wmt_set_chipid);
EXPORT_SYMBOL(mtk_wcn_wmt_set_soc_chipid);
