...
int hif_sdio_match_chipid_by_dev_id (const struct sdio_device_id *id)
{
	...
	for (index = 0; index < maxIndex; index++)
	{
		...
		if ((localId->vendor == id->vendor) && (localId->device == id->device))
		{
			...
			gCombo_ChipId = chipId;
			mtk_wcn_wmt_set_chipid(gCombo_ChipId);
			break;
		}
	}
	if (0 > chipId)
...
int sdio_detect_query_chipid(int waitFlag)
{
	...
	//gCombo_ChipId = 0x6628;
	if (0 == waitFlag)
		return gCombo_ChipId;
	if (0 <= hif_sdio_is_chipid_valid(gCombo_ChipId))
		return gCombo_ChipId;
