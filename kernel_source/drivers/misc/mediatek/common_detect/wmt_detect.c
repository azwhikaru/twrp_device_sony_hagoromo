...
unsigned int gWmtDetectDbgLvl = WMT_DETECT_LOG_ERR;
#else
...
static long wmt_detect_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		...
		case COMBO_IOCTL_SET_SOC_CHIP_ID:
			mtk_wcn_wmt_set_soc_chipid(arg);
			break;
		case COMBO_IOCTL_GET_REMOVE_KO:
			#if (MTK_WCN_REMOVE_KO)
				retval = 1;
				WMT_DETECT_INFO_FUNC("MTK_WCN_REMOVE_KO defined :%d\n", retval);
			#else
				retval = 0; 
				WMT_DETECT_INFO_FUNC("no MTK_WCN_REMOVE_KO defined\n");
			#endif
...
{
	WMT_DETECT_INFO_FUNC("wmt_detect_exit: 1111\n");
	...
	WMT_DETECT_INFO_FUNC("wmt_detect_exit: 2222\n");
	if(pDetectClass)
	{
		class_destroy(pDetectClass);
		pDetectClass = NULL;
	...
	WMT_DETECT_INFO_FUNC("wmt_detect_exit: 3333\n");
	cdev_del(&gWmtDetectCdev);
	unregister_chrdev_region(dev, WMT_DETECT_DEV_NUM);

	WMT_DETECT_INFO_FUNC("wmt_detect_exit: 4444\n");
#if !(MTK_WCN_REMOVE_KO)
/*deinit SDIO-DETECT module*/
	sdio_detect_exit();
#endif
	WMT_DETECT_INFO_FUNC("wmt_detect_exit: 5555\n");
