...
INT32 wmt_ctrl_rx(P_WMT_CTRL_DATA pWmtCtrlData/*UINT8 *pBuff, UINT32 buffLen, UINT32 *readSize*/)
{
    ...
    /* read_len = mtk_wcn_stp_receive_data(data, size, WMT_TASK_INDX, CHIP_TYPE_SOC); */
    readLen = mtk_wcn_stp_receive_data(pBuff, buffLen, WMT_TASK_INDX, CHIP_TYPE_SOC);
    ...
    )
{
    ...
    /*retval = mtk_wcn_stp_send_data(data, size, WMTDRV_TYPE_WMT, CHIP_TYPE_SOC);*/
    mtk_wcn_stp_flush_rx_queue(WMT_TASK_INDX);
    if (bRawFlag) {
        iRet = mtk_wcn_stp_send_data_raw(pData, size, WMT_TASK_INDX, CHIP_TYPE_SOC);
    }
    else {
        iRet = mtk_wcn_stp_send_data(pData, size, WMT_TASK_INDX, CHIP_TYPE_SOC);
    }
...
INT32  wmt_ctrl_stp_close(P_WMT_CTRL_DATA pWmtCtrlData)
{
    ...
    iRet = mtk_wcn_stp_register_event_cb(WMT_TASK_INDX, NULL, CHIP_TYPE_SOC); /* mtk_wcn_stp_register_event_cb */
    if (iRet) {
...
INT32  wmt_ctrl_stp_open(P_WMT_CTRL_DATA pWmtCtrlData)
{
    ...
    iRet = mtk_wcn_stp_register_event_cb(WMT_TASK_INDX, wmt_dev_rx_event_cb, CHIP_TYPE_SOC); /* mtk_wcn_stp_register_event_cb */
    if (iRet) {
