...
#define WMT_DEV_MAJOR 200 // never used number
#define WMT_DEV_NUM 1
#define WMT_DEV_INIT_TO_MS (2 * 1000)

#if CFG_WMT_DBG_SUPPORT
#define WMT_DBG_PROCNAME "driver/wmt_dbg_a"
#endif

#define WMT_DRIVER_NAME "mtk_stp_wmt_soc"
...
#define WMT_AEE_PROCNAME "driver/wmt_aee_a"
...
static void wmt_pwr_on_off_handler (struct work_struct *work)
{
    ...
    osal_lock_sleepable_lock(&g_es_lr_lock);
  	...
  	{
		do {
			if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_LPBK, CHIP_TYPE_SOC)) {
  	...
  	{
  		if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_LPBK, CHIP_TYPE_SOC)) {
...
INT32 wmt_dbg_dsns_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
    if (WMTDSNS_FM_DISABLE <= par2 && WMTDSNS_MAX > par2 )
    {
        ...
        mtk_wcn_wmt_dsns_ctrl(par2, CHIP_TYPE_SOC);
    }
    else
    {
...
INT32 wmt_dbg_hwver_get(INT32 par1, INT32 par2, INT32 par3)
{
    ...
    mtk_wcn_wmt_hwver_get(CHIP_TYPE_SOC);
    return 0;
}
...
INT32 wmt_dbg_func_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
    if (WMTDRV_TYPE_WMT > par2 || WMTDRV_TYPE_LPBK == par2)
    {
        if (0 == par3)
        {
            ...
            mtk_wcn_wmt_func_off(par2, CHIP_TYPE_SOC);
        }
        else
        {
            WMT_INFO_FUNC("function on test, type(%d)\n", par2);
            mtk_wcn_wmt_func_on(par2, CHIP_TYPE_SOC);
        }
    }
    else
    {
...
INT32 wmt_dbg_ut_test(INT32 par1, INT32 par2, INT32 par3)
{
    ...
    while((i--) > 0){
        ...
        while((j--) > 0){
            ...
            iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_BT, CHIP_TYPE_SOC); 
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### GPS On .... (%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_GPS, CHIP_TYPE_SOC); 
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### FM  On .... (%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_FM, CHIP_TYPE_SOC);
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### WIFI On .... (%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_WIFI, CHIP_TYPE_SOC); 
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### BT  Off .... (%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_BT, CHIP_TYPE_SOC); 
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### GPS  Off ....(%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_GPS, CHIP_TYPE_SOC); 
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### FM  Off .... (%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_FM, CHIP_TYPE_SOC); 
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
            WMT_INFO_FUNC("#### WIFI  Off ....(%d, %d) \n", i, j);
            iRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_WIFI, CHIP_TYPE_SOC);           
            if(iRet == MTK_WCN_BOOL_FALSE){
                break;
            }
        }
...
LONG wmt_dev_tm_temp_query(void)
{
    ...
    if(query_cond)
    {
        ...
        mtk_wcn_wmt_therm_ctrl(WMTTHERM_ENABLE, CHIP_TYPE_SOC);
        current_temp = mtk_wcn_wmt_therm_ctrl(WMTTHERM_READ, CHIP_TYPE_SOC);
        mtk_wcn_wmt_therm_ctrl(WMTTHERM_DISABLE, CHIP_TYPE_SOC);
...
#define WMT_IOCTL_ADIE_LPBK_TEST		_IOWR(WMT_IOC_MAGIC,26,char*)
    ...
    case WMT_IOCTL_FUNC_ONOFF_CTRL: /* test turn on/off func */
        ...
        do {
            ...
            if (arg & 0x80000000)
            {
                bRet = mtk_wcn_wmt_func_on(arg & 0xF, CHIP_TYPE_SOC);
            }
            else
            {
                bRet = mtk_wcn_wmt_func_off(arg & 0xF, CHIP_TYPE_SOC);
            }
            iRet = (MTK_WCN_BOOL_FALSE == bRet) ? -EFAULT : 0;
         } while (0);
        ...
        do{
            ...
            if (arg & 0x01)
            {
                bRet = mtk_wcn_wmt_func_on(WMTDRV_TYPE_LPBK, CHIP_TYPE_SOC);
            }
            else
            {
                bRet = mtk_wcn_wmt_func_off(WMTDRV_TYPE_LPBK, CHIP_TYPE_SOC);
            }
            iRet = (MTK_WCN_BOOL_FALSE == bRet) ? -EFAULT : 0;
          }while(0);
...
#if WMT_CREATE_NODE_DYNAMIC
	wmt_class = class_create(THIS_MODULE,"stpwmtA");
	if(IS_ERR(wmt_class))
		goto error;
	wmt_dev = device_create(wmt_class,NULL,devID,NULL,"stpwmtA");
	if(IS_ERR(wmt_dev))
		goto error;
#endif
