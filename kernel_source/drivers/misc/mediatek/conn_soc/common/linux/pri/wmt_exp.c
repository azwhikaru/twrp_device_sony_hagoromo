...
{
	if(on){
		mtk_wcn_wmt_func_on(type, CHIP_TYPE_SOC);
	}else{
		mtk_wcn_wmt_func_off(type, CHIP_TYPE_SOC);
	}
	return;
}
...
VOID mtk_wcn_wmt_exp_init()
{
	...
	mtk_wcn_wmt_exp_cb_reg(&wmtExpCb, CHIP_TYPE_SOC);
	return;
}

VOID mtk_wcn_wmt_exp_deinit()
{
	mtk_wcn_wmt_exp_cb_unreg(CHIP_TYPE_SOC);
