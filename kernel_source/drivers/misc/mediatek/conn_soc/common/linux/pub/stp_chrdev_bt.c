...
#define	CFG_SUPPORT_PROC_ENTRY				1
#define CFG_SUPPORT_MEDIA_SYNCHRONIZATION	1

#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
unsigned int  g_u4AddrOffset=0x80050;
UINT8 __iomem *pEmibaseaddr2;
UINT8 g_start = 0;
extern phys_addr_t gConEmiPhyBase;

int get_latched_tsf(unsigned int *ptr)
{
	//printk("%s() gConEmiPhyBase=0x%x g_u4AddrOffset=0x%x\n", __FUNCTION__, gConEmiPhyBase, g_u4AddrOffset);
	
	if (g_start == 0)
	{
		g_start ++;
		pEmibaseaddr2 = ioremap_nocache(gConEmiPhyBase ,0x100000);
	}
	...
	/* current_bt_clk @ address 0x80050 */
	*(ptr +2) = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset));
	printk("%s() *(ptr+2)=0x%x\n", __FUNCTION__, *(ptr+2));
	
	/* intraslot_offset @ address 0x80054 */
	*(ptr +3) = *((volatile UINT32 *)(pEmibaseaddr2 + g_u4AddrOffset+4));
	printk("%s() *(ptr+3)=0x%x\n", __FUNCTION__, *(ptr+3));

	/* ptr[5]: 0 for wifi; 1 for bt */
	*(ptr +5) = 1;
	printk("%s() *(ptr+5)=0x%x\n", __FUNCTION__, *(ptr+5));
...
//    down(&wr_mtx);
     ...
     */
    if (mtk_wcn_stp_is_rxqueue_empty(BT_TASK_INDX, CHIP_TYPE_SOC))
    {
        poll_wait(filp, &inq,  wait);

        /* empty let select sleep */
        if((!mtk_wcn_stp_is_rxqueue_empty(BT_TASK_INDX, CHIP_TYPE_SOC)) || retflag)
        {
...
ssize_t BT_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if (count > 0)
    {
        ...
        written = mtk_wcn_stp_send_data(&o_buf[0], copy_size, BT_TASK_INDX, CHIP_TYPE_SOC);
        if(0 == written)
        {
            retval = -ENOSPC;
...
ssize_t BT_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    retval = mtk_wcn_stp_receive_data(i_buf, count, BT_TASK_INDX, CHIP_TYPE_SOC);
...
long BT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case COMBO_IOC_BT_HWVER:
            ...
            hw_ver_sym = mtk_wcn_wmt_hwver_get(CHIP_TYPE_SOC);
        ...
        case COMBO_IOCTL_FW_ASSERT:
            ...
            bRet = mtk_wcn_wmt_assert(WMTDRV_TYPE_BT, arg, CHIP_TYPE_SOC);
...
static int BT_open(struct inode *inode, struct file *file)
{
    BT_INFO_FUNC("%s: major %d minor %d (pid %d)\n", __func__,
     ...
     /* turn on BT */
    if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_BT, CHIP_TYPE_SOC)) {
        BT_WARN_FUNC("WMT turn on BT fail!\n");
        return -ENODEV;
    }else{
        retflag = 0;
        mtk_wcn_wmt_msgcb_reg(WMTDRV_TYPE_BT, bt_cdev_rst_cb, CHIP_TYPE_SOC);
        BT_INFO_FUNC("WMT register BT rst cb!\n");
    }
#endif

    if (mtk_wcn_stp_is_ready(CHIP_TYPE_SOC)) {
...
#endif
        mtk_wcn_stp_set_bluez(0, CHIP_TYPE_SOC);
        ...
        mtk_wcn_stp_register_event_cb(BT_TASK_INDX, BT_event_cb, CHIP_TYPE_SOC);
...
static int BT_close(struct inode *inode, struct file *file)
{
    ...
    retflag = 0;
    mtk_wcn_wmt_msgcb_unreg(WMTDRV_TYPE_BT, CHIP_TYPE_SOC);
    mtk_wcn_stp_register_event_cb(BT_TASK_INDX, NULL, CHIP_TYPE_SOC);

    if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_BT, CHIP_TYPE_SOC)) {
...
#if CFG_SUPPORT_PROC_ENTRY
static ssize_t btmtk_read_proc(struct file *file, char __user *buf,
...
static ssize_t btmtk_write_proc(struct file *file, char __user *buf,
    size_t count, loff_t *ppos)
{
    printk("%s()\n", __FUNCTION__);
...
static const struct file_operations btmtk_proc_fops = {
    .read = btmtk_read_proc,
    .write = btmtk_write_proc,
};
#endif
...
#if CFG_SUPPORT_PROC_ENTRY
	proc_entry = proc_create("btmtk", 0660, NULL, &btmtk_proc_fops);
	if (proc_entry == NULL)
		printk("proc_entry null!\n");
#endif
#if CFG_SUPPORT_MEDIA_SYNCHRONIZATION
...
static void BT_exit(void)
{
    ...
    retflag = 0;
    mtk_wcn_stp_register_event_cb(BT_TASK_INDX, NULL, CHIP_TYPE_SOC);  // unregister event callback function
#if WMT_CREATE_NODE_DYNAMIC
	if(bt_dev)
	{
		device_destroy(bt_class,dev);
