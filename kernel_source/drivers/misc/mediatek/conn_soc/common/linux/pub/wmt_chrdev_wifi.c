...
#define WIFI_DRIVER_NAME "mtk_wmt_WIFI_chrdev_soc"
#define WIFI_DEV_MAJOR 153 // never used number
#define WIFI_DEV_FILE_NAME "wmtWifi"
...
void register_set_p2p_mode_handler_soc(set_p2p_mode handler, struct net_device *netdev) {
    WIFI_INFO_FUNC("(pid %d) register set p2p mode handler %p\n", current->pid, handler);
    pf_set_p2p_mode = handler;
	g_netdev = netdev;
}
EXPORT_SYMBOL(register_set_p2p_mode_handler_soc);
...
void register_set_dbg_level_handler_soc(set_dbg_level handler) {
    pf_set_dbg_level = handler;
}
EXPORT_SYMBOL(register_set_dbg_level_handler_soc);
/* endof mtk80707 */
...
EXPORT_SYMBOL(wifi_reset_start_soc);
...
int wifi_reset_end_soc()
{
    ...
    if (powered == 1) {
        ...
        if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_WIFI, CHIP_TYPE_SOC)) {
...
EXPORT_SYMBOL(wifi_reset_end_soc);
...
ssize_t WIFI_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
        ...
        if (local[0] == '0') {
            ...
            if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_WIFI, CHIP_TYPE_SOC)) {
...
static void WIFI_exit_soc(void)
{
    dev_t dev = MKDEV(WIFI_major, 0);
    retflag = 0;
#if WMT_CREATE_NODE_DYNAMIC
