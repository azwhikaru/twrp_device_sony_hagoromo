...
#endif
#ifdef CONFIG_BACKLIGHT_ICX_LM3630
...
#define SYSINFO_LCD_OTM8018 (0x0)
#define SYSINFO_LCD_HX8379 (0x1)
static int lcd_pwm_max = 1023;
static int lcd_pwm_default = 512;
static int lcd_pwm_min = 0;
static int lcd_default_level = 128;
#endif
...
#ifdef CONFIG_ARCH_MT8590_ICX
    switch (icx_pm_helper_sysinfo & SYSINFO_LCD_MASK){
        case SYSINFO_LCD_OTM8018 ://otm8108b lm3630
            lcd_pwm_max = 960;
            lcd_pwm_default = 228;
            lcd_pwm_min = 41;
            lcd_default_level = 128;
            break;
        case SYSINFO_LCD_HX8379: //hx8379c
            lcd_pwm_max = 900;
            lcd_pwm_default = 300;
            lcd_pwm_min = 100;
            lcd_default_level = 128;
...
#if defined(CONFIG_ARCH_MT8590_ICX)
    if((0 < level) && level < lcd_default_level){
        lcd_pwm_inclination = (float)(lcd_pwm_default-lcd_pwm_min)/lcd_default_level;
        param->pwmDuty = brightness_mapping(lcd_pwm_inclination*level + lcd_pwm_min);
    }
    else if(lcd_default_level <= level){
        lcd_pwm_inclination = (float)(lcd_pwm_max-lcd_pwm_default)/(LED_FULL-lcd_default_level);
        param->pwmDuty = brightness_mapping(lcd_pwm_inclination*(level-lcd_default_level) + lcd_pwm_default);
    }
    else{
        param->pwmDuty = brightness_mapping(0);
    }
#else
