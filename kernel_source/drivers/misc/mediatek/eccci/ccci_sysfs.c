...
static ssize_t ccci_attr_store(struct kobject *kobj, struct attribute *attr, const char *buf,
			       size_t count)
{
	ssize_t len = 0;
	struct ccci_attribute *a = container_of(attr, struct ccci_attribute, attr);
...
static ssize_t ccci_md_attr_store(struct kobject *kobj, struct attribute *attr, const char *buf,
				  size_t count)
{
	ssize_t len = 0;
	struct ccci_md_attribute *a = container_of(attr, struct ccci_md_attribute, attr);
...
int ccci_sysfs_add_modem(struct ccci_modem *md)
{
	...
	ret =
	    kobject_init_and_add(&md->kobj, &ccci_md_ktype, &ccci_sys_info->kobj, "MDSYS%d",
				 md->index + 1);
	if (ret < 0) {
		kobject_put(&md->kobj);
