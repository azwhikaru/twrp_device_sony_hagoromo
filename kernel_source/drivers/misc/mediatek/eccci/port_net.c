...
static int ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	int ret;
	static unsigned int ut_seq_num;
	int skb_len = skb->len;
	...
	if (skb_headroom(skb) < sizeof(struct ccci_header)) {
		CCCI_ERR_MSG(port->modem->index, NET,
			     "not enough header room on CH%d, len=%d header=%d hard_header=%d\n",
 ...
 tx_busy:
	if (tx_busy_retry_cnt % 20000 == 0) {
		CCCI_INF_MSG(port->modem->index, NET, "%s TX busy: retry_times=%d", port->name,
			     tx_busy_retry_cnt);
	}
	tx_busy_retry_cnt++;
