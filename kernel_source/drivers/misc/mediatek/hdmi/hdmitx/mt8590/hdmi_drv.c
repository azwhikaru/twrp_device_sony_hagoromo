...
static struct hrtimer r_cec_timer;
...
static enum hrtimer_restart cec_poll_isr(struct hrtimer *timer);
...
static void hdmi_hdmi_early_suspend(struct early_suspend *h)
{
	...
	spm_set_sleep_wakesrc(WAKE_SRC_CEC, true, 0);
}
...
static void hdmi_hdmi_late_resume(struct early_suspend *h)
{
	...
	spm_set_sleep_wakesrc(WAKE_SRC_CEC, false, 0);
}
...
{        
	...
	hrtimer_init(&r_cec_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
...
{
	hrtimer_start(&r_cec_timer, ktime_set(0, 1000000), HRTIMER_MODE_REL);
}
...
const HDMI_DRIVER* HDMI_GetDriver(void)
{
	static const HDMI_DRIVER HDMI_DRV_ops =
	{
        ...
        .cecsetretrycnt	  = hdmi_CECMWSetRetryCnt,
