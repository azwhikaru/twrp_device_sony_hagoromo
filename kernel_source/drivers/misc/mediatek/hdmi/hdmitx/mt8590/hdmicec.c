...
static DEFINE_KFIFO(notified_kfifo, CEC_FRAME_DESCRIPTION, RX_Q_SIZE);
...
void hdmi_cec_init(void)
{
	...
	if(1) //32k
	{
       //32k
		...
		vRegWrite4B(TR_TEST, 0x40004002);// Bpad enable¡BTx compared timing 0x19

		// CYJ.NOTE TX_EN, RX_EN: disable it
		vRegWrite4B(TR_CONFIG, 0x00000001);

		vRegWrite4B(RX_T_START_R, 0x007F0070);
	...
	vRegWrite4B(TR_CONFIG, 0x8fff1105);
...
void vHDMICECSetFreeTime(unsigned int freetime)
{
	vRegWriteFldAlign(TX_ARB,freetime,BCNT_NEW_INIT);
}

unsigned char u1HDMICECLineLevel(void)
{
	unsigned int temp;
	
	temp = *((volatile unsigned int*)(0xf00056a0));
	if (temp & (1<<10))
		return 1;
	return 0;
}

unsigned int u1HDMICECTxFsm(void)
{
	unsigned int temp;
	
	temp = *((volatile unsigned int*)(0xF00120AC));
	return temp & 0x7fff;
}

static int arbitration_err = 0;
static int arbitration_start = 0;
static unsigned char is_trigger_fail = 0;

void cec_arbitration_start(void)
{
	arbitration_err = 0;
	arbitration_start = 1;
	is_trigger_fail = 0;
...
void cec_arbitration_detect(void)
{
	unsigned int tx_fsm;
	unsigned char cec_line;

	if (arbitration_start != 1)
		return;

	tx_fsm = u1HDMICECTxFsm();
	cec_line = u1HDMICECLineLevel();

	if ((tx_fsm == 0x02) && (cec_line == 0)) {
		RESET_HW_TX();
		SetCECStatus(STATE_HW_RETX);
		arbitration_err = 1;
		arbitration_start = 0;
		is_trigger_fail = 1;
		ActiveTXFrame->arbitration = true;		
		return;
	}

	if ((!(tx_fsm == 0x02)) && (!(tx_fsm == 0x01))) {
		arbitration_err = 0;
		arbitration_start = 0;
		return;
	}
            ...
            if (IsCECStatus(STATE_HW_RETX))
	     {
			if (TX_FAIL_SOURCE())
			{
				...
				CLR_TX_FAIL_SOURCE();
	              ...
	              if ((TX_FAIL_RECORD()!=0) || (arbitration_err == 1))
			{
				DISABLE_ALL_TX_INT();
				...
				if (((TX_FAIL_RECORD() & ((1<<12)|(1<<16)|(1<<24)))!=0) && (arbitration_err != 1)) {
					ActiveTXFrame->is_need_retry = true;
				}
				arbitration_err = 0;
				RESET_HW_TX();
			}
...
{
		kfifo_in(&notified_kfifo, frame, 1);
...
static unsigned char _CEC_SendFrame(CEC_FRAME_DESCRIPTION* frame)
{
	...
	if (frame->is_need_retry)
		vHDMICECSetFreeTime(3);
	else
		vHDMICECSetFreeTime(5);
	frame->is_need_retry = false;

	TRIGGER_TX_HW();
	cec_arbitration_start();
	return errcode;
}
...
void CTS_RXProcess(CEC_FRAME_DESCRIPTION* frame)
{
	...
	if (frame->blocks.opcode == OPCODE_ABORT)
	{
		...
		CTSTestFrame.blocks.header.initiator = _rCECLaAddr.aui1_la[0];
 ...
 unsigned char hdmi_cec_isrprocess(unsigned char u1rxmode)
{
	...
	if (HW_RX_HEADER_ARRIVED())
	{
		if (IsCECStatus(STATE_TXING_FRAME))
		{
		ActiveTXFrame->is_need_retry = false;
		vHDMICECSetFreeTime(5);
		}		
	...
	{
		if (!HW_RX_HEADER_ARRIVED() && !IsCECStatus(STATE_RX_GET_NEW_HEADER) && !IsCECStatus(STATE_WAIT_RX_FRAME_COMPLETE))
		{
			u1ReceivedDst = GET_DST_FIELD_RECEIVING();
			HDMI_CEC_LOG( "u1ReceivedDst = 0x%08x\n", u1ReceivedDst);
			if ((u1ReceivedDst == _rCECLaAddr.aui1_la[0]) || (u1ReceivedDst == _rCECLaAddr.aui1_la[1]) ||
...
{
	HDMI_CEC_FUNC();
	CEC_FRAME_DESCRIPTION _frame;
	kfifo_out(&notified_kfifo,&_frame, 1);
    memcpy(frame, &_frame, sizeof(CEC_FRAME_DESCRIPTION));
}
 ...
 void hdmi_CECMWSend(CEC_SEND_MSG_T* msg)
{
	...
	cecMwTxMsg.arbitration = false;
	cecMwTxMsg.is_need_retry = false;
...
void hdmi_CECMWSetRetryCnt(unsigned int retry_cnt)
{
	HDMI_CEC_LOG("retry count:%d\n", retry_cnt);
	vRegWrite4BMsk(TX_ARB, retry_cnt << 12, 0xf << 12);
}
