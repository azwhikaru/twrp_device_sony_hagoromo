...
extern HDMI_RX_PACKET_INFO _RxPacket[MAX_PACKET];
 ...
 static unsigned char get_video_id_code(int res)
 {
	 switch (res)
	 {
 ...
 static const char* szHdmiResStr[HDMI_VIDEO_RESOLUTION_NUM] =
{
    [RES_480P] = "RES_480P",
    [RES_576P] = "RES_576P", 
    [RES_720P60HZ] = "RES_720P60HZ",
    [RES_720P50HZ] = "RES_720P50HZ",
    [RES_1080I60HZ] = "RES_1080I60HZ",
    [RES_1080I50HZ] = "RES_1080I50HZ",
    [RES_1080P30HZ] = "RES_1080P30HZ",
    [RES_1080P25HZ] = "RES_1080P25HZ", 
    [RES_1080P24HZ] = "RES_1080P24HZ", 
    [RES_1080P23_976HZ] = "RES_1080P23_976HZ", 
    [RES_1080P29_97HZ] = "RES_1080P29_97HZ", 
    [RES_1080P60HZ] = "RES_1080P60HZ", 
    [RES_1080P50HZ] = "RES_1080P50HZ", 
    [RES_3D_720P60HZ] = "RES_720p3d60Hz",
    [RES_3D_720P50HZ] = "RES_720p3d50Hz",
    [RES_3D_1080I60HZ] = "RES_1080i3d60Hz",
    [RES_3D_1080I50HZ] = "RES_1080i3d50Hz",
    [RES_3D_1080P24HZ] = "RES_1080P3d24Hz",
    [RES_3D_1080P23HZ] = "RES_1080P3d23Hz",
    [RES_480I] = "RES_480I",
    [RES_576I] = "RES_576I",
};
...
void vSetHDMITxPLL(unsigned char bResIndex, unsigned char bdeepmode)
{
  ...
  HDMI_PLL_FUNC();
  printk("bResIndex = %d@L%d\n",bResIndex,__LINE__);
  if((bResIndex==HDMI_VIDEO_720x480p_60Hz)||(bResIndex==HDMI_VIDEO_720x576p_50Hz) 
  	|| (bResIndex==HDMI_VIDEO_720x480i_60Hz) || (bResIndex==HDMI_VIDEO_720x576i_50Hz))
   u4Feq = 0; //27M
...
void vConfigHdmiSYS(unsigned char bResIndex)
{
  ...
  u4data = dReadHdmiTOPCK(HDMICLK_CFG_4);
  printk("[%s] u4data = 0x%x\n",__FUNCTION__,u4data);
...
#if CONFIG_MTK_HDMIRX_SUPPORT
		printk("enter %s bRes = %d\n",__FUNCTION__,bRes);
		vconfig_hdmirx_sys(bRes);
#endif
	}
...
void vSetHdmiHighBitrate(unsigned char fgHighBitRate)
{
  ...
  if(fgHighBitRate ==TRUE)
  {
	...
	if(_stAvdAVInfo.e_hdmi_aud_in == SV_SPDIF_RAW)
...
vEnableDSDConfig(BYTE chnNum)
{
	BYTE bData, bData1,bData2,bData3;
...
void vSetHDMIAudioIn(void)
{
  ...
  if(bData2 == PCM_24BIT)
   ...
   vAOUT_BNUM_SEL(AOUT_16BIT);
	...
	if(_stAvdAVInfo.e_hdmi_aud_in == SV_SPDIF && _stAvdAVInfo.e_iec_frame == IEC_768K)
	{
		vSetHdmiHighBitrate(1);
	}
	else
	{
  ...
  else //I2S input
  {
	...
	{
		vEnableDSDConfig(_stAvdAVInfo.ui1_aud_out_ch_number);
	}else
	{
...
#if CONFIG_MTK_HDMIRX_SUPPORT
void vHalBypassHDMI_NCTS(UINT32 u4RxNValue,UINT32 u4RxCTSValue)
{
  BYTE bData, bData1[NCTS_BYTES];
  BYTE bTemp;
  bData=0;
  vWriteByteHdmiGRL(GRL_NCTS, bData);//YT suggest 3 dummy N-CTS
  vWriteByteHdmiGRL(GRL_NCTS, bData);
  ...
  printk("[vHalBypassHDMI_NCTS] RxN=%d, Rx CTS=%d\n", u4RxNValue, u4RxCTSValue);

  bData1[0]= (u4RxCTSValue >> 24)& 0xff;
  bData1[1]= (u4RxCTSValue >> 16)& 0xff;
  bData1[2]= (u4RxCTSValue >> 8)& 0xff;
  bData1[3]= (u4RxCTSValue)& 0xff;
  bData1[4]= (u4RxNValue >> 16)& 0xff;
  bData1[5]= (u4RxNValue >> 8)& 0xff;
  bData1[6]= (u4RxNValue)& 0xff;
...
void vSetHDMISRC4N(void)
{
	BYTE bData;

	bData = bReadByteHdmiGRL(GRL_SHIFT_L1);
	bData |= 0x10;
	vWriteByteHdmiGRL(GRL_SHIFT_L1, bData);
}
...
#if CONFIG_MTK_HDMIRX_SUPPORT
if((fgIsHdmiRepeater()) && (hdmirx_audio_is_bypass_mode()))
{
	bhdmi_LCh_status[3]= _stAvdAVInfo.bhdmiLChstatus[3];
	bhdmi_LCh_status[4]= _stAvdAVInfo.bhdmiLChstatus[4];
	bhdmi_RCh_status[3]= _stAvdAVInfo.bhdmiRChstatus[3];
	bhdmi_RCh_status[4]= _stAvdAVInfo.bhdmiRChstatus[4];
}
else
{
#endif
...
static enum HDMI_AVI_INFO_RATIO aspect_ratio = HDMI_AVI_RATIO_16_9;

void vSetAVIAspectRatio(enum HDMI_AVI_INFO_RATIO ratio)
{
	aspect_ratio = ratio;
}
...
void vSendAVIInfoFrame(unsigned char ui1resindex, unsigned char ui1colorspace)
{
  ...
  if(aspect_ratio == HDMI_AVI_RATIO_4_3)
  	_bAviInfoFm[1] |= 0x10;
  else 
  	_bAviInfoFm[1] |= 0x20;
  _bAviInfoFm[1] |= 0x08;
  ...
  _bAviInfoFm[3] = get_video_id_code(ui1resindex);//bData4
  ...
  if((ui1resindex==HDMI_VIDEO_720x480i_60Hz) || (ui1resindex==HDMI_VIDEO_720x576i_50Hz))
    _bAviInfoFm[4]  = 0x01;
  else
  	_bAviInfoFm[4]  = 0x00;
...
void vSendVendorSpecificInfoFrame(unsigned char ui1resindex)
{
   ...
   bResTableIndex = get_video_id_code(ui1resindex);//bData4
...
void vChgtoSoftNCTS(unsigned char ui1resindex, unsigned char ui1audiosoft, unsigned char ui1hdmifs, unsigned char bdeepmode)
{
  ...
  if((fgIsHdmiRepeater()) && (hdmirx_audio_is_bypass_mode())){
	  u4N = u4GetRxHwNValue();
	  u4CTS = u4GetRxHwCTSValue();
	  vHalBypassHDMI_NCTS(u4N,u4CTS);
  	}
  else
#endif
  vHDMI_NCTS(ui1hdmifs, ui1resindex,  bdeepmode);
...
void vHalSendHdmiPacket(unsigned char bType, bool fgEnable, unsigned char *pr_bData)
{
	...
	{
		_bAviInfoFm[0] = *(pr_bData+4);
		_bAviInfoFm[1] = *(pr_bData+5);
		_bAviInfoFm[2] = *(pr_bData+6);
		_bAviInfoFm[3] = *(pr_bData+7);
		_bAviInfoFm[4] = *(pr_bData+8);
		vWriteHdmiGRLMsk(GRL_CTRL, 0, CTRL_AVI_EN);
		bLength = 14;
	}
...
{
	printk("HalHdmiRxI2S Bypass\n");
	vWriteHdmiGRLMsk(GRL_MIX_CTRL,MIX_CTRL_FLAT,MIX_CTRL_FLAT);//FOR spdif RAW data
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<4),(1<<4));//spdif clk(rx_mclk)
	//SPDIF  to HDMI1
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<8),(1<<8));//hdmi spdif bypass
	//bclk,lrck
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<5),(1<<5));
	//I2S DATA to HDMI1
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<7),(1<<7)|(1<<6));
...
void HalHdmiRxDSDBypass(void)
{
	...
	//MCLK to HDMI1
	vWriteHdmiGRLMsk(GRL_MIX_CTRL,1<<7,(1<<7));//FOR spdif RAW data
	
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<4),(1<<4));//spdif clk
	//SPDIF  to HDMI1
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<8),(1<<8));//mclk
	
	//bclk,lrck
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<5),(1<<5));

	//I2S to HDMI1
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<6),((1<<7)|(1<<6)));
	
	//TX setting
	vWriteHdmiGRLMsk(GRL_I2S_UV,1<<2,0x0F<<2);
	vWriteHdmiGRLMsk(GRL_AUDIO_CFG,0,1<<0);
	vWriteHdmiGRLMsk(GRL_CFG0,0x02,3<<0);	
	vWriteByteHdmiGRL(GRL_CH_SW0,0x10);	
	vWriteByteHdmiGRL(GRL_CH_SW1,0x9d);	
	vWriteByteHdmiGRL(GRL_CH_SW2,0xf5);	
	vWriteByteHdmiGRL(GRL_CH_SWAP,0x00);
	
	vWriteHdmiGRLMsk(GRL_CTRL,0,(1<<3));
	vWriteHdmiGRLMsk(GRL_AOUT_BNUM_SEL,0,1<<7); //don't aline
	vWriteHdmiGRLMsk(GRL_CFG1,0,1<<1); //i2s input
	vWriteHdmiGRLMsk(GRL_CFG5,0x04<<4,7<<4); //don't aline
	vWriteHdmiGRLMsk(GRL_I2S_UV,7<<2,7<<2); //6 ch
	vWriteHdmiGRLMsk(GRL_AUDIO_CFG,1<<7,1<<7); //dsd
}
void HalHdmirxSpdifBypass(void)
{
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<4),(1<<4));//spdif clk
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<8),(1<<8));//spdif data
	vWriteHdmiGRLMsk(GRL_CTRL,0,(1<<3));
	vWriteHdmiGRLMsk(GRL_AOUT_BNUM_SEL,0,1<<7); //don't aline
	vWriteHdmiGRLMsk(GRL_AUDIO_CFG,1<<1,1<<1); //Enable HBR packet
	vWriteHdmiGRLMsk(GRL_SHIFT_L1,1<<4,1<<4); //SRC 4N

}

/**********select video data and timing from dgi or dpi*/ 
void vselecthdmisrcpath(void)
{
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20,(1<<3),(1<<3));////select video data from rgb2hdmi
}
