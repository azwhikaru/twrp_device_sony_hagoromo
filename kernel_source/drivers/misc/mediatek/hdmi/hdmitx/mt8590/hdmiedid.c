...
void vShowEdidRawData(void)
{
  ...
  char *log_buf;
	...
	log_buf = kzalloc(2048, GFP_KERNEL);
	if(!log_buf)
		return ;
  ...
  HDMI_EDID_FUNC();
   ...
   for(bTemp=0; bTemp<2; bTemp++)
   {
   	 ...
   	 sprintf(log_buf + strlen(log_buf),"   EDID Block Number=#%d \n", bTemp);
   	 sprintf(log_buf + strlen(log_buf),"   | 00  01  02  03  04  05  06  07  08  09  0a  0b  0c  0d  0e  0f\n");
   	 sprintf(log_buf + strlen(log_buf),"===================================================================\n");
   	 j = 	bTemp*EDID_BLOCK_LEN;
     for(i=0; i< 8; i++)
     {
      if(((i*16)+j) < 0x10)
        sprintf(log_buf + strlen(log_buf),"0%x:  ", (i*16)+j);
      else
        sprintf(log_buf + strlen(log_buf),"%x:  ", (i*16)+j);
      ...
      for(k=0;k<16;k++)
      {
          ...
          sprintf(log_buf + strlen(log_buf),"%2x\n", _bEdidData[j+ (i*16+k)]);
          else
          sprintf(log_buf + strlen(log_buf),"0%x\n", _bEdidData[j+ (i*16+k)]);
        }
        }
      	else
      	{
      	   ...
      	   {
        if(_bEdidData[j+ (i*16+k)]> 0x0f)
          sprintf(log_buf + strlen(log_buf),"%2x  ", _bEdidData[j+ (i*16+k)]);
        else
          sprintf(log_buf + strlen(log_buf),"0%x  ", _bEdidData[j+ (i*16+k)]);
     }
     }
...
BOOL fgExtEdidChecksumOk(void)
{
	...
	for(u4Index=128; u4Index< EDID_SIZE; u4Index++)
	  bCheckSum =bCheckSum+_bEdidData[u4Index];

	if(bCheckSum==0)
	{
	  HDMI_LOG("[HDMI]EXT EDID CheckSum Ok\n");
      return (TRUE);
    }
	else
	{
	  HDMI_LOG("[HDMI]EXT EDID CheckSum Fail\n");
	  return (FALSE);
	}
...
void vShowEdidInformation(void)
{
    ...
    HDMI_EDID_FUNC();
  	...
  	sprintf(log_buf + strlen(log_buf),"[HDMI]EDID ver:%d/rev:%d \n",_HdmiSinkAvCap.ui1_Edid_Version, _HdmiSinkAvCap.ui1_Edid_Revision );
    sprintf(log_buf + strlen(log_buf),"[HDMI]EDID Extend Rev:%d  \n", _HdmiSinkAvCap.ui1_ExtEdid_Revision);
    if(_HdmiSinkAvCap.b_sink_support_hdmi_mode)
    {
      sprintf(log_buf + strlen(log_buf),"[HDMI]SINK Device is HDMI \n");
    }
    else
    {
      sprintf(log_buf + strlen(log_buf),"[HDMI]SINK Device is DVI \n");
    }

    if(_HdmiSinkAvCap.b_sink_support_hdmi_mode)
    sprintf(log_buf + strlen(log_buf),"[HDMI]CEC ADDRESS:%x  \n",_HdmiSinkAvCap.ui2_sink_cec_address);

	sprintf(log_buf + strlen(log_buf),"[HDMI]max clock limit : %d\n",_HdmiSinkAvCap.ui1_sink_max_tmds_clock);
    ...
    if(u4Res & SINK_480I)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1440x480I 59.94hz \n");
    if(u4Res & SINK_480I_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 2880x480I 59.94hz \n");
    if(u4Res & SINK_480P)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 720x480P 59.94hz\n");
    if(u4Res & SINK_480P_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1440x480P 59.94hz \n");
    if(u4Res & SINK_480P_2880)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 2880x480P 59.94hz \n");
    if(u4Res & SINK_720P60)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1280x720P 59.94hz \n");
    if(u4Res & SINK_1080I60)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080I 59.94hz \n");
    if(u4Res & SINK_1080P60)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080P 59.94hz \n");

    if(u4Res & SINK_576I)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1440x576I 50hz \n");
    if(u4Res & SINK_576I_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 2880x576I 50hz \n");
    if(u4Res & SINK_576P)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 720x576P 50hz \n");
    if(u4Res & SINK_576P_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1440x576P 50hz \n");
    if(u4Res & SINK_576P_2880)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 2880x576P 50hz \n");
    if(u4Res & SINK_720P50)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1280x720P 50hz \n");
    if(u4Res & SINK_1080I50)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080I 50hz \n");
    if(u4Res & SINK_1080P50)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080P 50hz \n");
    if(u4Res & SINK_1080P30)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080P 30hz \n");
    if(u4Res & SINK_1080P24)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080P 24hz \n");
    if(u4Res & SINK_1080P25)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT 1920x1080P 25hz \n");

    u4Res= (_HdmiSinkAvCap.ui4_sink_native_ntsc_resolution|_HdmiSinkAvCap.ui4_sink_native_pal_resolution);
    sprintf(log_buf + strlen(log_buf),"[HDMI]NTSC Native =%x \n", _HdmiSinkAvCap.ui4_sink_native_ntsc_resolution);
    sprintf(log_buf + strlen(log_buf),"[HDMI]PAL Native =%x \n", _HdmiSinkAvCap.ui4_sink_native_pal_resolution);
    if(u4Res & SINK_480I)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1440x480I 59.94hz \n");
    if(u4Res & SINK_480I_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 2880x480I 59.94hz \n");
    if(u4Res & SINK_480P)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 720x480P 59.94hz\n");
    if(u4Res & SINK_480P_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1440x480P 59.94hz \n");
    if(u4Res & SINK_480P_2880)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 2880x480P 59.94hz \n");
    if(u4Res & SINK_720P60)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1280x720P 59.94hz \n");
    if(u4Res & SINK_1080I60)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080I 59.94hz \n");
    if(u4Res & SINK_1080P60)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080P 59.94hz \n");
    if(u4Res & SINK_576I)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1440x576I 50hz \n");
    if(u4Res & SINK_576I_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 2880x576I 50hz \n");
    if(u4Res & SINK_576P)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 720x576P 50hz \n");
    if(u4Res & SINK_576P_1440)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1440x576P 50hz \n");
    if(u4Res & SINK_576P_2880)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 2880x576P 50hz \n");
    if(u4Res & SINK_720P50)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1280x720P 50hz \n");
    if(u4Res & SINK_1080I50)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080I 50hz \n");
    if(u4Res & SINK_1080P50)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080P 50hz \n");
    if(u4Res & SINK_1080P30)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080P 30hz \n");
    if(u4Res & SINK_1080P24)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080P 24hz \n");
    if(u4Res & SINK_1080P25)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Native resolution is 1920x1080P 25hz \n");


    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT RGB\n");
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_YCBCR_444)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT YCBCR 444\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_YCBCR_422)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT YCBCR 422\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_XV_YCC709)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT xvYCC 709\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_XV_YCC601)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT xvYCC 601\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA0)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT metadata P0\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA1)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT metadata P1\n");
    }
    if(_HdmiSinkAvCap.ui2_sink_colorimetry & SINK_METADATA2)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT metadata P2\n");
    }


    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_10_BIT)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT YCBCR 30 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_12_BIT)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT YCBCR 36 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit & HDMI_SINK_DEEP_COLOR_16_BIT)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT YCBCR 48 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_ycbcr_color_bit == HDMI_SINK_NO_DEEP_COLOR)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Not SUPPORT YCBCR Deep Color \n");

    if(_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_10_BIT)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT RGB 30 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_12_BIT)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT RGB 36 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_rgb_color_bit & HDMI_SINK_DEEP_COLOR_16_BIT)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT RGB 48 Bits Deep Color \n");
    if(_HdmiSinkAvCap.e_sink_rgb_color_bit == HDMI_SINK_NO_DEEP_COLOR)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Not SUPPORT RGB Deep Color \n");

	HDMI_LOG("%s\n",log_buf);
	memset(log_buf, 0, 2048);
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_LPCM)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT LPCM \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_AC3)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT AC3 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MPEG1)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT MPEG1 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MP3)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT AC3 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MPEG2)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT MPEG2 Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_AAC)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT AAC Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DTS)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT DTS Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_ATRAC)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT ATRAC Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DSD)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT SACD DSD Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DOLBY_PLUS)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT Dolby Plus Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DTS_HD)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT DTS HD Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_MAT_MLP)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT MAT MLP Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_DST)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT SACD DST Decode \n");
    if(_HdmiSinkAvCap.ui2_sink_aud_dec & HDMI_SINK_AUDIO_DEC_WMA)
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT  WMA Decode \n");
    ...
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT PCM Max 2CH, Fs is: %s \n", &cDstStr[0]);
    ...
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT PCM Max 6CH Fs is: %s \n", &cDstStr[0]);
    ...
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT PCM Max 7CH Fs is: %s \n", &cDstStr[0]);
    ...
    sprintf(log_buf + strlen(log_buf),"[HDMI]SUPPORT PCM Max 8CH, FS is: %s \n", &cDstStr[0]);

    }

    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FL_FR)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker FL/FR allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_LFE)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker LFE allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FC)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker FC allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RL_RR)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker RL/RR allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RC)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker RC allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_FLC_FRC)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker FLC/FRC allocated \n");
    if(_HdmiSinkAvCap.ui1_sink_spk_allocation & SINK_AUDIO_RLC_RRC)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Speaker RLC/RRC allocated \n");

	sprintf(log_buf + strlen(log_buf),"[HDMI]HDMI edid support content type =%x\n",_HdmiSinkAvCap.ui1_sink_content_cnc);
    sprintf(log_buf + strlen(log_buf),"[HDMI]Lip Sync Progressive audio latency = %d\n", _HdmiSinkAvCap.ui1_sink_p_audio_latency);
    sprintf(log_buf + strlen(log_buf),"[HDMI]Lip Sync Progressive video latency = %d\n", _HdmiSinkAvCap.ui1_sink_p_video_latency);
    if(_HdmiSinkAvCap.ui1_sink_i_latency_present)
    {
    sprintf(log_buf + strlen(log_buf),"[HDMI]Lip Sync Interlace audio latency = %d\n", _HdmiSinkAvCap.ui1_sink_i_audio_latency);
    sprintf(log_buf + strlen(log_buf),"[HDMI]Lip Sync Interlace video latency = %d\n", _HdmiSinkAvCap.ui1_sink_i_video_latency);
    }

    if(_HdmiSinkAvCap.ui1_sink_support_ai == 1)
    sprintf(log_buf + strlen(log_buf),"[HDMI]Support AI \n");
    else
    sprintf(log_buf + strlen(log_buf),"[HDMI]Not Support AI \n");

    sprintf(log_buf + strlen(log_buf),"[HDMI]Monitor Max horizontal size = %d\n", _HdmiSinkAvCap.ui1_Display_Horizontal_Size);
    sprintf(log_buf + strlen(log_buf),"[HDMI]Monitor Max vertical size = %d\n", _HdmiSinkAvCap.ui1_Display_Vertical_Size);


     if( _HdmiSinkAvCap.b_sink_hdmi_video_present==TRUE)
     	sprintf(log_buf + strlen(log_buf),"[HDMI]HDMI_Video_Present\n");
     else
     	sprintf(log_buf + strlen(log_buf),"[HDMI]No HDMI_Video_Present\n");

     if( _HdmiSinkAvCap.b_sink_3D_present==TRUE)
     	sprintf(log_buf + strlen(log_buf),"[HDMI]3D_present\n");
     else
     	sprintf(log_buf + strlen(log_buf),"[HDMI]No 3D_present\n");
