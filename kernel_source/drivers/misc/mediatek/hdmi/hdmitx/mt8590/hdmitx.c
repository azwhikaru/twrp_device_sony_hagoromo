 ...
 int flag_resolution_interlace(HDMI_VIDEO_RESOLUTION resolution)
{
  if((resolution==HDMI_VIDEO_1920x1080i_60Hz)||
  	...
  	(resolution==HDMI_VIDEO_1920x1080i3d_sbs_60Hz)||
  	 ...
  	 (resolution == HDMI_VIDEO_720x480i_60Hz)||
  	 (resolution == HDMI_VIDEO_720x576i_50Hz))
  	 return true;
  else
  	 return false;
}
...
static  HDMI_STATUS hdmi_drv_deinit(void)
{
    ...
    hdmi_dpi_free_irq();
    HDMI_LOG("hdmi_drv_deinit finish\n");
    return HDMI_STATUS_OK;
}
...
static void hdmi_dpi_config_update(void)
{
    ...
    if ((HDMI_VIDEO_1920x1080i_60Hz == p->output_video_resolution) ||
            ...
            (HDMI_VIDEO_720x480i_60Hz == p->output_video_resolution) ||
            (HDMI_VIDEO_720x576i_50Hz == p->output_video_resolution)
    #if defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT)
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
    ...
    if (enable)
    {
		...
		if (true == need_config)
        {
			...
			*(volatile unsigned int *)0xf4014f00 = 0x00000050;
        }
        else
        {
            disp_path_get_mutex_(dp_mutex_dst);
    ...
    else
    {
        if (-1 != dp_mutex_dst)
        {
            ...
            DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(dp_mutex_dst), 0);
            DISP_REG_SET(DISP_REG_CONFIG_MUTEX_SOF(dp_mutex_dst), 0);
...
{
    HDMI_FUNC();
	printk("[%s]vformat = %d L%d \n",__FUNCTION__,vformat,__LINE__);
    if(IS_HDMI_NOT_ON())
    {
...
static void hdmi_enable_clock(unsigned int enable)
{
	static unsigned int g_current_clk_status = 0;

	if (g_current_clk_status == 0 && enable == 1) {
		HDMI_MSG("hdmi enable clock\n");
		...
		enable_clock(MT_CG_INFRA_CEC, "HDMITX");
		g_current_clk_status = 1;
		disp_rdma1_intr_restore();
	} else if (g_current_clk_status == 1 && enable == 0) {
		disp_rdma1_intr_disable_and_clear();
		// Better to reset DMA engine before disable their clock
		RDMAStop(1);
		RDMAReset(1);
		pr_err("hdmi disable clock\n");
		...
		//disable_clock(MT_CG_INFRA_CEC, "HDMITX");
		//disable_clock(MT_CG_PERI_I2C3, "HDMITX"); // For CEC, can not disable
		g_current_clk_status = 0;
	}
}
...
void hdmi_video_config_ext(HDMI_VIDEO_RESOLUTION res, HDMI_VIDEO_OUT_MODE mode)
{
        ...
        HDMI_LOG("video resolution configuration, arg,mode =%ld, %ld,%ld\n", res, hdmi_reschange,mode);
	...
	HDMI_LOG(" hdmi power state(%d) is not on,change resolution \n",p->state);
        ...
        if ((hdmi_reschange == res) && (mode == HDMI_VIDEO_OUT_MODE_SRC))
        {
...
)
{
   ...
   hdmi_drv->colordeep(hdmi_colorspace);
	...
	if(fgHdmiRepeaterIsBypassMode())
	{
		...
		hdmi_drv->colordeep(hdmibypass_colorspace,_stAvdAVInfo.e_deep_color_bit);
	}
		...
		HDMI_LOG(" p->output_video_resolution = %d ,%s @ %d\n",p->output_video_resolution, __FUNCTION__,__LINE__);
        {
	    ...
	    *(volatile unsigned int *)0xf4014f00 = 0x00000051;
        up(&hdmi_update_mutex);
...
static void dpi_setting_res(u8 arg)
{
    ...
    switch (arg)
    {
      ...
      case HDMI_VIDEO_720x480i_60Hz:
	  {
			 ...
			 p->hdmi_width = 720;
			 p->hdmi_height = 480;
			 p->output_video_resolution = HDMI_VIDEO_720x480i_60Hz;
			  break;
	  }
      ...
      case HDMI_VIDEO_720x576i_50Hz:
	  {
			 ...
			 p->hdmi_width = 720;
			 p->hdmi_height = 576;
			 p->output_video_resolution = HDMI_VIDEO_720x576i_50Hz;
			  break;
	  }
...
void hdmi_test_on(bool flag)
{
	if (flag) {
		//hdmi_power_on();
		hdmi_av_enable(1);
		hdmi_video_config_ext(HDMI_VIDEO_720x480p_60Hz, HDMI_VIDEO_OUT_MODE_SRC);
	} else {
		hdmi_av_enable(0);
	}
}

extern void vSetAVIAspectRatio(enum HDMI_AVI_INFO_RATIO ratio);
extern void vSendAVIInfoFrame(unsigned char ui1resindex, unsigned char ui1colorspace);
extern void vSetHDCPSrm(struct HDMI_HDCP_SRM *srm);
...
static long hdmi_ioctl_ex(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
	   ...
	   case MTK_HDMI_CEC_SET_RETRY_CNT:
	   {
			hdmi_drv->cecsetretrycnt(arg);
			break;
	   }
	   ...
	   case MTK_HDMI_GET_EDID_RAW:
	   {
			...
			edidbuffer = kzalloc(EDID_SIZE, GFP_KERNEL);
			if(!edidbuffer){
				HDMI_LOG("err!!! no memory!\n");
				r = -ENOMEM;
				break;
			}

			memset(edidbuffer, 0 , EDID_SIZE);
	   		hdmi_drv->getedidrawdata(edidbuffer, EDID_SIZE);
		    if (copy_to_user((void __user *)arg, edidbuffer, EDID_SIZE)) {
	  ...
	  case MTK_HDMI_GET_EDID_SEGMENT:
	 {
		  HDMITX_EDID_SEGMENT *edid_segment;
		  char *edidbuffer;
		...
		edid_segment = kzalloc(sizeof(*edid_segment), GFP_KERNEL);
		if(!edid_segment){
			HDMI_LOG("err!!! no memory!\n");
			r = -ENOMEM;
			kfree(edidbuffer);
		  ...
		  memset(edidbuffer, 0 , EDID_SIZE);
		  memset(edid_segment, 0, sizeof(*edid_segment));
		  if (copy_from_user(edid_segment, (void __user *)arg, sizeof(*edid_segment))) {
			 HDMI_LOG("copy_from_user failed! line:%d \n", __LINE__);
			 r = -EFAULT;
		 } else {
			 if(edid_segment->segment >= 2){
				  HDMI_LOG("invalid arg, segment = %d \n", edid_segment->segment);
				  r = -EINVAL;
				  kfree(edidbuffer);
				  kfree(edid_segment);
				  break;
			 ...
			 hdmi_drv->getedidrawdata(edidbuffer, EDID_SIZE);
			 if(edid_segment->segment == 0)
			  memcpy(edid_segment->edid, edidbuffer, sizeof(edid_segment->edid));
			 else if(edid_segment->segment == 1)
			  memcpy(edid_segment->edid, &edidbuffer[256], sizeof(edid_segment->edid));

			 if (copy_to_user((void __user *)arg, edid_segment, sizeof(*edid_segment))) {
				  HDMI_LOG("copy_to_user failed! line:%d \n", __LINE__);
				  r = -EFAULT;
			 }
		 }
	  ...
	  case MTK_HDMI_SET_AVI_RATIO:
	  {
		vSetAVIAspectRatio(arg);
		vSendAVIInfoFrame(_stAvdAVInfo.e_resolution, _stAvdAVInfo.e_video_color_space);
		break;
	  }

	  case MTK_HDMI_HDCP_SET_SRM:
	  {
		struct HDMI_HDCP_SRM *srm;

		srm = kzalloc(sizeof(*srm), GFP_KERNEL);
		if(!srm) {
			HDMI_LOG("err!!! no memory!\n");
			r = -ENOMEM;
			break;
		 ...
		 if (copy_from_user(srm, (void __user *)arg, sizeof(*srm))) {
			 HDMI_LOG("copy_from_user failed! line:%d \n", __LINE__);
			 r = -EFAULT;
		 } else {
			HDMI_LOG("copy_from_user success! line:%d \n", __LINE__);
			vSetHDCPSrm(srm);
		 }
	...
	case MTK_HDMI_COLOR_DEEP:
       {
		...
		{
			unsigned int hdmi_sta;
			hdmi_sta = hdmi_drv->get_state();
			if (copy_to_user((void __user *)arg, &hdmi_sta, sizeof(unsigned int))) {
		           HDMI_LOG("copy_to_user failed! line:%d \n", __LINE__);
		           r = -EFAULT;
		       }
		 	break;
       ...
       case MTK_HDMI_AUDIO_VIDEO_ENABLE:
		...
		case MTK_HDMI_TMDS_ON_OFF:
		{
			if(hdmi_drv->tmdsonoff)
            	hdmi_drv->tmdsonoff(arg);
			break;
		}
