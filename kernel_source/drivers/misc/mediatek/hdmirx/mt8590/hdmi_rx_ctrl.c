...
HDMIRX_HDCP_KEY usersethdmirxkey;
...
void vHDMIRxHpdLoop(void)
{
	...
	{
		bInitHDCP++;
        printk(" **************only internel hdcp test1******************* \n");
#if 0//def CONFIG_MTK_IN_HOUSE_TEE_SUPPORT
        printk(" **************only internel hdcp test2******************* \n");
		fgCaHDMIRXInstallHdcpKey(usersethdmirxkey.u1Hdcprxkey,384);
//#else
		vHDMIRxLoadHdcpKey();
#endif
		vHDMIHDCPRst();
...
//digtal part power on 
	...
	/*********************PMIC enable 3.3V,1.8V,1.2V*********************/
	hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_3300, "HDMIRX");
	hwPowerOn(MT6323_POWER_LDO_VGP2, VOL_1800, "HDMIRX");
	hwPowerOn(MT6323_POWER_LDO_VGP3, VOL_1200, "HDMIRX");
	...
	ret += enable_clock(MT_CG_INFEA_DDCCI, "hdmirx");
...
// digital part power off
	...
	ret += disable_clock(MT_CG_INFEA_DDCCI, "hdmirx");
    return HDMIRX_STATUS_OK;
...
{//check color space
    ...
    if (vHalCheckRxIsHdmiMode())
    {
        ...
        if ((bReadData & 0x60)== 0x00) // RGB
            return HDMIRX_RGB;
        else if((bReadData & 0x60)== 0x20)//Ycbcr422
            return HDMIRX_YCBCR_422;
		else if((bReadData & 0x60)== 0x40) //YCBCR444
			return HDMIRX_YCBCR_444;
		else
			return HDMIRX_XV_YCC;
    }
    else
    {
        return 1;
...
UINT8 bHDMI422Input(void)
{
    ...
    else
    {
        if ((u1HalReadAviType() &0x60) == 0x20 )
            return HDMIRX_YCBCR_422;
        else if((u1HalReadAviType() &0x60) == 0x40 )
            return HDMIRX_YCBCR_444;
		else if((u1HalReadAviType() &0x60) == 0x00 )
			return HDMIRX_RGB;
		else 
			return HDMIRX_XV_YCC;
    }
...
void vHDMIRXColorSpaceConveter(void)
{
    ...
    {//YUV
        if(bHDMI422Input()== HDMIRX_YCBCR_444)
        {//YUV444 input
...
void vRxHDCPSetTxKsv(BYTE bTxDownStream, UINT16 u2TxBStatus, BYTE *prbTxBksv, BYTE *prbTxKsvlist, BOOL fgTxVMatch)
{
    ...
    {
        printk("[HDMI RX][HDCP] !!!Error!! TX dowstream over 9 = %d \n", _TxDownStreamCount) ;
...
void RxMergeBstatus(void)
{
    ...
    printk(" [HDMI RX][HDCP]_fgUseModifiedDepth = 0x%x \n",_fgUseModifiedDepth);
    printk(" [HDMI RX][HDCP]_u1ModifiedDepth = 0x%x \n",_u1ModifiedDepth);
...
void hdmirx_hdcpkey(unsigned char *pbhdcprxkey)
{
#if defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT)
	int i = 0;
	if (pbhdcprxkey != NULL) {
		for(i = 0; i < 384; i += 8) {
			printk("key1 = %3d: 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x\n", i, pbhdcprxkey[i], pbhdcprxkey[i+1], pbhdcprxkey[i+2], pbhdcprxkey[i+3], pbhdcprxkey[i+4], pbhdcprxkey[i+5], pbhdcprxkey[i+6], pbhdcprxkey[i+7]);
		}
		memcpy(usersethdmirxkey.u1Hdcprxkey, (unsigned char*)pbhdcprxkey, sizeof(usersethdmirxkey.u1Hdcprxkey));
	}
#endif
}
...
void vSetRxHdcpStatus(RxHDCPStateType bStatus)
{
    ...
    printk("[HDMI RX][HDCP]Set RX HDCP STATUS = %s\n",cRxHdcpStatus[_RxHDCPState]);
}
...
void vHDMIRxHdcpService(void)
{
    ...
    if(_bHDMIRxHDCPStatus != _bNewHDMIRxHDCPStatus)
    {
        ...
        if(!_bNewHDMIRxHDCPStatus)
        {
            if(!fgAVMUTE)
            {
                printk("[HDMI RX][HDCP] Upstream disable Enc,notify Tx disable HDCP\n" );
                RxAuthStartInt();
				hdmi_enablehdcp(1);
            }
        }
    ...
    if(!u1HalChkDataEnableExist())
    {
        ...
        {
            printk("[HDMI RX][HDCP] No detect Data Enable signal,Reset Rx Hdcp\n" );
            RxAuthStartInt();
    ...
    {
        {
            ...
            if(_u2RxRiShadow!=_u2RxRiShadowOld)
            {
                ...
                printk("[HDMI RX][HDCP][SINK MODE] Rx Ri =0x%x\n",_u2RxRiShadow);
            }
        }
        return;
    }
...
{
    if(bHDMIInputType()!=HDMIRX_RGB) // YCbCr
    {
        if(bHDMI422Input() == HDMIRX_YCBCR_444)	// 4:4:4
        {
            printk("[HDMI RX]Color Space = YCbCr444 \n");
        }
        else if(bHDMI422Input() == HDMIRX_YCBCR_422)
        {
...
void vHdmiRxGetPacketData(void)
{
    ...
    if(fgHalIsINTR3_NEW_AVI())
    {
        ...
        if(bcount == 19)
        {
			...
			HDMIRX_LOG("[%s] enter L%d _RxPacket[%d].fgValid = %d\n",__FUNCTION__,__LINE__,bIndex,_RxPacket[bIndex].fgValid);
...
extern unsigned char oEDIDBuff[128];
void vEdidUpdataChk(void)
{
   ...
   // bData = fgHDMIAudRxBypassMode();
    if((_u1TxEdidReady!= _u1TxEdidReadyOld)&&(rx_edid_change))
    {
		printk( "[HDMI RX][EDID]_u1TxEdidReadyOld = %d,rx_edid_change =%d\n",_u1TxEdidReadyOld,rx_edid_change);
		...
		/**** EDID is handle by sony*/
		HDMIRX_LOG("MTK set hpd Low in %s\n",__FUNCTION__);
		vHDMIHPDLow(HDMI_SWITCH_1);
        ...
        else
        {
			...
			for(i=0;i<128;i++){
				oEDIDBuff[i] = _Usersetedidtodrv.u1HdmiRxEDID[i];
			}
			vWriteEDIDBlk0(EEPROM0, oEDIDBuff);
			for(i=0;i<128;i++){
				oEDIDBuff[i] = _Usersetedidtodrv.u1HdmiRxEDID[i+0x80];
			}
			vWriteEDIDBlk1(EEPROM0, oEDIDBuff);
			
			if(vGetTxPortEdid(oEDIDBuff,0,128) == TRUE){
				HDMIRX_LOG("MTK set hpd High in %s\n",__FUNCTION__);
...
bool hdmirx_audio_is_bypass_mode(void)
{
    if(hdmirxbypassmode)
        return TRUE;
    else
...
unsigned int hdmirx_drv_read(unsigned short u2Reg)
{
    unsigned int u4Data;
    internal_hdmirx_read(HDMIRX_REG_BASE+u2Reg, &u4Data);
	HDMIRX_LOG("[R]addr = 0x%04x, data = 0x%08x\n", u2Reg, u4Data);
	return u4Data;
}

void hdmirx_drv_write(unsigned short u2Reg, unsigned int u4Data)
{
    HDMIRX_LOG("[W]addr= 0x%04x, data = 0x%08x\n", u2Reg, u4Data);
    internal_hdmirx_write(HDMIRX_REG_BASE+u2Reg, u4Data);
}

unsigned int hdmirx_ana_read(unsigned short u2Reg)
{
    unsigned int u4Data;
    internal_hdmirx_read(HDMIRX_ANA_REG_BASE+u2Reg, &u4Data);
	HDMIRX_LOG("[R]addr = 0x%04x, data = 0x%08x\n", u2Reg, u4Data);
	return u4Data;
}

void hdmirx_ana_write(unsigned short u2Reg, unsigned int u4Data)
{
    HDMIRX_LOG("[W]addr= 0x%04x, data = 0x%08x\n", u2Reg, u4Data);
    internal_hdmirx_write(HDMIRX_ANA_REG_BASE+u2Reg, u4Data);
}
