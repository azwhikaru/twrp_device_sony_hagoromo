...
UINT8 bDviStdTimingSearch(UINT8 bMode, UINT16   wDviHclk, UINT8 bDviVclk, UINT16 wDviHtotal,UINT16 wDviWidth)
{
	...
	HDMI_RX_SYNC_LOG("[HDMI RX] bMode = %d,wDviHclk = %d,bDviVclk = %d,wDviHtotal = %d,wDviWidth =%d\n",bMode,wDviHclk,bDviVclk,wDviHtotal,wDviWidth);
    //setch hdmi timing
    do
    {   
	...
	switch (_bDviModeDetState)
	{
		...
		case DVI_SEARCH_STATE:
			...
			if(_bDviTiming == MODE_NOSUPPORT)
			{
				...
				{
					HDMI_RX_SYNC_LOG("[HDMI RX]md: MODE_NOSUPPORT %d found\n", _bNotSupportCnt);
					break;
				}
			}
		...
		case DVI_MODE_CHG_DONE_STATE:
			...
			HDMI_RX_SYNC_LOG("[HDMI RX]_bDviTiming = %d\n", _bDviTiming);  
			...
			if(fgHdmiRepeaterIsBypassMode() && (_bVDITiming < RES_MODE_NUM))
			{
				if(_bHDMIColorSpace == HDMIRX_YCBCR_422)
					rInput.ePinType = 6;
				else
					rInput.ePinType = 9;
				rInput.eInputMode = 1;
...
void vDviChkModeChange(void)
{
	...
	if (_bDviModeChged)
	{
		...
		if((Linux_HAL_GetDeltaTime(&rDelta, &_rDviModeChgTime, &rCurTime)== TRUE))
		{
			...
			HDMI_RX_SYNC_LOG("[HDMI RX]mc:  Time out to force mode detect done \n");
			...
			vVSWGetHDMIRXStatus(_HdmiRxTimingStatus);
			HDMI_RX_SYNC_LOG("[HDMI RX][NFY]HDMI RX UNLOCK L%d\n",__LINE__);  
			vDVISetModeDone();
		}
	}
	...
	switch (_bDviChkState)
	{
		...
		if (bDVIGetHAct())
		{
			HDMI_RX_SYNC_LOG("[HDMI RX]mc: from no signal to wait stable\n");
			_bDVIEQFlag=1;
		...
		else
		{
			...
			{
				HDMI_RX_SYNC_LOG("[HDMI RX]mc: DVI_NO_SIGNAL to force mode done \n");
				_bPWOFFCnt=0;
				_bDviTiming = MODE_NOSIGNAL;
				_HdmiRxTimingStatus = VSW_COMP_NFY_UNLOCK;
				vVSWGetHDMIRXStatus(_HdmiRxTimingStatus);
	...
	case DVI_CHK_MODECHG:
		...
		{
			vHDMIRXColorSpaceConveter();
			printk("[HDMI RX]mc: color space change and trigger mode change bHDMIInputType() =%d,_bHDMIColorSpace = %d \n", bHDMIInputType(),_bHDMIColorSpace );
			_bDVIEQFlag=0;									
			_bDviChkState = DVI_WAIT_STABLE  ;
			vDviInitial();		
		}
		...
		||(_bHDMI422Input!= bHDMI422Input())||(_bHDMIITCFlag!= bHDMIITCFlag())||(_bHDMIITCContent!= bHDMIITCContent())||(_bHDMIHDCPStatus!= bHDMIHDCPStatusGet()))
		{
			...
			{		
				if((_bHDMIAspectRatio!= bHDMIAspectRatio())||(_bHDMIAFD!= bHDMIAFD()))
				{
					...
					_bHDMIAspectRatio= bHDMIAspectRatio();
					_bHDMIAFD = bHDMIAFD();
					printk("[HDMI RX][NFY]HDMI RX ASPECT CHG _bHDMIAspectRatio = %d,_bHDMIAFD = %d\n",_bHDMIAspectRatio,_bHDMIAFD);  
			...
			if(_bHDMI422Input!= bHDMI422Input())
			{
				...
				printk("[HDMI RX][NFY]HDMI RX COLOR SPACE CHG,_bHDMI422Input = %d\n",_bHDMI422Input);  
			...
			((wvtemp >= (_wDviTmpVtotal - 2)) && (wvtemp <= (_wDviTmpVtotal + 2))))
			{
				...
				if(_bDviMdChgCnt++ > _wDVI_WAIT_STABLE_COUNT)
				{
					...
					if(fgHalCheckRxIsVResStable())
					{
						...
						HDMI_RX_SYNC_LOG("[HDMI RX]_bDviMdChgCnt =%d  H/V Clk Chg, V:%d H:%d \n",_bDviMdChgCnt, wvtemp, whtemp);
						HDMI_RX_SYNC_LOG("[HDMI RX]pre H/V Clk Chg, V:%d H:%d\n", _wDviVTotal, _wDviHtotal);
...
{ 
	...
	if(bHDMIInputType()!= HDMIRX_RGB) {//YUV
		if(bHDMI422Input()== HDMIRX_YCBCR_422) {
			printk("[HDMI RX]YC422_24BIT,SRC CSP is HDMIRX_YCBCR_422\n");
			pv_get_info->ePinType = YC422_24BIT;
			pv_get_info->esrccsp = HDMIRX_YCBCR_422;
		} else {
			printk("[HDMI RX]YC444_36BIT,SRC CSP is HDMIRX_YCBCR_444\n");
			pv_get_info->ePinType = YC444_36BIT;
			pv_get_info->esrccsp = HDMIRX_YCBCR_444;
		}
   	} else {
		printk("[HDMI RX]YC444_36BIT,SRC CSP is HDMIRX_RGB\n");
		pv_get_info->ePinType = YC444_36BIT;
		pv_get_info->esrccsp = HDMIRX_RGB;
   	}
    
    if(bHDMIDeepColorStatus()== RX_NON_DEEP)
    {
        printk("[HDMI RX]DEEP COLOR = 24 BIT \n");
		pv_get_info->esrcdeepcolor = HDMIRX_NO_DEEP_COLOR;
    }
    else if(bHDMIDeepColorStatus()== RX_30BITS_DEEP_COLOR)
    {
        printk("[HDMI RX]DEEP COLOR = 30 BIT \n");
		pv_get_info->esrcdeepcolor = HDMIRX_DEEP_COLOR_10_BIT;
    }
    else if(bHDMIDeepColorStatus()== RX_36BITS_DEEP_COLOR)
    {
        printk("[HDMI RX]DEEP COLOR = 36 BIT \n");
		pv_get_info->esrcdeepcolor = HDMIRX_DEEP_COLOR_12_BIT;
    }
    else if(bHDMIDeepColorStatus()== RX_48BITS_DEEP_COLOR)
    {
        printk("[HDMI RX]DEEP COLOR = 48 BIT \n");
		pv_get_info->esrcdeepcolor = HDMIRX_DEEP_COLOR_16_BIT;
    }
    ...
    else
	{
		...
		printk("pv_get_info->eAspectRatio = %d @ %d \n", pv_get_info->eAspectRatio,__LINE__);
	}
