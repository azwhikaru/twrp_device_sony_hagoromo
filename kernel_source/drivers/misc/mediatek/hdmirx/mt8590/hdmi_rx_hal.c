...
extern HDMI_RX_EDID_T _Usersetedidtodrv;
...
void vHalRxHwInit(void)
{
	...
	HDMIRX_WRITE32_MASK(REG_SYS_CTRL,RX_EN,RX_EN);
	printk("HdmiRxPHYInit %s %d\n",__FUNCTION__,__LINE__);
	...
	*/
	...
	// HDCP Keymask
	HDMIRX_WRITE32_MASK(REG_EPST,_wHalHDMI_HDCP_MASk1<<24,KS_MASK_7_0);
	HDMIRX_WRITE32_MASK(REG_KS_MASK,_wHalHDMI_HDCP_MASk2<<0,0xff<<0);
	...
	HDMIRX_WRITE32_MASK(REG_INTR_MASK0,NEW_ACP_ONLY|NEW_AVI_ONLY|NEW_SPD_ONLY|NEW_AUD_ONLY|NEW_MPEG_ONLY|NEW_UNREC_ONLY,NEW_ACP_ONLY|NEW_AVI_ONLY|NEW_SPD_ONLY|NEW_AUD_ONLY|NEW_MPEG_ONLY|NEW_UNREC_ONLY);
	...
	memcpy(_Usersetedidtodrv.u1HdmiRxEDID, pdRepeaterEdid, 256);
...
void bHDMIPHYReset(UINT8 u1ResetSel)
{
    ...
    {
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON00,0x1<<31,RG_HDMI_CDR_RST);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON00,0x0,RG_HDMI_CDR_STOP);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON00,0x0,RG_HDMI_CDR_RST);
		udelay(10);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON01,0x1<<10,RG_HDMI_EQ_RST);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON01,0x1<<11,RG_HDMI_EQ_SWRSTSEL);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON01,0x0,RG_HDMI_EQ_RST);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON01,0x0,RG_HDMI_EQ_SWRSTSEL);
		...
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON02,0x0,RG_HDMI_0_DEEPCLRCLK_PDB);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON02,0x0,RG_HDMI_0_DEEPCLRCLK_RSTN);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON02,0x1<<1,RG_HDMI_0_DEEPCLRCLK_PDB);
		HDMIRX_A_WRITE32_MASK(REG_HDMI_RX_CON02,0x1<<0,RG_HDMI_0_DEEPCLRCLK_RSTN);
    }
...
void vHalSetRxI2sAudFormat(UINT8 u1fmt, UINT8 u1Cycle)
{
	...
	{
		// Right-Justified
		HDMIRX_WRITE32_MASK(REG_I2S_CTRL, (0x1<<18), JUSTIFY);
...
void vHalHDMISubPortFuncSwtich(BOOL fgRxMode)
{
	if(fgRxMode == TRUE) {
#ifdef GPIO_HDMI_IN_SWITCH_PORT_PIN
		mt_set_gpio_out(GPIO_HDMI_IN_SWITCH_PORT_PIN, GPIO_OUT_ZERO);
#endif
	}
	else {
#ifdef GPIO_HDMI_IN_SWITCH_PORT_PIN
		mt_set_gpio_out(GPIO_HDMI_IN_SWITCH_PORT_PIN, GPIO_OUT_ONE);
#endif
	}
}
...
{
	/***standby,Both HPD1,2 same to HPD****/
    if(bPort == 0) {
		vHalRxHDMITMDSCTRL(FALSE);
	}
	else if(bPort == 1) {
		/** HDMI RX PORT***/
		printk("[hdmirx] select externel hdmi as hdmi source\n");
		vHalRxHDMITMDSCTRL(TRUE);
		vHalHDMISubPortFuncSwtich(TRUE);
	}
	else if(bPort == 2){
		/** 89G PORT***/
		printk("[hdmirx] select 89G as hdmi source\n");
		vHalRxHDMITMDSCTRL(TRUE);
		vHalHDMISubPortFuncSwtich(FALSE);
...
void vHalSelANABand(void)
{
	...
	else if((26*1024) <= (u4CKPRD*(250)))
	{
		...
		HDMIRX_A_WRITE32(0x168,0x00000004);
		//msleep(1);
		HDMIRX_A_WRITE32(0x16C,0x0632c000);
		msleep(1);
...
{
	UINT32 u4Data;
	u4Data = HDMIRX_READ32(REG_INTR_VS_ISRC1);
	u4Data &= (~(0x0F<<16));
	HDMIRX_WRITE32(REG_INTR_VS_ISRC1,(u4Data |INTR_NEW_VS_PKT));
}
...
void vHalClearNewNOVSIntStatus(void)
{
	...
	u4Data = HDMIRX_READ32(REG_INTR_VS_ISRC1);
	u4Data &= (~(0x0F<<16));
	HDMIRX_WRITE32(REG_INTR_VS_ISRC1,(u4Data |INTR_NO_VS_PKT));
}
...
void vHalClearNewISRC1IntStatus(void)
{
	...
	u4Data = HDMIRX_READ32(REG_INTR_VS_ISRC1);
	u4Data &= (~(0x0F<<16));
	HDMIRX_WRITE32(REG_INTR_VS_ISRC1,(u4Data |INTR_NEW_ISRC1_PKT));
}
...
void vHalClearVSYNCIntStatus(void)
{
	...
	u4Data = HDMIRX_READ32(REG_INTR_VS_ISRC1);
	u4Data &= (~(0x0F<<16));
	HDMIRX_WRITE32(REG_INTR_VS_ISRC1,(u4Data |INTR_VSYNC));
...
{
    UINT32 u4Value  = 0;
    u4Value = HDMIRX_READ32(REG_CTS_HVAL) & CTS_VAL_HW_19_0;
    return u4Value ;
}
...
{
	HDMIRX_WRITE32_MASK(REG_SYS_CTRL,u1Data,0xff);
}
...
{
    UINT8 u1AudChstat2 ;
    u1AudChstat2 = HDMIRX_READ32(REG_CHST0) & 0x000000FF;
...
{
    UINT8 u1AudChstat3 ;
    u1AudChstat3 = HDMIRX_READ32(REG_CHST1) & 0x000000FF;
...
enum HDMI_RX_AUDIO_FS eGetACRFs(void)
{
	...
	u64 a = (UINT64)216000 * (UINT64)u4HalGetRxHwNValue();
	u32 b = u4Temp;
	u32 c = u4HalGetRxHwCTSValue();;

	do_div(a,b);
	do_div(a,c);
	UINT32 ACRFs = (UINT32)a;
...
void vHalSetHDMIRxHBR(BOOL fgHBR)
{
    if (fgHBR)
    {
        ...
        HDMIRX_WRITE32_MASK(REG_N_HDMI_CTRL,(0x01<<9)|(0x01<<11)|(0x01<<16)|(0x0<<17),(0x01<<9)|(0x01<<11)|(0x01<<16)|(0x01<<17));
...
void vHalHdmiRxAudBypass(BOOL fgBypass,BOOL dsd)
{
	 if (fgBypass)
	 {
	 	vRxWriteRegMsk(DISP_HDMI_SYS_CFG_01,HDMI_ACLK_SEL|HDMI_BCLK_SEL|HDMI_AUDIO_DATA_BYPASS|HDMI_SPDIF_BYPASS,
						HDMI_ACLK_SEL|HDMI_BCLK_SEL|HDMI_AUDIO_DATA_BYPASS|HDMI_SPDIF_BYPASS);
		if(dsd)
	 		vRxWriteRegMsk(DISP_HDMI_SYS_CFG_01,HDMI_AUDIO_DSD_BYPASS,HDMI_AUDIO_DSD_BYPASS);
		else
	 		vRxWriteRegMsk(DISP_HDMI_SYS_CFG_01,0,HDMI_AUDIO_DSD_BYPASS);
   	 }
	 else
	 {
		 vRxWriteRegMsk(DISP_HDMI_SYS_CFG_01,0,
						 HDMI_ACLK_SEL|HDMI_BCLK_SEL|HDMI_AUDIO_DATA_BYPASS|HDMI_SPDIF_BYPASS);
	 }
}
...
{
	if (HDMIRX_READ32(REG_INTR) & V_RDY_INTR ) {
		return TRUE;
	}else {
		return FALSE;
	}
...
{
	HDMIRX_WRITE32_MASK(REG_SHD_BSTATUS,(u2Bstatus & 0x7F)<<24,DEV_CNT);
	HDMIRX_WRITE32_MASK(REG_HDCP_STAT, ((u2Bstatus>>8) & 0xFF),0xFF);
}
