...
extern unsigned int hdmirx_drv_read(unsigned short u2Reg);
extern void hdmirx_drv_write(unsigned short u2Reg, unsigned int u4Data);
extern unsigned int hdmirx_ana_read(unsigned short u2Reg);
extern void hdmirx_ana_write(unsigned short u2Reg, unsigned int u4Data);
...
#define MMSYS_CONFIG_BASE    0x4000000
...
#define HDMIRX_READ32(dAddr)           hdmirx_drv_read((dAddr))
#define HDMIRX_WRITE32(dAddr, dVal)     hdmirx_drv_write((dAddr), (dVal))
#define HDMIRX_WRITE32_MASK(dAddr, dVal, dMsk) HDMIRX_WRITE32((dAddr), (HDMIRX_READ32(dAddr) & (~(dMsk))) | ((dVal) & (dMsk)))
...
// base is 0x00000000
#define vRxWriteReg(dAddr, dVal) *(volatile UINT32 *)(TOP_CKGEN_BASE + dAddr) = dVal
...
#define DISP_HDMI_SYS_CFG_01 (MMSYS_CONFIG_BASE + 0x904)
	#define HDMI_ACLK_SEL      0x1 << 4
	#define HDMI_BCLK_SEL 0x1 << 5
	#define HDMI_AUDIO_DSD_BYPASS      0x1 << 6
	#define HDMI_AUDIO_DATA_BYPASS 0x1 << 7
	#define HDMI_SPDIF_BYPASS      0x1 << 8
	#define HDMI_RX_SPDIF_SEL	   0x1 << 9
