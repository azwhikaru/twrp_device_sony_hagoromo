...
{
	vRxHDCPSetTxKsv(bTxDownStream, u2TxBStatus, prbTxBksv, prbTxKsvlist, fgTxVMatch);
}
...
void hdmirx_tmr_isr(unsigned long n)
{
	...
	for (i=0; i<MAX_HDMI_RX_TMR_NUMBER; i++)
	{
        ...
        else if (_HdmiRxTmrValue[i] >= HDMI_RX_TMR_ISR_TICKS)
        {
			_HdmiRxTmrValue[i] -= HDMI_RX_TMR_ISR_TICKS;
			if((i==HDMI_RX_SRV_TMR_INX)&&( _HdmiRxTmrValue[HDMI_RX_SRV_TMR_INX] ==0 ))
			{
				vSendHdmiRxCmd(HDMI_RX_SERVICE_CMD);
			}
...
static void vHdmiRxDetectService(void)
{
    if((_fgDisableHdmiRx)||(_bRxHDCPMode==5))
		return;
    vHDMIRxHpdLoop();
	if(fgIsHdmiRepeater())    
	{
...
#ifdef CONFIG_MTK_IN_HOUSE_TEE_SUPPORT
	printk("[HDMIRX]fgCaHDMIRxCreate\n");
	fgCaHDMIRXCreate();	
#endif
    if (VinTask == NULL) {
