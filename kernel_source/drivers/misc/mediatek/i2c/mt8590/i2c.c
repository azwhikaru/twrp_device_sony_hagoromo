...
static int dma_reset(mt_i2c *i2c)
{
	...
	udelay(50);
	//hw reset
	mt65xx_reg_sync_writel(1<<1, i2c->pdmabase + OFFSET_RST);
	udelay(5);
	mt65xx_reg_sync_writel(0x0, i2c->pdmabase + OFFSET_RST);
	udelay(5);
...
static void _i2c_write_reg(mt_i2c *i2c)
{
  ...
  if(i2c->dma_en)
  {
    ...
    if((U32)i2c->msg_buf > DMA_ADDRESS_HIGH){
        if (I2C_MASTER_RD == i2c->op) {
            rpaddr = dma_map_single(i2c->adap.dev.parent, i2c->msg_buf, i2c->trans_data.trans_len, DMA_FROM_DEVICE);
            if (dma_mapping_error(i2c->adap.dev.parent, rpaddr))
                return;
        } else if (I2C_MASTER_WR == i2c->op) {
		    wpaddr = dma_map_single(i2c->adap.dev.parent, i2c->msg_buf, i2c->trans_data.trans_len, DMA_TO_DEVICE);
		    ...
		    if (dma_mapping_error(i2c->adap.dev.parent, wpaddr))
			    return;
		    rpaddr = dma_map_single(i2c->adap.dev.parent, i2c->msg_buf, i2c->trans_data.trans_auxlen, DMA_FROM_DEVICE);
		    if (dma_mapping_error(i2c->adap.dev.parent, rpaddr)) {
			    dma_unmap_single(i2c->adap.dev.parent, wpaddr, i2c->trans_data.trans_len, DMA_TO_DEVICE);
			    return;
		    }
        }
    }
    ...
    if (I2C_MASTER_RD == i2c->op) {
      ...
      mt65xx_reg_sync_writel((U32)rpaddr, i2c->pdmabase + OFFSET_RX_MEM_ADDR);
      ...
      mt65xx_reg_sync_writel((U32)wpaddr, i2c->pdmabase + OFFSET_TX_MEM_ADDR);
...
static S32 _i2c_transfer_interface(mt_i2c *i2c)
{
  ...
  if(i2c->dma_en)
  {
    ...
    if((U32)ptr > DMA_ADDRESS_HIGH){
      I2CINFO( I2C_T_DMA, " DMA mode should use physical buffer address!\n");
  ...
  if (i2c->dma_en && (U32)i2c->msg_buf > DMA_ADDRESS_HIGH) {
    if (i2c->op == I2C_MASTER_WR) {
        dma_unmap_single(i2c->adap.dev.parent, wpaddr, i2c->trans_data.trans_len, DMA_TO_DEVICE);
    } else if (i2c->op == I2C_MASTER_RD) {
        dma_unmap_single(i2c->adap.dev.parent, rpaddr, i2c->trans_data.trans_len, DMA_FROM_DEVICE);
    } else {
        dma_unmap_single(i2c->adap.dev.parent, wpaddr, i2c->trans_data.trans_len, DMA_TO_DEVICE);
	    dma_unmap_single(i2c->adap.dev.parent, rpaddr, i2c->trans_data.trans_auxlen, DMA_FROM_DEVICE);
    }
  }
...
static void mt_i2c_shutdown(struct platform_device *pdev)
{
	mt_i2c *i2c = platform_get_drvdata(pdev);
	if (i2c) {
