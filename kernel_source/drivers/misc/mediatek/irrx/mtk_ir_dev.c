...
void mtk_ir_dev_put_scancode(u32 ui4scancode)
{
  ...
  kfifo_in_locked(&fifo_scancode, (void *)&ui4scancode, SCANCODE_SIZE,&fifo_scancode_lock ); // put scan code
...
static ssize_t mtk_ir_dev_read(struct file *file,
                                      char *buffer,
                                       size_t length,
                                       loff_t *ppos)
{
...
{ 
    ...
    {   
		...
		case CMD_IR_GET_DEVNAME:
		    ...
		    ret = copy_to_user((void *)arg,
				              (void *)(rcdev->input_name),
	     	              strlen(rcdev->input_name) + 1);
			break;
...
{   
	...
	while (!kthread_should_stop())
	{
        ...
        _ret = kfifo_out_locked(&fifo_scancode,&scancode, sizeof(scancode),&fifo_scancode_lock); // get scancode 
