...
static u32 mtk_ir_nec_set_customer_code(void * preserve)
{
    u32 customer_code;
	if (preserve == NULL)
	{
...
{  
   ...
   else
   {
	 ...
	 struct rc_map_table *p_table = mtk_nec_map.map.scan;
	 int size = mtk_nec_map.map.size;
	 int i = 0;
...
u32  mtk_ir_nec_decode( void * preserve)
{
   ...
   static unsigned long last_jiffers = 0; //  this is only the patch for NEC disturbed repeat key
    ...
    if ((u4BitCnt == MTK_NEC_BITCNT_REPEAT) )
    {
        ...
        if (time_after(current_jiffers,last_jiffers) > 0)
        {
		  ...
		  if (jiffies_to_msecs(current_jiffers - last_jiffers ) > NEC_REPEAT_MS)
		  {
					...
					so ignore it
					*/
			 IR_LOG_KEY("### this is the ng repeat key repeat_out=%dms!!!!!!###\n",jiffies_to_msecs(current_jiffers - last_jiffers ));
			_u4PrevKey = BTN_NONE;
		   }
	    }
