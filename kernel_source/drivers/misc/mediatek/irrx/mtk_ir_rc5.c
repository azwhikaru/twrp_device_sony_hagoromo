...
static u32 mtk_ir_rc5_set_customer_code(void * preserve)
{
    u32 customer_code;
	if (preserve == NULL)
	{
...
static u32  mtk_ir_rc5_decode( void * preserve)
{
    ...
    u16 u4BitCnt = RC5_INFO_TO_BITCNT(_u4Info);  
    u16 u2RC5Custom = MTK_RC5_GET_CUSTOM(pu1Data[0]);
    u16 u2RC5Toggle = MTK_RC5_GET_TOGGLE(pu1Data[0]);
    u32 u4RC5key = MTK_RC5_GET_KEYCODE(pu1Data[0], pu1Data[1]);
   
 	
	IR_LOG_KEY( "RxIsr Info:0x%08x data: 0x%08x%08x\n", _u4Info,
	 	                          _au4IrRxData[1], _au4IrRxData);
...
static int mtk_ir_rc5_init_hw(void)
{
  ...
  MTK_IR_MODE ir_mode = mtk_ir_core_getmode();
   ...
   else
   {
	 ...
	 mtk_rc5_map.map.size = ARRAY_SIZE(mtk_rc5_table);
	  ...
	  struct rc_map_table *p_table = mtk_rc5_map.map.scan;
	  int size = mtk_rc5_map.map.size;
	  int i = 0;
  ...
  rc_map_register(&mtk_rc5_map);
   ...
   IR_WRITE32(IRRX_CONFIG_HIGH_REG,  MTK_RC5_CONFIG); // 0xf0021
