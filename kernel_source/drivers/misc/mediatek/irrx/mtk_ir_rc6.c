...
static u32 mtk_ir_rc6_set_customer_code(void * preserve)
{
    u32 customer_code;
	if (preserve == NULL)
	{
...
static u32  mtk_ir_rc6_decode( void * preserve)
{
   ...
   u32 u4RC6key = MTK_RC6_GET_KEYCODE(pu1Data[1], pu1Data[2]);
	...
	IR_LOG_KEY( "Bitcnt: 0x%02x, Leader: 0x%02x, Custom: 0x%02x, Toggle: 0x%02x, Keycode; 0x%02x\n", 
...
static int mtk_ir_rc6_init_hw(void)
{
   ...
   else
   {
	 ...
	 struct rc_map_table *p_table = mtk_rc6_map.map.scan;
	 int size = mtk_rc6_map.map.size;
	 int i = 0;
