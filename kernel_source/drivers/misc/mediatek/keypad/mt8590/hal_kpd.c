...
typedef struct _KPD_CUST_PIN_INFO
{
	unsigned int PinNum;
...
void mtk_kpd_gpios_get(KPD_CUST_PIN_INFO_T ROW_REG[], KPD_CUST_PIN_INFO_T COL_REG[])
{
	int i = 0;

	for (; i < 8; i++)
	{
		memset (&ROW_REG[i], 0, sizeof(ROW_REG[0]));
		memset (&COL_REG[i], 0, sizeof(COL_REG[0]));
	}
	
	#ifdef GPIO_KPD_KROW0_PIN
	ROW_REG[0].PinNum = GPIO_KPD_KROW0_PIN;
	ROW_REG[0].PinMode = GPIO_KPD_KROW0_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW1_PIN
	ROW_REG[1].PinNum  = GPIO_KPD_KROW1_PIN;
	ROW_REG[1].PinMode = GPIO_KPD_KROW1_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW2_PIN
	ROW_REG[2].PinNum = GPIO_KPD_KROW2_PIN;
	ROW_REG[2].PinMode= GPIO_KPD_KROW2_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW3_PIN
	ROW_REG[3].PinNum = GPIO_KPD_KROW3_PIN;
	ROW_REG[3].PinMode= GPIO_KPD_KROW3_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW4_PIN
	ROW_REG[4].PinNum = GPIO_KPD_KROW4_PIN;
	ROW_REG[4].PinMode = GPIO_KPD_KROW4_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW5_PIN
	ROW_REG[5].PinNum = GPIO_KPD_KROW5_PIN;
	ROW_REG[5].PinMode = GPIO_KPD_KROW5_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW6_PIN
	ROW_REG[6].PinNum = GPIO_KPD_KROW6_PIN;
	ROW_REG[6].PinMode = GPIO_KPD_KROW6_PIN_M_KROW;
	#endif

	#ifdef GPIO_KPD_KROW7_PIN
	ROW_REG[7].PinNum = GPIO_KPD_KROW7_PIN;
	ROW_REG[7].PinMode = GPIO_KPD_KROW7_PIN_M_KROW;
	#endif


	#ifdef GPIO_KPD_KCOL0_PIN
	COL_REG[0].PinNum = GPIO_KPD_KCOL0_PIN;
	COL_REG[0].PinMode = GPIO_KPD_KCOL0_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL1_PIN
	COL_REG[1].PinNum = GPIO_KPD_KCOL1_PIN;
	COL_REG[1].PinMode = GPIO_KPD_KCOL1_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL2_PIN
	COL_REG[2].PinNum = GPIO_KPD_KCOL2_PIN;
	COL_REG[2].PinMode = GPIO_KPD_KCOL2_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL3_PIN
	COL_REG[3].PinNum = GPIO_KPD_KCOL3_PIN;
	COL_REG[3].PinMode = GPIO_KPD_KCOL3_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL4_PIN
	COL_REG[4].PinNum = GPIO_KPD_KCOL4_PIN;
	COL_REG[4].PinMode = GPIO_KPD_KCOL4_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL5_PIN
	COL_REG[5].PinNum = GPIO_KPD_KCOL5_PIN;
	COL_REG[5].PinMode = GPIO_KPD_KCOL5_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL6_PIN
	COL_REG[6].PinNum = GPIO_KPD_KCOL6_PIN;
	COL_REG[6].PinMode = GPIO_KPD_KCOL6_PIN_M_KCOL;
	#endif

	#ifdef GPIO_KPD_KCOL7_PIN
	COL_REG[7].PinNum = GPIO_KPD_KCOL7_PIN;
	COL_REG[7].PinMode = GPIO_KPD_KCOL7_PIN_M_KCOL;
	#endif

	#ifdef CONFIG_MTK_LDVT
    for(i = 0; i< 8; i++)
	{
		printk("ROW_REG[%d].PinNum=%08X\n", i, ROW_REG[i].PinNum);
		printk("ROW_REG[%d].PinMode=%08X\n", i, ROW_REG[i].PinMode);
		printk("COL_REG[%d].PinNum=%08X\n", i, COL_REG[i].PinNum);
		printk("COL_REG[%d].PinMode=%08X\n", i, COL_REG[i].PinMode);
	}
	#endif
}
...
{
	KPD_CUST_PIN_INFO_T ROW_REG[8];
	KPD_CUST_PIN_INFO_T COL_REG[8];
	int i;
	...
	for(i = 0; i < 8; i++)
	{
		if (COL_REG[i].PinNum!= 0)
		{
			/* KCOL: GPIO INPUT + PULL ENABLE + PULL UP */
			mt_set_gpio_mode(COL_REG[i].PinNum, COL_REG[i].PinMode);
			mt_set_gpio_dir(COL_REG[i].PinNum, 0);
			mt_set_gpio_pull_enable(COL_REG[i].PinNum, 1);
			mt_set_gpio_pull_select(COL_REG[i].PinNum, 1);
		}
		
		if(ROW_REG[i].PinNum != 0)
		{
			/* KROW: GPIO output + pull disable + pull down */
			mt_set_gpio_mode(ROW_REG[i].PinNum, ROW_REG[i].PinMode);
			mt_set_gpio_dir(ROW_REG[i].PinNum, 1);
			mt_set_gpio_pull_enable(ROW_REG[i].PinNum, 0);	
			mt_set_gpio_pull_select(ROW_REG[i].PinNum, 0);
		}
	}
}
#endif
...
void long_press_reboot_function_setting(void)
{
	if(kpd_enable_lprst&& get_boot_mode() == NORMAL_BOOT) {
		...
		kpd_print("Enable normal mode LPRST\n");
            ...
            #ifndef CONFIG_DISABLE_MTK_LONG_PRESS_RESET
