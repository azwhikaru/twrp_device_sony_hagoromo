...
void UDVT_TaskFun(void *pv_arg)
{
  ...
  while(1)
  {
    ...
    if(tmpReceivedN > 0)
    {
	  ...
	  //printk("8  %d, 16 %d,32 %d ,32 %d  \n ",sizeof(UINT8),sizeof(UINT16),sizeof(UINT32),sizeof(u32));
	  //printk(KERN_ERR"GetDataFun  receivedN %d,count %d ,tmpReceived % p,tmpReceived+receivedN %p = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x",receivedN,tmpReceivedN,tmpReceived,tmpReceived+receivedN,tmpReceived[0],
	  //	tmpReceived[1],tmpReceived[2],tmpReceived[3],tmpReceived[4],tmpReceived[5]);
