...
#include <drivers/misc/mediatek/pmic_wrap/mt8590/pwrap_hal.h>
...
static void __pwrap_soft_reset()
{
...
static U64 _pwrap_get_current_time(void)
{
	...
	//only used in FPGA DVT
	//U64 t=0;
	//gpt_get_cnt(GPT6,&t);
	//return ((t/13)*1000); //GPT6 clock 13M, change to ns
}
//U64 elapse_time=0;
...
S32 _pwrap_manual_modeAccess( U32  write, U32  adr, U32  wdata, U32 *rdata )
{
    ...
    {
		_pwrap_manual_mode(1,  OP_CK, 8, &man_rdata);
		_pwrap_manual_mode(0,  OP_IND, 0, &man_rdata);
		*rdata = (GET_MAN_RDATA( man_rdata )<<8);
...
static S32 _pwrap_wrap_access_test( void )
{
	...
	PWRAPLOG("[WACS0] DEW_WRITE_TEST = 0x%x after write test.\n", rdata);
	if( rdata != 0x1234 )
	{
	...
	PWRAPLOG("[WACS1] DEW_WRITE_TEST = 0x%x after write test.\n", rdata);
	if( rdata != 0x1234 )
	{
	...
	return_value=pwrap_wacs2(0, DEW_READ_TEST, 0, &rdata);
	PWRAPLOG("[WACS2] READ DATA = 0x%x\n",rdata);
  	if( rdata != DEFAULT_VALUE_READ_TEST )
  	{
  	...
  	rdata=0;
	pwrap_wacs2(1, WRAP_ACCESS_TEST_REG, 0x1234, &rdata);
  	return_value=pwrap_wacs2(0, WRAP_ACCESS_TEST_REG, 0, &rdata);
	PWRAPLOG("[WACS2] DEW_WRITE_TEST = 0x%x after write test.\n", rdata);
  	if( rdata != 0x1234 )
  	{
...
static S32 _pwrap_man_access_test( void )
{
	...
	PWRAPLOG("read test (using manual mode),rdata=%x\n",rdata);
	if( rdata != DEFAULT_VALUE_READ_TEST )
	{
	...
	PWRAPLOG("write test (using manual mode),rdata=%x\n",rdata);
	if( rdata != 0x1234 )
	{
			...
			for(j=0; j<4; j++) {
				*((volatile unsigned int *)(pmic_wrap_reg_tbl[i][0]+PMIC_WRAP_BASE)) = (RegWriteValue[j]&pmic_wrap_reg_tbl[i][2]);
				if(((*((volatile unsigned int *)(pmic_wrap_reg_tbl[i][0]+PMIC_WRAP_BASE))&pmic_wrap_reg_tbl[i][2])!=(RegWriteValue[j]&pmic_wrap_reg_tbl[i][2]))) {
					PWRAPLOG("Reg offset %.3x R/W test fail. write %.8x, read %.8x \r\n",
...
static S32 tc_reset_pattern_test(  )
{
	...
	PWRAPLOG("Begin tc_reset_pattern_test.\n");
	WRAP_CLR_BIT(1 << 1, PMIC_WRAP_INT_EN);
	PWRAPLOG("Diable SIG_ERR INT, PMIC_WRAP_INT_EN:0x%x\n",WRAP_RD32(PMIC_WRAP_INT_EN));

	return_value = pwrap_wacs2(0, DEW_READ_TEST, 0, &rdata);
	PWRAPLOG("DEW_READ_TEST: 0x%x\n",rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPERR("read test error(using WACS2),return_value=%x, rdata=%x\n", return_value,
			 rdata);
		res += 1;
	...
	return_value = pwrap_wacs2(1, WRAP_ACCESS_TEST_REG, 0x1234,&rdata);
	return_value = pwrap_wacs2(0, WRAP_ACCESS_TEST_REG, 0, &rdata);
	if (rdata != 0x1234) {
		PWRAPERR("write test error(using WACS2),return_value=%x, rdata=%x\n", res, rdata);
		res += 1;
	}
	PWRAPLOG("WRAP_ACCESS_TEST_REG has be written as 0x%x\n",rdata);
	...
	PWRAPLOG("_pwrap_reset_spislv done.\n");

	res = pwrap_init();
	if(res != 0)
		PWRAPERR("error, pwrap_init fail, res = %x\n",res);
	...
	PWRAPLOG("WRAP_ACCESS_TEST_REG be 0x%x after spislv reset\n",rdata);
...
static S32 tc_soft_reset_test(  )
{
	...
	if ((regValue & 0x01) != 0x00) {
		PWRAPLOG("tc_soft_reset_test fail.regValue =%d\n", regValue);
		res += 1;
	}
	...
	if (res == 0)
		...
		PWRAPLOG("tc_soft_reset_test fail!\n");
...
{
	int res=0;
	enable_clock(MT_CG_INFRA_PMICWRAP, "pmic_wrap");
	disable_clock(MT_CG_INFRA_PMICWRAP, "pmic_wrap");
...
static S32 tc_infrasys_pmicspi_pdn_test(void)
{
	...
	/*#####################################
	// Turn on the clock, function is work
	//####################################*/
	res = _pwrap_wrap_access_test();
	if( res == 0){
		PWRAPLOG("turn on the pmicspi clock: function work!.\n");
	}
	else {
		PWRAPLOG("turn on the pmicspi clock: function fail! res = %d.\n",res);
		return res;
	}

	/*##########################################
	//  Turn off the clock, function is not work
	//#########################################*/
	...
	if( res != 0)
		PWRAPLOG("tc_infrasys_pmicspi_pdn_test pass!\n");
	else
		PWRAPLOG("tc_infrasys_pmicspi_pdn_test fail!\n");
...
static S32 tc_infrasys_pmic_wrap_pdn_test(void)
{
	U32 rdata = 0;
	UINT32 return_value = 0;
	...
	if( return_value == 0){
		PWRAPLOG("turn on the pmic_wrap clock: function work!.\n");
	}
	else {
		PWRAPLOG("turn on the pmic_wrap clock: function fail! res = %d.\n",res);
		return return_value;
	}
...
#if 0
	WRAP_WR32(PMIC_WRAP_DIO_EN, 1);
	return_value = WRAP_RD32(PMIC_WRAP_DIO_EN);
	PWRAPLOG("PMIC_WRAP_DIO_EN : 0x%x.\n",return_value);
	if (return_value == 0){
		PWRAPLOG("turn off the pmic_wrap clock: function not work! The value have not been written to WRAP register.\n");
		PWRAPLOG("tc_infrasys_pmic_wrap_pdn_test pass!\n");
	}
	else{
		PWRAPLOG("turn off the pmic_wrap clock: function still work! The value have been written to WRAP register.\n");
		PWRAPLOG("tc_infrasys_pmic_wrap_pdn_test fail!\n");
	}
#endif
	return_value = pwrap_read(DEW_READ_TEST, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
...
static irqreturn_t mt_pwrap_dvt_irq(int irqno, void *dev_id)
{
	...
	//spin_unlock_irqrestore(&mt_wrp->lock,flags);
...
#endif
		{
			//PWRAPLOG("_concurrence_wacs0_test.\n");
			rdata = 0;
...
#endif
		{
			//PWRAPLOG("_concurrence_wacs1_test.\n");
			rdata = 0;
			mutex_lock(&wrap_read_write_sync);
...
#endif
		{
			//PWRAPLOG("_concurrence_wacs2_test.\n");
			rdata = 0;
			mutex_lock(&wrap_read_write_sync);
...
static S32 mt_pwrap_dvt(U32 nbr)
{
	...
	switch(nbr){
		...
		case WDT_4:
			tc_wdt_test_sub4();
			break;
		case INFRASYS_PMICSPI:	/* 19 */
			tc_infrasys_pmicspi_pdn_test();
			break;
		case INFRASYS_PMIC_WRAP:
			tc_infrasys_pmic_wrap_pdn_test();
			break;		
		default:
...
// some test case should disable PWRAP INTERRUPT
#if 1
	free_irq(MT_PMIC_WRAP_IRQ_ID, NULL);
	ret = request_irq(MT_PMIC_WRAP_IRQ_ID, mt_pwrap_dvt_irq, IRQF_TRIGGER_HIGH, "pwrap_dvt",
			NULL);
	if (ret) {
