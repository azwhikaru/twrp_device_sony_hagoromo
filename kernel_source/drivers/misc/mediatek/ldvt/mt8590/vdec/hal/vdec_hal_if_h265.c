...
void vVDEC_HAL_H265_SetStillImageInfo(UINT32 u4VDecID, H265_SPS_Data *prSPS, H265_PPS_Data *prPPS)
{
	...
	#if 1
    //Begin add 8590E2 Modification
	UINT32 uiTempVal = u4VDecReadVLDTOP(u4VDecID, 72 * 4); 
	printk("VLD_TOP_72 Default Value: 0x%8x\n", uiTempVal);
	vVDecWriteVLDTOP(u4VDecID, 72 * 4, uiTempVal & (~(0x1<<1)));
	uiTempVal = u4VDecReadVLDTOP(u4VDecID, 72 * 4); 
	printk("VLD_TOP_72 New Value: 0x%8x\n", uiTempVal);
	//END add 8590E2 Modification
	#endif
...
UINT32 vVDEC_HAL_H265_VDec_VPmode(UINT32 u4VDecID)
{
    ...
    if ( risc_val1 & 0x1 ){
        ...
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_PIC_MB_SIZE_M1, VLD_TOP_26);      /* 26 * 4 */
        vVDecWriteVLDTOP(u4VDecID, HEVC_VLD_TOP_PIC_PIX_SIZE, VLD_TOP_28);  /* 28 * 4 */
        ...
        risc_val1 = u4VDecReadHEVCMISC(u4VDecID, RW_HEVC_DEC_COMPLETE); /* 41 * 4 */
...
void vVDEC_HAL_H265_Dump_ErrReg(UINT32 u4VDecID, int frameNum)
{
    UINT32 risc_val1;
    risc_val1= u4VDecReadHEVCVLD(u4VDecID, 58 * 4);
    printk(" frame #%d  HEVC VLD 58 =  0x%x  \n", frameNum, risc_val1);
    risc_val1= u4VDecReadHEVCVLD(u4VDecID, 57 * 4);
    printk(" frame #%d  HEVC VLD 57 =  0x%x  \n", frameNum, risc_val1);
}
...
UINT32 vVDEC_HAL_H265_VDec_VPmode(UINT32 u4VDecID)
{
    ...
    risc_val1= u4VDecReadHEVCVLD(u4VDecID, 58 * 4);
    printk(" yaochuang dump pic #%d  HEVC VLD 58 =  0x%x  \n", _ptH265CurrFBufInfo[u4VDecID]->u4PicCnt, risc_val1);
    
    printk(" yaochuang Test to decide enter VP Mode or not?? \n");
    risc_val1= u4VDecReadHEVCVLD(u4VDecID, 57 * 4);
    //if (_u4PicCnt[u4VDecID] >= 1){  risc_val1 = 1; }      // force frame num turn on VP mode for debug
    if ( risc_val1 & 0x1 )
    {        
        printk("[INFO] VP mode %x\n", risc_val1);
        UINT32 SliceStartLCURow, u4LCUsize, u4RealWidth, u4W_Dram;
        UINT32 pic_real_wifth, pic_real_height,i, minDPOC;
        ...
        risc_val1= u4VDecReadHEVCVLD(u4VDecID, RW_HEVLD_SPS_SIZE);
        u4LCUsize = 1<<((risc_val1>>4) & 0x7); /* max cu size */
        risc_val1= u4VDecReadHEVCVLD(u4VDecID, RO_VLD_VWPTR);
        SliceStartLCURow = (risc_val1>>16)  & 0x3ff;
        ...
        printk("[INFO] Decode error Enter VP mode!!  SliceStartLCURow %d; u4LCUsisze %d; refBufferIndex %d(pic #%d)\n", 
               SliceStartLCURow, u4LCUsize, ucRefFBIndex, _ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4PicCnt);
        ...
        if (_u4Main10Mode == 0) 
        {
            vVDecWriteVLDTOP(u4VDecID, 64*4, 0x0); // set bitdepth 8 bit
        }
        else
        {
            vVDecWriteVLDTOP(u4VDecID, 64*4, 0x2); // set bitdepth 10 bit
        }

        /* set pic height & width in MB unit */
        vVDecWriteVLDTOP(u4VDecID, 26 * 4, VLD_TOP_26);

        /* set vp dma src & dst address */
        vVDecWriteVLDTOP(u4VDecID, 56*4, PHYSICAL(_ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4YStartAddr));
        vVDecWriteVLDTOP(u4VDecID, 57*4, PHYSICAL(_ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4CStartAddr));
        vVDecWriteVLDTOP(u4VDecID, 58*4, PHYSICAL(_ptH265CurrFBufInfo[u4VDecID]->u4YStartAddr));
        vVDecWriteVLDTOP(u4VDecID, 59*4, PHYSICAL(_ptH265CurrFBufInfo[u4VDecID]->u4CStartAddr));

        /* set vp dma size in MB unit */
        UINT32 MoveSizeInMB = (VLD_26_W / 16 * (((pic_real_height + 15) >> 4) << 4)/16) - SliceStartLCURow * u4LCUsize / 16  *  VLD_26_W / 16;
        UINT32 Start_X = 0;
        printk("Start X:0x%x Size:%d MB\n", Start_X, MoveSizeInMB);        
        ...
        if (bIsUFO)
        {
            ...
            printk ("[INFO] VP UFO settings\n");
            #endif
            printk ("[INFO] VP UFO settings : src & dst address \n");
...
//	            }
        ...
        /* disable int_wait_bits_nop after Oryx E3 SET BIT[20] = 1 */
        risc_val1 = u4VDecReadHEVCMISC(u4VDecID, 41 * 4);
        vVDecWriteHEVCMISC (u4VDecID, 41 * 4, risc_val1 | (0x1 << 20) ) ;

        /* set vp dma & trigger vp */
        vVDecWriteVLDTOP(u4VDecID, 65*4, (0x1 << 8) | 0x1);

        /* polling the operation finish */
        return Wait_decode_finished( jiffies );
      }
...
{
    printk ("[INFO] Function: %s line:%d  do nothing\n", __FUNCTION__, __LINE__);

	//Dump_reg function is not available,so disable dump 
    #if 1  
    char pBitstream_name[200] = {0};

    memcpy (pBitstream_name , _bFileStr1[u4VDecID][1] + 3 , (strlen(_bFileStr1[u4VDecID][1]) - 25) );
    pBitstream_name[(strlen(_bFileStr1[u4VDecID][1]) - 25)] = '\0';
