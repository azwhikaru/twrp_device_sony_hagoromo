...
#endif
    vVDecSetVLDVFIFO(0, u4VDecID, (UINT32)PHYSICAL(prMpegVFifoPrm->u4VFifoSa), (UINT32)PHYSICAL(prMpegVFifoPrm->u4VFifoEa));
...
INT32 i4VDEC_HAL_MPEG_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_MPEG_BS_INIT_PRM_T *prMpegBSInitPrm)
{
   ...
   vVDecSetVLDVFIFO(u4BSID, u4VDecID, (UINT32)PHYSICAL((ULONG) prMpegBSInitPrm->u4VFifoSa), (UINT32)PHYSICAL((ULONG) prMpegBSInitPrm->u4VFifoEa));

   u4VLDRemainByte =  ((prMpegBSInitPrm->u4ReadPointer)) & 0xf;


   vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), (UINT32)PHYSICAL((ULONG) prMpegBSInitPrm->u4ReadPointer));
   vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),(UINT32)PHYSICAL((ULONG) prMpegBSInitPrm->u4WritePointer));
   ...
   if (fgVDecWaitVldFetchOk(u4BSID, u4VDecID))
   {
    ...
    vVDecSetVLDVFIFO(u4BSID, u4VDecID, (UINT32)PHYSICAL(prMpegBSInitPrm->u4VFifoSa), (UINT32)PHYSICAL(prMpegBSInitPrm->u4VFifoEa));
 
    u4VLDRemainByte =  ((UINT32)PHYSICAL(prMpegBSInitPrm->u4ReadPointer)) & 0xf;
    
    // prevent initialize barrel fail
    for (i = 0; i < 5; i++)
    {
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR, u4VDecReadVLD(u4VDecID, WO_VLD_WPTR) | VLD_CLEAR_PROCESS_EN);
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), (UINT32)PHYSICAL(prMpegBSInitPrm->u4ReadPointer));
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), (UINT32)PHYSICAL(prMpegBSInitPrm->u4ReadPointer));       
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10), (UINT32)PHYSICAL(prMpegBSInitPrm->u4WritePointer));
