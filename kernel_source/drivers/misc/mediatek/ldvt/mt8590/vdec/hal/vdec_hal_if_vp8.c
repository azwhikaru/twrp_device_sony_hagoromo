...
INT32 i4VDEC_HAL_VP8_InitBarrelShifter(UINT32 u1VP8VLD, UINT32 u4VDecID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
 ...
 //   HalDisableDCache();
   ...
   //printk("i4VDEC_HAL_VP8_InitBarrelShifter, V fifostart 0x%p, end 0x%p\n", pVp8DecInfo->u4FifoStart, pVp8DecInfo->u4FifoEnd);
   //printk("i4VDEC_HAL_VP8_InitBarrelShifter, p fifostart 0x%p, end 0x%p\n", PHYSICAL((ULONG)pVp8DecInfo->u4FifoStart), PHYSICAL((ULONG)pVp8DecInfo->u4FifoEnd));
...
UINT32 i4VDEC_HAL_VP8_DecStart(UINT32 u4VDecID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
   ...
   UINT32 u4DramPicArea;
    ...
    // for pp out (UFO ENC)
    vVDecWriteMC(u4VDecID, 698 * 4, PHYSICAL((ULONG) pVp8DecInfo->u4CurYAddr) + u4PIC_SIZE_BS[u4VDecID]);
    vVDecWriteMC(u4VDecID, 699 * 4, PHYSICAL((ULONG) pVp8DecInfo->u4CurYAddr) + u4PIC_SIZE_BS[u4VDecID] + u4UFO_LEN_SIZE_Y[u4VDecID]);
    // for MC (UFO DEC)
...
/*
   ...
   */
   ...
   vVDecWriteVP8MC(u4VDecID, RW_MC_LUMA_SIZE,(UINT32)(pVp8DecInfo->u4CurCAddr-pVp8DecInfo->u4CurYAddr));
   #if VMMU_SUPPORT
...
//   vVDecWriteVP8MC(u4VDecID, 0x660, (u4VDecReadVP8MC(u4VDecID, 0x660) |(0x80000000)));
    ...
    *(volatile ULONG *)(0xF6000008) = 0xFF;
    ...
    #if 1
    u4HalValue = u4VDecReadVLDTOP(u4VDecID, 72 *4);

    printk("Before VLD TOP 72 value 0x%x\n", u4HalValue);
    u4HalValue = u4HalValue & 0xFFFD;
	printk("After VLD TOP 72 value 0x%x\n", u4HalValue);
	vVDecWriteVLDTOP(u4VDecID, 72 * 4, u4HalValue);
	#endif
#endif
