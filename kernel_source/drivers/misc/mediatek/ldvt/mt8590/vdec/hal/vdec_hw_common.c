...
{
    //printk("vWriteReg VA_VDEC_BASE is 0x%lx, dAddr 0x%p, value 0x%x\n", VA_VDEC_BASE, (ULONG)(VA_VDEC_BASE + dAddr),dVal);
	mt65xx_reg_sync_writel(dVal, (ULONG)(VA_VDEC_BASE + dAddr)); //*(volatile UINT32 *)(IO_BASE + dAddr) = dVal
}

UINT32 u4ReadReg(UINT32 dAddr)
{
    //printk("u4ReadReg VA_VDEC_BASE is 0x%lx, dAddr 0x%p\n", VA_VDEC_BASE, (VA_VDEC_BASE + dAddr));
    return (UINT32)(*(volatile UINT32 *)(VA_VDEC_BASE + dAddr));
}

void vWriteGconReg(UINT32 dAddr, UINT32 dVal)  
{
	mt65xx_reg_sync_writel(dVal, (ULONG)(VA_VDEC_GCON_BASE + dAddr)); //*(volatile UINT32 *)(IO_BASE + dAddr) = dVal
}
...
#endif
{
...
#endif
    //u4VDecReadDV(u4VDecID, 0);
    //vVDecWriteDV(u4VDecID, 0, 0xffffffff);
	//u4VDecReadDV(u4VDecID, 0);
...
#endif
        u4VDecPDNCtrlSpec = 0xFFFE;
        #if VDEC_UFO_ENABLE
        u4VDecPDNCtrlModule = 0x3F6A151D;
		u4VDecPDNCtrlModule2 = 0x3F;
		#else
        u4VDecPDNCtrlModule = 0x7F6A151D;
		u4VDecPDNCtrlModule2 = 0xFFFFFFFF;
		#endif
        break;
    case VDEC_DIVX3:
    case VDEC_MPEG4:
    case VDEC_H263:         
        ...
        u4VDecPDNCtrl = 0x00000EB8;
#endif
        u4VDecPDNCtrlSpec = 0xFFFD;
        #if VDEC_UFO_ENABLE
        u4VDecPDNCtrlModule = 0x0;//0x3E6A1108;
		u4VDecPDNCtrlModule2 = 0x0;//0x1F;
		#else
...
#endif
        u4VDecPDNCtrlSpec = 0xFFF7;
        u4VDecPDNCtrlModule = 0x53E20180;
        break;
		
    case VDEC_H265:   //HEVC POWER setting 
        u4VDecPDNCtrlSpec    = 0xFEFF;
        u4VDecPDNCtrlModule  = 0x77E30180;
    	u4VDecPDNCtrlModule2 = 0xFFFFFA60;
        break;
    ...
    case VDEC_RM:
        ...
        u4VDecPDNCtrlSpec = 0xFFEF;
        u4VDecPDNCtrlModule = 0x7B241100;
		u4VDecPDNCtrlModule2 = 0xFFFFFFFF;
    ...
    case VDEC_VP8:
        ...
        u4VDecPDNCtrlSpec = 0xFF7F;

		#if VDEC_UFO_SUPPORT
        u4VDecPDNCtrlModule = 0x31201100;   
		u4VDecPDNCtrlModule2 = 0x3B; 

		#else
        u4VDecPDNCtrlModule = 0x71201100;   
		u4VDecPDNCtrlModule2 = 0xFFFFFFFF; 
		#endif
    ...
    vVDecWriteDV( u4VDecID, 0x178, u4VDecPDNCtrlModule2);

    //vVDecWriteDV( u4VDecID, 0xC8, 0);
    //vVDecWriteDV( u4VDecID, 0xCC, 0);
	//vVDecWriteDV( u4VDecID, 0x178, 0);
...
void vVDecSetVLDVFIFO(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4VFifoSa, UINT32 u4VFifoEa)
{
	...
	//printk("vVDecSetVLDVFIFO VFifosa 0x%p, VFifoEa 0x%p\n", u4VDecReadVLD(u4VDecID, RW_VLD_VSTART), u4VDecReadVLD(u4VDecID, RW_VLD_VEND + (u4BSID << 10)));
}
        ...
        if (DUMP_FOR_SIM)
        {
            ...
            #endif
            {
				...
				case AVS_VLD_REG_OFFSET0:
					DBG_H264_PRINTF(" RISCWrite(`AVS_VLD_REG_OFFSET0%d + 4*%d, 32'h%x)\n", u4VDecID,u4Addr >> 2, u4Val);
					break;
				case VP8_VLD_REG_OFFSET0:
					DBG_H264_PRINTF(" RISCWrite(`VP8_VLD_REG_OFFSET0%d + 4*%d, 32'h%x)\n", u4VDecID,u4Addr >> 2, u4Val);
					break;
				case VP8_VLD2_REG_OFFSET0:
					DBG_H264_PRINTF(" RISCWrite(`VP8_VLD2_REG_OFFSET0%d + 4*%d, 32'h%x)\n", u4VDecID,u4Addr >> 2, u4Val);
					break;
                default:
    ...
    if (szFunction)
    {
        if (DUMP_FOR_SIM)
        {
            ...
            #endif
            {
                ...
                break;
                case HEVC_VLD_REG_OFFSET0:
                    DBG_H264_PRINTF(" RISCRead_HEVC_VLD( %d); /* return 0x%08X */\n", u4Addr >> 2, u4Val);
                    break;

				case AVS_VLD_REG_OFFSET0:
					DBG_H264_PRINTF(" RISCRead(`AVS_VLD_REG_OFFSET0%d + 4*%d, 32'h%x)\n", u4VDecID,u4Addr >> 2, u4Val);
					break;
					
                case VP8_VLD_REG_OFFSET0:
					DBG_H264_PRINTF(" RISCRead(`VP8_VLD_REG_OFFSET0%d + 4*%d, 32'h%x)\n", u4VDecID,u4Addr >> 2, u4Val);
					break;

				case VP8_VLD2_REG_OFFSET0:
					DBG_H264_PRINTF(" RISCRead(`VP8_VLD2_REG_OFFSET0%d + 4*%d, 32'h%x)\n", u4VDecID,u4Addr >> 2, u4Val);
					break;
                default:
