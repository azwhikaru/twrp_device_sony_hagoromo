...
//        printk("\n MPV Dump register end \n");
    ...
    //printk("ucMpegSpecType is %d\n",prDecPrm->ucMpegSpecType);
    // Process Decode
...
//	vVDecWriteVLD(u4VDecID,RW_VLD_MP4_ERR_DET2,0xffffffff);
    ...
    */
	...
	#if (POWER_TEST_CASE == POWER_TEST_DCM)
	{
		...
		u4Value = u4VDecReadDV(u4VDecID, 42*4);
		u4Value = u4Value | 0x1;
		vVDecWriteDV( u4VDecID, 42*4, u4Value); 
		u4Value = u4VDecReadDV(u4VDecID, 42*4);
    ...
    else
    {
      //#if ((VDEC_DDR3_SUPPORT && (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8560))||VDEC_UFO_ENABLE) // DDR3+PPOUT setting mtk40343
		...
		vVDecWriteMC(u4VDecID, RW_MC_PP_WB_BY_POST, 1); // wirte PP out only
            ...
            {
                vVDecWriteMC(u4VDecID, 698*4, u4AbsDramANc((ULONG)(prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa+prDecPrm->u4PIC_SIZE_BS)));//698*4 = 0xAE8
                vVDecWriteMC(u4VDecID, 699*4, u4AbsDramANc((ULONG)(prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa+prDecPrm->u4PIC_SIZE_BS+prDecPrm->u4UFO_LEN_SIZE_Y)));//699*4 = 0xAEC
            ...
            {
                vVDecWriteMC(u4VDecID, 698*4, u4AbsDramANc((ULONG)(prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa+prDecPrm->u4PIC_SIZE_BS)));//698*4 = 0xAE8
                vVDecWriteMC(u4VDecID, 699*4, u4AbsDramANc((ULONG)(prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa+prDecPrm->u4PIC_SIZE_BS+prDecPrm->u4UFO_LEN_SIZE_Y)));//699*4 = 0xAEC
            ...
            {
                vVDecWriteMC(u4VDecID, 698*4, u4AbsDramANc((ULONG)(prMpegDecPrm->rMpegFrameBufSa.u4Pic2YSa+prDecPrm->u4PIC_SIZE_BS)));//698*4 = 0xAE8
                vVDecWriteMC(u4VDecID, 699*4, u4AbsDramANc((ULONG)(prMpegDecPrm->rMpegFrameBufSa.u4Pic2YSa+prDecPrm->u4PIC_SIZE_BS+prDecPrm->u4UFO_LEN_SIZE_Y)));//699*4 = 0xAEC
