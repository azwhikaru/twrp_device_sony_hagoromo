...
#define PROB_TBL_BUFFER  6
#define INPUT_BUFFER  7
...
#define VP9_PRINT_SHOW(x, ...)   printk("[VP9][INFO] --> "x" \n", ##__VA_ARGS__)
...
#define VP9_DPB_SZ           0x9000000
//#define VP9_DPB_SZ           0x5000000
...
#define MAX_REF_LF_DELTAS_VP9       4   //qianqian@20150215
#define MAX_MODE_LF_DELTAS_VP9      2
...
{
    ULONG ulBufVAddr;    //qianqian@20150215
    UINT32 u4BufSize;
}VP9_DRAM_BUF_T;
...
typedef struct _VP9_INPUT_CTX_T_
{
    ...
    ULONG ulVaFifoStart;                 ///< Video Fifo memory start address    
    ULONG ulVaFifoEnd;                 ///< Video Fifo memory end address     
    UINT32 u4FileOffset;    
    UINT32 u4FileLength;    
    UINT32 u4BitstreamLoadingCnt;    
    CHAR   ucBitStreamName[256];
    ULONG ulVaFrameStart;    //qianqian@20150215
    ULONG ulVaFrameEnd;
    UINT32 u4FrameSize;    
...
typedef struct {
  ...
  // 0 = Intra, Last, GF, ARF
  INT8 ref_deltas[MAX_REF_LF_DELTAS_VP9];
  INT8 last_ref_deltas[MAX_REF_LF_DELTAS_VP9];

  // 0 = ZERO_MV, MV
  INT8 mode_deltas[MAX_MODE_LF_DELTAS_VP9];
  INT8 last_mode_deltas[MAX_MODE_LF_DELTAS_VP9];
...
typedef struct _VP9_COMMON_T_
{
    ...
    VP9_REF_CNT_BUF_T FRAME_BUFS[MAX_FRAMES];//should not be changed except reallocate
