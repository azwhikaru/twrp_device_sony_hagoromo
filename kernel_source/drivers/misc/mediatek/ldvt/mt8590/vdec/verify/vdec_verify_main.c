  ...
  //Read rom table(inter and intra) from PC
  	{
		...
		else
		{
			...
			printk("Succeed in opening intra table\n");
		}
        //Open inter
        _tInFileInfo[u4InstID].fgGetFileInfo = TRUE;
		...
		else
		{
			...
			printk("Succeed in opening inter talbe\n");
		}
  	}
...
void vVDecVerifyThread(void* vpdata)
{
 ...
 //  VP9
  _u4K2KMode[u4InstID] = 0; //pparam->u4K2KMode;
    _u4LaeMode[u4InstID] = 0; //pparam->u4LaeMode;
    _u4DualCoreEnable[u4InstID] = 0; //pparam->u4DualCoreEnable;    
    _u4VP9LogOption = 0; //pparam->u4VP9LogOption;
 //~VP9
