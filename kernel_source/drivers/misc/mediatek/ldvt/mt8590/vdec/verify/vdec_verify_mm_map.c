...
void * x_alloc_aligned_verify_mem_ex(UINT32 u4Size, UINT32 u4Align, BOOL fgChannelA, const char *szFunction, INT32 i4Line)
{
    ...
    if (g_u4AllocSize + u4Size < 0xf400000)
...
{    
    ...
    printk("========== VP9 memory allocate ==============\n");
    
    _pucFileListSa[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(FILE_LIST_SZ,1024, 0);   
    printk("_pucFileListSa[u4InstID] = 0x%x, size is 0x%lx\n", _pucFileListSa[u4InstID], FILE_LIST_SZ);   

    //allocate VFIFO buffer
     _pucVFifo[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_V_FIFO_SZ, 1024, 1);
    printk("// <buffer> bitstream dram physical address: 32'h%x, _pucVFifo[u4InstID] = 0x%x\n", PHYSICAL(_pucVFifo[u4InstID]), _pucVFifo[u4InstID]);

#if VP9_CRC_ENABLE
    _pucVP9CRCYBuf0[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_CRC_BUFFER_SZ,1024, 1);
    _pucVP9CRCCBuf0[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_CRC_BUFFER_SZ,1024, 1);
    _pucVP9CRCYBuf1[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_CRC_BUFFER_SZ,1024, 1);
    _pucVP9CRCCBuf1[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_CRC_BUFFER_SZ,1024, 1);
    _pucVP9CRCYBuf2[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_CRC_BUFFER_SZ,1024, 1);
    _pucVP9CRCCBuf2[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_CRC_BUFFER_SZ,1024, 1);
    ...
    _pucDumpYBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_GOLD_Y_SZ,1024, 1);
    printk("g2YGolden buffer[%d] 0x%08x(0x%08x) size 0x%x\n",u4InstID,_pucDumpYBuf[u4InstID], PHYSICAL(_pucDumpYBuf[u4InstID]),VP9_GOLD_Y_SZ);
    
    //allocate C golden buffer 
    _pucDumpCBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_GOLD_Y_SZ,1024, 1);
    printk("g2CGolden buffer[%d] 0x%08x(0x%08x) size 0x%x\n",u4InstID,_pucDumpCBuf[u4InstID], PHYSICAL(_pucDumpCBuf[u4InstID]),VP9_GOLD_C_SZ );
 
#endif  

    //allocate Seg buffer, for core 0
    _pucSegId[u4InstID][0] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_SEG_ID_SZ,1024, 1);
    printk("_pucSegId[u4InstID][0] = 0x%x, size is 0x%x\n", _pucSegId[u4InstID][0], VP9_SEG_ID_SZ);    
    
    //allocate Seg buffer, for core 1
     _pucSegId[u4InstID][1] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_SEG_ID_SZ,1024, 1);
    printk("_pucSegId[u4InstID][1] = 0x%x, size is 0x%x\n", _pucSegId[u4InstID][1], VP9_SEG_ID_SZ);
	

   if(_u4DualCoreEnable[u4InstID])
    {
        //g2LAEBuffer[u4InstID][0] =  g2SegIdWrapper[u4InstID][1] +  PATTERN_SEG_ID_SZ;
        _pucLAEBuffer[u4InstID][0] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_LAE_BUFFER_SZ,1024, 1);
        printk("g2LAEBuffer[u4InstID][0] = 0x%08x(0x%08x), size is 0x%x\n", _pucLAEBuffer[u4InstID][0],
                    PHYSICAL(_pucLAEBuffer[u4InstID][0]), VP9_LAE_BUFFER_SZ);
            
        //g2ErrBuffer[u4InstID][0] =  g2LAEBuffer[u4InstID][0] +  VP9_LAE_BUFFER_SZ;
        _pucErrBuffer[u4InstID][0] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_ERR_BUFFER_SZ,1024, 1);
        printk("g2ErrBuffer[u4InstID][0] = 0x%08x(0x%08x), size is 0x%x\n", _pucErrBuffer[u4InstID][0],
        ...
        _pucDPB[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_DPB_SZ,4096, 1);
        printk("VP9 buffer[%d] 0x%08x size 0x%x\n",u4InstID,_pucDPB[u4InstID],VP9_DPB_SZ);
        
    }
   else
    {
        //allocate DPB buffer
        _pucDPB[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VP9_DPB_SZ,4096, 1); //_pucSegId[u4InstID][1] + VP9_SEG_ID_SZ;
        printk("VP9 buffer[%d] 0x%08x size 0x%x\n",u4InstID, _pucDPB[u4InstID], VP9_DPB_SZ);                  
    }

   _pucTileBuf = (UCHAR *)x_alloc_aligned_verify_mem(VP9_TILE_BUFFER_SIZE,1024, 1);
   _pucCountTBLBuffer= (UCHAR *)x_alloc_aligned_verify_mem(VP9_COUNT_TBL_SZ,1024, 1);
   _pucProbTBLBuffer= (UCHAR *)x_alloc_aligned_verify_mem(VP9_PROB_TBL_SZ,1024, 1);
...
//    printk("CRCBUF_SA = 0x%x\n", CRCBUF_SA);
  ...
  printk("_pucCRCBuf[u4InstID] = 0x%x 0x%x 0x%x\n", _pucCRCBuf[u4InstID], CRCBUF_SA, CRC_SZ);
    ...
    {
        _pucFileListSa[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(FILE_LIST_SZ, 1024, 1);
        printk("// <buffer> _pucFileListSa[u4InstID] = 0x%p\n", PHYSICAL(_pucFileListSa[u4InstID]));
...
#if  defined(MPEG4_CRC_CMP) || defined(VDEC_MP2_COMPARE_CRC)
...
//~AVC Working Buffer
  ...
  // WMV Part
      ...
      _pucPic0Y[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(UFO_PIC_SIZE_REF,4096, WORKING_AREA_CHANEL_A); //UCHAR address
      _pucPic0C[u4InstID] = _pucPic0Y[u4InstID];
      _pucPic1Y[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(UFO_PIC_SIZE_REF,4096, WORKING_AREA_CHANEL_A);
      _pucPic1C[u4InstID] = _pucPic1Y[u4InstID];
      _pucPic2Y[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(UFO_PIC_SIZE_REF,4096, WORKING_AREA_CHANEL_A);
...
#endif
        printk("_pucSegIdWrapper[u4InstID] = 0x%lx\n", _pucSegIdWrapper[u4InstID]);
