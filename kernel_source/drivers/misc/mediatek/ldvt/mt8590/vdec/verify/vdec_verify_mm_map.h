...
#else
#define V_FIFO_SZ 0x1900000 //0x01900000//0x05000000 //0x1000000
#endif
...
#endif
#else
#define PIC_Y_SZ          (2048 *1152)*2 //(2048*1088) *2
#define PIC_C_SZ          (2048 *1152) //(2048*1088 / 2) *2
#endif
...
// WMV PP Work Bufer
#define DEC_PP_SZ         ((4096/16)*9*8)*2
...
// ~WMV PP Work Bufer
#define DEC_PP_Y_SZ      (4096*3072) *2  //(3840*2176) *2//(2048*1088) *2
#define DEC_PP_C_SZ      (4096*3072/2) *2  //(3840*2176/2) *2//(2048*1088/2) *2
#define UFO_PIC_SIZE_REF       (4096*3072) *3//(2048*1088) *3
...
//6589NEW 2.2 2.6
#define DATA_PARTITION_SZ   ((2 * (4096 / 16) * (3072 / 16) * 128) / 8) // (2 * MBx * MBy * 128) bits = 261120 bytes
// 6589
#define NOT_CODED_SZ        (((((4096 / 16) * (3072 / 16) + 127) >> 7) << 7) / 8)*2 // ((((1 * MBx * MBy) + 127) >> 7) << 7) bits = 1024 bytes
// 6582
//#define NOT_CODED_SZ        (((((1920 / 16) * (1088 / 16) + 128) >> 7) << 7) / 8) // ((((1 * MBx * MBy) + 127) >> 7) << 7) bits = 1024 bytes


//6589NEW 4.1
#define MV_DIRECT_SZ        (((4096 / 16) * (3072 / 16) * 128) / 8)*2 // (MBx * MBy) * 128 bit = 130560 bytes 
