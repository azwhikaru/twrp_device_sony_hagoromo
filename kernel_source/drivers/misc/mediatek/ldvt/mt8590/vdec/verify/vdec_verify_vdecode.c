...
void vVDecProc(UINT32 u4InstID)
{
  ...
  else if(_u4CodecVer[u4InstID] == VDEC_H265)
  {
	...
	//Begin The code if  for debug STILL IMAGE 
	{
		printk("Yaochuang=====>BitStream File = %s \n",_bFileStr1[u4InstID][1]);
		printk("Yaochuang=====>VFIFO SIZE = %d, BitStream Size = %d\n",V_FIFO_SZ,_tInFileInfo[u4InstID].u4FileLength);
		vH265DumpVFIFO(u4InstID, _pucVFifo[u4InstID], _tInFileInfo[u4InstID].u4FileLength, 0);
		vVDEC_HAL_H265_VDec_DumpReg(u4InstID, 0); 
	}
...
//    BSP_InvDCacheRange((UINT32)_pucDPB[u4InstID],DPB_SZ);
  ...
  else 
  {
    ...
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegFrameBufSa.u4Pic0CSa = (ULONG)_pucPic0C[u4InstID];
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegFrameBufSa.u4Pic0YSa = (ULONG)_pucPic0Y[u4InstID];
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegFrameBufSa.u4Pic1CSa = (ULONG)_pucPic1C[u4InstID];
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegFrameBufSa.u4Pic1YSa = (ULONG)_pucPic1Y[u4InstID];
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegFrameBufSa.u4Pic2CSa = (ULONG)_pucPic2C[u4InstID];
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegFrameBufSa.u4Pic2YSa = (ULONG)_pucPic2Y[u4InstID];
...
BOOL fgIsMPEGVDecComplete(UINT32 u4InstID)
{
  ...
  if(_fgVDecComplete[u4InstID])
  {
    #if (POWER_TEST_CASE == POWER_TEST_DCM)
	 {
        i4VDEC_HAL_Common_Gcon_Enable_After_Complete(u4InstID, VDEC_MPEG4);
	 }
	 #endif
...
*/
  ...
  {
    rMpegVFifoInitPrm.u4VFifoSa = (ULONG)_pucVFifo[u4InstID];
    rMpegVFifoInitPrm.u4VFifoEa = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
    i4VDEC_HAL_MPEG_InitVDecHW(u4InstID,&rMpegVFifoInitPrm);
    ...
    rMpegBSInitPrm.u4VFifoSa =  (ULONG)_pucVFifo[u4InstID];
    rMpegBSInitPrm.u4VFifoEa = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
    rMpegBSInitPrm.u4ReadPointer =  (ULONG)_pucVFifo[u4InstID] + _u4Divx3SetPos[u4InstID];
  #ifndef  RING_VFIFO_SUPPORT
    rMpegBSInitPrm.u4WritePointer = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
  #else
  //		rMpegBSInitPrm.u4WritePointer = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
  ...
  else // MPEG
  {
    ...
    rMpegBSInitPrm.u4VFifoSa =  (ULONG)_pucVFifo[u4InstID];
    rMpegBSInitPrm.u4VFifoEa = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
    rMpegBSInitPrm.u4ReadPointer =  (ULONG)_pucVFifo[u4InstID] + u4VldByte;
  #ifndef  RING_VFIFO_SUPPORT
    rMpegBSInitPrm.u4WritePointer = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
  #else
  //		rMpegBSInitPrm.u4WritePointer = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
...
/*          if(u4InstID == 0)
        ...
        else
        {
          u4MbX_last = u4MbX;
          u4MbY_last = u4MbY;
          #if (POWER_TEST_CASE == POWER_TEST_DCM) // notice: only for IRQ enable
          if (_fgVDecComplete[u4InstID])
          {
              i4VDEC_HAL_Common_Gcon_Enable_After_Complete(u4InstID, VDEC_MPEG4);
          }
...
void vChkVDec(UINT32 u4InstID)
{
  ...
  //printk("_u4CodecVer[u4InstID] is 0x%x\n",_u4CodecVer[u4InstID]);
...
void vVerifyDx3SufxChk(UINT32 u4InstID)
{
  ...
  if(_ucVopCdTp[u4InstID] == VCT_I)
  {
    ...
    if(dwShift >= 17)
    {
      _fgVerSwitchRounding[u4InstID] = (u4VDecReadVLD(0, 60 * 4) >> 15) & 0x1;
...
{     
    ...
    {  
        ...
        if (isFail) 
		{
            printk("Function: %s line:%d Failed \n", __FUNCTION__, __LINE__);
            //vVDEC_HAL_H265_VDec_DumpReg(u4InstID, 1);
            _u4VerBitCount[u4InstID] = 0xFFFFFFFF;
        } 
        else if (_u4PicCnt[u4InstID] == _u4DumpRegPicNum[u4InstID])
        {
            printk("Function: %s line:%d (%d / %d) DumpReg\n", __FUNCTION__, __LINE__, _u4PicCnt[u4InstID], _u4DumpRegPicNum[u4InstID]);
            //vVDEC_HAL_H265_VDec_DumpReg(u4InstID, 1);
        }
		...
		//Begin The code if  for debug STILL IMAGE 
		{
			printk("Yaochuang=====>BitStream File = %s \n",_bFileStr1[u4InstID][1]);
			printk("Yaochuang=====>VFIFO SIZE = %d, BitStream Size = %d\n",V_FIFO_SZ,_tInFileInfo[u4InstID].u4FileLength);
			vH265DumpVFIFO(u4InstID, _pucVFifo[u4InstID], _tInFileInfo[u4InstID].u4FileLength, 1);
			vVDEC_HAL_H265_VDec_DumpReg(u4InstID, 1); 
			
        }
		//END The code if  for debug STILL IMAGE 
    }
