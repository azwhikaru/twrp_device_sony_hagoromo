...
UINT32 vHEVCSearchRealPic(UINT32 u4InstID)
{
    ...
    do
    {
        ...
        if (u4RetVal == NOT_SUPPORT)
        {
            printk(" [ %s  %d ] \t", __FUNCTION__, __LINE__);
            printk(" bit depth not support, return  %d\n", u4RetVal);
            break;
        }
...
UINT32 vHEVCVerifySPS_Rbsp(UINT32 u4InstID)
{
    ...
    if(pH265_SPS_DataInst->u4BitDepthLumaMinus8 == 2 && pH265_SPS_DataInst->u4BitDepthChromaMinus8 == 2)
    {
       printk("// bit-depth == 10, Trigger 10-bit Compact mode!!\n");
       _u4Main10Mode = HEVC_COMPACT_MODE;
    }
    else if(pH265_SPS_DataInst->u4BitDepthLumaMinus8 == 0 && pH265_SPS_DataInst->u4BitDepthChromaMinus8 == 0)
    {
       printk("// bit-depth == 8!!\n");
       _u4Main10Mode = 0;
    }
