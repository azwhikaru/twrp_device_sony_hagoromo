...
char _RMDUMPFileName[10][100] = {"D:\\FwdBufY.bin\0","D:\\FwdBufC.bin\0","D:\\BwdBufY.bin\0","D:\\BwdBufC.bin\0",
    "D:\\OutBufY.bin\0", "D:\\OutBufC.bin\0", "D:\\SourFwdBufY.bin\0","D:\\SourFwdBufC.bin\0",
    "D:\\SourBwdBufY.bin\0","D:\\SourBwdBufC.bin\0"};
BOOL fgDump = FALSE;
...
void vVDEC_HAL_RM_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
    ...
    INT32 i4TopVldEnd   = 128;
#endif
    INT32 i4AVCMVStart = 131;
    INT32 i4AVCMVEnd   = 181;
    INT32 i4PPStart = 0;
    INT32 i4PPEnd   = 639;
    INT32 i4MISCStart = 0;
    INT32 i4MISCEnd   = 639;
    ...
    printk("RV_VLD Registers\n");
    for (i = i4RVldStart; i < i4RVldEnd; i++)
    {
        u4Data = u4VDecReadRMVLD(u4VDecID, (i<<2));
        printk("RV VLD %04d: 0x%08X\n", i, u4Data);
    }
    ...
    for (i = i4TopVldStart; i < i4TopVldEnd; i++)
    {
        ...
        printk("VLDTOP %04d: 0x%08X\n", i, u4Data);
    }
#endif

    printk("AVCMV Registers\n");
    for (i = i4AVCMVStart; i < i4AVCMVEnd; i++)
    {
        u4Data = u4VDecReadAVCMV(u4VDecID, (i<<2));
        printk("AVCMV %04d: 0x%08X\n", i, u4Data);
    }

    printk("PP Registers\n");
    for (i = i4PPStart; i < i4PPEnd; i++)
    {
        u4Data = u4VDecReadPP(u4VDecID, (i<<2));
        printk("PP %04d: 0x%08X\n", i, u4Data);
    }

    printk("MISC Registers\n");
    for (i = i4MISCStart; i < i4MISCEnd; i++)
    {
        u4Data = u4VDecReadMISC(u4VDecID, (i<<2));
        printk("MISC %04d: 0x%08X\n", i, u4Data);
    }
    
    printk("Imgresz Registers\n");
    for (i = i4ImgStart; i < i4ImgEnd; i++)
    {
        u4Data = u4RM_VDecRead_ImgResz(u4VDecID, (i<<2));
        printk("Imgresz %04d: 0x%08X\n", i, u4Data);
    }
}
#endif
...
void vRM_TriggerDecode(UINT32 u4InstID, VDEC_INFO_RM_PICINFO_T *prParsingPic)
{
  ...
  //Log Input Window before Trigger VDec
  {
    ...
    //vVDEC_HAL_RM_VDec_DumpReg(u4InstID, TRUE);
  }
...
*/
  ...
  vVDEC_HAL_RM_EnableDramTable(u4InstID, PHYSICAL(_pucRMIntraTableBuf[u4InstID]), PHYSICAL(_pucRMInterTableBuf[u4InstID]));
  #endif
...
*/
      ...
      if (prParsingPic->u4OrgWidth != prParsingPic->u4PrevDispWidth || prParsingPic->u4OrgHeight != prParsingPic->u4PrevDispHeight)  // RPR
      {
        ...
        if (0) //(51 == prParsingPic->u4DecodedPicCnt)
        {
            UINT32 u4DumpSize = prParsingPic->u4PrevDecWidth*prParsingPic->u4PrevDecHeight;
            
            fgWrData2PC(prParsingPic->u4FwdBufY, u4DumpSize, 7, _RMDUMPFileName[6]);
            fgWrData2PC(prParsingPic->u4FwdBufC, u4DumpSize/2, 7, _RMDUMPFileName[7]);
            printk("Source Y Buffer VA: 0x%x C Buffer VA: 0x%x ", prParsingPic->u4FwdBufY, prParsingPic->u4FwdBufC);
            printk("Y Buffer PA: 0x%x C Buffer PA: 0x%x", PHYSICAL(prParsingPic->u4FwdBufY)/16, PHYSICAL(prParsingPic->u4FwdBufC)/16);
        }
        ...
        if (0) //(51 == prParsingPic->u4DecodedPicCnt)
        {
            UINT32 u4DumpSize = prParsingPic->u4Width*prParsingPic->u4Height;
            
            fgWrData2PC(prParsingPic->u4FwdBufY, u4DumpSize, 7, _RMDUMPFileName[0]);
            fgWrData2PC(prParsingPic->u4FwdBufC, u4DumpSize/2, 7, _RMDUMPFileName[1]);
            printk("Dst Y Buffer VA: 0x%x C Buffer VA: 0x%x ", prParsingPic->u4FwdBufY, prParsingPic->u4FwdBufC);
            printk("Y Buffer PA: 0x%x C Buffer PA: 0x%x", PHYSICAL(prParsingPic->u4FwdBufY)/16, PHYSICAL(prParsingPic->u4FwdBufC)/16);

            
            fgWrData2PC(prParsingPic->u4OutBufY, u4DumpSize, 7, _RMDUMPFileName[4]);
            fgWrData2PC(prParsingPic->u4OutBufC, u4DumpSize/2, 7, _RMDUMPFileName[5]);
            printk("Out Y Buffer VA: 0x%x C Buffer VA: 0x%x ", prParsingPic->u4OutBufY, prParsingPic->u4OutBufC);
            printk("Y Buffer PA: 0x%x C Buffer PA: 0x%x", PHYSICAL(prParsingPic->u4OutBufY)/16, PHYSICAL(prParsingPic->u4OutBufC)/16);
        }
      }
...
//  HalFlushInvalidateDCache();
  ...
  for (y = 0; y < u4Height/2 && fgUOK && fgVOK; y++)
  {
    for (x = 0; x < u4Width/2 && fgUOK && fgVOK; x += 4)
    {
      ...
      if (u4ErrVal != u4GolVal)
      {
        ...
        printk("<vdec> Cb mismatch. x=%d, y=%d. HW Val = 08X, Golden Val = 0x%08X\n", x, y, u4ErrVal, u4GolVal);
      ...
      if (u4ErrVal2 != u4GolVal2)
      {
        ...
        printk("<vdec> Check Golden UV Error\n");
        printk("<vdec> Cr mismatch. x=%d, y=%d. HW Val = 08X, Golden Val = 0x%08X\n", x, y, u4ErrVal2, u4GolVal2);
        //ASSERT(0);
        //while (1);
      }
    }
  ...
  else
  {
    if (0) //(FALSE == fgDump)
    {
        UINT32 u4DumpSize = _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4Height*_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4Width;
        
        if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.ePtype == RM_INTERPIC) || (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.ePtype == RM_TRUEBPIC))
        {
            fgWrData2PC(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4FwdBufY, u4DumpSize, 7, _RMDUMPFileName[0]);
            fgWrData2PC(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4FwdBufC, u4DumpSize/2, 7, _RMDUMPFileName[1]);
        }

        
        if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.ePtype == RM_TRUEBPIC)
        {
            fgWrData2PC(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4BwdBufY, u4DumpSize, 7, _RMDUMPFileName[2]);
            fgWrData2PC(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4BwdBufC, u4DumpSize/2, 7, _RMDUMPFileName[3]);
        }

        fgWrData2PC(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4OutBufY, u4DumpSize, 7, _RMDUMPFileName[4]);
        fgWrData2PC(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4OutBufC, u4DumpSize/2, 7, _RMDUMPFileName[5]);
...
void vRM_CheckCRCResult(UINT32 u4InstID, UINT32 u4DecFrameCnt, UINT32 u4CRCResBuf)
{
    ...
    for (i=0; i<4; i++) {
        ...
        if (u4SWResult != u4CRCResult) {
            printk("Y CRC Compare Golden Error (0x%x 0x%x)!!!!!!!!!!!!!!!!!!!!!\n", u4SWResult, u4CRCResult);
            //ASSERT(0);
            //break;
        }
    }
    ...
    for (i=0; i<4; i++) {
        ...
        if (u4SWResult != u4CRCResult) {
            printk("UV CRC Compare Golden Error (0x%x 0x%x)!!!!!!!!!!!!!!!!!!!!!\n", u4SWResult, u4CRCResult);
            //ASSERT(0);
            //break;
        }
    }
...
void vRM_VDecDecEndProc(UINT32 u4InstID)
{
  ...
  {   
    ...
    /*        if(u4InstID == 0)
      ...
      if (0) //(51 == _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt)
      {
          vVDEC_HAL_RM_VDec_DumpReg(u4InstID, FALSE);
      }
