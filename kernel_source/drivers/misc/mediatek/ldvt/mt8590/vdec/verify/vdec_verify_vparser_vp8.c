...
//    i4VDEC_HAL_VP8_InitVDecHW(u4InstID, &rVp8VDecInitPrm);
    ...
    prVDecVp8FrmHdr->u4WritePos= (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
...
//  prVDecVp8FrmHdr->u4FifoEnd = VIRTUAL(prVDecVp8FrmHdrPhysical.u4FifoEnd);
  ...
  if(VDEC_FLGSET(u4FlagParam,VP8PARAM_MODEREF_LFDELTA_ENABLE))
  {
     ...
     if(VDEC_FLGSET(u4FlagParam,VP8PARAM_MODEREF_IFDELTA_UPDATE))
     {
        ...
        for(u4DeltaIndex=0;u4DeltaIndex<VP8_MAX_REF_LF_DELTAS;u4DeltaIndex++)
        {
           if(u4VDEC_VP8_VLDReadLiteral(u4InstID, 1))
           {
        ...
        for(u4DeltaIndex=0;u4DeltaIndex<VP8_MAX_MODE_LF_DELTAS;u4DeltaIndex++)
        {
            if(u4VDEC_VP8_VLDReadLiteral(u4InstID, 1))
            {
...
*/
    ...
    prVDecVp8FrmHdr->u4SegIdWrapper = (ULONG)_pucSegIdWrapper[u4InstID];

#if VDEC_VP8_IMAGERESIZE_SUPPORT

       prVDecVp8FrmHdr->u4TmpBuf = (ULONG)_pucWebpReszWorkBuf[u4InstID];
	   prVDecVp8FrmHdr->u4ImgReszY = (ULONG)_pucWebpReszFrameYAddr[u4InstID];
	   prVDecVp8FrmHdr->u4ImgReszC = (ULONG)_pucWebpReszFrameCAddr[u4InstID];
#endif
                                                         ...
                                                         UINT32 u4HwSwapMode)
{
   ...
   UINT8 auAddrSwapMapTable[8] =
 {
  ...
  printk("vVP8_InvAddressSwap ++ u4AddrSwapMode %d\n", u4AddrSwapMode);
...
static CHAR _bPathAddStr[256] = {"pattern\golden\/"};
...
#if VDEC_UFO_SUPPORT
static CHAR _bFileAddStrY[20] = {"_bits_Y.out\0"};
static CHAR _bFileAddStrC[20] = {"_bits_CbCr.out\0"};
static CHAR _bFileAddStrYLen[20] = {"_len_Y.out\0"};
static CHAR _bFileAddStrCLen[20] = {"_len_CbCr.out\0"};
#else
static CHAR _bFileAddStrY[20] = {".y.dat\0"};
static CHAR _bFileAddStrC[20] = {".c.dat\0"};
#endif
...
//REG_9: c_crc_checksum[127:96]
  ...
  //UINT32 u4SWResult;
#if 1
    static UINT32 u4HWCRC_Y0, u4HWCRC_Y1, u4HWCRC_Y2, u4HWCRC_Y3;
    static UINT32 u4HWCRC_C0, u4HWCRC_C1, u4HWCRC_C2, u4HWCRC_C3;
    
    UINT32 u4Temp;
//Searh path name
    static UINT32 path_byte_addr = 0;
...
#if VDEC_UFO_SUPPORT
    ULONG u4NonSwapYLengthBase;
    ULONG u4NonSwapCLengthBase;
    ULONG *pu4WorkBuf;
    ULONG *pu4GoldenBuf;
        ...
        #endif    
        {
         ...
         u4NonSwapYBase = prVDecVp8FrmHdr->u4CurYAddr;
...
#endif
#if VDEC_UFO_SUPPORT
            u4NonSwapYLengthBase = (ULONG)(u4NonSwapYBase + _tVerMpvDecPrm[u4InstID].u4PIC_SIZE_BS);
            u4NonSwapCLengthBase = (ULONG)(u4NonSwapYLengthBase + _tVerMpvDecPrm[u4InstID].u4UFO_LEN_SIZE_Y);
            ...
            if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode != 4)
            {
                ...
                u4NonSwapYBase = (ULONG)_pucAddressSwapBuf[u4InstID];
                u4NonSwapCBase = (ULONG)_pucAddressSwapBuf[u4InstID] + PIC_Y_SZ;
                memcpy((UCHAR*)u4NonSwapYBase, _pucDumpYBuf[u4InstID],(u4AlignWidth*u4AlignHeight) +u4AlignSize);
                memcpy((UCHAR*)u4NonSwapCBase, _pucDumpCBuf[u4InstID],(u4AlignWidth*u4AlignHeight/2) + u4AlignSize);                 
 ...
 //               Printf("MPV emu VP8 inverse swap mode %d\n", _tVerMpvDecPrm[u4InstID].ucAddrSwapMode);
  ...
  //      BSP_dma_unmap_single((UINT32)_pu1VP8VDecYGoldPhy, GOLD_Y_SZ, FROM_DEVICE);
       ...
       //  msleep(1000);
        ...
        u4YBase = (ULONG)_pucVDSCLBuf[u4InstID];
            ...
            for (mbh=0; mbh < u4Ty1; mbh++)
            {
                ...
                if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode != 4) // do not compare Align to 4MB width unused data
                  ...
                  u4Offset = 0;
                    ...
                    pbGoldenBuf = (UCHAR*) (((ULONG) (_pucDumpYBuf[u4InstID])) + u4StartGld);
                            ...
                            if(1)
                            {
                                ...
                                else
                                {
                                    if ((*(pbDecBuf)) != (*(pbGoldenBuf)))
                                    {
                                        ...
                                        printk(strMessage,"Y Data Mismatch at [x= 0x%d, y=0x%d] = 0x%x, Golden = 0x%x !!! \n", u4XPix, u4YPix, (*pbDecBuf), (*pbGoldenBuf));
                                        //fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tFileListRecInfo[u4InstID]);
  ...
  //      BSP_dma_unmap_single((UINT32)_pu1VP8VDecCGoldPhy, GOLD_C_SZ, FROM_DEVICE);
         ...
         //   msleep(1000);
            ...
            u4CBase = (ULONG)_pucVDSCLBuf[u4InstID] + u4DramPicSize;
            #else
                ...
                for (mbh=0; mbh < u4Ty1; mbh++)
                {
                    ...
                    if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode != 4)
                       ...
                       u4StartGld = (mbh*u4Tx1 + mbw) * (16*16);
                        pbGoldenBuf = (UCHAR*) (((ULONG) (_pucDumpCBuf[u4InstID])) + u4StartGld);
...
#endif
        {
            ...
            pu4WorkBuf = (ULONG *)((ULONG)pbDecBuf);
            pu4GoldenBuf = (ULONG *)((ULONG)_pucDumpYBuf[u4InstID]);
...
#endif
        {
            ...
            pbDecBuf = (UCHAR*)u4NonSwapCLengthBase;
            pu4WorkBuf = (ULONG *)((ULONG)pbDecBuf);
            pu4GoldenBuf = (ULONG *)((ULONG)_pucDumpCBuf[u4InstID]);

            for (u4Idx = 0; u4Idx < _u4GoldenCSize[u4InstID]/sizeof(ULONG); u4Idx++) {
...
BOOL fgIsVP8VDecComplete(UINT32 u4InstID)
{
  ...
  if(_fgVDecComplete[u4InstID])
  {
      ...
      *(volatile ULONG *)(0xF6000008) = 0xFF;
