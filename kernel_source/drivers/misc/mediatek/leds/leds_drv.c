...
int leds_drv_brightness = LED_FULL;
module_param_named(brightness, leds_drv_brightness, int, S_IWUSR | S_IRUGO );
...
static enum led_brightness mt65xx_led_get(struct led_classdev *led_cdev)
{
	return mt_mt65xx_led_get(led_cdev);
}
...
#ifdef CONFIG_ARCH_MT8590_ICX
	if(g_leds_data[type]->cust.mode == MT65XX_LED_MODE_CUST_BLS_PWM){
		mt65xx_led_set(&(g_leds_data[type]->cdev), level);
	}
	else{
...
#ifdef CONFIG_ARCH_MT8590_ICX
		/* not MT65xx PMIC gpio, but MT8590 GPIO */
		if (cust_led_list[i].mode == MT65XX_LED_MODE_GPIO) {
			pr_warn("mt65xx_leds_probe: set GPIO[%d] as a LED port\n", cust_led_list[i].data);
			mt_set_gpio_mode(cust_led_list[i].data, GPIO_MODE_GPIO);
			mt_set_gpio_dir(cust_led_list[i].data, GPIO_DIR_OUT);
			mt_set_gpio_out(cust_led_list[i].data, GPIO_OUT_ZERO);	/* low active */

			g_leds_data[i]->cdev.brightness_get = mt65xx_led_get;
#ifdef CUST_LEDS_HAVE_TRIGGER
			g_leds_data[i]->cdev.default_trigger = cust_led_list[i].default_trigger;
#endif
		}
		else if(cust_led_list[i].mode == MT65XX_LED_MODE_CUST_BLS_PWM){
			g_leds_data[i]->cdev.brightness_get = mt65xx_led_get;
			if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
				mt65xx_led_set(&(g_leds_data[i]->cdev),leds_drv_brightness);
			}
		}
#endif /* CONFIG_ARCH_MT8590_ICX */
