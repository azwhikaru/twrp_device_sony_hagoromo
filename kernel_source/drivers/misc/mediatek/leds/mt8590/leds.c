	...
	*/	
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_GPIO:
			...
			pr_debug("mt65xx_leds_set_cust: LED brightness [%d]\n", level);
#ifdef CUST_LEDS_GPIO_HIGH_ACTIVE
			mt_set_gpio_out(cust->data, level ? GPIO_OUT_ONE : GPIO_OUT_ZERO);
#else
			mt_set_gpio_out(cust->data, level ? GPIO_OUT_ZERO : GPIO_OUT_ONE); 
#endif
			return 1;
...
#ifdef CONFIG_ARCH_MT8590_ICX
enum led_brightness mt_mt65xx_led_get(struct led_classdev *led_cdev)
{
	struct mt65xx_led_data *led_data =
		container_of(led_cdev, struct mt65xx_led_data, cdev);
	struct cust_mt65xx_led *cust = &led_data->cust;
	int val, rval = LED_OFF;

	switch (cust->mode) {
	case MT65XX_LED_MODE_GPIO:
		val = mt_get_gpio_out(cust->data);
		pr_warn("[LED] mt_mt65xx_led_get: GPIO (%d)\n", val);
		rval = (val) ? LED_OFF : LED_FULL;
		break;
	case MT65XX_LED_MODE_CUST_BLS_PWM:
		rval = mt_get_bl_brightness();
		break;
	default:
		pr_warn("mt_mt65xx_led_get: not supported\n");
		break;
	}
	return rval;
}
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if (strcmp(led_data->cust.name, "lcd-backlight") == 0) {
		...
		/* If not LCD backlight, always refrect the blightness */
		led_data->level = level;
		schedule_work(&led_data->work);
	}
#else /* CONFIG_ARCH_MT8590_ICX */
