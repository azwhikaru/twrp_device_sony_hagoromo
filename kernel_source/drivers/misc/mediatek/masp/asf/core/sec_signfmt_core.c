				     ...
				     unsigned int offset, unsigned int size)
{
	...
	/* hash file                           */
	/* ----------------------------------- */
	if (SEC_OK != (ret = sec_hash((uchar *) file_buf, file_size, (uchar *) hash_buf, hash_len))) {
					...
					char *hash_buf, unsigned int hash_len, char *ext_buf)
{
	...
	if (SEC_OK !=
	    (ret =
	     sec_signfmt_calculate_filelist_hash_core(fp, (char *)verify_buf + SEC_IMG_HEADER_SIZE,
				     ...
				     unsigned int hash_len, char *ext_buf)
{
	...
	if (SECCFG_V3 == get_seccfg_ver()
		...
		if (SEC_OK !=
		    (ret =
		     sec_signfmt_calculate_image_hash_v4(part_name, &img_if->v3, hash_buf, hash_len,
							 ext_buf))) {
			goto _end;
		}
	} else if (SECCFG_V3 == get_seccfg_ver()
		...
		if (SEC_OK !=
		    (ret =
		     sec_signfmt_calculate_image_hash_v3(part_name, &img_if->v3, hash_buf, hash_len,
							 ext_buf))) {
			goto _end;
		}
	} else {
		if (SEC_OK !=
		    (ret =
		     sec_signfmt_calculate_image_hash_v2(part_name,
