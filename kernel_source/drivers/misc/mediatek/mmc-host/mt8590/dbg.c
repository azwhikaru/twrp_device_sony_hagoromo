...
#ifndef FPGA_PLATFORM
extern void msdc_set_driving(struct msdc_host* host,struct msdc_hw* hw,int sd);
...
static int msdc_debug_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	...
	if(cmd == SD_TOOL_ZONE) {
		          ...
		          if((unsigned char)p3 > 7 || (unsigned char)p4 > 7 ||(unsigned char)p5 > 7) {
		             MSDC_TRC_PRINT(MSDC_DBGDRV,"[****SD_Debug****]Some drving value was not right(correct:0~7)\n");
						 ...
						 if(p6 == 0x33){
							 ...
							 MSDC_TRC_PRINT(MSDC_DBGDRV,"[****SD_Debug****][MSDC%u][0x%x] clk_drv=%d, cmd_drv=%d, dat_drv=%d\n",host->id, p6,
							 ...
							 MSDC_TRC_PRINT(MSDC_DBGDRV,"[****SD_Debug****][MSDC%u][0x%x] clk_drv=%d, cmd_drv=%d, dat_drv=%d\n",host->id, p6,
							 	host->hw->clk_drv_sd_18, host->hw->cmd_drv_sd_18, host->hw->dat_drv_sd_18);
		...
		if(id == 0 || id == 3) //for msdc0
		{
#ifdef CONFIG_MTK_EMMC_SUPPORT
		   sd_multi_rw_compare(id, COMPARE_ADDRESS_MMC, compare_count);//test the address 0 of eMMC card, since there a little memory.
#else
		   sd_multi_rw_compare(id, COMPARE_ADDRESS_SD, compare_count); //test a larger address of SD card
#endif
		}else {
