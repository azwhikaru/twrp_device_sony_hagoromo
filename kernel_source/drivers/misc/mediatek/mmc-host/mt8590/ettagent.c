...
static int create_test_nodes(int size, struct kobject *parent_kobj)
{
    ...
    for(i=0; i<size; i++){
        ...
        for(j=0; j<TEST_MODE_COUNT; j++){    
			...
			sysfs_attr_init(&test_kattr[i * TEST_MODE_COUNT + j]->attr);
...
static int create_reg_node(int size, struct kobject *parent_kobj)
{
    ...
    for(i=0; i<size; i++){
		...
		sysfs_attr_init(&reg_kattr[i]->attr);
...
static int create_freq_node(int size, struct kobject *parent_kobj)
{
    ...
    for(i=0; i<size; i++){
		...
		sysfs_attr_init(&freq_kattr[i]->attr);
...
static E_RESULT_TYPE autok_cmd_test(struct msdc_host *host, unsigned int test_data[], int test_len)
{
    ...
    if ( res != E_RESULT_PASS )
	   MSDC_TRC_PRINT(MSDC_ETTAGENT,"[%s] failure since error: %i\n", __func__, res);
...
static int msdc_ettagent_read(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func, void *pBuffer, unsigned int u4Len, unsigned int u4Cmd)
{
    ...
    {
        MSDC_ERR_PRINT(MSDC_ETTAGENT,"[%s] pBuffer = %p, host = %p\n", __func__, pBuffer, host);
        return -1;
    }
    ...
    if( ((u4Cmd == 53) && (u4Len < 4)) ||
        ((u4Cmd == 52) && (u4Len > 1)) )
    {
        MSDC_ERR_PRINT(MSDC_ETTAGENT,"[%s] u4Cmd = %d, u4Len = %d\n", __func__, u4Cmd, u4Len);
        return -1;
    }
    ...
    {
        MSDC_ERR_PRINT(MSDC_ETTAGENT,"[%s] Doesn't support u4Cmd = %d\n", __func__, u4Cmd);
        ret = -1;
    }
...
//    printk("Isaac: host->error = %d\n", host->error);
    if(ret)
	   MSDC_TRC_PRINT(MSDC_ETTAGENT,"[%s] CMD%u failure since error: %i\n", __func__, u4Cmd, ret);
