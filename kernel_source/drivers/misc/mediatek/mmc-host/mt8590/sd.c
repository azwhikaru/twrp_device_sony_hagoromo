...
#include "mt_sd.h"
#ifdef MTK_DUALCARD_SUPPORT
#define CUST_EINT_MSDC2_INS_NUM 77
#define CUST_EINT_MSDC2_INS_DEBOUNCE_CN 1
#define CUST_EINT_MSDC2_INS_TYPE CUST_EINT_TRIGGER_HIGH
...
#include <mach/eint.h>
#include <cust_eint.h>
#ifdef CUST_EINT_MC1_DET_NUM
#define CUST_EINT_MSDC1_INS_NUM           CUST_EINT_MC1_DET_NUM
#define CUST_EINT_MSDC1_INS_DEBOUNCE_CN   CUST_EINT_MC1_DET_DEBOUNCE_CN
#define CUST_EINT_MSDC1_INS_TYPE          CUST_EINT_MC1_DET_TYPE
#define CUST_EINT_MSDC1_INS_DEBOUNCE_EN   CUST_EINT_MC1_DET_DEBOUNCE_EN
#endif
...
#define ICX_AD_BID2     (5)     // BID3 = AD5ch
#define ICX_AD_BID3     (6)     // BID3 = AD6ch
#define ICX_AD_VAL_TH   (0x73C) // BoardID3 Threshold(BB-SET)
#define ICX_AD_MAX      (0xfff) // AUXADC resolution is 4095(0-0xfff)
static void msdc_reorganize_drv_strength(struct msdc_host *host);

#define CMD_BUSY_TMO    (HZ*2) 	/* 2secs for Generic R1b busy */
#define CMD_ERASE_TMO   (HZ*16) /* 16secs for CMD38(Erase) busy */
#endif /*CONFIG_ARCH_MT8590_ICX*/

#if defined(CONFIG_ENABLE_CARDDET)
#include <mach/icx_carddet.h>
#endif /*CONFIG_ENABLE_CARDDET*/
...
#if CONFIG_MSDC_GATE_CLOCK_TIMEOUT_MSECS
#define CLK_TIMEOUT                      msecs_to_jiffies(CONFIG_MSDC_GATE_CLOCK_TIMEOUT_MSECS)
#else
...
#if defined(CONFIG_ENABLE_CARDDET)
static struct carddet_eint_ops *msdc1_carddet_ops = NULL;
static struct carddet_func_tbl *msdc_carddet_func = NULL;
#endif /*CONFIG_ENABLE_CARDDET*/

//#define MSDC_EMMC_USE_RST_N

//=================================

#define MSDC_LOWER_FREQ
#define MSDC_MAX_FREQ_DIV                (2)  /* 200 / (4 * 2) */
#define MSDC_MAX_TIMEOUT_RETRY           (3)
...
{
const u32 hclk[] =  { 26,273,182,78, 0 };
    u32 base = host->base;
	u32 hclksrc=MSDC_DONOTCARE_HCLK, clksrc;
	...
	MSDC_ASSERT(host);
    if ( host->id == 3) {
		sdr_get_field(TOPCKGEN_CTRL(CLKCFG6) ,MSDC_CKGENHCLK_SEL(MSDC3,CLKCFG6), hclksrc);

		sdr_get_field(TOPCKGEN_CTRL(CLKCFG14), MSDC_CKGEN_SEL(MSDC3,CLKCFG14), clksrc);
	}
	else if ( host->id == 2) {
	   sdr_get_field(TOPCKGEN_CTRL(CLKCFG3), MSDC_CKGEN_SEL(MSDC2,CLKCFG3), clksrc);
	}
	else if ( host->id == 1) {
	   sdr_get_field(TOPCKGEN_CTRL(CLKCFG3), MSDC_CKGEN_SEL(MSDC1,CLKCFG3), clksrc);
	}
	else {
	   sdr_get_field(TOPCKGEN_CTRL(CLKCFG2), MSDC_CKGEN_SEL(MSDC0,CLKCFG2), clksrc);
	}

    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] MSDCPLL_CON0(0x121) = 0x%x\n", host->id, APMIXEDSYS_CTRL(MSDCPLL_CON0),     sdr_read32(APMIXEDSYS_CTRL(MSDCPLL_CON0)));
    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] MSDCPLL_PWR_CON0    = 0x%x\n", host->id, APMIXEDSYS_CTRL(MSDCPLL_PWR_CON0), sdr_read32(APMIXEDSYS_CTRL(MSDCPLL_PWR_CON0)));

    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] TOPCKGEN_CLK_CFG2   = 0x%x\n", host->id, TOPCKGEN_CTRL(CLKCFG2),  sdr_read32(TOPCKGEN_CTRL(CLKCFG2)));
    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] TOPCKGEN_CLK_CFG3   = 0x%x\n", host->id, TOPCKGEN_CTRL(CLKCFG3),  sdr_read32(TOPCKGEN_CTRL(CLKCFG3)));
    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] TOPCKGEN_CLK_CFG6   = 0x%x\n", host->id, TOPCKGEN_CTRL(CLKCFG6),  sdr_read32(TOPCKGEN_CTRL(CLKCFG6)));
    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] TOPCKGEN_CLK_CFG14  = 0x%x\n", host->id, TOPCKGEN_CTRL(CLKCFG14), sdr_read32(TOPCKGEN_CTRL(CLKCFG14)));

    if(hclksrc==MSDC_DONOTCARE_HCLK) {
      MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] clksrc %uMHz:%u\n", host->id, host->hclk/1000000, clksrc);
    }
	else {
		MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] clksrc %uMHz:%u, hclksrc %uMHz:%u\n", host->id, host->hclk/1000000, clksrc, hclk[hclksrc], hclksrc);
	}

    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] PERI_GLOBALCON_PDN0_STA    = 0x%x\n", host->id, PERICFG_CTRL(PERI_GLOBALCON_PDN0_STA),    sdr_read32(PERICFG_CTRL(PERI_GLOBALCON_PDN0_STA)));
    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] PERI_GLOBALCON_PDN_MD1_STA = 0x%x\n", host->id, PERICFG_CTRL(PERI_GLOBALCON_PDN_MD1_STA), sdr_read32(PERICFG_CTRL(PERI_GLOBALCON_PDN_MD1_STA)));
    MSDC_TRC_PRINT(MSDC_INIT,"[SD%d] Reg[%x] PERI_GLOBALCON_PDN_MD2_STA = 0x%x\n", host->id, PERICFG_CTRL(PERI_GLOBALCON_PDN_MD2_STA), sdr_read32(PERICFG_CTRL(PERI_GLOBALCON_PDN_MD2_STA)));

    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 00] MSDC_CFG              = 0x%.8x\n", sdr_read32(MSDC_CFG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 04] MSDC_IOCON            = 0x%.8x\n", sdr_read32(MSDC_IOCON));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 08] MSDC_PS               = 0x%.8x\n", sdr_read32(MSDC_PS));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 0C] MSDC_INT              = 0x%.8x\n", sdr_read32(MSDC_INT));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 10] MSDC_INTEN            = 0x%.8x\n", sdr_read32(MSDC_INTEN));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 14] MSDC_FIFOCS           = 0x%.8x\n", sdr_read32(MSDC_FIFOCS));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 18] MSDC_TXDATA           = not read\n");
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 1C] MSDC_RXDATA           = not read\n");
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 30] SDC_CFG               = 0x%.8x\n", sdr_read32(SDC_CFG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 34] SDC_CMD               = 0x%.8x\n", sdr_read32(SDC_CMD));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 38] SDC_ARG               = 0x%.8x\n", sdr_read32(SDC_ARG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 3C] SDC_STS               = 0x%.8x\n", sdr_read32(SDC_STS));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 40] SDC_RESP0             = 0x%.8x\n", sdr_read32(SDC_RESP0));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 44] SDC_RESP1             = 0x%.8x\n", sdr_read32(SDC_RESP1));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 48] SDC_RESP2             = 0x%.8x\n", sdr_read32(SDC_RESP2));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 4C] SDC_RESP3             = 0x%.8x\n", sdr_read32(SDC_RESP3));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 50] SDC_BLK_NUM           = 0x%.8x\n", sdr_read32(SDC_BLK_NUM));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 54] SDC_VOL_CHG           = 0x%.8x\n", sdr_read32(SDC_VOL_CHG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 58] SDC_CSTS              = 0x%.8x\n", sdr_read32(SDC_CSTS));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 5C] SDC_CSTS_EN           = 0x%.8x\n", sdr_read32(SDC_CSTS_EN));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 60] SDC_DCRC_STS          = 0x%.8x\n", sdr_read32(SDC_DCRC_STS));

	if((host->id == 0)||(host->id == 3)){
       MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 70] EMMC_CFG0             = 0x%.8x\n", sdr_read32(EMMC_CFG0));
       MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 74] EMMC_CFG1             = 0x%.8x\n", sdr_read32(EMMC_CFG1));
       MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 78] EMMC_STS              = 0x%.8x\n", sdr_read32(EMMC_STS));
       MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 7C] EMMC_IOCON            = 0x%.8x\n", sdr_read32(EMMC_IOCON));
	}

    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 80] SDC_ACMD_RESP         = 0x%.8x\n", sdr_read32(SDC_ACMD_RESP));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 84] SDC_ACMD19_TRG        = 0x%.8x\n", sdr_read32(SDC_ACMD19_TRG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 88] SDC_ACMD19_STS        = 0x%.8x\n", sdr_read32(SDC_ACMD19_STS));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 8C]*MSDC_DMA_HIGH4BIT     = 0x%.8x\n", sdr_read32(MSDC_DMA_HIGH4BIT)); /* new register @mt8590 */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 90] MSDC_DMA_SA           = 0x%.8x\n", sdr_read32(MSDC_DMA_SA));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 94] MSDC_DMA_CA           = 0x%.8x\n", sdr_read32(MSDC_DMA_CA));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 98] MSDC_DMA_CTRL         = 0x%.8x\n", sdr_read32(MSDC_DMA_CTRL));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ 9C] MSDC_DMA_CFG          = 0x%.8x\n", sdr_read32(MSDC_DMA_CFG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ A0] MSDC_DBG_SEL          = 0x%.8x\n", sdr_read32(MSDC_DBG_SEL));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ A4] SW_DBG_OUT            = 0x%.8x\n", sdr_read32(MSDC_DBG_OUT));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ A8] MSDC_DMA_LEN          = 0x%.8x\n", sdr_read32(MSDC_DMA_LEN));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ B0] MSDC_PATCH_BIT0       = 0x%.8x\n", sdr_read32(MSDC_PATCH_BIT0));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ B4] MSDC_PATCH_BIT1       = 0x%.8x\n", sdr_read32(MSDC_PATCH_BIT1));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ B8]*MSDC_PATCH_BIT2       = 0x%.8x\n", sdr_read32(MSDC_PATCH_BIT2)); /* new register @mt8590 */


	/* Only for SD/SDIO Controller 6 registers below */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ C0]*DAT0_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT0_TUNE_CRC)); /* new register @mt8590 */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ C4]*DAT1_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT1_TUNE_CRC)); /* new register @mt8590 */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ C8]*DAT2_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT2_TUNE_CRC)); /* new register @mt8590 */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ CC]*DAT3_TUNE_CRC         = 0x%.8x\n", sdr_read32(DAT3_TUNE_CRC)); /* new register @mt8590 */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ D0]*CMD_TUNE_CRC          = 0x%.8x\n", sdr_read32(CMD_TUNE_CRC)); /* new register @mt8590 */
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ D4]*SDIO_TUNE_WIND        = 0x%.8x\n", sdr_read32(SDIO_TUNE_WIND)); /* new register @mt8590 */

    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ F0] MSDC_PAD_TUNE0	     = 0x%.8x\n", sdr_read32(MSDC_PAD_TUNE0));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ F4]*MSDC_PAD_TUNE1	     = 0x%.8x\n", sdr_read32(MSDC_PAD_TUNE1));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ F8] MSDC_DAT_RDDLY0	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY0));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[ FC] MSDC_DAT_RDDLY1	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY1));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[100]*MSDC_DAT_RDDLY2	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY2));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[104]*MSDC_DAT_RDDLY3	     = 0x%.8x\n", sdr_read32(MSDC_DAT_RDDLY3));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[110] MSDC_HW_DBG	         = 0x%.8x\n", sdr_read32(MSDC_HW_DBG));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[114] MSDC_VERSION	         = 0x%.8x\n", sdr_read32(MSDC_VERSION));
    MSDC_TRC_PRINT(MSDC_INIT,"Reg[118] MSDC_ECO_VER  	     = 0x%.8x\n", sdr_read32(MSDC_ECO_VER));

    if(host->id == 3){
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[180] EMMC50_PAD_CTL0	     = 0x%.8x\n", sdr_read32(EMMC50_PAD_CTL0));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[184] EMMC50_PAD_DS_CTL0	 = 0x%.8x\n", sdr_read32(EMMC50_PAD_DS_CTL0));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[188] EMMC50_PAD_DS_TUNE	 = 0x%.8x\n", sdr_read32(EMMC50_PAD_DS_TUNE));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[18C] EMMC50_PAD_CMD_TUNE   = 0x%.8x\n", sdr_read32(EMMC50_PAD_CMD_TUNE));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[190] EMMC50_PAD_DAT01_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT01_TUNE));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[194] EMMC50_PAD_DAT23_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT23_TUNE));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[198] EMMC50_PAD_DAT45_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT45_TUNE));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[19C] EMMC50_PAD_DAT67_TUNE = 0x%.8x\n", sdr_read32(EMMC50_PAD_DAT67_TUNE));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[204] EMMC51_CFG0	         = 0x%.8x\n", sdr_read32(EMMC51_CFG0));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[208] EMMC50_CFG0	         = 0x%.8x\n", sdr_read32(EMMC50_CFG0));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[20C] EMMC50_CFG1	         = 0x%.8x\n", sdr_read32(EMMC50_CFG1));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[21C] EMMC50_CFG2	         = 0x%.8x\n", sdr_read32(EMMC50_CFG2));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[220] EMMC50_CFG3	         = 0x%.8x\n", sdr_read32(EMMC50_CFG3));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[224] EMMC50_CFG4	         = 0x%.8x\n", sdr_read32(EMMC50_CFG4));
	    MSDC_TRC_PRINT(MSDC_INIT,"Reg[228] EMMC50_BLOCK_LENGTH   = 0x%.8x\n", sdr_read32(EMMC50_BLOCK_LENGTH));
    }

    /* dump gpio register */
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C10] MSDC0_CTRL0_CLK        = 0x%.8x", sdr_read32(0xF0005000 + 0xC00));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C20] MSDC0_CTRL1_CMD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC10));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C30] MSDC0_CTRL2_DAT        = 0x%.8x", sdr_read32(0xF0005000 + 0xC20));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C40] MSDC0_CTRL3_PAD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC30));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C50] MSDC1_CTRL0_CLK        = 0x%.8x", sdr_read32(0xF0005000 + 0xC40));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C60] MSDC1_CTRL1_CMD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC50));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C70] MSDC1_CTRL2_DAT        = 0x%.8x", sdr_read32(0xF0005000 + 0xC60));
    MSDC_TRC_PRINT(MSDC_INIT,"Rg[C80] MSDC1_CTRL3_PAD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC70));
		...
		if(host->id == 0){
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%u Pad RDSEL(%u) TDSEL(%u)\r\n", host->id, u4rdsel,u4tdsel);
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uCLK :GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)	\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uCMD :GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT0:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT1:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT2:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT3:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT4:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT5:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT6:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDAT7:GPIO%03u:%u: SR(%u) TX(%u), IES(%u) SMT(%u) RX(%u)\r\n",\
			...
			MSDC_TRC_PRINT(MSDC_INIT,"MSDC%uDSL :GPIO%03u:%u: SMT(%u) RX(%u)\r\n",\
...
static int msdc_clk_stable(struct msdc_host *host,u32 mode, u32 div, u32 hs400_src)
{
	...
	if(loop >= 0) {
		...
		/* error not stable */
		printk(KERN_ERR "msdc%d on clock failed\n",host->id);
...
static u32 hclks_msdc50[] = { 26000000, 400000000, 182000000, 136000000, 156000000, 200000000, 100000000, 50000000};
#endif
...
#ifdef CFG_DEV_MSDC0
			case 0:
				sdr_set_field(MSDC_CTRL(MSDC0, 0), MSDC_CLK_SR(MSDC0),	clk);
...
#ifdef CFG_DEV_MSDC1
			case 1:
				sdr_set_field(MSDC_CTRL(MSDC1, 0), MSDC_CLK_SR(MSDC1),	clk);
...
#ifdef CFG_DEV_MSDC2
			case 2:
				sdr_set_field(MSDC_CTRL(MSDC2, 0), MSDC_CLK_SR(MSDC2),	clk);
...
#ifdef CFG_DEV_MSDC3
			case 3:
				sdr_set_field(MSDC_CTRL(MSDC3, 0), MSDC_CLK_SR(MSDC3),	clk);
...
#ifdef CFG_DEV_MSDC0
		case 0:
            sdr_set_field(MSDC_CTRL(MSDC0, 0), BIT_MSDC_CLK_R0R1PUPD,  MSDC_PST_PD_50KOHM);
...
#ifdef CFG_DEV_MSDC1
		case 1:
            sdr_set_field(MSDC_CTRL(MSDC1, 0), BIT_MSDC_CLK_R0R1PUPD,  MSDC_PST_PD_50KOHM);
...
#ifdef CFG_DEV_MSDC2
		case 2:
            sdr_set_field(MSDC_CTRL(MSDC2, 0), BIT_MSDC_CLK_R0R1PUPD,  MSDC_PST_PD_50KOHM);
...
#ifdef CFG_DEV_MSDC3
        case 3:
            sdr_set_field(MSDC_CTRL(MSDC3, 0), BIT_MSDC_CLK_R0R1PUPD,  MSDC_PST_PD_50KOHM);
			...
			sdr_set_field(EMMC50_PAD_DS_CTL0, MSDC_EMMC50_PAD_DS_CTL0_PUPD, mode);
            break;
#endif
         default:
			break;
...
#ifdef CFG_DEV_MSDC0
			case 0:
				//sdr_set_field(MSDC_CTRL(MSDC0, 0), MSDC_CLK_SMT(MSDC0), 	 smt);
...
#ifdef CFG_DEV_MSDC1
			case 1:
				//sdr_set_field(MSDC_CTRL(MSDC1, 0), MSDC_CLK_SMT(MSDC1), 	 smt);
...
#ifdef CFG_DEV_MSDC2
			case 2:
				//sdr_set_field(MSDC_CTRL(MSDC2, 0), MSDC_CLK_SMT(MSDC2), 	 smt);
...
#ifdef CFG_DEV_MSDC3
			case 3:
				//sdr_set_field(MSDC_CTRL(MSDC3, 0), MSDC_CLK_SMT(MSDC3), 	 smt);
				...
				sdr_set_field(EMMC50_PAD_DS_CTL0, MSDC_EMMC50_PAD_DS_CTL0_SMT, smt);
				break;
#endif
			default:
				break;
...
void msdc_set_driving(struct msdc_host *host,struct msdc_hw *hw, int sd)
{
#ifndef FPGA_PLATFORM
		u32 id;
		...
		if(host && hw){
            ...
            {
                clkdrv = hw->clk_drv_sd_18;
                cmddrv = hw->cmd_drv_sd_18;
                datdrv = hw->dat_drv_sd_18;
            }
            else if(sd == 2)
            {
                clkdrv = hw->clk_drv_sd_18_sdr50;
                cmddrv = hw->cmd_drv_sd_18_sdr50;
                datdrv = hw->dat_drv_sd_18_sdr50;
            }
            else if(sd == 3)
            {
                clkdrv = hw->clk_drv_sd_18_ddr50;
                cmddrv = hw->cmd_drv_sd_18_ddr50;
                datdrv = hw->dat_drv_sd_18_ddr50;
            }
			switch(host->id){
#ifdef CFG_DEV_MSDC0
				case 0:
				...
				case 3:
					...
					sdr_set_field(EMMC50_PAD_DS_CTL0, MSDC_EMMC50_PAD_DS_CTL0_DRV, hw->ds_drv);
					//sdr_set_field(EMMC50_PAD_DS_CTL0, MSDC_EMMC50_PAD_DS_CTL0_DRV, hw->rst_drv);
					break;
#endif
				default:
					break;
...
#ifndef FPGA_PLATFORM
    u32 id;
	u32 clkdrv=0, cmddrv=0, datdrv=0, dsdrv = 0;
	 u32 gpiobase;
#endif
	...
	if(host && msdc_ctl){
		switch(id){
			...
			case 3:
				...
				sdr_get_field(EMMC50_PAD_DS_CTL0, MSDC_EMMC50_PAD_DS_CTL0_DRV, dsdrv);
	            break;
#endif
			default:
				break;
...
static void msdc_emmc_power(struct msdc_host *host,u32 on)
{
    ...
    if (host->mmc && host->mmc->card && (host->mmc->caps & MMC_CAP_WAIT_WHILE_BUSY) && (on == 0)){
        ...
        while ((sdr_read32(MSDC_PS) & 0x10000) != 0x10000) {
            if (time_after(jiffies, tmo)){
                MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u]Dat0 keep low before power off, sa_timeout = 0x%x\n", host->id, host->mmc->card->ext_csd.sa_timeout);
                break;
            }
        }
    }
	...
	switch(host->id){
		case 0:
			msdc_set_smt(host,1);
#ifndef CONFIG_DONOT_CONTROL_VEMC3V3_ON_EMMC
#ifndef FPGA_PLATFORM
...
//			msdc_set_driving(host,host->hw,0);
            ...
            MSDC_TRC_PRINT(MSDC_CONFIG,"[MSDC%u] fixed io 3.3V and flash 3.3V)\n", host->id);
#ifdef BENTEN_M1_SD_UHS1
#ifdef BENTEN_M1_SD_POWER_SWITCH_GPIO
			mt_set_gpio_out(host->hw->pow_pin, GPIO_OUT_ZERO);
#else
...
#ifdef BENTEN_M1_SD_POWER_SWITCH_GPIO
            if(on){
                mt_set_gpio_out(host->hw->pow_en, GPIO_OUT_ONE);
            } else {
                mt_set_gpio_out(host->hw->pow_en, GPIO_OUT_ZERO);
            }
#endif /*BENTEN_M1_SD_POWER_SWITCH_GPIO*/

#ifndef FPGA_PLATFORM
#if 0   // fixed 3.3Vs always.
		...
		case 2:
			...
			msdc_set_driving(host,host->hw,0);
			msdc_set_rdtdsel(host,0);
			mt_set_gpio_mode(PIN4MSDC0_DAT5|0x80000000,GPIO_MODE_GPIO);
			mt_set_gpio_dir(PIN4MSDC0_DAT5|0x80000000,GPIO_DIR_OUT);
			mt_set_gpio_out(PIN4MSDC0_DAT5|0x80000000,GPIO_OUT_ONE);
			mt_set_gpio_mode(PIN4MSDC0_DAT4|0x80000000,GPIO_MODE_GPIO);
			mt_set_gpio_dir(PIN4MSDC0_DAT4|0x80000000, GPIO_DIR_OUT);
			mt_set_gpio_out(PIN4MSDC0_DAT4|0x80000000, GPIO_OUT_ZERO);
#else
...
static int msdc_sd_power_switch(struct msdc_host *host,u32 on)
{
    int err = -EINVAL;
	switch(host->id){
		case 1:
#ifndef BENTEN_M1_SD_UHS1
            MSDC_TRC_PRINT(MSDC_CONFIG,"[MSDC%u] CANNOT switch to 1.8V since MSDC1 fixed io 3.3V and flash 3.3V)\n", host->id);
#else
#ifndef FPGA_PLATFORM
...
#ifdef BENTEN_M1_SD_POWER_SWITCH_GPIO
			if(on) {
				mt_set_gpio_out(host->hw->pow_pin, GPIO_OUT_ONE);
			}
			else {
				mt_set_gpio_out(host->hw->pow_pin, GPIO_OUT_ZERO);
			}
...
//			msdc_set_driving(host,host->hw,1);
		...
		case 2:
                ...
                mt_set_gpio_mode(PIN4MSDC0_DAT4|0x80000000,GPIO_MODE_GPIO);
		mt_set_gpio_dir(PIN4MSDC0_DAT4|0x80000000,GPIO_DIR_OUT);
		mt_set_gpio_out(PIN4MSDC0_DAT4|0x80000000, GPIO_OUT_ONE);
		msdc_set_smt(host,1);
		msdc_set_rdtdsel(host,1);
		msdc_set_driving(host,host->hw,1);
#else
...
#else
#ifndef FPGA_PLATFORM
			msdc_ldo_power(on, MT6323_POWER_LDO_VMCH, VOL_3300, &g_msdc2_io);
			//msdc_set_enio18(host,1);
#endif
#endif
			msdc_set_smt(host,1);
...
static void msdc_sdio_power(struct msdc_host *host,u32 on)
{
	switch(host->id){
		case 2:
            ...
            MSDC_TRC_PRINT(MSDC_CONFIG,"[MSDC%u] fixed io 1.8V and flash 1.8V)\n", host->id);
			msdc_set_smt(host,1);
			msdc_set_rdtdsel(host,1);
			msdc_set_driving(host,host->hw,1);
#else
			if(MSDC_VIO18_MC2 == host->power_domain) {
				if(on) {
					...
					//msdc_ldo_power(on, MT6323_POWER_LDO_VMCH, VOL_3300, &g_msdc2_io); // Bus & device keeps 3.3v
				}
...
{
    MSDC_TRC_PRINT(MSDC_CONFIG, "When connect to MTK SDIO 3.0 Device, The CMD11 will be performed but no need to change voltage really. ");
}
...
static void msdc_reset_tmo_tune_counter(struct msdc_host *host,TUNE_COUNTER index)
{
    if(index >= 0 && index <= all_counter)
    {
        switch (index)
        {
            case cmd_counter:
                if(host->rwcmd_time_tune != 0)
                    MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] TMO TUNE CMD Times(%d)", host->id, host->rwcmd_time_tune);
                host->rwcmd_time_tune = 0;
                break;
            case read_counter  :
                if(host->read_time_tune != 0)
                    MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] TMO TUNE READ Times(%d)", host->id, host->read_time_tune);
                host->read_time_tune = 0;
                break;
            case write_counter :
                if(host->write_time_tune != 0)
                    MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] TMO TUNE WRITE Times(%d)", host->id, host->write_time_tune);
        ...
        MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] ==> reset tmo counter index(%d) error!\n",host->id,index);
    }
}
static void msdc_reset_crc_tune_counter(struct msdc_host *host,TUNE_COUNTER index)
{
    ...
    if(index >= 0 && index <= all_counter)
    {
        switch (index)
        {
            case cmd_counter:
                if(host->t_counter.time_cmd != 0){
                    MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] CRC TUNE CMD Times(%d)", host->id, host->t_counter.time_cmd);
                    if(g_ett_tune){
            ...
            case read_counter  :
                if(host->t_counter.time_read != 0){
                    MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] CRC TUNE READ Times(%d)", host->id, host->t_counter.time_read);
                    if(g_ett_tune){
            ...
            case write_counter :
                if(host->t_counter.time_write != 0){
                    MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] CRC TUNE WRITE Times(%d)", host->id, host->t_counter.time_write);
                    if(g_ett_tune){
        ...
        MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] ==> reset crc counter index(%d) error!\n",host->id,index);
    }
}
...
static void msdc_set_bad_card_and_remove(struct msdc_host *host)
{
    ...
    if(host == NULL) {
        MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] WARN: host is NULL",host->id);
        return;
    }
    host->card_inserted = 0;

    if((host->mmc == NULL) || (host->mmc->card == NULL)) {
        MSDC_ERR_PRINT(MSDC_ERROR, "[MSDC%u] WARN: mmc or card is NULL",host->id);
        return;
    }
    if(host->mmc->card){
...
#if defined(CONFIG_ENABLE_CARDDET)
        msdc_carddet_func->card_removed();
#endif /*CONFIG_ENABLE_CARDDET*/
        ...
        MSDC_TRC_PRINT(MSDC_INFO, "[MSDC%u]Do remove the bad card, block_bad_card=%d, card_inserted=%d",host->id, host->block_bad_card, host->card_inserted);
    }
}
...
//    u32 status = 0;
	...
	if(hw->flags & MSDC_UHS1){
		...
		MSDC_TRC_PRINT(MSDC_INFO,"[MSDC%u] %s: Support UHS1",host->id, __func__);
	}
#ifdef CONFIG_EMMC_50_FEATURE
...
{
	u32 clk_src;
	int mux_id[] = {MT_MUX_MSDC30_0, MT_MUX_MSDC30_1, MT_MUX_MSDC30_2, MT_MUX_MSDC30_3};
	char name[20];
	u32 hclksrc = MSDC50_CLKSRC4HCLK_182MHZ;
	...
	if(host->id == 3)
	{
		if ( host->mmc && host->mmc->card && mmc_card_hs400(host->mmc->card)) {
			...
			MSDC_TRC_PRINT(MSDC_INFO,"[info][%s] hs400 mode, change pll mux to 400Mhz\n", __func__);
		}
		else {
	...
	else
	{
		...
		sprintf(name, "MSDC%d", host->id);
		clkmux_sel(mux_id[host->id], 1, name);
	}
}
...
static void msdc_set_mclk(struct msdc_host *host, int ddr, u32 hz)
{
    ...
    if (!hz) { // set mmc system clock to 0
        MSDC_DBG_PRINT(MSDC_INFO,"msdc%d -> set mclk to 0",host->id);  // fix me: need to set to 0
    ...
    if(hz >= 25000000)
    	MSDC_DBG_PRINT(MSDC_INFO,"msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> DDR<%d> mode<%d> div<%d>" ,
		                host->id, hz/1000, hclk/1000, sclk/1000, ddr, mode, div);
	else
		MSDC_DBG_PRINT(MSDC_INFO,"msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> DDR<%d> mode<%d> div<%d>" ,
		                host->id, hz/1000, hclk/1000, sclk/1000, ddr, mode, div);

	printk("msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> DDR<%d> mode<%d> div<%d> hs400_src<%d>\n" ,
		                host->id, hz/1000, hclk/1000, sclk/1000, ddr, mode, div, hs400_src);
...
extern int mmc_emmc_power_cycle(struct mmc_host *host, u32 ocr,struct mmc_card *card);
...
static u32 msdc_power_tuning(struct msdc_host *host)
{
    ...
    card = mmc->card;
    if (card == NULL) {
        MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] mmc->card is NULL", host->id);
        return 1;
    }
...
#ifdef MSDC_EMMC_USE_RST_N
        sdr_set_field(MSDC_CFG, MSDC_CFG_CKPDN, 1); /* Clock On */
#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH

        // HGRM15BG-1741 H/W Reset
//        mt_set_gpio_out(GPIO22, GPIO_OUT_ZERO); /* emmc2 disable */
        mdelay(1);
...
//        mt_set_gpio_out(GPIO22, GPIO_OUT_ONE);  /* emmc2_en */
        mdelay(1);
#else  /*CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH*/
        sdr_set_bits(EMMC_IOCON, EMMC_IOCON_BOOTRST);
        mdelay(1);
        sdr_clr_bits(EMMC_IOCON, EMMC_IOCON_BOOTRST);
        mdelay(1);        
#endif /*CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH*/
        sdr_set_field(MSDC_CFG, MSDC_CFG_CKPDN, 0); /* Clock Off */
        
#endif /*MSDC_EMMC_USE_RST_N*/
        ...
        spin_unlock(&host->lock);
        ret = mmc_emmc_power_cycle(mmc, mmc->ocr, card);
        spin_lock(&host->lock);
	...
	if(host->power_control)
		...
		MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] No power control callback. Please check host_function<0x%lx>",host->id,host->hw->host_function);
#endif
        mdelay(10);
#ifdef FPGA_PLATFORM
	...
	if(host->power_control)
        ...
        if((host->sclk > 100000000) && (host->power_cycle >= 1)) {
			...
			MSDC_TRC_PRINT(MSDC_INFO,"[MSDC%u] %s: unsupport UHS1(SDR104) since current sclk %uHz and power_cycle%u",host->id, __func__, host->sclk,host->power_cycle);
		}
		if(((host->sclk <= 100000000) && ((host->sclk > 50000000) || host->ddr)) && (host->power_cycle >= 1)){
#ifdef CONFIG_ARCH_MT8590_ICX
			mmc->caps &= ~(MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 | MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR104 | MMC_CAP_UHS_DDR50 | MMC_CAP_1_8V_DDR);
#else
			mmc->caps &= ~(MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR104 | MMC_CAP_UHS_DDR50);
#endif/*CONFIG_ARCH_MT8590_ICX*/
			MSDC_TRC_PRINT(MSDC_INFO,"[MSDC%u] %s: unsupport UHS1 since current sclk %uHz",host->id, __func__, host->sclk);
		}
		...
		(host->power_cycle)++;
            ...
            MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] the %d time, exceed the max power cycle time %d, go to remove the bad card, power_cycle_enable=%d", host->id, host->power_cycle, MSDC_MAX_POWER_CYCLE, host->power_cycle_enable);
    ...
    while (state != 4) { // until status to "tran"
        while ((err = msdc_get_card_status(mmc, host, &status))) {
            ...
            if (err != (unsigned int)-EIO) {
                ...
                MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] update cmd para failed", host->id);
                return 1;
            }
        }
...
static void msdc_set_power_mode(struct msdc_host *host, u8 mode)
{
    ...
    if (host->power_mode == MMC_POWER_OFF && mode != MMC_POWER_OFF) {
		...
		if(host->power_control) {
			...
			MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] No power control callback. Please check host_function<0x%lx> and Power_domain<%d>",host->id,host->hw->host_function,host->power_domain);
		}
#endif
                                      ...
                                      unsigned long       timeout)
{
    ...
    if (opcode == MMC_SEND_STATUS) {
        ...
        while(sdc_is_cmd_busy() || sdc_is_busy()){
            if (time_after(jiffies, tmo)) {
                ERR_MSG("XXX cmd_busy / sdc_busy timeout: before CMD<%d> Resptype<%d>", opcode, resp);
                cmd->error = (unsigned int)-ETIMEDOUT;
                msdc_reset_hw(host->id);
                return cmd->error;  /* Fix me: error handling */
            }
        ...
        unsigned long       timeout)
{
    ...
    while (1){
        ...
        if (time_after(jiffies, tmo)) {
            MSDC_DBG_PRINT(MSDC_INFO,"XXX CMD<%d> polling_for_completion timeout ARG<0x%.8x>", cmd->opcode, cmd->arg);
            cmd->error = (unsigned int)-ETIMEDOUT;
            host->sw_timeout++;
            msdc_dump_info(host->id);
    ...
    if (intsts & cmdsts) {
...
#endif
        {
            ...
            MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x> CMDBSY<0x%08x> SDCBSY<0x%08x> RSP<0x%08x> ", 
                    host->id, cmd->opcode, cmd->arg, sdc_is_cmd_busy() , sdc_is_busy(), sdr_read32(SDC_RESP0));
        } else if (intsts & MSDC_INT_CMDTMO) {
            cmd->error = (unsigned int)-ETIMEDOUT;
            MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x> CMDBSY<0x%08x> SDCBSY<0x%08x>\n", 
                host->id, cmd->opcode, cmd->arg, sdc_is_cmd_busy() , sdc_is_busy());
        }
#else /*CONFIG_ARCH_MT8590_ICX*/
        } else if (intsts & MSDC_INT_RSPCRCERR) {
            cmd->error = (unsigned int)-EIO;
            MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>", host->id, cmd->opcode, cmd->arg);
            msdc_reset_hw(host->id);
        } else if (intsts & MSDC_INT_CMDTMO) {
            cmd->error = (unsigned int)-ETIMEDOUT;
            MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>", host->id, cmd->opcode, cmd->arg);
            msdc_reset_hw(host->id);
        }
#endif/*CONFIG_ARCH_MT8590_ICX*/
        ...
        if ((sbc != NULL) && (host->autocmd & MSDC_AUTOCMD23)) {
             if (intsts & MSDC_INT_ACMDRDY) {
                 ...
                 MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] autocmd23 crc error\n", host->id);
                 ...
                 MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] autocmd23 to error\n", host->id);
...
#ifdef CONFIG_ARCH_MT8590_ICX
    if(cmd->error != 0 && host->cmd_rsp == RESP_R1B){
		if (cmd->opcode == MMC_ERASE) 		/* Erase */
	        tmo = jiffies + CMD_ERASE_TMO;
		else								/* Other Command */
	        tmo = jiffies + CMD_BUSY_TMO;
        while(sdc_is_cmd_busy() || sdc_is_busy()){
            if (time_after(jiffies, tmo)) {
                ERR_MSG("CMD<%d> Busy Timeout",  cmd->opcode);
                goto out;
            }
        }
    }
...
#ifdef CONFIG_ARCH_MT8590_ICX
    if(cmd->error != 0){
        printk(KERN_ERR "[MSDC%u] CMD<%d> Arg<0x%.8x> error<%d>\n", host->id, cmd->opcode, cmd->arg, cmd->error);
        msdc_reset_hw(host->id);
    }
#endif /*CONFIG_ARCH_MT8590_ICX*/
    host->cmd = NULL;
    ...
    while (1) {
        ...
        if(ptr==NULL||(PageHighMem((struct page *)(sg->page_link & ~0x3))))
        {
            ...
            for(i=0;i< totalpages;i++)
            {
                ...
                if(!kaddr[i])
                    MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] msdc0:kmap failed %x", host->id,kaddr[i]);
            ...
            if(ptr==NULL)
                MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] sg_virt %x", host->id, ptr);
                        ...
                        if ((left >=  MSDC_FIFO_THD) && (msdc_rxfifocnt() >= MSDC_FIFO_THD)) {
                            ...
                            if ((ints & MSDC_INT_DATCRCERR) || (ints & MSDC_INT_DATTMO)) {
                                if (ints & MSDC_INT_DATCRCERR){
                                    MSDC_TRC_PRINT(MSDC_INFO,"[msdc%d] DAT CRC error (0x%x), Left DAT: %d bytes\n",  host->id, ints, left);
                                    data->error = (unsigned int)-EIO;
                                }
                                if (ints & MSDC_INT_DATTMO){
                                    MSDC_TRC_PRINT(MSDC_INFO,"[msdc%d] DAT TMO error (0x%x), Left DAT: %d bytes\n",  host->id, ints, left);
                                    data->error = (unsigned int)-ETIMEDOUT;
                                }
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
                                if ((!sdio_enable_tune) && ((host->hw->host_function != MSDC_SDIO)
...
int msdc_pio_write(struct msdc_host* host, struct mmc_data *data)
{
    ...
    while (1) {
        ...
        if(ptr==NULL||(PageHighMem((struct page *)(sg->page_link & ~0x3))))
        {
            ...
            for(i=0;i< totalpages;i++)
            {
                ...
                if(!kaddr[i])
                    MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] kmap failed %x\n", host->id,kaddr[i]);
            }

            ptr = sg_virt(sg);

            if(ptr==NULL)
                MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] :write sg_virt %x\n",host->id, ptr);
    ...
    if(data->error) //MSDC_ERR_PRINT(MSDC_ERROR,"write pio data->error<%d>", data->error);
        MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] write pio data->error<%d> left<%d> size<%d>", host->id,data->error, left, size);
...
static void msdc_dma_stop(struct msdc_host *host)
{
    ...
    if(retry == 0){
		MSDC_ERR_PRINT(MSDC_ERROR,"[MSDC%u] !!ASSERT!!", host->id);
    }
    mb();
    if (host->hw->flags & MSDC_SDIO_IRQ)
        spin_lock_irqsave(&SDIO_IRQ_lock, flags_lock);
...
#ifdef CONFIG_EMMC_50_FEATURE
        sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_AXI_WRAP_CKEN, 1);
#endif
...
int hs400_restore_pad_tune(int restore)
{
    ...
    if(mtk_msdc_host[3]){
        ...
        mtk_msdc_host[3]->saved_para.pad_tune = sdr_read32(MSDC_PAD_TUNE);
    }
    return 0;
}
int hs400_restore_pb1(int restore)
{
    ...
    if(mtk_msdc_host[3]){
        ...
        sdr_get_field((MSDC_PATCH_BIT1), MSDC_PB1_WRDAT_CRCS_TA_CNTR, mtk_msdc_host[3]->saved_para.wrdat_crc_ta_cntr);
    }
    return 0;
}
int hs400_restore_ddly0(int restore)
{
    ...
    if(mtk_msdc_host[3]){
        base = mtk_msdc_host[3]->base;
        mtk_msdc_host[3]->saved_para.ddly0 = sdr_read32(MSDC_DAT_RDDLY0);
    }
    return 0;
}
int hs400_restore_ddly1(int restore)
{
    ...
    if(mtk_msdc_host[3]){
        base = mtk_msdc_host[3]->base;
        mtk_msdc_host[3]->saved_para.ddly1 = sdr_read32(MSDC_DAT_RDDLY1);
    }
    return 0;
}
...
static struct msdc_reg_control hs400_backup_reg_list[HS400_BACKUP_REG_NUM] = {
    ...
    {(MSDC_3_BASE + OFFSET_MSDC_PATCH_BIT0),        (MSDC_PB0_INT_DAT_LATCH_CK_SEL), 0x0, 0x0, NULL},
    {(MSDC_3_BASE + OFFSET_MSDC_PATCH_BIT1),        (MSDC_PB1_WRDAT_CRCS_TA_CNTR), 0x0, 0x0, hs400_restore_pb1}, // the defalut init value is 0x1, but HS400 need the value be 0x0
    {(MSDC_3_BASE + OFFSET_MSDC_IOCON),             (MSDC_IOCON_R_D_SMPL | MSDC_IOCON_DDLSEL | MSDC_IOCON_R_D_SMPL_SEL | MSDC_IOCON_R_D0SPL | MSDC_IOCON_W_D_SMPL_SEL | MSDC_IOCON_W_D_SMPL), 0x0, 0x0, NULL},
    {(MSDC_3_BASE + OFFSET_MSDC_PAD_TUNE),          (MSDC_PAD_TUNE_DATWRDLY | MSDC_PAD_TUNE_DATRRDLY), 0x0, 0x0, hs400_restore_pad_tune},
    {(MSDC_3_BASE + OFFSET_MSDC_DAT_RDDLY0),        (MSDC_DAT_RDDLY0_D3 | MSDC_DAT_RDDLY0_D2 | MSDC_DAT_RDDLY0_D1 | MSDC_DAT_RDDLY0_D0), 0x0, 0x0, hs400_restore_ddly0},
    {(MSDC_3_BASE + OFFSET_MSDC_DAT_RDDLY1),        (MSDC_DAT_RDDLY1_D7 | MSDC_DAT_RDDLY1_D6 | MSDC_DAT_RDDLY1_D5 | MSDC_DAT_RDDLY1_D4), 0x0, 0x0, hs400_restore_ddly1},
    {(MSDC_3_BASE + OFFSET_EMMC50_PAD_CMD_TUNE),    (MSDC_EMMC50_PAD_CMD_TUNE_TXDLY), 0x0, 0x00000200, hs400_restore_cmd_tune},
    {(MSDC_3_BASE + OFFSET_EMMC50_PAD_DS_TUNE),     (MSDC_EMMC50_PAD_DS_TUNE_DLY1 | MSDC_EMMC50_PAD_DS_TUNE_DLY3), 0x0, 0x0, hs400_restore_ds_tune},
    {(MSDC_3_BASE + OFFSET_EMMC50_PAD_DAT01_TUNE),  (MSDC_EMMC50_PAD_DAT0_TXDLY | MSDC_EMMC50_PAD_DAT1_TXDLY), 0x0, 0x01000100, hs400_restore_dat01_tune},
    {(MSDC_3_BASE + OFFSET_EMMC50_PAD_DAT23_TUNE),  (MSDC_EMMC50_PAD_DAT2_TXDLY | MSDC_EMMC50_PAD_DAT3_TXDLY), 0x0, 0x01000100, hs400_restore_dat23_tune},
    {(MSDC_3_BASE + OFFSET_EMMC50_PAD_DAT45_TUNE),  (MSDC_EMMC50_PAD_DAT4_TXDLY | MSDC_EMMC50_PAD_DAT5_TXDLY), 0x0, 0x01000100, hs400_restore_dat45_tune},
    {(MSDC_3_BASE + OFFSET_EMMC50_PAD_DAT67_TUNE),  (MSDC_EMMC50_PAD_DAT6_TXDLY | MSDC_EMMC50_PAD_DAT7_TXDLY), 0x0, 0x01000100, hs400_restore_dat67_tune},
};
...
int emmc_hs400_tune_rw(struct msdc_host *host)
{
    ...
    if((host->id != 3) || (host->ddr != MSDC_STATE_HS400)){
        return err;
    }
                                  ...
                                  struct mmc_command *sbc)
{
    ...
    if (sbc) {
        MSDC_DBG_PRINT(MSDC_DEBUG,"XXX SBC<%d><0x%x> Error<%d> Resp<0x%x>", sbc->opcode, sbc->arg, sbc->error, sbc->resp[0]);
    }
        ...
        ((cmd->error == -EIO) || (data->error == -EIO)))
    {
        ...
        //MSDC_ERR_PRINT(MSDC_ERROR,"XXX Vcore<0x%x> CPU_Temperature<%d>", vcore_uv, cur_temperature);
        MSDC_ERR_PRINT(MSDC_ERROR,"XXX CPU_Temperature<%d>", cur_temperature);
#endif // MTK_SDIO30_DETECT_THERMAL
    }
#endif
...
static u32 msdc_status_verify_case1(struct msdc_host *host, struct mmc_command *cmd)
{
    ...
    while (state != 4) {    /* until status to "tran" */
        ...
        if (time_after(jiffies, tmo)) {
            ...
            if ((host->hw->host_function == MSDC_SD)
                && (host->sclk >= 100000000 || (host->ddr == MSDC_STATE_DDR)))
                host->sd_30_busy++;
            return msdc_power_tuning(host);
        }
    }
...
#ifdef CONFIG_ARCH_MT8590_ICX
        /* CMD7 cannot re-send the same command */
        if (cmd->opcode == MMC_SELECT_CARD && cmd->error != 0) {
                    goto out;
        }
#endif/*CONFIG_ARCH_MT8590_ICX*/
...
out:
    ...
    msdc_gate_clock(host, 1);   /* clear flag. */
    //msdc_gate_clock(host, 0);   /* clear flag. */
    spin_unlock(&host->lock);
sdio_error_out:
    mmc_request_done(mmc, mrq);
...
static void msdc_tune_async_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
    ...
    if(host->mrq){
        ...
        WARN_ON(host->mrq);
#endif
        MSDC_ERR_PRINT(MSDC_ERROR,"XXX host->mrq<0x%.8x> cmd<%d>arg<0x%x>", (int)host->mrq,host->mrq->cmd->opcode,host->mrq->cmd->arg);
        if(host->mrq->data){
            MSDC_ERR_PRINT(MSDC_ERROR,"XXX request data size<%d>",host->mrq->data->blocks * host->mrq->data->blksz);
            MSDC_ERR_PRINT(MSDC_ERROR,"XXX request attach to host force data timeout and retry");
            host->mrq->data->error = (unsigned int)-ETIMEDOUT;
		}
		else{
            MSDC_ERR_PRINT(MSDC_ERROR,"XXX request attach to host force cmd timeout and retry");
            host->mrq->cmd->error = (unsigned int)-ETIMEDOUT;
        }
        MSDC_ERR_PRINT(MSDC_ERROR,"XXX current request <0x%.8x> cmd<%d>arg<0x%x>",(int)mrq,mrq->cmd->opcode,mrq->cmd->arg);
        if(mrq->data)
            MSDC_ERR_PRINT(MSDC_ERROR,"XXX current request data size<%d>",mrq->data->blocks * mrq->data->blksz);
    }
    ...
    */
    ...
    do{
            ...
            || (host->t_counter.time_write % 16 == 15))
        {
            spin_unlock(&host->lock);
            msleep(150);
            MSDC_ERR_PRINT(MSDC_ERROR,"sleep 150ms here!");   //sleep in tuning flow, to avoid printk watchdong timeout
            spin_lock(&host->lock);
            goto out;
        }*/
        ...
        {
            if (msdc_tune_cmdrsp(host)){
                MSDC_ERR_PRINT(MSDC_ERROR,"failed to updata cmd para");
                goto out;
            }
        }
        ...
        if (data && (data->error == (unsigned int)-EIO)) {
            ...
            if (data->flags & MMC_DATA_READ) {  // read
                if (msdc_tune_read(host)) {
                    MSDC_ERR_PRINT(MSDC_ERROR,"failed to updata read para");
                    goto out;
                }
            } else {
                if (msdc_tune_write(host)) {
                    MSDC_ERR_PRINT(MSDC_ERROR,"failed to updata write para");
                    goto out;
                }
            }
#endif
        ...
        if (data) {
            if (msdc_abort_data(host)) {
                MSDC_ERR_PRINT(MSDC_ERROR,"abort failed");
                data_abort = 1;
                if(host->hw->host_function == MSDC_SD){
        ...
        if (cmd->error == (unsigned int)-ETIMEDOUT) {
            if(cmd->opcode == MMC_READ_SINGLE_BLOCK || cmd->opcode == MMC_READ_MULTIPLE_BLOCK || cmd->opcode == MMC_WRITE_BLOCK || cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK ){
                if((host->sw_timeout) || (++(host->rwcmd_time_tune) > MSDC_MAX_TIMEOUT_RETRY)){
                    MSDC_ERR_PRINT(MSDC_ERROR,"msdc%d exceed max r/w cmd timeout tune times(%d) or SW timeout(%d),Power cycle\n",host->id,host->rwcmd_time_tune,host->sw_timeout);
                    if(msdc_power_tuning(host))
                        goto out;
                }
            }else {
...
static void msdc_set_buswidth(struct msdc_host *host, u32 width)
{
    ...
    MSDC_TRC_PRINT(MSDC_INFO,"Bus Width = %d-bit", \
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    static char *timing[] = {
        "LEGACY", "MMC_HS", "SD_HS","UHS_SDR12","UHS_SDR25",
		"UHS_SDR50","UHS_SDR104","UHS_DDR50","MMC_HS200","MMC_HS400","UNKNOWN"
    };

    MSDC_DBG_PRINT(MSDC_INFO,"[MSDC%u] SET_IOS: CLK(%dkHz), BUS(%s), BW(%u), PWR(%s), VDD(%s), TIMING(%s)", host->id,
        ios->clock / 1000, bus_mode[ios->bus_mode],
        (ios->bus_width == MMC_BUS_WIDTH_8) ?8 : (ios->bus_width == MMC_BUS_WIDTH_4) ? 4 : 1,
        power_mode[ios->power_mode], vdd[ios->vdd], timing[ios->timing > 9 ? 10: ios->timing]);
	...
	if (host->id == 3) {
        ...
        if(ios->timing == MMC_TIMING_UHS_DDR50){
            ...
            ddr = MSDC_STATE_HS200;
        }else {
            ddr = MSDC_STATE_DEFAULT;
        }
	if (host->id == 3) {
	...
	if(host->id == 1)	{
		if (ios->timing == MMC_TIMING_UHS_SDR50)
			msdc_set_driving(host,hw,2);
		else if (ios->timing == MMC_TIMING_UHS_DDR50)
			msdc_set_driving(host,hw,3);
    ...
    if (host->mclk != ios->clock || host->ddr != ddr) { /* not change when clock Freq. not changed ddr need set clock*/
        if(ios->clock >= 25000000) {
           ...
           //     hw->clk_drv_sd_18 -= 1;
            ...
            if ((host->hw->host_function == MSDC_EMMC) || (host->hw->host_function == MSDC_SD)){
                sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_GET_BUSY_MARGIN, host->saved_para.write_timeout_eco1);
                sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_GET_BUSY_MARGIN, host->saved_para.write_timeout_eco2);
            }
        ...
        if (ios->clock == 0) {
            ...
            if(ios->power_mode == MMC_POWER_OFF && !is_card_sdio(host)){
                ...
                sdr_get_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_CMD_RSP,    host->saved_para.cmd_resp_ta_cntr);
                sdr_get_field(MSDC_PATCH_BIT1, MSDC_PATCH_BIT1_WRDAT_CRCS, host->saved_para.wrdat_crc_ta_cntr);
                sdr_get_field(MSDC_PATCH_BIT1, MSDC_PB1_GET_BUSY_MARGIN, host->saved_para.write_timeout_eco1);
                sdr_get_field(MSDC_PATCH_BIT1, MSDC_PB1_GET_CRC_MARGIN, host->saved_para.write_timeout_eco2);
#ifdef CONFIG_EMMC_50_FEATURE
               if(host->id == 3 ){
                   sdr_get_field(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY1, host->saved_para.ds_dly1);
                   sdr_get_field(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY3, host->saved_para.ds_dly3);
...
#ifdef CONFIG_EMMC_50_FEATURE
        if((host->id == 3) && (host->ddr != state)) {
            if (ddr == MSDC_STATE_HS400){//switch from eMMC 4.5 backward speed mode to HS400
                emmc_hs400_backup();
                msdc_apply_ett_settings(host, MSDC_HS400_MODE);
            }
            if (host->ddr == MSDC_STATE_HS400){//switch from HS400 to eMMC 4.5 backward speed mode
                emmc_hs400_restore();
            }
            if(ddr == MSDC_STATE_HS200){
...
static void msdc2_eint_handler(void)
{
    ...
    if (0 == host->sd_cd_polarity){
        mt_eint_set_polarity(CUST_EINT_MSDC2_INS_NUM, MT_POLARITY_LOW);
    } else {
        mt_eint_set_polarity(CUST_EINT_MSDC2_INS_NUM, MT_POLARITY_HIGH);
    }
    ...
    if(got_bad_card == 0)
        tasklet_hi_schedule(&host->card_tasklet);
	MSDC_DBG_PRINT(MSDC_DEBUG,"SD2 card %s(%x:%x)",(host->hw->cd_level ^ host->sd_cd_polarity) ? "insert":"remove", host->hw->cd_level, host->sd_cd_polarity);
}
#endif
#endif
...
#endif
    {
        ...
        if (intsts & datsts) {
            ...
            if (intsts & MSDC_INT_DATTMO){
               	data->error = (unsigned int)-ETIMEDOUT;
#ifdef CONFIG_ARCH_MT8590_ICX
                printk(KERN_WARNING "XXX CMD<%d> Arg<0x%.8x> MSDC_INT_DATTMO  \n", host->mrq->cmd->opcode, host->mrq->cmd->arg);
#else  /*CONFIG_ARCH_MT8590_ICX*/
                ...
                data->error = (unsigned int)-EIO;
#ifdef CONFIG_ARCH_MT8590_ICX
                printk(KERN_WARNING "XXX CMD<%d> Arg<0x%.8x> MSDC_INT_DATCRCERR, SDC_DCRC_STS<0x%x> \n",
...
//#endif
	...
	if(enable){
		...
		if(host->id == 1 && (MSDC_SD == hw->host_function) && (hw->flags & MSDC_CD_PIN_EN)){
...
#else /*CONFIG_ENABLE_CARDDET*/
                mt_eint_registration(CUST_EINT_MSDC1_INS_NUM, CUST_EINTF_TRIGGER_LOW, msdc1_carddet_ops->notifier_call, 1);
...
#else /*CONFIG_ENABLE_CARDDET*/
                mt_eint_registration(CUST_EINT_MSDC1_INS_NUM, CUST_EINTF_TRIGGER_HIGH, msdc1_carddet_ops->notifier_call, 1);
                mt_eint_unmask(CUST_EINT_MSDC1_INS_NUM);
#endif /*CONFIG_ENABLE_CARDDET*/
            }
			...
			MSDC_DBG_PRINT(MSDC_DEBUG,"SD1 card detection eint resigter.");
		#endif
		}
		if(host->id == 2 && (MSDC_SD == hw->host_function) && (hw->flags & MSDC_CD_PIN_EN)){
		    // sd2 need handle eint11
		    #ifdef CUST_EINT_MSDC2_INS_NUM
			if(hw->cd_level)
				host->sd_cd_polarity = (~EINT_MSDC2_INS_POLARITY)&0x1;
			else
				host->sd_cd_polarity = EINT_MSDC2_INS_POLARITY;
			mt_eint_set_hw_debounce(CUST_EINT_MSDC2_INS_NUM, CUST_EINT_MSDC2_INS_DEBOUNCE_CN);
			if (0 == host->sd_cd_polarity){
				mt_eint_registration(CUST_EINT_MSDC2_INS_NUM, CUST_EINTF_TRIGGER_LOW, msdc2_eint_handler, 1);
			} else {
				mt_eint_registration(CUST_EINT_MSDC2_INS_NUM, CUST_EINTF_TRIGGER_HIGH, msdc2_eint_handler, 1);
			}
			mt_eint_unmask(CUST_EINT_MSDC2_INS_NUM);
			MSDC_DBG_PRINT(MSDC_DEBUG,"SD2 card detection eint resigter.");
			#endif
		}
	}
#ifdef CONFIG_ARCH_MT8590_ICX
        ...
        if(host->id == 1){
            /* disable card detection for msdc1*/
            mt_eint_mask(CUST_EINT_MSDC1_INS_NUM);
            mt_eint_registration(CUST_EINT_MSDC1_INS_NUM, CUST_EINTF_TRIGGER_LOW, NULL, 0);
        }
        if(host->id == 2){
            /* disable card detection for msdc2*/
            #ifdef CUST_EINT_MSDC2_INS_NUM
            mt_eint_mask(CUST_EINT_MSDC2_INS_NUM);
            mt_eint_registration(CUST_EINT_MSDC2_INS_NUM, CUST_EINTF_TRIGGER_LOW, NULL, 0);
            #endif
        }
    }
#else /*CONFIG_ARCH_MT8590_ICX*/
...
*    <0: error code
*************************************************************************/
extern unsigned int mt8590_vcore_tbl[];
...
int sdio_start_ot_transfer(void)
{
    ...
    for(hostidx = 0; hostidx < HOST_MAX_NUM; hostidx++)
    {
        ...
        if(is_card_sdio(mtk_msdc_host[hostidx]))
        {
            ...
            vcore_uv = mt8590_vcore_tbl[vcore_uv_off];
...
static int msdc_drv_probe(struct platform_device *pdev)
{
    ...
    struct msdc_host *host;
    struct msdc_hw *hw;
    u32  base,fpgabase,efusebase,gpiobase,ckgenbase;
    int ret, irq;
	...
	if (pdev->id == 0 || pdev->id == 3) {	
	    ...
	    if (memefuse != NULL) {
			...
			if (pdev->id == 0) {
			    ...
			    MSDC_TRC_PRINT(MSDC_INIT, "MSDC0 efuse(0x%x) current %s \n",efusebase, efuse ? "Disable" : "Enable");
			    if (efuse != 0 ) {
					return -ENODEV;
			    }
			}
				...
				MSDC_TRC_PRINT(MSDC_INIT, "MSDC3 efuse(0x%x) current %s \n",efusebase, efuse ? "Disable" : "Enable");
				if (efuse != 0 ) {
					return -ENODEV;
				}
			}
    ...
    if (first_probe == 0) {
        ...
        switch(pdev->id){
		    ...
		    case 3:
#ifdef CFG_DEV_MSDC3
				enable_clock(MT_CG_PERI_MSDC30_4, "SD");
				enable_clock(MT_CG_PERI_MSDC30_3, "SD");
				disable_clock(MT_CG_PERI_MSDC30_3, "SD");
#endif
	        break;
        	}
#endif
...
/*
    ...
    l_irq_data.irq = irq;
	MSDC_TRC_PRINT(MSDC_INIT, "MSDC%u io:0x%x, irq: %u \n",pdev->id,base, irq);
    if (mem == NULL) {
        mmc_free_host(mmc);
        return -EBUSY;
    }
	...
	if (pdev->id == 0 || pdev->id == 3) {
		...
		MSDC_TRC_PRINT(MSDC_INIT, "MSDC%u fpga_gpio:0x%x \n",pdev->id,fpgabase);
		...
		MSDC_TRC_PRINT(MSDC_INIT, "MSDC%u gpio:0x%x \n",pdev->id,gpiobase);
		...
		MSDC_TRC_PRINT(MSDC_INIT, "MSDC%u ckgenio:0x%x \n",pdev->id,ckgenbase);
	}
	else {
	    memfpga  = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	    if (memfpga == NULL) {
	...
	if(hw->flags & MSDC_SDR50){
		mmc->caps |= MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |
			MMC_CAP_UHS_SDR50;    // | MMC_CAP_SET_XPC_180;
			mmc->caps2 |= MMC_CAP2_HS200_1_8V_SDR;
			MSDC_TRC_PRINT(MSDC_INFO,"[MSDC%u] %s %d: Support SDR50",pdev->id, __func__, __LINE__);
	}
	...
	if(hw->flags & MSDC_UHS1){
		mmc->caps |= MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |
				MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR104;    // | MMC_CAP_SET_XPC_180;
		mmc->caps2 |= MMC_CAP2_HS200_1_8V_SDR;
		MSDC_TRC_PRINT(MSDC_INFO,"[MSDC%u] %s: Support UHS1",pdev->id, __func__);
	}
#ifdef CONFIG_EMMC_50_FEATURE
    if(hw->flags & MSDC_HS400){
	...
	if(host->id == 3)
		host->hclk  = hclks_msdc50[hw->clk_src];  /* emmc 5.0 hclk: clock of clock source to msdc controller */
	else
		host->hclk  = hclks[hw->clk_src];  /* emmc 4.5 hclk: clock of clock source to msdc controller */
    ...
    if(pdev->id == 1){
#ifdef CFG_DEV_MSDC1
		/*msdc1 card power enable*/
		mt_set_gpio_mode(hw->pow_en, GPIO_MODE_GPIO);
		mt_set_gpio_dir(hw->pow_en, GPIO_DIR_OUT);
		mt_set_gpio_out(hw->pow_en, GPIO_OUT_ZERO);
		mmc->caps2 |= MMC_CAP2_NO_PRESCAN_POWERUP;
		
		/*msdc1 Volt Select*/
		mt_set_gpio_mode(hw->pow_pin, GPIO_MODE_GPIO);
		mt_set_gpio_dir(hw->pow_pin, GPIO_DIR_OUT);
		mt_set_gpio_out(hw->pow_pin, GPIO_OUT_ZERO);
...
#ifdef CONFIG_ARCH_MT8590_ICX
    msdc_reorganize_drv_strength(host);
#endif /*CONFIG_ARCH_MT8590_ICX*/
...
#if defined(MTK_DUALCARD_SUPPORT) && defined(CONFIG_ENABLE_CARDDET)
    if(host->id == 2){
        /* Config card detection pin and enable interrupts */
        if (hw->flags & MSDC_CD_PIN_EN) {  /* set for card */
            msdc_enable_cd_irq(host, 1);
...
#endif /*MTK_DUALCARD_SUPPORT && CONFIG_ENABLE_CARDDET*/
...
release:
    ...
    if(pdev->id == 1){
#ifdef CFG_DEV_MSDC1
    	msdc_ldo_power(0, MT6323_POWER_LDO_VMC, VOL_3300, &g_msdc1_io);
#endif
	}
#endif
...
static void msdc_drv_shutdown(struct platform_device *pdev)
{
    ...
    MSDC_TRC_PRINT(MSDC_INFO,"shutdown !!!");

    if (pdev->id != 3){
        msdc_drv_remove(pdev);
    }
#endif
}
...
#ifdef CONFIG_KEEP_EXT_PMIC_EN_ON_AT_WARM_SLEEP
    upmu_set_strup_ext_pmic_en(1);
...
/*set gpio for msdc2*/
#if defined(MTK_DUALCARD_SUPPORT) && defined(CONFIG_ARCH_MT8590_ICX)

    mt_set_gpio_mode(GPIO85, GPIO_MODE_01);
    mt_set_gpio_mode(GPIO86, GPIO_MODE_01);
    mt_set_gpio_mode(GPIO87, GPIO_MODE_01);
    mt_set_gpio_mode(GPIO88, GPIO_MODE_01);
    mt_set_gpio_mode(GPIO89, GPIO_MODE_01);
    mt_set_gpio_mode(GPIO90, GPIO_MODE_01);

    mt_set_gpio_dir(GPIO85, GPIO_DIR_IN);
    mt_set_gpio_dir(GPIO86, GPIO_DIR_IN);
    mt_set_gpio_dir(GPIO87, GPIO_DIR_IN);
    mt_set_gpio_dir(GPIO88, GPIO_DIR_IN);
    mt_set_gpio_dir(GPIO89, GPIO_DIR_IN);
    mt_set_gpio_dir(GPIO90, GPIO_DIR_IN);

    mt_set_gpio_pull_enable(GPIO85, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_enable(GPIO86, GPIO_PULL_DISABLE);
    mt_set_gpio_pull_enable(GPIO87, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_enable(GPIO88, GPIO_PULL_ENABLE);
...
#if defined(CONFIG_ENABLE_CARDDET)
int msdc_eint_register(void *ops, bool enable)
{
#ifdef CFG_DEV_MSDC1
    struct msdc_host *host = mtk_msdc_host[1];
    ...
    msdc1_carddet_ops = (struct carddet_eint_ops *)ops;

    if (enable) {
        /*enable card detection*/
        if (host->hw->flags & MSDC_CD_PIN_EN) {  /* set for card */
            msdc_enable_cd_irq(host, 1);
        } else {
            msdc_enable_cd_irq(host, 0);
        }
...
EXPORT_SYMBOL(msdc_eint_register);

void msdc_set_carddet_func(void *func)
{

  MSDC_ASSERT(func);

  msdc_carddet_func = (struct carddet_func_tbl *)func;

}
EXPORT_SYMBOL(msdc_set_carddet_func);
#endif /*CONFIG_ENABLE_CARDDET*/
...
static void msdc_reorganize_drv_strength(struct msdc_host *host)
{
    ...
    if(host->id != 1 && host->id != 3){
        //Do nothing!! 
        return;
    }
    
    //Only for SD / EMMC.
    ret = IMM_GetOneChannelValue(ICX_AD_BID3, data, &ad_val);
    if(!ret && ad_val < ICX_AD_VAL_TH){
        //SET (HM or FS)
        ret = IMM_GetOneChannelValue(ICX_AD_BID2, data, &ad_val);
        if(!ret && ad_val < (ICX_AD_MAX/2)){
            //HM Drive Strength
            host->hw->clk_drv_sd_18 = host->hw->clk_drv_sd_18_hm;
            host->hw->cmd_drv_sd_18 = host->hw->cmd_drv_sd_18_hm;
            host->hw->dat_drv_sd_18 = host->hw->dat_drv_sd_18_hm;
        }else{
            //FS Drive Strength
            host->hw->clk_drv_sd_18 = host->hw->clk_drv_sd_18_fs;
            host->hw->cmd_drv_sd_18 = host->hw->cmd_drv_sd_18_fs;
            host->hw->dat_drv_sd_18 = host->hw->dat_drv_sd_18_fs;
        }
    }
