...
//    preempt_disable();
	...
	sdr_set_field(MSDC_PAD_TUNE,MSDC_PAD_TUNE_CMDRDLYSEL, 1);
	
	sdr_set_field(MSDC_PAD_TUNE1,MSDC_PAD_TUNE1_DATRDLY2SEL, 1);
	sdr_set_field(MSDC_PAD_TUNE1,MSDC_PAD_TUNE1_CMDRDLY2SEL, 1); 
...
//    BOOTMODE btmod;
	...
	host = mmc_priv(mmc);
	MSDC_TRC_PRINT(MSDC_SDIODRV, "[mtk sdio][msdc%u] + wait_sdio_autok_ready(0x%p)\n", id, data);
    base = host->base;
...
EXIT_WAIT_AUTOK_READY:        
	...
	MSDC_TRC_PRINT(MSDC_SDIODRV, "[mtk sdio] - wait_sdio_autok_ready(0x%p)\n", data);
...
static int __init autok_init(void)
{
	...
	MSDC_TRC_PRINT(MSDC_INIT,"+ %s init",__func__);
	...
	do {
		...
		MSDC_ASSERT(p_autok_thread_data);
	    p_autok_thread_data->log = autok_log_info;
	...
	MSDC_TRC_PRINT(MSDC_INIT,"- %s init return %u",__func__,retval);
