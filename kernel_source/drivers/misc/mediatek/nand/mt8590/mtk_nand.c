...
bool get_device_info(u8*id, flashdev_info *devinfo)
{
    ...
    for (i = 0; i<flash_number; i++){
		MSG(INIT, "gen_FlashTable_p[target].devciename: %s \n",gen_FlashTable_p[i].devciename);
		mismatch=0;
...
static bool mtk_nand_ready_for_write(struct nand_chip *nand, u32 u4RowAddr, u32 col_addr, bool full, u8 * buf)
{
    ...
    if (full)
    {
        ...
        if (!phys)
        {
            printk(KERN_ERR "[mt65xx_nand_ready_for_write]convert virt addr (%x) to phys add fail!!!\n", (u32) buf);
            return false;
        } else
        {
            DRV_WriteReg32(NFI_STRADDR_REG32, phys);
...
static int mtk_nand_probe(struct platform_device *pdev)
{
    ...
    ret=pmic_read_interface(0x508, &temp_val, 0xFFFF, 0x0);
	printk("[Power/PMIC in nand probe 0x508 = 0x%x]\n",temp_val);
  ...
  hwPowerOn(MT65XX_POWER_LDO_VEMC_3V3, VOL_3300, "NFI");
 #else
  hwPowerOn(MT6323_POWER_LDO_VEMC_3V3, VOL_3300, "NFI");
 #endif
    mtk_nand_gpio_init();
...
//          host->saved_para.sECC_FDMADDR_REG32 = DRV_Reg32(ECC_FDMADDR_REG32);
		  ...
		  #ifdef MTK_PMIC_MT6397
		  hwPowerDown(MT65XX_POWER_LDO_VEMC_3V3, "NFI");
		  #else
		  hwPowerDown(MT6323_POWER_LDO_VEMC_3V3, "NFI");
		  #endif
          nand_disable_clock();
