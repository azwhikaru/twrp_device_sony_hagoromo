...
static int pmt_region_read(struct pmt_region *region, void *buf)
{
	...
	err = eMMC_rw_x(region->base_addr, (unsigned int *)buf, 3, 0, region->size, 1, EMMC_PART_USER);
#else
...
#ifdef CONFIG_MTK_EMMC5
	err = eMMC_rw_x(region->base_addr, (unsigned int *)buf, 3, 1, region->size, 1, EMMC_PART_USER);
#else
...
#ifdef CONFIG_MTK_EMMC5
	err = eMMC_rw_x(this_addr, (u32*)buf, 3, 0, 512, 1, EMMC_PART_USER);
#else
...
#ifdef CONFIG_MTK_EMMC5
	err = eMMC_rw_x(this_addr, (u32*)buf, 3, 1, 512, 1, EMMC_PART_USER);
#else
