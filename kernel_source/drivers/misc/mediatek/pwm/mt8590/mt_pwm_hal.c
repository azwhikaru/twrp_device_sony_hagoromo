...
uint32_t mt_get_pwm_all_enable_hal(void)
{	uint32_t	reg;

	reg = INREG32(PWM_ENABLE);
	return reg;
}

void mt_set_pwm_all_enable_hal(uint32_t en)
{	OUTREG32(PWM_ENABLE, en);
}
...
uint16_t mt_get_pwm_HiDur_hal(U32 pwm_no)
{
	unsigned long	addr_HiDur;
	uint32_t	reg;

	addr_HiDur = PWM_register[pwm_no]+4*PWM_HDURATION;
	reg = INREG32(addr_HiDur);
	return (uint16_t)reg;
}
uint16_t mt_get_pwm_LowDur_hal (U32 pwm_no)
{
	unsigned	addr_LowDur;
	uint32_t	reg;

	addr_LowDur = PWM_register[pwm_no] + 4*PWM_LDURATION;
	reg = INREG32 (addr_LowDur);
	return (uint16_t)reg;
}
