...
static int rtc_ops_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	rtc_tm_to_time(tm, &time);
#if RTC_OVER_TIME_RESET
	if(tm->tm_year < (RTC_DEFAULT_YEA - 1900) || (RTC_MAX_YEAR - 1900)  <= tm->tm_year){
...
static int rtc_set_alarm_irq_enable(struct device *dev, unsigned int enabled)
{
	unsigned long flags;

	spin_lock_irqsave(&rtc_lock, flags);
	if (enabled)
		hal_rtc_set_alarm_irq(true);
	else
		hal_rtc_set_alarm_irq(false);
	spin_unlock_irqrestore(&rtc_lock, flags);
...
static struct rtc_class_ops rtc_ops = {
	...
	.alarm_irq_enable = rtc_set_alarm_irq_enable,
};
