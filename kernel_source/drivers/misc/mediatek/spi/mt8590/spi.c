...
static void enable_clk(void)
{
	...
	enable_clock(MT_CG_PERI_SPI2, "spi");
#endif
	return;
}
...
static void disable_clk(void)
{
	...
	disable_clock(MT_CG_PERI_SPI2, "spi");
#endif
	return;
}
...
mt_spi_next_xfer(struct mt_spi_t *ms, struct spi_message *msg)
{
	...
	if (ms->running == PAUSED) {
		...
		SPI_DBG("spi_start_transfer.\n");		
		//spi_dump_reg(ms); /*Dump register before transfer*/
#if 0
		vaule = spi_readl(ms, SPI_STATUS0_REG);
		while (vaule != 0) {
			msleep(1);
			vaule = spi_readl(ms, SPI_STATUS0_REG);
			printk("SPI_STATUS0_REG(%x),SPI_STATUS1_REG(%x)\n", vaule,spi_readl(ms, SPI_STATUS0_REG));
		}
#endif
	} else {
...
/*		if (!chip_config){
		...
		chip_config->setuptime = 50;
		chip_config->holdtime = 50;
		chip_config->high_time = 25;
		chip_config->low_time = 25;
		chip_config->cs_idletime = 50;
		chip_config->ulthgh_thrsh = 50;
