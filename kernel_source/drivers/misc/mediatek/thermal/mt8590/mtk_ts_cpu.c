...
#define INVALID_HOT_TEMP (150000)
#define INVALID_COLD_TEMP (-50000)
...
static unsigned int trip_temp[10] = {117000,95000,85000,76000,70000,68000,65000,60000,55000,50000};
...
static void thermal_reset_and_initial(void)
{
	...
	*/
	...
	printk(KERN_WARNING"[mtk_ts_cpu] temperature sampling control, 2 out of 4 samples");
	...
	printk(KERN_WARNING"[mtk_ts_cpu] temperature sampling control, 4 out of 6 samples");
    THERMAL_WRAP_WR32(0x03FF03FF, TEMPMONCTL2);     // both filt and sen interval is 15.86ms
	...
	printk(KERN_WARNING"[mtk_ts_cpu] temperature sampling control, 8 out of 10 samples");
	...
	printk(KERN_WARNING"[mtk_ts_cpu] temperature sampling control, 16 out of 18 samples");
...
#else // default 1
	printk(KERN_WARNING"[mtk_ts_cpu] temperature sampling control, 1 out of 1 samples");
...
static void thermal_cal_prepare(void)
{
	...
	temp0 = get_devinfo_with_index(41);
	temp1 = get_devinfo_with_index(42);
	temp2 = get_devinfo_with_index(43);

	printk(KERN_CRIT  "[Power/CPU_Thermal] [Thermal calibration] temp0=0x%x, temp1=0x%x, , temp2=0x%x\n", temp0, temp1, temp2);
	//mtktscpu_dprintk("thermal_cal_prepare\n");

    g_adc_ge_t     = ((temp0 & 0x003FF000)>>12);//ADC_GE_T [9:0] *(0x10206424)[21:12]
	g_adc_oe_t     = ((temp0 & 0xFFC00000)>>22);//ADC_OE_T [9:0] *(0x10206424)[31:22]


	g_o_vtsmcu1    = (temp1 & 0x000001FF);  //O_VTSMCU1    (9b) *(0x10206428)[8:0]
	g_o_vtsmcu2    = (temp1 & 0x0003FE00)>>9;   //O_VTSMCU2    (9b) *(0x10206428)[17:9]

 	g_o_vtsabb     = (temp2 & 0x0003FE00)>>9;		//O_VTSABB     (9b) *(0x1020642c)[17:9]

	g_degc_cali    = (temp2 & 0x00FC0000)>>18;   //DEGC_cali    (6b) *(0x1020642c)[23:18]
	g_adc_cali_en_t= (temp2 & 0x01000000)>>24;		//ADC_CALI_EN_T(1b) *(0x1020642c)[24]
	g_o_slope      = (temp2 & 0xFC000000)>>26;  //O_SLOPE      (6b) *(0x1020642c)[31:26]
	g_o_slope_sign = (temp2 & 0x02000000)>>25;   //O_SLOPE_SIGN (1b) *(0x1020642c)[25]

	g_id           = (temp1 & 0x08000000)>>27;   //ID           (1b) *(0x10206428)[27]
...
int CPU_Temp(void)
{
	int curr_temp, curr_temp2, max_temp;
	curr_temp = get_immediate_temp1();
	curr_temp2 = get_immediate_temp2();
	max_temp = (curr_temp > curr_temp2) ? curr_temp : curr_temp2;
							...
							unsigned long *t)
{
    ...
    {
	/* CPU temp invalid range for project 8590 */
        if ((curr_temp > INVALID_HOT_TEMP) || (curr_temp < INVALID_COLD_TEMP)) // invalid range
        {
