...
#ifdef CONFIG_ICX_ALLOW_NULL_CONSOLE
static int icx_set_console_cmdline_null(char *cmd)
{
    int cmd_len = strlen(cmd);
    char *p, *cmd_end = cmd + cmd_len;
    const char *root_str = "root=/dev/ram";
    const char *cons_str = "console=/dev/null";

    // find "root=" and point there
    for(p = cmd; p < cmd_end; p++) {
        if(!memcmp(p, root_str, strlen(root_str))) {
            break;
        }
    }

    if(p < cmd_end) {
        snprintf(cmd, cmd_len, "%s %s", cons_str, p);
    }
...
#ifdef CONFIG_ICX_ALLOW_NULL_CONSOLE
		/* printf("U-boot console setting is null, use uboot  setting, return\n"); */
		icx_set_console_cmdline_null(kernel_cmd_line);
#else
...
#if defined(CONFIG_ENABLE_WMPORT)
static void (*mtk_uart_event_notifier_func)( int ) = NULL;

void mtk_uart_register_event_notifier( void (*notifier)( int ) )
{
	mtk_uart_event_notifier_func = notifier;
}

void mtk_uart_call_event_notifier(int type, struct mtk_uart *up )
{
	void (*notifier)(int) = mtk_uart_event_notifier_func;
	if ( up->port.irq == MT_UART2_IRQ_ID ) 
		if ( notifier ) (*notifier)( type );
}
#endif /* CONFIG_ENABLE_WMPORT */
...
#if defined(CONFIG_ENABLE_WMPORT)
		if ( !err && ( uart->line_status & ( UART_LSR_FE | UART_LSR_BI ) ) ) {
			err++;
			mtk_uart_call_event_notifier( MTK_UART_EVENT_TYPE_RD_ERR, port);
		}
#endif /* CONFIG_ENABLE_WMPORT */
...
#if defined(CONFIG_ENABLE_WMPORT)
	mtk_uart_call_event_notifier(MTK_UART_EVENT_TYPE_OPEN, port);
#endif /* CONFIG_ENABLE_WMPORT */
