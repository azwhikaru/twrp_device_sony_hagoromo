...
#if defined (CONFIG_ENABLE_MIPI_SWAP)
#include <../../auxadc/mt_auxadc.h>
#define ICX_BID_MIN     1
#define ICX_BID_MAX     3
static int icx_get_bid(int bid)
{
    const int ad_max = 0xfff;               // AUXADC resolution is 4095(0-0xfff)
    const int ad_ch[] = { -1, 4, 5, 6 };    // BID1=AD4, BID2=AD5, BID3=AD6
    int ad_err, ad_raw_data, ad_tmp[4];

    if(bid < ICX_BID_MIN || ICX_BID_MAX < bid) {
        // invalid bid error
        return -1;
    }

    ad_err = IMM_GetOneChannelValue(ad_ch[bid], ad_tmp, &ad_raw_data);
    if(ad_err) {
        // ad read error
        return ad_err;
    }

    if(ad_raw_data < (ad_max/2)) {
        // bid is lo
        return 0;
    }
    else {
...
//      pmod = 433.33;
        ...
        OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_FRA_EN,1);
#if defined (CONFIG_ENABLE_MIPI_SWAP)
    if(icx_get_bid(3) == 0) {  // BID3=lo SET
        // if SET,check model
        if(icx_get_bid(2) == 1) {  // BID2=hi need swap model
            //swap data1 and clk
            printk("[MIPI] will swap mipi line.\n");
            OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG->MIPITX_DSI_PHY_SEL,MIPI_TX_PHYC_SEL,0x1);
            OUTREGBIT(MIPITX_DSI_PHY_SEL_REG,DSI_PHY_REG->MIPITX_DSI_PHY_SEL,MIPI_TX_PHY1_SEL,0x4);
            OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_CKLANE_EN,0x0);
            OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_CKLANE_EN,0x1);
        }
    }
#endif
...
#if defined (CONFIG_ENABLE_MIPI_SWAP)
    if(icx_get_bid(3) == 0) {  // BID3=lo SET
        if(icx_get_bid(2) == 0) {  // BID2=low
            printk("[MIPI] Change lane rt_code.\n");
            OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_RT_CODE,0x2);
            OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_RT_CODE,0x2);
            OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_RT_CODE,0x2);
        }
    }
#endif
...
void DSI_PHY_clk_switch(bool on)
{
	...
	/* RG_DSI_PAD_TIE_LOW_EN Set to 0 is better by DE YH.CHIEN advice */
        OUTREGBIT(MIPITX_DSI_TOP_CON_REG,DSI_PHY_REG->MIPITX_DSI_TOP_CON,RG_DSI_PAD_TIE_LOW_EN, 0);
...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (count)
            {
#if defined(CONFIG_ARCH_MT8590_ICX) && defined(CONFIG_OTM8018B_DSI_XXX)
                t0.Data_ID = DSI_DCS_SHORT_PACKET_ID_1;
#else
