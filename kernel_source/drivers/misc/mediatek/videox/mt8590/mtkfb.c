...
#endif
{
    ...
    case MTKFB_POWERON:
    {
        MTKFB_FUNC();
        if(!is_early_suspended)  {
                pr_err("MTKFB_POWERON return directly\n");
                return r;
        }
        if (down_interruptible(&sem_early_suspend))
        {
...
static void mtkfb_late_resume(struct early_suspend *h)
{
    ...
    pr_notice("[FB Driver] enter late_resume\n");
    ...
    {
#ifndef CONFIG_MTK_FB_DISPLAY_DIRECTLY
        mtkfb_clear_lcm();
#endif
    }
...
#endif
#endif
    pr_notice("[FB Driver] leave late_resume\n");
