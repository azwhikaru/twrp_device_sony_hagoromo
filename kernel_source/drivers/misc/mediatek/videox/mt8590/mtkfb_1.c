...
#ifndef CONFIG_MTK_FB_NO_MEM
	// use fb0 buffer directly
	fbdev->fb_pa_base = fb_pa + DISP_GetFBRamSize() + DAL_GetLayerSize();
	fbdev->fb_va_base = fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize();

	memset(fbdev->fb_va_base, 0, alloc_size);
#else
...
static int mtkfb_1_probe(struct device *dev)
{
    ...
    if (r) {
		MTKFB_1_LOG("mtkfb_1_fbinfo_init failed\n");
        goto cleanup;
    }
    init_state++;   // 4
    ...
    if (r) {
		MTKFB_1_LOG("mtkfb_1_register_sysfs failed\n");
        goto cleanup;
    }
    init_state++;   // 5
