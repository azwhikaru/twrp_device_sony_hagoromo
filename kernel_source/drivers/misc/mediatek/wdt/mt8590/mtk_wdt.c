...
/*
 * watchdog mode:
 * dual_mode_en: debug module enable.
 * irq: generate interrupt instead of reset first.
 * ext_en: output reset signal to outside
 * ext_pol: polarity of external reset signal
 * wdt_en: enable watchdog timer
 */
			...
			BOOL wdt_en)
{
	...
	printk(KERN_INFO "mtk_wdt_mode_config  mode=%x, tmp:%x, pid=%d\n",
...
int mtk_wdt_confirm_hwreboot(void)
{
	...
	unsigned int tmp;
	aee_sram_fiq_log("mt8590 only hw reboot mode test\n");
	mtk_wdt_restart(WD_TYPE_NOLOCK);

	/* set rgu 20s time out */
	tmp = (unsigned int)(20 * (1 << 6));
	tmp = tmp << 5;
	DRV_WriteReg32(MTK_WDT_LENGTH, (tmp | MTK_WDT_LENGTH_KEY));
...
void mtk_wdt_restart(enum wd_restart_type type)
{
	if (type == WD_TYPE_NORMAL) {
		...
		*(volatile u32 *)(MTK_WDT_RESTART) = MTK_WDT_RESTART_KEY;
		...
		printk("WDT:[mtk_wdt_restart] type %d error, pid = %d\n", type, current->pid);
}
...
{
	DRV_WriteReg32(MTK_WDT_DEGLITCH_EN, MTK_WDT_DEGLITCH_KEY);
	return 0;
}
...
{
	/* dual_mode, irq, ext_en, low pol, dis wdt */
...
MODULE_AUTHOR("MTK");
MODULE_DESCRIPTION("MT8590 Watchdog Device Driver");
MODULE_LICENSE("GPL");
