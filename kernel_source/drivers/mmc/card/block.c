...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if ((is_legal == 1) && (host->card_num == 2) && (host->card2 == NULL)) {
		is_legal = 0;
	}
#endif
	return is_legal;
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if ((host->card_num == 2) && (host->card2) && (host->select == (host->card2->rca << 16)))
		md = mmc_get_drvdata(host->card2);
#endif
...
void met_mmc_dma_map(struct mmc_host *host, struct mmc_async_req *areq)
{
	...
	md = mmc_get_drvdata(host->card);
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if ((host->card_num == 2) && (host->select == (host->card2->rca << 16)))
		md = mmc_get_drvdata(host->card2);
#endif
	if (!check_met_mmc_blk_data_legal(md))
		return;
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if ((host->card_num == 2) && (host->card2 == NULL))
		return;
#endif
	md = mmc_get_drvdata(host->card);
#ifdef CONFIG_MMC_MULTIPLE_CARDS
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
		part_err = mmc_blk_part_switch(card, md);
#else
...
#include <linux/delay.h>
#include "../../misc/mediatek/mmc-host/mt8590/mt_sd.h"

#define ICX_MMC_READ  1
#define ICX_MMC_WRITE 0

static struct mmc_card *loader_part_chip_card = NULL;

static int icx_mmc_read_write_core(
	struct mmc_card    *card,
	int                is_read,
...
)
{
	...
	brq.data.sg     = buf_sg;
	brq.data.sg_len = buf_sg_len;

	if (brq.data.sg_len < 1) {
		mmc_release_host(card->host);
		return -EINVAL;
	}
	...
	return (brq.cmd.error || brq.data.error) ? -1 : 0;
}
...
#define ICX_MMC_TEST_HEAD_SECTOR 529664			/* 0x81500 32KB Baundary */
/* #define ICX_MMC_TEST_HEAD_SECTOR 532480 */	/* 0x82000  4MB Baundary */

#define ICX_MMC_RESUME_TIME_TEST_COUNT    10
#define ICX_MMC_PERFORMANCE_TEST_COUNT  2048

#define KERN_TEST KERN_ERR

static const int test_param[] = {
	    512,
	   1024,
	   2048,
	   4096,
...
static const int test_param_count = sizeof(test_param)/sizeof(int);

static int icx_mmc_resume_time_test(void)
{
	struct scatterlist sg[1];
	struct timeval pre; 
	struct timeval post;
	char * buf; 
	int log[ICX_MMC_RESUME_TIME_TEST_COUNT];
	int total_noi;
	int total_int;
	int total_int2;
	int rv;
	...
	printk(KERN_TEST "eMMC auto resume time test -------------------------------------------\n");
	printk(KERN_TEST " \n");
	...
	sg_set_page(&sg[0], virt_to_page(buf), 512, offset_in_page(buf));

	/* dummy read */
	rv = icx_mmc_read_write_core(
			loader_part_chip_card,
			ICX_MMC_READ,
			ICX_MMC_TEST_HEAD_SECTOR - 1,
			sg, /* sg list  */
			1,  /* sg count */
			512 /* size     */
	);
	if (rv) {
		printk(KERN_ERR "%s(): icx_mmc_read_write_core() error\n", __FUNCTION__);
		kfree(buf);
		return(-1);
	}
	...
	printk(KERN_TEST "  executing no interval read test ...\n");

	for (n = 0; n < ICX_MMC_RESUME_TIME_TEST_COUNT; n++){
		...
		rv = icx_mmc_read_write_core(
				loader_part_chip_card,
				ICX_MMC_READ,
				ICX_MMC_TEST_HEAD_SECTOR + n,
				sg, /* sg list  */
				1,  /* sg count */
				512 /* size     */
		);

		do_gettimeofday(&post);
		...
		log[n] = (post.tv_sec*1000000+post.tv_usec) - (pre.tv_sec*1000000+pre.tv_usec);
	}

	total_noi = 0;
	for (n = 0; n < ICX_MMC_RESUME_TIME_TEST_COUNT; n++) {
		total_noi = total_noi + log[n];
		printk(KERN_TEST "    %02d: %8d [us]\n", n + 1, log[n]);
	}
	printk(KERN_TEST "  total time= %d [us]\n", total_noi);
	printk(KERN_TEST " \n");

	/**************************************************************/

	printk(KERN_TEST "  executing interval read test ... (interval=10ms)\n");
	...
	printk(KERN_TEST "  executing interval read test ... (interval=2s)\n");
	...
	printk(
		KERN_TEST
		"auto sleep resume delay (10ms) = %d [us]\n",
		(total_int-total_noi)/ICX_MMC_RESUME_TIME_TEST_COUNT
	);   
	printk(
		KERN_TEST
		"auto sleep resume delay (2s)   = %d [us]\n",
		(total_int2-total_noi)/ICX_MMC_RESUME_TIME_TEST_COUNT
	);
	printk(KERN_TEST " \n");
...
#define ADMA_MAX_TRANSFER_SIZE  0x10000
static struct scatterlist sg_perf[64];

static int icx_mmc_performance_test(void)
{
	struct timeval pre; 
	struct timeval post;
	char * buf; 
    ...
    int sg_count;
    int time_diff;
    unsigned int mbps_upper;
    unsigned int mbps_lower;

	printk(KERN_TEST "eMMC performance test ------------------------------------------------\n");
	printk(KERN_TEST " \n");

	chip_sectors = loader_part_chip_card->ext_csd.sectors;
	if (chip_sectors == 0)
		chip_sectors = loader_part_chip_card->csd.capacity << (loader_part_chip_card->csd.read_blkbits - 9);
	
	buf = kmalloc(test_param[test_param_count-1], GFP_KERNEL);
	if (buf == NULL) {
		printk(KERN_ERR "%s(): kmalloc() error\n", __FUNCTION__);
		return(-1);
	}
	...
	printk(KERN_TEST "  executing the read test ...\n");

	printk(KERN_TEST "    test size [byte], test count, time [us], bit rate[Mbps]\n");

	for (p=0; p<test_param_count && btos(test_param[p])<=MAX_BD_NUM; p++) {
		
		count = minimum(
			ICX_MMC_PERFORMANCE_TEST_COUNT,
			(chip_sectors-ICX_MMC_TEST_HEAD_SECTOR)/btos(test_param[p])
		);

        sg_count = test_param[p] / ADMA_MAX_TRANSFER_SIZE;
        if(test_param[p] % ADMA_MAX_TRANSFER_SIZE) sg_count++;

		for (n=0; n<sg_count; n++) {
		    sg_perf[n].length = minimum(ADMA_MAX_TRANSFER_SIZE, test_param[p]);
        	sg_set_page(&sg_perf[n], virt_to_page(buf + ADMA_MAX_TRANSFER_SIZE * n), minimum(ADMA_MAX_TRANSFER_SIZE, test_param[p]), offset_in_page(buf + ADMA_MAX_TRANSFER_SIZE * n));
        }
		...
		for (n=0; n<count; n++) {
			...
			rv = icx_mmc_read_write_core(
					loader_part_chip_card,
					ICX_MMC_READ,
					ICX_MMC_TEST_HEAD_SECTOR + btos(test_param[p])*n,
					sg_perf,
					sg_count,
					test_param[p]
			);
        ...
        time_diff = (int)(((post.tv_sec*1000000+post.tv_usec) - (pre.tv_sec*1000000+pre.tv_usec))/count);
        mbps_upper = test_param[p] * 8 / time_diff;
        mbps_lower = (test_param[p] * 8 - mbps_upper * time_diff) * 1000 / time_diff;
	...
	printk(KERN_TEST "  executing the write test ...\n");
	
	printk(KERN_TEST "    test size [byte], test count, time [us], bit rate[Mbps]\n");

	for(p=0; p<test_param_count && (test_param[p]>>9)<=MAX_BD_NUM; p++){
...
static void icx_mmc_test(void)
{
	int rv = 0;

	printk(KERN_TEST " \n");

	mmc_claim_host(loader_part_chip_card->host);
	mmc_select_card(loader_part_chip_card);

	rv = icx_mmc_resume_time_test();
	if (rv < 0) {
		mmc_release_host(loader_part_chip_card->host);
		while (1) {}
	}

	rv = icx_mmc_performance_test();
	if (rv < 0) {
		mmc_release_host(loader_part_chip_card->host);
		while (1) {}
	}
	...
	printk(KERN_ERR "test finisihed\n");
	while (1) {}
}
...
static int icx_get_name_idx(const unsigned long *addr, unsigned long size, int is_sd)
{
    int i = 0;

    for(i = 0; i < size; i++) {
        i = find_next_zero_bit(addr, size, i);
        if(( is_sd && !(i == 0 || i == 3)) || // 0,3 is reserved for mmc
           (!is_sd && !(i == 1 || i == 2))) { // 1,2 is reserved for sdc
			return i;
        }
    }
    return i;
						...
						int area_type)
{
	...
	if (!subname) {
#if defined(CONFIG_ARCH_MT8590_ICX)
		md->name_idx = icx_get_name_idx(name_use, max_devices, mmc_card_sd(card));
#else
...
#ifdef CONFIG_ICX_MMC_TEST
	if(md->name_idx == 0){
		/* set chip 0 card structure. */
		loader_part_chip_card = card;
	}
