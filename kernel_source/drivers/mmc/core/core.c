...
uint mmc_boot_serial = 0;
module_param_named(emmc_boot_serial, mmc_boot_serial, uint, 0644);
MODULE_PARM_DESC(
	emmc_serial,
	"Serial ID of booted eMMC");
#endif
...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (host->card2 && (host->select == (host->card2->rca << 16)))
		card = host->card2;
#endif
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if (err && cmd->retries && !mmc_card_removed(card)) {
#else
...
#if defined(CONFIG_ICX_DEBUG_MMC_STATISTICS_M) || defined(CONFIG_ICX_DEBUG_MMC_STATISTICS_C)
#include <linux/ktime.h>
#include <linux/icx_debug.h>

static ktime_t icx_debug_mmc_statistics_t0;
static int icx_debug_mmc_statistics_rw;
static int icx_debug_mmc_statistics_address;
static int icx_debug_mmc_statistics_count;
#endif
...
static int __mmc_start_req(struct mmc_host *host, struct mmc_request *mrq)
{
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if (host->card2 && mmc_card_removed(host->card2)) {
		mrq->cmd->error = -ENOMEDIUM;
		complete(&mrq->completion);
		return -ENOMEDIUM;
	}
	...
	if ((mrq->cmd->opcode == MMC_READ_SINGLE_BLOCK)
	 || (mrq->cmd->opcode == MMC_READ_MULTIPLE_BLOCK)) {
	 	icx_debug_mmc_statistics_rw = 1;
		icx_debug_mmc_statistics_address = mrq->cmd->arg | ((host->select >> 17) << 27);
		icx_debug_mmc_statistics_count   = mrq->data->blocks;
		icx_debug_mmc_statistics_t0      = ktime_get();
	}
	else if ((mrq->cmd->opcode==MMC_WRITE_BLOCK)
		  || (mrq->cmd->opcode==MMC_WRITE_MULTIPLE_BLOCK)) {
	 	icx_debug_mmc_statistics_rw = 2;
		icx_debug_mmc_statistics_address = mrq->cmd->arg | ((host->select >> 17) << 27);
		icx_debug_mmc_statistics_count   = mrq->data->blocks;
		icx_debug_mmc_statistics_t0      = ktime_get();
	}
	else {
		icx_debug_mmc_statistics_rw = 0;
		icx_debug_mmc_statistics_address = 0;
		icx_debug_mmc_statistics_count   = 0;
		memset(&icx_debug_mmc_statistics_t0, 0, sizeof(ktime_t));
	}
#endif
...
#ifdef CONFIG_ICX_DEBUG_MMC_STATISTICS_M
	if (icx_debug_mmc_statistics_rw) {
		t1 = icx_debug_get_usec(icx_debug_mmc_statistics_t0);
		icx_debug_statistics_save(icx_debug_mmc_statistics_rw,
								  icx_debug_mmc_statistics_address,
								  icx_debug_mmc_statistics_count,
								  t1>>4);
	}
#endif
#ifdef CONFIG_ICX_DEBUG_MMC_STATISTICS_C
	if (icx_debug_mmc_statistics_rw == 1) {
		t1 = icx_debug_get_usec(icx_debug_mmc_statistics_t0);
		printk(KERN_ERR "R%07X%03X%05X\n",
			   icx_debug_mmc_statistics_address,
			   icx_debug_mmc_statistics_count,
			   t1>>4);
	}
	if (icx_debug_mmc_statistics_rw == 2) {
		t1 = icx_debug_get_usec(icx_debug_mmc_statistics_t0);
		printk(KERN_ERR "W%07X%03X%05X\n",
			   icx_debug_mmc_statistics_address,
			   icx_debug_mmc_statistics_count,
			   t1>>4);
	}
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
		err = host->areq->err_check(card, host->areq);
#else
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	else (host->card2 && (mmc_card_mmc(host->card2)) && (host->card2->ext_csd.cache_ctrl & 0x1)) {
		if (mmc_cache_ctrl(host, 0)) {
			pr_err("%s: failed to disable cache\n", mmc_hostname(host));
			return;
		}
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if (host->card2)
		host->card2->state &= ~(MMC_STATE_HIGHSPEED | MMC_STATE_HIGHSPEED_DDR);
#endif
	if (mmc_host_is_spi(host)) {
		host->ios.chip_select = MMC_CS_HIGH;
		host->ios.bus_mode = MMC_BUSMODE_PUSHPULL;
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
		if (host->card2)
			mmc_card_set_removed(host->card2);
#endif /* CONFIG_MMC_MULTIPLE_CARDS */
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if (host->card_num == 2)
		ret |= mmc_card_removed(host->card2);
#endif
	/*
...
void mmc_start_host(struct mmc_host *host)
{
    ...
    // EMMC(MSDC0/MSDC3) : Execute detect change.
    // SD  (MSDC1/MSDC2) : Do not execute detect change.
    //                     For now only support MSDC1.
    if(host->dev_id != 1) {
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
			if (host->card_num == 2) {
				if (mmc_card_doing_bkops(host->card2)) {
					err = mmc_stop_bkops(host->card2);
					if (err)
						goto out;
				}
			}
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
		if (host->card2 && mmc_card_mmc(host->card2) &&
		    mmc_card_doing_bkops(host->card2)) {
			err = mmc_stop_bkops(host->card2);
			if (err) {
				pr_err("%s: didn't stop bkops\n",
					mmc_hostname(host));
			...
			mmc_card_clr_doing_bkops(host->card2);
		}
#endif
