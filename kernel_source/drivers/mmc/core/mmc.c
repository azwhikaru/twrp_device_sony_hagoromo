...
extern unsigned int mmc_boot_serial;
#endif
	...
	struct mmc_card *oldcard)
{
	...
	struct mmc_card *cards[2];
	struct mmc_card *oldcards[2];
	int i;
#endif
	...
	if (oldcard) {
#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH
		if (host->boot_serial == oldcard->cid.serial) {
			oldcards[0] = host->card;
			oldcards[1] = host->card2;
		}
		else {
			oldcards[0] = host->card2;
			oldcards[1] = host->card;
		}
#else
		oldcards[0] = host->card;
		oldcards[1] = host->card2;
	...
	for (i = 0; i < 2; i++) {
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH
			if (host->rca[i] != 0)
				card->rca = host->rca[i];	/* need to swap RCA */
			else
				card->rca = i + 1;
#else
			card->rca = i + 1;
		...
		/*
		 * For native busses:  set card RCA.
		 */
	...
	/*
	 * For native busses:  quit open drain mode.
	 */
	mmc_set_bus_mode(host, MMC_BUSMODE_PUSHPULL);
#else /* CONFIG_MMC_MULTIPLE_CARDS */
	if (mmc_host_is_spi(host))
...
#endif /* CONFIG_MMC_MULTIPLE_CARDS */
	...
	/* 1st step above: get serial no. all devices */

	/* 2nd step: multiple eMMC and first eMMC's RCA is different to booted device */
	/*           -> save the RCA in order to swap these */
	if (host->card_num >= 2) {
		if (host->rca[0] == 0) {
			if (host->boot_serial == 0) {
				err = -ENODEV;
				goto free_card;
			}

			if (host->boot_serial != cards[0]->cid.serial) {
				host->rca[0] = cards[1]->rca;
				host->rca[1] = cards[0]->rca;

				mmc_remove_card(cards[0]);
				mmc_remove_card(cards[1]);
		...
		host->boot_serial = cards[0]->cid.serial;
	}

	/* 3rd step: get other informations */
	for (i = 0; i < host->card_num; i++) {
		card = cards[i];
		oldcard = oldcards[i];
#endif
	...
	if (!oldcard) {
		...
		if (i == 0) {
			...
			cards[i]->ext_csd = cards[0]->ext_csd;
			if (cards[i]->ext_csd.sectors)
				mmc_card_set_blockaddr(card);
...
#if defined(CONFIG_MMC_MULTIPLE_CARDS) && defined(CONFIG_MMC_HS200)
	}
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH
		if (host->boot_serial == cards[i]->cid.serial)
			host->card = cards[i];
		else
			host->card2 = cards[i];
#else
...
int mmc_emmc_power_cycle(struct mmc_host *host, u32 ocr, struct mmc_card *card)
{
	int err = -1;

#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH
	u32 rca = 0;
    struct mmc_card *oldcard = NULL;

    rca = (host->select) >> 16;
    printk(KERN_ERR "%s rca(%d) select(%d) \n", __func__, rca, host->select);

    if (host->card_num >= 2) {

        if (host->card->cid.serial < host->card2->cid.serial) {
            oldcard = host->card;
        } else {
            oldcard = host->card2;
        }

        err = mmc_init_card(host, ocr, oldcard);
        if (err)
            goto err;

        if (host->card->rca == rca) {
            card = host->card;
        } else if(host->card2->rca == rca) {
            card = host->card2;
        } else {
            err = -1;
            goto err;
        ...
        err = mmc_init_card(host, ocr, host->card);
        if (err)
            goto err;
...
static int mmc_alive(struct mmc_host *host)
{
	...
	if (host->card_num == 2)
		err |= mmc_send_status(host->card2, NULL);
...
static int mmc_suspend(struct mmc_host *host)
{
	...
	for (i = 0; i < host->card_num; i++) {
		...
		if (mmc_can_poweroff_notify(card))
			err = mmc_poweroff_notify(card, EXT_CSD_POWER_OFF_SHORT);
#if (1 == LINUX_34_DEBUG)
		else if (mmc_card_can_sleep(host)) {
			err = mmc_card_sleep(host);
			if (!err)
				mmc_card_set_sleep(card);
		}
#else
		else if (mmc_card_can_sleep(host))
			err = mmc_card_sleep(host);
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if (host->card2)
		host->card2->state &= ~(MMC_STATE_HIGHSPEED | MMC_STATE_HIGHSPEED_200 | MMC_STATE_HIGHSPEED_400);
#endif
#else
	host->card->state &= ~(MMC_STATE_HIGHSPEED | MMC_STATE_HIGHSPEED_200);
#ifdef CONFIG_MMC_MULTIPLE_CARDS
	if (host->card2)
		host->card2->state &= ~(MMC_STATE_HIGHSPEED | MMC_STATE_HIGHSPEED_200);
#endif
#endif
...
int mmc_attach_mmc(struct mmc_host *host)
{
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH
	/* save a serial no. of booted device */
	host->boot_serial = mmc_boot_serial;
...
#ifdef CONFIG_MMC_MULTIPLE_CARDS_CMD_SWITCH
	/* If there are multiple eMMC cards and first one's RCA is different to booted device, */
	/* reinitialize eMMC cards in order to swap their RCA */
	if ((host->card_num >= 2) && (host->rca[0] != 0)) {
		err = mmc_init_card(host, host->ocr, NULL);
		if (err)
			goto err;
	}
	...
	if (host->card_num == 2) {
		mmc_release_host(host);
		err = mmc_add_card(host->card2);

		/* NOTE:                                                     */
		/*   Not implemented the process related to POWEROFF_NOTIFY. */
		/*   Because of MMC_CAP2_POWEROFF_NOTIFY hasn't been set.    */
...
#ifndef CONFIG_MMC_MULTIPLE_CARDS
	if (err)
		goto remove_card;
#endif
