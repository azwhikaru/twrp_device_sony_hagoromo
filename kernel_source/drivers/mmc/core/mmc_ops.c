...
static int _mmc_select_card(struct mmc_host *host, struct mmc_card *card)
{
    ...
    struct mmc_command retry_cmd = {0};
    u32 status;
    int retry_cnt = 0;
	...
	if (card) {
		...
		if (card->type == MMC_TYPE_MMC && host->select == cmd.arg)
			return 0; /* through */
#endif
	} else {
		cmd.arg = 0;
...
#ifdef CONFIG_ARCH_MT8590_ICX
    //prepare retry cmd
    retry_cmd.opcode = MMC_SELECT_CARD;
    retry_cmd.arg = 0;
    retry_cmd.flags = MMC_RSP_NONE | MMC_CMD_AC;

    while(retry_cnt < MMC_CMD_RETRIES){
        //CMD7  SELECT CARD
        ...
        if (err) {
            ...
            //For Retry : CMD7  SELECT NONE CARD
            err = mmc_wait_for_cmd(host, &retry_cmd, 0);
        } else {
            break;
        }
    }
    
    if(retry_cnt >= MMC_CMD_RETRIES){
        return -EIO;
    }
    return 0;
		...
		u32 opcode, void *buf, unsigned len)
{
	...
	mmc_select_card(card);	/* for multi-card support */
