...
static struct device_attribute dev_torture =
	...
	__ATTR(torture, S_IRUGO, dev_attribute_show, NULL);
#else
...
#endif
#ifndef CONFIG_MTK_SLC_NAND_SUPPORT
...
//MTK end
	...
	ubi_msg("after ubi->peb_buf = kmalloc(ubi->peb_size, GFP_KERNEL);\n");
	...
	if (!ubi->fm_buf)
		goto out_free;
	ubi_msg("after CONFIG_MTD_UBI_FASTMAP\n");
#endif
	ubi_msg("before sched_clock\n");
	attach_time = sched_clock();
	
	err = ubi_attach(ubi, 0);
	ubi_msg("after ubi_attach, %d\n", err);
	if (err) {
