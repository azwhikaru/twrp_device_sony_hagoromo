...
int raeth_resume_fe_dma(struct net_device *dev)
{
	int		i;
	unsigned int	regVal;
	END_DEVICE* ei_local = netdev_priv(dev);
	...
	if((NULL != ei_local->tx_ring0) && (NULL != ei_local->rx_ring0)) {
		regVal = (TX_WB_DDONE | RX_DMA_EN | TX_DMA_EN | PDMA_BT_SIZE_16DWORDS | ADMA_RX_BT_SIZE_32DWORDS);
		regVal |= (RX_2B_OFFSET);
		sysRegWrite(DMA_GLO_CFG, regVal);
	}
	return 1;
}


inline int raeth_send(struct net_device* dev, struct sk_buff *skb, int gmac_no)
{
