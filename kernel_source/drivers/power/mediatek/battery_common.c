...
/* // Global Variable */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
#ifndef CONFIG_USB_CAN_SLEEP_CONNECTING
				 ...
				 enum power_supply_property psp, union power_supply_propval *val)
{
	...
	battery_log(BAT_LOG_CRTI, "[wireless_get_property] start\n");
	switch (psp) {
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	battery_log(BAT_LOG_FULL, "Power/Battery",
...
static void wireless_update(struct wireless_data *wireless_data)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE || g_wireless_state) {
		if ((BMT_status.charger_type == WIRELESS_CHARGER) || g_wireless_state) {
			...
			battery_log(BAT_LOG_CRTI, "[wireless_update]\n");
		}
	} else {
...
{
#if defined(CONFIG_POWER_VERIFY)
	battery_log(BAT_LOG_CRTI, "Power/Battery",
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		if (upmu_is_chr_det() == KAL_TRUE) {
			battery_log(BAT_LOG_CRTI, "Open infra misc....\n");
#if defined(CONFIG_USB_MTK_OTG) && !(defined(CONFIG_USB_MTK_PORT0_CHARGE))
			musb_do_infra_misc(false);
#endif
		}
		if(upmu_is_chr_det() == KAL_FALSE && mt_usb_is_device()) {
			battery_log(BAT_LOG_CRTI, "Close infra misc....\n");
#if defined(CONFIG_USB_MTK_OTG) && !(defined(CONFIG_USB_MTK_PORT0_CHARGE))
...
{
	battery_log(BAT_LOG_CRTI, "******** battery : bat_thread_wakeup  ********\n");
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ADC_CHANNEL_READ:
		...
		} else if (adc_in_data[0] == 30)	/* V_Bat_temp magic number */
		{
			...
			battery_log(BAT_LOG_FULL, "unknown channel(%d,%d)%d\n",
...
static int battery_probe(struct platform_device *dev)
{
	...
	battery_charging_control(CHARGING_CMD_GET_BATTERY_STATUS, &bat_status);
	if(bat_status == 0) {
		battery_log(BAT_LOG_CRTI, "[battery_probe] battery exist\n");
		...
		battery_log(BAT_LOG_CRTI, "[battery_probe] don't use battery for system power\n");
				   ...
				   const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		battery_log(BAT_LOG_CRTI, "[Battery] buf is %s and size is %d\n", buf,
				    size);
