...
#if (defined(USB_HOST_QMU_ENABLE) || defined(USB11_HOST_QMU_ENABLE))
int usb_hcd_link_going_urb_to_ep(struct urb *urb)
{
	int		rc = 0;
	...
	urb->unlinked = 0;
	list_add_tail(&urb->urb_list, &urb->ep->urb_going_list);
	spin_unlock(&hcd_urb_list_lock);
	return rc;
}
EXPORT_SYMBOL_GPL(usb_hcd_link_going_urb_to_ep);
#endif
