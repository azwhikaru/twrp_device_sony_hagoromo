		...
		void (*release)(struct device *dev))
{
	...
	printk("[long]%s[%d] name %s\n", __FUNCTION__, __LINE__, gadget->name);
...
int usb_gadget_probe_driver(struct usb_gadget_driver *driver)
{
	...
	printk("[long]%s[%d] name %s\n", __FUNCTION__, __LINE__, driver->function);

	mutex_lock(&udc_lock);
	list_for_each_entry(udc, &udc_list, list) {
		printk("[long]%s[%d] name %s\n", __FUNCTION__, __LINE__, udc->gadget->name);
		/* For now we take the first one */
		if (!udc->driver)
		{	
#if defined(CONFIG_USB_MTK_PORT0_CHARGE)
			if((0 == (strcmp(udc->gadget->name, "musbfsh-hdrc"))) &&
				(0 == (strcmp(driver->function, "android_usb"))))
			{
				printk("[long]%s[%d] android gadget bind with port1\n", __FUNCTION__, __LINE__);
				goto found;
			}else
			if((0 == (strcmp(udc->gadget->name, "musb-hdrc"))) &&
				(0 == (strcmp(driver->function, "g_hid"))))
			{
				printk("[long]%s[%d] g_hid gadget bind with port0\n", __FUNCTION__, __LINE__);
				goto found;
			}
#else			
			goto found;
