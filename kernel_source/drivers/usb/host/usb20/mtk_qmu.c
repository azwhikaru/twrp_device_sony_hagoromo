...
extern void musb_h_tx_flush_fifo(struct musb_hw_ep *ep);
...
extern void musb_giveback_stage1(struct musb *musb, struct urb *urb, int status);
extern void musb_giveback_stage2(struct musb *musb, struct urb *urb, int status);
...
//#ifdef CONFIG_MT7118_HOST
    ...
    musb_giveback_stage1(musb, urb, status);
    qh->is_ready = ready;
	DBG(4,"\n");
...
#if 0	
	musb_giveback_stage2(musb, urb, status);
#endif
}
...
void h_qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	bool done = true;
	//trying to give_back the request to gadget driver.
	#if (defined(USB_HOST_QMU_ENABLE))
...
success:
	...
	if(qh->type != USB_ENDPOINT_XFER_CONTROL) {
		if (!(musb_allocate_ep_fifo(musb, qh)))
			return -1;
	}
	if (idle) {
