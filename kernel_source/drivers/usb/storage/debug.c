...
#if (defined(CONFIG_ARCH_MT8590_ICX))
/*! Debug printk flags, default: Supress printk. */
uint usb_stor_debug_printk = 0;
EXPORT_SYMBOL_GPL(usb_stor_debug_printk);

module_param_named(debug_printk,usb_stor_debug_printk,uint,0644);
/*! Debug printk level. */
int usb_stor_debug_printlv = 7;
EXPORT_SYMBOL_GPL(usb_stor_debug_printlv);

module_param_named(debug_printlv,usb_stor_debug_printlv,int,0644);
#endif /* (defined(CONFIG_ARCH_MT8590_ICX)) */
...
/* ICX platform. */
	/* always do */ {
		char	key_hex[16];
		char	asc_ascq_hex[24];

		if (keystr == NULL) {
							/*  01234567890 */
			snprintf(key_hex, sizeof(key_hex), "key=0x%.2x", key);
			keystr = key_hex;
		}
		if (what == NULL) {
								/*    012345678901234567890123456789 */
			snprintf(asc_ascq_hex, sizeof(asc_ascq_hex), "ASC.ASCQ=0x%.2x.0x%.2x", asc, ascq);
			what = asc_ascq_hex;
		}
		usb_stor_dbg(us, "%s: ", keystr);
		US_DEBUGPX(what, ascq);
...
#else /* (!defined(CONFIG_ARCH_MT8590_ICX)) */
	r = dev_vprintk_emit(usb_stor_debug_printlv, &us->pusb_dev->dev, fmt, args);
#endif /* (!defined(CONFIG_ARCH_MT8590_ICX)) */
