...
/*msk the data first, then umsk with the umsk.*/
static inline void os_writelmskumsk(unsigned int addr, unsigned int data, unsigned int msk,
				    unsigned int umsk)
{
	unsigned int tmp = readl((void __iomem *)addr);
	mb();
...
EXTERN DEV_INT32 wait_for_value(DEV_INT32 addr, DEV_INT32 msk, DEV_INT32 value, DEV_INT32 ms_intvl,
				DEV_INT32 count);
