#ifndef _MTK11_QMU_H_
#define _MTK11_QMU_H_

#ifdef MUSB11_QMU_SUPPORT

/* for musb_read/write api */
#include "mtk11_musb.h"
#include "musb11_debug.h"
#include "musb11_io.h"
#include <linux/musb/mtk_musb.h>
#include <linux/dmapool.h>

/* CUSTOM SETTING */
#define MUSB11_GPD_LEN_ALIGNED (64)  	/* > gpd len (16) and cache line size aligned */
#define MUSB11_GPD_EXT_LEN (48)		/* MUSB11_GPD_LEN_ALIGNED - 16(should be sizeof(TGPD) */
#define MUSB11_GPD_SZ (16)
#define MUSB11_MAX_GPD_NUM 128
#define MUSB11_RXQ_NUM 8
#define MUSB11_TXQ_NUM 8
#define MUSB11_MAX_QMU_EP MUSB11_RXQ_NUM
#define MUSB11_TXQ	0
#define MUSB11_RXQ	1

/* QMU SETTING */
#define MUSB11_NO_ZLP 0
#define MUSB11_HW_MODE 1
#define MUSB11_GPD_MODE 2
//#define MUSB11_TXZLP MUSB11_GPD_MODE
//#define MUSB11_TXZLP MUSB11_HW_MODE

#define MUSB11_TXZLP MUSB11_NO_ZLP

//#define CFG_RX_ZLP_EN
//#define CFG_RX_COZ_EN

#define MUSB11_CFG_CS_CHECK
...
#define USB11_QMU_DBG_ON

#ifdef USB11_QMU_DBG_ON
#define QMU_ERR(format, args...) do {if(mtk_dbg_level(LOG_ERR)){printk(KERN_WARNING "QMU11_ERR,<%s %d>, " format , __func__, __LINE__ , ## args);}}while(0)
#define QMU_WARN(format, args...) do {if(mtk_dbg_level(LOG_WARN)){printk(KERN_WARNING "QMU11_WARN,<%s %d>, " format , __func__, __LINE__ , ## args);}}while(0)
#define QMU_INFO(format, args...) do {if(mtk_dbg_level(LOG_INFO)){printk(KERN_WARNING "QMU11_INFO,<%s %d>, " format , __func__, __LINE__ , ## args);}}while(0)
#define QMU_DBG(format, args...) do {if(mtk_dbg_level(LOG_DBG)){printk(KERN_WARNING "QMU11_DBG,<%s %d>, " format , __func__, __LINE__ , ## args);}}while(0)
#else
...
#define MGC_ReadQMU8(base,_offset) \
	musb11_readb(base, (USB_HW_QMU_OFF + _offset))

#define MGC_ReadQUCS8(base,_offset) \
	musb11_readb(base, (USB_HW_QUCS_OFF + _offset))

#define MGC_ReadQIRQ8(base,_offset) \
	musb11_readb(base, (USB_HW_QIRQ_OFF + _offset))

#define MGC_ReadQMU16(base,_offset) \
	musb11_readw(base, (USB_HW_QMU_OFF + _offset))

#define MGC_ReadQUCS16(base,_offset) \
	musb11_readw(base,(USB_HW_QUCS_OFF + _offset))

#define MGC_ReadQIRQ16(base,_offset) \
	musb11_readw(base, (USB_HW_QIRQ_OFF + _offset))
#define MGC_ReadQMU32(base,_offset) \
	musb11_readl(base, (USB_HW_QMU_OFF + _offset))

#define MGC_ReadQUCS32(base,_offset) \
	musb11_readl(base, (USB_HW_QUCS_OFF + _offset))

#define MGC_ReadQIRQ32(base,_offset) \
	musb11_readl(base, (USB_HW_QIRQ_OFF + _offset))

#define MGC_WriteQMU32(base,_offset, _data) \
	musb11_writel(base, (USB_HW_QMU_OFF + _offset), _data)

#define MGC_WriteQUCS32(base,_offset, _data) \
	musb11_writel(base, (USB_HW_QUCS_OFF + _offset), _data)

#define MGC_WriteQIRQ32(base,_offset, _data) \
	musb11_writel(base,(USB_HW_QIRQ_OFF + _offset), _data)
...
#define TGPD_SET_CHKSUM(_pd, _n)    ((TGPD *)_pd)->chksum = usb11_PDU_calcCksum((u8 *)_pd, _n)
#define TGPD_SET_CHKSUM_HWO(_pd, _n)    ((TGPD *)_pd)->chksum = usb11_PDU_calcCksum((u8 *)_pd, _n)-1
...
extern void qmu11_destroy_gpd_pool(struct device *dev);
extern int qmu11_init_gpd_pool(struct device *dev);
extern void qmu11_reset_gpd_pool(u32 ep_num, u8 isRx);
extern bool mtk_is_qmu11_enabled(u8 EP_Num, u8 isRx);
extern void mtk_qmu11_enable(struct musb *musb, u8 EP_Num, u8 isRx);
extern void mtk_qmu11_insert_task(u8 EP_Num, u8 isRx, u8* buf, u32 length, u8 zlp);
extern void mtk_qmu11_resume(u8 EP_Num, u8 isRx);
extern void qmu11_done_rx(struct musb *musb, u8 ep_num);
extern void qmu11_done_tx(struct musb *musb, u8 ep_num);
extern void mtk11_disable_q(struct musb *musb, u8 ep_num, u8 isRx);
extern void mtk_qmu11_irq_err(struct musb *musb, u32 qisar);
extern void usb11_flush_ep_csr(struct musb *musb, u8 ep_num, u8 isRx);
extern void mtk_qmu11_stop(u8 ep_num, u8 isRx);
#ifdef MUSB11_QMU_SUPPORT_HOST
extern void h_qmu11_done_rx(struct musb *musb, u8 ep_num);
extern void h_qmu11_done_tx(struct musb *musb, u8 ep_num);
extern void mtk_qmu11_insert_task_ioc(u8 EP_Num, u8 isRx, u8* buf, u32 length, u8 zlp,u8 isioc);
#endif
#endif
#endif
