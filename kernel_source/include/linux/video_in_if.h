...
typedef struct _HDMIRX_SINK_AV_CAP_T
{
  ...
  unsigned char ui1_sink_dts_ch_sampling[7];//n: channel number index, value: each bit means sampling rate for this channel number (SINK_AUDIO_32k..)
...
typedef struct {
	bool pcmchinfo; /*0:2CH, 1:MULTI-CH*/
	bool dsdaudio;  /*if dsd*/
	bool hbraudio;  /*if hbr*/
	unsigned char   hdmirxaudfs;/*fs*/
	unsigned char	chsts[5];/*channel status info*/
} HDMIRX_AUDIO_INFO;
