			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	printk("[USB]%s:%d bits:%d, sample bits:%d frame bits:%d\n", __FUNCTION__, __LINE__,
			bits, runtime->sample_bits, runtime->frame_bits);
	#endif
	runtime->sample_bits = bits;
	bits *= runtime->channels;
	runtime->frame_bits = bits;
	#ifdef MTK_ADD_DSD_DOP_SUPPORT
	if(runtime->format == 0x30)
		frames = 4;
	else
		frames = 1;
	#else
	...
	#endif
	#ifdef MTK_ADD_DSD_DOP_SUPPORT
	printk("[USB]%s:%d frames:%d bits:%d, sample bits:%d frame bits:%d\n", __FUNCTION__, __LINE__,
			frames, bits, runtime->sample_bits, runtime->frame_bits);
	#endif
	while (bits % 8 != 0) {
		bits *= 2;
		frames *= 2;
...
#if SNDRV_PCM_RATE_5512 != 1 << 0 || SNDRV_PCM_RATE_768000 != 1 << 15 //SNDRV_PCM_RATE_192000 != 1 << 12
