				 ...
				 struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->count = 2;
#ifdef CONFIG_ARCH_MT8590_ICX
	uinfo->value.integer.min = VOL_MIN;
...
#ifdef CONFIG_ARCH_MT8590_ICX
#define DUMMY_PLSRC(xname, xindex, addr) \
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
  .info = snd_dummy_plsrc_info, \
  .get = snd_dummy_plsrc_get, .put = snd_dummy_plsrc_put, \
  .private_value = addr }

#define snd_dummy_plsrc_info snd_ctl_boolean_mono_info
 
static int snd_dummy_plsrc_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_dummy *dummy = snd_kcontrol_chip(kcontrol);
	int addr = kcontrol->private_value;
	...
	spin_lock_irq(&dummy->mixer_lock);
	ucontrol->value.integer.value[0] = dummy->playback_source[addr][0];
	spin_unlock_irq(&dummy->mixer_lock);
	return 0;
}

static int snd_dummy_plsrc_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_dummy *dummy = snd_kcontrol_chip(kcontrol);
	int change, addr = kcontrol->private_value;
	int new = ucontrol->value.integer.value[0] & 1;
	spin_lock_irq(&dummy->mixer_lock);
	change = dummy->playback_source[addr][0] != new;
	dummy->playback_source[addr][0] = new;
	spin_unlock_irq(&dummy->mixer_lock);
	return change;
}
#endif
...
#ifdef CONFIG_ARCH_MT8590_ICX
DUMMY_PLSRC("Master Playback Switch", 0, MIXER_ADDR_MASTER),
#endif
{
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
