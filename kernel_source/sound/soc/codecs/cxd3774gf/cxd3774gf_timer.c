...
int cxd3774gf_timer_initialize(void)
{
	print_trace("%s()\n",__FUNCTION__);

	timer_thread=kthread_create(timer_thread_main,NULL,"cxd3774gf_thread");
...
int cxd3774gf_timer_finalize(void)
{
	print_trace("%s()\n",__FUNCTION__);
...
static int timer_thread_main(void * dummy)
{
	...
	while(1){
		...
		printk(KERN_INFO "####### CXD3774GF TIMER\n");
#endif
		cxd3774gf_check_jack_status(force);
