...
static int cxd3776er_register_write_safe(
	unsigned int    address,
	unsigned char * value,
	int	     size
);

static int cxd3776er_register_read_safe(
	unsigned int    address,
	unsigned char * value,
	int	     size
);

static int cxd3776er_register_write_core(
	unsigned int    address,
	unsigned char * value,
	int	     size
);

static int cxd3776er_register_read_core(
	unsigned int    address,
	unsigned char * value,
	int	     size
);
...
)
{
	...
	rv=cxd3776er_register_write_safe(address,value,size);
	if(rv<0) {
		back_trace();
...
)
{
	...
	rv=cxd3776er_register_read_safe(address,value,size);
	if(rv<0) {
		back_trace();
		mutex_unlock(&access_mutex);
		return(rv);
...
)
{
	...
	rv=cxd3776er_register_read_safe(address,&uc,1);
	if(rv<0) {
		back_trace();
		mutex_unlock(&access_mutex);
		return(rv);
	...
	rv=cxd3776er_register_write_safe(address,&uc,1);
	if(rv<0) {
		back_trace();
		mutex_unlock(&access_mutex);
		return(rv);
...
)
{
	...
	while(retry>=0){
		rv=cxd3776er_register_write_core(address,value,size);
		if(rv==0)
			break;
...
)
{
	...
	while(retry>=0){
		rv=cxd3776er_register_read_core(address,value,size);
		if(rv==0)
			break;
