...
static int change_external_osc(
	struct cxd3778gf_status * status,
	int pwm_nowait
);

static unsigned int suitable_external_osc(
	int output_device,
	int headphone_amp,
	int icx_playback_active,
	int std_playback_active,
...
static struct delayed_work hp_det_event_work;
...
static struct cxd3778gf_status present = {
	...
	.board_type		     = TYPE_A,
	.i2s_mode		     = SND_SOC_DAIFMT_CBS_CFS,
...
int cxd3778gf_core_initialize(struct mutex * mutex)
{
	print_trace("%s()\n",__FUNCTION__);
	...
	INIT_DELAYED_WORK(&hp_det_event_work, do_hp_det_event_work);
...
//	digiamp_initialize();
	...
	/* dummy, dnc driver is not loaded. */
	present.noise_cancel_active=cxd3778gf_dnc_judge(
...
int cxd3778gf_startup(int icx_playback, int std_playback, int capture)
{
...
int cxd3778gf_shutdown(int icx_playback, int std_playback, int capture)
{
	...
	if(icx_playback){
		...
		if(present.i2s_mode == SND_SOC_DAIFMT_CBM_CFM)
...
#ifdef ICX_ENABLE_AU2CLK
	change_external_osc(&present,ON);
#endif
...
int cxd3778gf_set_board_type(unsigned int type)
{
        print_trace("%s(%u)\n",__FUNCTION__);
...
int cxd3778gf_get_board_type(unsigned int * type)
{
        print_trace("%s(%u)\n",__FUNCTION__);
...
int cxd3778gf_set_icx_playback_dai_rate(unsigned int rate)
{
	...
	if (present.format==PCM_MODE) {
		...
		/* avoid puchi-noise by disable dsd-path before clk-change */
		if(dsd_enable&0x01){
			cxd3778gf_set_output_device_mute(&present,ON,TRUE);
...
int cxd3778gf_set_icx_playback_dai_format(unsigned int format)
{
	...
	if (present.board_type == TYPE_Z && !(board_set_flag&BOARD_ID_NO_PULLUP_MASK)){
...
#ifdef CONFIG_SND_SOC_MT8590
	if (present.board_type == TYPE_Z && present.i2s_mode == SND_SOC_DAIFMT_CBS_CFS){
		if (now_osc == EXTERNAL_OSC_441){ /* for DSD native */
			if((afe_read(CLK_AUDDIV_3)&0x00000021)!=0x00000021 || (afe_read(CLK_AUDDIV_0)&0x00010001)!=0x00010001){
				afe_msk_write(ASYS_TOP_CON, 0x01, A1SYS_TIMING_ON_MASK);
				msleep(1);
				afe_msk_write(CLK_AUDDIV_3, 0x1 << 22, 0x1 << 22);
				usleep_range(1,10);
				afe_msk_write(CLK_AUDDIV_3, 0x1 << 0, AUD_MUX1_CLK_MASK); /* internal APLL 48K domain */
				afe_msk_write(CLK_AUDDIV_3, 0x4 << 3, AUD_MUX2_CLK_MASK); /* cxd3778 mclk out 44K domain */
				afe_msk_write(CLK_AUDDIV_0, 0x0 << 24, AUD_A2SYS_K1_MASK); /* APLL2 no divide */
				afe_msk_write(CLK_AUDDIV_0, 0x1 << 16, AUD_A1SYS_K1_MASK);      /* APLL1 DIV Fix:APLL1/2 */
				afe_msk_write(CLK_AUDDIV_0, 0x1 << 0,  AUD_EXT1_K1_MASK);       /* EXT_CLK divide 1/2 */
				afe_msk_write(CLK_AUDDIV_3, 0x0 << 22, 0x1 << 22);
				afe_msk_write(ASYS_TOP_CON, A1SYS_TIMING_ON, A1SYS_TIMING_ON_MASK);
				msleep(1);
			}
		} else if (now_osc == EXTERNAL_OSC_480){ /* no use */
			if((afe_read(CLK_AUDDIV_3)&0x00000014)!=0x00000014 || (afe_read(CLK_AUDDIV_0)&0x01000001)!=0x01000001){
				afe_msk_write(CLK_AUDDIV_3, AUD_A1SYS_K1_OFF,AUD_A1SYS_K1_MASK);
				afe_msk_write(CLK_AUDDIV_3, 0x4 << 0, AUD_MUX1_CLK_MASK); /* cxd3778 mclk out 48K domain */
				afe_msk_write(CLK_AUDDIV_3, 0x2 << 3, AUD_MUX2_CLK_MASK); /* internal APLL 44K domain */
				afe_msk_write(CLK_AUDDIV_0, 0x0 << 16, AUD_A1SYS_K1_MASK); /* APLL1 no divide */
				afe_msk_write(CLK_AUDDIV_0, 0x1 << 0 , AUD_EXT1_K1_MASK);      /* EXT_CLK divide 1/2 */
				afe_msk_write(CLK_AUDDIV_0, 0x1 << 24, AUD_A2SYS_K1_MASK);     /* APLL2 DIV Fix:APLL2/2 */
				afe_msk_write(CLK_AUDDIV_3, AUD_A1SYS_K1_ON,AUD_A1SYS_K1_MASK);
			}
		}
	} else if (present.board_type == TYPE_Z && present.i2s_mode == SND_SOC_DAIFMT_CBM_CFM){  /* for PCM */
		if((afe_read(CLK_AUDDIV_3)&0x00000011)!=0x000000011 || (afe_read(CLK_AUDDIV_0)&0x01010000)!=0x01010000){
			afe_msk_write(ASYS_TOP_CON, 0x01, A1SYS_TIMING_ON_MASK);
			msleep(1);
			afe_msk_write(CLK_AUDDIV_3, 0x1 << 22, 0x1 << 22);
			usleep_range(1,10);
			afe_msk_write(CLK_AUDDIV_3, 0x1 << 0, AUD_MUX1_CLK_MASK);  /* internal APLL 44K domain */
			afe_msk_write(CLK_AUDDIV_3, 0x2 << 3, AUD_MUX2_CLK_MASK);  /* internal APLL 48K domain */
			afe_msk_write(CLK_AUDDIV_0, 0x1 << 16, AUD_A1SYS_K1_MASK); /* APLL1 DIV Fix:APLL1/2 */
			afe_msk_write(CLK_AUDDIV_0, 0x1 << 24, AUD_A2SYS_K1_MASK); /* APLL2 DIV Fix:APLL2/2 */
			afe_msk_write(CLK_AUDDIV_0, 0x0 << 0 , AUD_EXT1_K1_MASK);  /* EXT_CLK no divide */
			afe_msk_write(CLK_AUDDIV_3, 0x0 << 22, 0x1 << 22);
			afe_msk_write(ASYS_TOP_CON, A1SYS_TIMING_ON, A1SYS_TIMING_ON_MASK);
			msleep(1);
		}
...
#ifdef CONFIG_SND_SOC_MT8590
		if (!(afe_read(PWR2_TOP_CON)&PDN_DSD_ENC_MASK)) {
			cxd3778gf_register_read(CXD3778GF_BLK_FREQ1,&tmp);
			if(present.sample_rate==88200){
				...
				if (present.board_type ==TYPE_Z)
					afe_msk_write(CLK_AUDDIV_0, 0x0 << 0 , AUD_EXT1_K1_MASK);
...
#ifdef CONFIG_SND_SOC_MT8590
	afe_msk_write(CLK_AUDDIV_0, 0x1 << 0,  AUD_EXT1_K1_MASK);
#endif

	if(dsd_enable&0x01){
		cxd3778gf_set_phv_mute(&present,TRUE);
...
//              cxd3778gf_register_modify(CXD3778GF_SMS_DSD_CTRL,0x10,0x70);
		...
		cxd3778gf_set_phv_mute(&present,FALSE);
		cxd3778gf_set_master_volume(&present, present.master_volume);
	}
...
int cxd3778gf_put_user_noise_cancel_gain(int index)
{
	...
	rv=cxd3778gf_dnc_set_user_gain(index);
	if(rv<0){
		back_trace();
		mutex_unlock(global_mutex);
		return(rv);
...
#else
				print_debug("no dsd digital volume");
#endif
                                break;
...
int cxd3778gf_put_input_device(int value)
{
	print_trace("%s(%d)\n",__FUNCTION__,value);
		...
		if(backup != status) {
			if(present.output_device==OUTPUT_DEVICE_HEADPHONE){
				...
				if(present.noise_cancel_mode) {
					if((present.noise_cancel_active == 1 && status != JACK_STATUS_SE_5PIN) ||
						(present.noise_cancel_active == 0 && status == JACK_STATUS_SE_5PIN)) {
						cxd3778gf_set_output_device_mute(&present,ON,TRUE);
						muted = 1;
					}
				}
...
int cxd3778gf_handle_pcm_event(void)
{
	int rv;
...
/*@ handle_hp_det_event */
/************************/
int cxd3778gf_handle_hp_det_event(void)
{
        int rv;
	...
	/* hp det */
	schedule_delayed_work(&hp_det_event_work, 0);
...
int cxd3778gf_put_headphone_detect_mode(int value)
{
	print_trace("%s(%d)\n",__FUNCTION__,value);
	...
	if(value==HEADPHONE_DETECT_POLLING || value==HEADPHONE_DETECT_SELECT){
		cxd3778gf_interrupt_finalize(present.board_type, present.headphone_detect_mode);
		...
		cxd3778gf_interrupt_initialize(present.board_type, value);
...
int cxd3778gf_put_clock_recovery(int value)
{
	if (value==1){
		print_error("Recovery clock workaround1.\n");
		mutex_lock(global_mutex);
		afe_msk_write(ASYS_TOP_CON, 0x01, A1SYS_TIMING_ON_MASK);
		msleep(1);
		afe_msk_write(ASYS_TOP_CON, A1SYS_TIMING_ON, A1SYS_TIMING_ON_MASK);
		...
		print_error("Recovery clock workaround2.\n");
		mutex_lock(global_mutex);
		afe_enable(0);
		msleep(1);
		afe_enable(1);
...
int cxd3778gf_get_clock_recovery(int * value)
{
	/* this interface is one shot call, so always return 0 */
        *value=0;
...
static int suspend_core(void)
{
	...
	if(present.board_type == TYPE_A && present.headphone_detect_mode == HEADPHONE_DETECT_INTERRUPT){
		mt_eint_mask(cxd3778gf_get_hp_det_irq());
	}
	shutdown_chip();
...
//	digiamp_switch_shunt_mute(ON);
	...
	if(present.board_type == TYPE_A && present.headphone_detect_mode == HEADPHONE_DETECT_INTERRUPT){
		mt_eint_unmask(cxd3778gf_get_hp_det_irq());
		schedule_delayed_work(&hp_det_event_work, 10);
	}
...
)
{
	...
	if(output_device==OUTPUT_DEVICE_HEADPHONE){
		...
		if(headphone_amp==HEADPHONE_AMP_SMASTER_SE){
			...
			cxd3778gf_switch_external_osc(osc);
			cxd3778gf_register_modify(CXD3778GF_MODE_CONTROL,0x01,0x01);
#endif
			change_external_osc(&present,OFF);
#if 0
...
static int change_sample_rate(unsigned int rate)
{
	...
	if(rate<=48000){
		cxd3778gf_register_modify(CXD3778GF_SD1_MODE,0x00,0x07);
		if(present.board_type == TYPE_Z)
			cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x02,0x06);
	}
	else if(rate<=96000){
		cxd3778gf_register_modify(CXD3778GF_SD1_MODE,0x01,0x07);
                if(present.board_type == TYPE_Z)
                        cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x02,0x06);
	}
	else if(rate<=192000){
		cxd3778gf_register_modify(CXD3778GF_SD1_MODE,0x02,0x07);
                if(present.board_type == TYPE_Z)
                        cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x02,0x06);
	}
	else{
		cxd3778gf_register_modify(CXD3778GF_SD1_MODE,0x03,0x07);
                if(present.board_type == TYPE_Z)
                        cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x04,0x06);
	}
...
static int change_sample_rate_std(unsigned int rate)
{
	...
	cxd3778gf_register_modify(CXD3778GF_SD_ENABLE,0x00,0x40);
        ...
        if(rate<=48000){
                cxd3778gf_register_modify(CXD3778GF_SD2_MODE,0x04,0x07);
		if(present.board_type == TYPE_Z)
                        cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x02,0x06);
        }
        else if(rate<=96000){
                cxd3778gf_register_modify(CXD3778GF_SD2_MODE,0x05,0x07);
                if(present.board_type == TYPE_Z)
                        cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x02,0x06);
        }
        else if(rate<=192000){
                cxd3778gf_register_modify(CXD3778GF_SD2_MODE,0x06,0x07);
                if(present.board_type == TYPE_Z)
                        cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x02,0x06);
        }
        else{
...
)
{
	...
	if(present.board_type==TYPE_A){
				...
				switch(jack_status_se){
					case JACK_STATUS_SE_5PIN:
						if(headphone_amp==HEADPHONE_AMP_SMASTER_SE || headphone_amp==HEADPHONE_AMP_SMASTER_BTL){
							...
							else if(headphone_type==NCHP_TYPE_BUNDLE)
		...
		for(n=0; n<CODEC_RAM_SIZE/(CODEC_RAM_WORD_SIZE*8); n++){
			...
			rv=cxd3778gf_register_write_multiple(
				CXD3778GF_MEM_WDAT,
				((unsigned char *)&cxd3778gf_tone_control_table[table])+CODEC_RAM_WORD_SIZE*8*n,
...
/* read data */
#if 0
	rv=cxd3778gf_register_modify(CXD3778GF_MEM_CTRL,0x8C,0xDF);
        if(rv<0){
                back_trace();
                return(-1);
        }
        ...
        for(n=0; n<CODEC_RAM_SIZE/(CODEC_RAM_WORD_SIZE*8); n++){
                ...
                rv=cxd3778gf_register_read_multiple(
                        CXD3778GF_MEM_RDAT,
                        ((unsigned char *)&cxd3778gf_tone_control_table[table])+CODEC_RAM_WORD_SIZE*8*n,
                        CODEC_RAM_WORD_SIZE*8
                );
	...
	cxd3778gf_register_modify(CXD3778GF_DEQ_CONTROL,0x00,0x80);

	cxd3778gf_register_write_multiple(
		CXD3778GF_DEQ_1_COEF_BASE,
		(unsigned char *)&cxd3778gf_tone_control_table[table].coefficient[0],
		sizeof(struct cxd3778gf_deq_coefficient)
	);
	cxd3778gf_register_write_multiple(
		CXD3778GF_DEQ_2_COEF_BASE,
		(unsigned char *)&cxd3778gf_tone_control_table[table].coefficient[1],
		sizeof(struct cxd3778gf_deq_coefficient)
	);
	cxd3778gf_register_write_multiple(
		CXD3778GF_DEQ_3_COEF_BASE,
		(unsigned char *)&cxd3778gf_tone_control_table[table].coefficient[2],
		sizeof(struct cxd3778gf_deq_coefficient)
	);
	cxd3778gf_register_write_multiple(
		CXD3778GF_DEQ_4_COEF_BASE,
		(unsigned char *)&cxd3778gf_tone_control_table[table].coefficient[3],
		sizeof(struct cxd3778gf_deq_coefficient)
	);
	cxd3778gf_register_write_multiple(
		CXD3778GF_DEQ_5_COEF_BASE,
		(unsigned char *)&cxd3778gf_tone_control_table[table].coefficient[4],
		sizeof(struct cxd3778gf_deq_coefficient)
	);

	cxd3778gf_register_modify(CXD3778GF_DEQ_CONTROL,0x80,0x80);
...
)
{
	...
	if(new_status!=old_status){
		...
		&& (         new_status==JACK_STATUS_SE_3PIN || new_status==JACK_STATUS_SE_5PIN || new_status==JACK_STATUS_SE_4PIN   )){
			...
			cxd3778gf_switch_set_headphone_value(2);
		}
		...
		&& (         new_status==JACK_STATUS_SE_NONE || new_status==JACK_STATUS_SE_ANTENNA)){
			...
			cxd3778gf_switch_set_headphone_value(0);
		}
		if (present.board_type == TYPE_A){
			if((force || old_status==JACK_STATUS_SE_3PIN) && (new_status==JACK_STATUS_SE_5PIN)){
				/* 5 = 3pin<->5pin special value */
				cxd3778gf_switch_set_headphone_value(5);
				cxd3778gf_switch_set_headphone_value(2);
			}
			if((force || old_status==JACK_STATUS_SE_5PIN) && (new_status==JACK_STATUS_SE_3PIN)){
				/* 5 = 3pin<->5pin special value */
				cxd3778gf_switch_set_headphone_value(5);
				cxd3778gf_switch_set_headphone_value(2);
			}
		...
		if((force || old_status==JACK_STATUS_SE_NONE) && (new_status!=JACK_STATUS_SE_NONE)){
			cxd3778gf_switch_set_antenna_value(1);
		}
		if((force || old_status!=JACK_STATUS_SE_NONE) && (new_status==JACK_STATUS_SE_NONE)){
			cxd3778gf_switch_set_antenna_value(0);
		}
	}
...
)
{
        ...
        if(new_status!=old_status){
                ...
                && (         new_status==JACK_STATUS_BTL   )){
                        cxd3778gf_switch_set_headphone_value(4);
                }
                if((force || old_status==JACK_STATUS_BTL)
                && (         new_status==JACK_STATUS_BTL_NONE)){
                        cxd3778gf_switch_set_headphone_value(3);
                }
        }
...
//	now=cxd3778gf_get_external_osc();
	...
	if (present.board_type == TYPE_Z && !(board_set_flag&BOARD_ID_NO_PULLUP_MASK)){
		...
		if(req!=now_osc){
			...
			if(pwm_nowait)
				cxd3778gf_set_output_device_mute(status,ON,FALSE);
			...
			cxd3778gf_register_modify(CXD3778GF_SD_ENABLE,0x44,0x44);
			if(pwm_nowait)
				cxd3778gf_set_output_device_mute(status,OFF,FALSE);
...
//			}
			...
			if(present.noise_cancel_active == FALSE){
				cxd3778gf_set_playback_mute(status,ON);
				cxd3778gf_register_modify(CXD3778GF_SMS_NS_PMUTE,0x80,0x80);
				msleep(5);
				cxd3778gf_set_line_mute(status,ON);
			}
...
//				cxd3778gf_set_phv_mute(&present,FALSE);
			...
			if(present.noise_cancel_active == FALSE){
				cxd3778gf_set_line_mute(status,OFF);
				cxd3778gf_register_modify(CXD3778GF_SMS_NS_PMUTE,0x00,0x80);
				msleep(5);
				cxd3778gf_set_playback_mute(status,OFF);
			}
//			if(present.noise_cancel_active == TRUE){
//				cxd3778gf_register_modify(CXD3778GF_PHV_CTRL0,0x80,0x80);
...
)
{
	...
	/* std path is asrc and don't change clk source */
#if 0
...
static int check_active(struct cxd3778gf_status * status)
{
	int ret=FALSE;
...
//	cxd3778gf_register_modify(CXD3778GF_POWER_CONTROL_2,0x00,0xF0);
	...
	if(present.board_type==TYPE_Z)
		cxd3778gf_register_modify(CXD3778GF_BLK_ON0,0x0A,0x0F);
	else
		cxd3778gf_register_modify(CXD3778GF_BLK_ON0,0x0F,0x0F);
...
//	cxd3778gf_register_modify(CXD3778GF_MODE_CONTROL,0xC2,0xFE);
	/* MCLOUT_ON */
	if(present.board_type!=TYPE_A && (board_set_flag&BOARD_ID_NO_PULLUP_MASK))
	...
	cxd3778gf_register_modify(CXD3778GF_CLK_EN0,0x17,0x17);

	/* USM_CLK_EN enable */
	if(present.board_type==TYPE_Z){
		cxd3778gf_register_modify(CXD3778GF_CLK_EN2,0x10,0x10);
		cxd3778gf_register_modify(CXD3778GF_DNC_INTP,0x00,0x08);
	}

	/* MTB_CLK_EN use codec block */
	cxd3778gf_register_modify(CXD3778GF_CLK_EN2,0x40,0x40);
...
//	msleep(20);
	...
	cxd3778gf_register_modify(CXD3778GF_SD2_MODE,0x5C,0x5F);
...
//	cxd3778gf_register_write(CXD3778GF_DITHER_LINE,0x8A);
	...
	msleep(30);
#endif
	cxd3778gf_register_modify(CXD3778GF_SD_ENABLE,0x11,0x11);

//	cxd3778gf_register_modify(CXD3778GF_NS_DAC,0x00,0xFF);
...
//	cxd3778gf_register_modify(CXD3778GF_POWER_CONTROL_2,0xFF,0xFF);
	...
	afe_msk_write(CLK_AUDDIV_3, 0x1 << 0, AUD_MUX1_CLK_MASK); /* internal APLL 44K domain */
	afe_msk_write(CLK_AUDDIV_3, 0x2 << 3, AUD_MUX2_CLK_MASK); /* internal APLL 48K domain */
	afe_msk_write(CLK_AUDDIV_0, 0x0 << 0 , AUD_EXT1_K1_MASK); /* External clk no dividd */
	afe_msk_write(CLK_AUDDIV_0, 0x1 << 16, AUD_A1SYS_K1_MASK);/* APLL1 DIV Fix:APLL1/2 */
	afe_msk_write(CLK_AUDDIV_0, 0x1 << 24, AUD_A2SYS_K1_MASK);/* APLL2 DIV Fix:APLL2/2 */
#endif
	cxd3778gf_reset();
...
static int switch_hpout2(int value)
{
		...
		/* smaster LR reverse */
		if(present.board_type==TYPE_A && (board_set_flag&BOARD_ID_PULLUP_MASK))
			cxd3778gf_register_modify(CXD3778GF_SMS_PWM_CTRL1,0x20,0x20);
...
static int switch_lineout(int value)
{
	...
	if(value){
		...
		/* lineout 1 L/R=on */
//		cxd3778gf_register_modify(CXD3778GF_POWER_CONTROL_3,0x00,0x0C);
...
//			cxd3778gf_register_modify(CXD3778GF_BLK_ON0,0x04,0x04);
		...
		/* lineout 1 L/R=off */
		//cxd3778gf_register_modify(CXD3778GF_POWER_CONTROL_3,0x0C,0x0C);
		/* CPCLK_EN disable */
//		cxd3778gf_register_modify(CXD3778GF_CPCTL1,0x00,0x80);

		/* NC_CLK_EN, CPCTL_CLK_EN?, PHV_CLK_EN disable */
//                cxd3778gf_register_modify(CXD3778GF_CLK_EN1,0x00,0x4A);

                /* sms_clk_en */
//              cxd3778gf_register_modify(CXD3778GF_CLK_EN2,0x00,0x20);

		/* DACL&R LINEOUTL&R disable */
...
static int switch_tuner(int value)
{
	...
	if(value){
		/* MCLKOUT_OFF */
		if(present.board_type!=TYPE_A && (board_set_flag&BOARD_ID_NO_PULLUP_MASK))
			cxd3778gf_register_modify(CXD3778GF_CLK_OUTPUT,0x00,0x04);
...
//                      nc=cxd3778gf_get_mic_det_value();
			...
			if(present.headphone_type==NCHP_TYPE_BUNDLE || present.headphone_type==NCHP_TYPE_MODEL1){
...
//                      	nc=cxd3778gf_get_mic_det_value();
	...
	hp=cxd3778gf_get_hp_det_value();

	if((hp&0x02 && hp&0x04)){
		status=JACK_STATUS_SE_4PIN;
	} else if(hp&0x01)
		status=JACK_STATUS_SE_3PIN;
	else{
		status=JACK_STATUS_SE_NONE;
...
//		cxd3778gf_register_modify(CXD3778GF_POWER_CONTROL_1,0x00,0x08);
		...
		cxd3778gf_register_modify(CXD3778GF_CLK_EN2,0x12,0x12);
	}else{
//		cxd3778gf_register_modify(CXD3778GF_POWER_CONTROL_1,0x08,0x08);
		if (present.board_type == TYPE_Z)
			cxd3778gf_register_modify(CXD3778GF_CLK_EN2,0x10,0x12);
		else
			cxd3778gf_register_modify(CXD3778GF_CLK_EN2,0x00,0x12);
	}
	return(0);
}
