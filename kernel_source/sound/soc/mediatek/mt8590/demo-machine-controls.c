...
static size_t mem_cmp(const UINT8 *mem1, const UINT8 *mem2, size_t size)
{
	size_t i;
	for (i = 0; i < size; ++i) {
		if (mem1[i] != mem2[i])
...
static int morning_test_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
	uinfo->value.integer.min = 0;
...
static int morning_test_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	printk("%s()", __func__);
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
volatile unsigned long long morning_t1;

static int morning_test_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	//int passthrough = ucontrol->value.integer.value[0];
	printk("%s()", __func__);
	mt_set_gpio_mode(GPIO264, GPIO_MODE_GPIO);
	mt_set_gpio_dir(GPIO264, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO264, GPIO_OUT_ZERO);
	morning_t1 = sched_clock();
	mt_set_gpio_out(GPIO264, GPIO_OUT_ONE);
	msleep(1);
	mt_set_gpio_out(GPIO264, GPIO_OUT_ZERO);
	return 0;
}
...
static const struct snd_kcontrol_new demo_controls[] = {
    {
		...
		.name = "morning test",
		.info = morning_test_info,
		.get = morning_test_get,
		.put = morning_test_put
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
