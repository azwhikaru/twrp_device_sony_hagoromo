...
static inline void afe_set_bit(u32 addr, int bit)
{
	...
	for (id = MEM_ASRC_1; id < MEM_ASRC_NUM; ++id) {
		if (addr == REG_ASRC_GEN_CONF + id * 0x100) {
			printk("%s() raise bit 24 of REG_ASRC_GEN_CONF + %d * 0x100\n", __func__, id);
			val |= (0x1 << 24);
			msk |= (0x1 << 24);
			break;
		}
...
void afe_enable(int en)
{
	...
	if (en) {
		afe_msk_write(CLK_AUDDIV_3, 0x3f << 23, 0x3f << 23); /* disable mclks of i2s1~6 by default */
		...
		afe_write(PWR1_ASM_CON1, 0x492);	/* i2s asrc clk is from asm_ck(high) 208M */
		afe_write(PWR2_ASM_CON1, 0x492492);	/* i2s asrc clk is from asm_ck(high) 208M */
#ifdef CONFIG_SND_SOC_ICX_AUDIO_MOBILE_NEXT
		/* boost i2s0   mclk,bck,lrck's driving to 0(4mA),2(8mA),4(12mA),6(16mA) */
                afe_write(0x10005F80, 0x0000);
                /* boost i2s1/2 mclk,bck,lrck's driving to 0(4mA),2(8mA),4(12mA),6(16mA) */
                afe_write(0x10005F70, 0x0040);
#else

#ifdef CONFIG_RICHO_RICOH5B
		/* boost i2s0   mclk,bck,lrck's driving to 0(4mA),2(8mA),4(12mA),6(16mA) */
		afe_write(0x10005F80, 0x0000);
		/* boost i2s1/2 mclk,bck,lrck's driving to 0(4mA),2(8mA),4(12mA),6(16mA) */
...
int a1sys_start(int working_pll, itrcon_action itrcon, int start)
{
	...
	if (start) {
		if (a1sys_started == 0) {
			if (working_pll != NR_PLLS)
				enable_pll(working_pll, "AUDIO");
...
int audio_irq_configurate(enum audio_irq_id id, const struct audio_irq_config *config)
{
	...
	case FS_96000HZ:
		afe_fs_mode = 6;
		break;
	case FS_192000HZ: case FS_384000HZ:
		afe_fs_mode = 7;
		break;
	case FS_11025HZ:
		afe_fs_mode = 9;
	...
	case FS_88200HZ:
		afe_fs_mode = 0xe;
		break;
	case FS_176400HZ: case FS_352800HZ:
		afe_fs_mode = 0xf;
		break;
	default:
		afe_fs_mode = 5;
	...
	/* hdmi-i2s-hdmi-tx irq counter */
		...
		afe_msk_write(AFE_IRQ_CON, afe_fs_mode << 4, IRQ1_FS_MASK);
		afe_msk_write(AFE_IRQ_CNT1, config->init_val, IRQ_CNT1_MASK);
	} else if (id == IRQ_AFE_IRQ2) {
		afe_msk_write(AFE_IRQ_CON, afe_fs_mode << 8, IRQ2_FS_MASK);
...
static DEFINE_SPINLOCK(asys_irqs_using_lock);
...
enum audio_irq_id asys_irq_acquire(void)
{
	int i;
	unsigned long flags;
	spin_lock_irqsave(&asys_irqs_using_lock, flags);
	for (i = 0; i < ASYS_IRQ_NUM; ++i) {
		if (!asys_irqs_using[i]) {
			asys_irqs_using[i] = 1;
			spin_unlock_irqrestore(&asys_irqs_using_lock, flags);
...
{
	static const enum cg_clk_id clks[AFE_I2S_IN_NUM] = {
		MT_CG_AUDIO_I2SIN1,
		MT_CG_AUDIO_I2SIN2,
		MT_CG_AUDIO_I2SIN3,
		MT_CG_AUDIO_I2SIN4,
...
#ifdef CONFIG_SND_SOC_ICX_AUDIO_MOBILE_NEXT
	/* don't invert bck when i2s-out slave
...
{
	static const enum cg_clk_id clks[AFE_I2S_OUT_NUM] = {
		MT_CG_AUDIO_I2SO1,
		MT_CG_AUDIO_I2SO2,
		MT_CG_AUDIO_I2SO3,
		MT_CG_AUDIO_I2SO4,
...
void afe_multilinein_enable(int en)
{
	en = !!en;
	if (en) {
		enable_clock(MT_CG_AUDIO_HDMIRX, "AUDIO");
...
{
	static const enum cg_clk_id clks[SAMPLE_ASRC_IN_NUM] = {
		MT_CG_AUDIO_ASRCI1,
		MT_CG_AUDIO_ASRCI2,
		MT_CG_AUDIO_ASRCI3,
		MT_CG_AUDIO_ASRCI4,
...
{
	static const enum cg_clk_id clks[SAMPLE_ASRC_OUT_NUM] = {
		MT_CG_AUDIO_ASRCO1,
		MT_CG_AUDIO_ASRCO2,
		MT_CG_AUDIO_ASRCO3,
		MT_CG_AUDIO_ASRCO4,
...
const UINT32 IIR_COEF_384_TO_352[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x0303b6d0,
    0x05cdf456,
    0x0303b6d0,
    0xc416d6b5,
...
const UINT32 IIR_COEF_256_TO_192[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02c5a70f,
    0x03eb90a0,
    0x02c5a70f,
    0xdc8ebb5e,
...
const UINT32 IIR_COEF_352_TO_256[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02b9c432,
    0x038d00b6,
    0x02b9c432,
    0xe15c8a85,
...
const UINT32 IIR_COEF_384_TO_256[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x0539819f,
    0x054951e5,
    0x0539819f,
    0xda57122a,
...
const UINT32 IIR_COEF_352_TO_192[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x04bd6a83,
    0x015bba25,
    0x04bd6a83,
    0x0d7cdab3,
...
const UINT32 IIR_COEF_384_TO_192[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x04ec93a4,
    0x000a1bf6,
    0x04ec93a4,
    0x1a59368b,
...
const UINT32 IIR_COEF_384_TO_176[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x04c0f379,
    0xfedb44d2,
    0x04c0f379,
    0x298f4134,
...
const UINT32 IIR_COEF_256_TO_96[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02545a7f,
    0xfe3ea711,
    0x02545a7f,
    0x2238ae5c,
...
const UINT32 IIR_COEF_352_TO_128[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02449fa8,
    0xfe2712a2,
    0x02449fa8,
    0x24311459,
...
const UINT32 IIR_COEF_384_TO_128[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02894a52,
    0xfd770f7a,
    0x02894a52,
    0x27008598,
...
const UINT32 IIR_COEF_352_TO_96[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x0217fde7,
    0xfd41917c,
    0x0217fde7,
    0x30c91f13,
...
const UINT32 IIR_COEF_384_TO_96[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x023fb27b,
    0xfcd2f985,
    0x023fb27b,
    0x326cca6b,
...
const UINT32 IIR_COEF_384_TO_88[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02a62553,
    0xfc064b8f,
    0x02a62553,
    0x334dac81,
...
const UINT32 IIR_COEF_256_TO_48[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02b72fb4,
    0xfb7c5152,
    0x02b72fb4,
    0x374ab8ef,
...
const UINT32 IIR_COEF_352_TO_64[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02a6b37b,
    0xfb888156,
    0x02a6b37b,
    0x37f859ff,
...
const UINT32 IIR_COEF_384_TO_64[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x029d40ba,
    0xfb79d6e0,
    0x029d40ba,
    0x392d8096,
...
const UINT32 IIR_COEF_352_TO_48[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02f3c075,
    0xfaa0f2bb,
    0x02f3c075,
    0x3b1e2ce5,
...
const UINT32 IIR_COEF_384_TO_48[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x02d975a0,
    0xfabc9d25,
    0x02d975a0,
    0x3bece279,
...
const UINT32 IIR_COEF_384_TO_44[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x03039659,
    0xfa5c23d4,
    0x03039659,
    0x3c785635,
...
const UINT32 IIR_COEF_352_TO_32[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x0314039e,
    0xfa16c22d,
    0x0314039e,
    0x3dc6a869,
...
const UINT32 IIR_COEF_384_TO_32[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x032f10f1,
    0xf9d95961,
    0x032f10f1,
    0x3e1158bc,
...
const UINT32 IIR_COEF_352_TO_24[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x035e047f,
    0xf96b61f8,
    0x035e047f,
    0x3ea9e77e,
...
const UINT32 IIR_COEF_384_TO_24[TBL_SZ_MEMASRC_IIR_COEF] = {
    0x038fcda4,
    0xf9036a1f,
    0x038fcda4,
    0x3eda69b0,
...
int afe_power_on_mem_asrc(enum afe_mem_asrc_id id, int on)
{
	...
	if (on) {
		/* afe_msk_write(0x100000d0, 0x3 << 0, 0x3 << 0);  test */
#if 0
		/* asrc_ck select asm_h_ck(208M) */
		afe_msk_write(CLK_CFG_0, 0x5 << 0, 0x7 << 0);
#else
...
{
	/*
	 * This spinlock can prevent race condition
	 * when enabling(disabling) of IBUF_EMPTY_INT and enabling(disabling) of OBUF_OV_INT
...
int afe_mem_asrc_configurate(enum afe_mem_asrc_id id, const struct afe_mem_asrc_config *config)
{
	...
	{
		u32 o_fs = fs_integer(output_fs);
		u32 i_fs = fs_integer(input_fs);
		u32 val = o_fs / i_fs + ((o_fs % i_fs) ? 1 : 0);
		pr_debug("%s() i_fs=%u, o_fs=%u, val=%u\n", __func__, i_fs, o_fs, val);
		afe_write_bits(REG_ASRC_MAX_OUT_PER_IN0 + addr_offset, /*val*/0, POS_CH01_MAX_OUT_PER_IN0, 4);
	}
...
int lp_configurate(volatile struct lp_info *lp, u32 base, u32 size,
		   unsigned int rate, unsigned int channels, unsigned int bitwidth, int i2s_id, enum audio_irq_id irq_id,int slave_clock)
{
...
int lp_cmd_excute(volatile struct lp_info *lp, unsigned int cmd)
{
	...
	int loop = 100;
	while((lp->cmd != LP_CMD_NONE || !is_cm4_soft0_irq_cleared()) && (--loop > 0))
		mdelay(1);
	WARN(!loop, "CM4 not ready?");
#else
...
#ifdef CONFIG_ARCH_MT8590_ICX
	loop = 100;
	while((lp->cmd != LP_CMD_NONE) && (--loop > 0))
		mdelay(1);
	WARN(!loop, "CM4 not respond?");
#else
...
static bool spdifrx_inited = FALSE;
...
static void spdifrx_select_port(enum afe_spdifrx_port port)
{
	if (port == SPDIFRX_PORT_OPT){
		afe_msk_write(AFE_SPDIFIN_INT_EXT,MULTI_INPUT_SEL_ARC,MULTI_INPUT_SEL_MASK);
	afe_write(AFE_SPDIFIN_CFG1,
			afe_read(AFE_SPDIFIN_CFG1) & (AFE_SPDIFIN_REAL_OPTICAL) & (AFE_SPDIFIN_SWITCH_REAL_OPTICAL));
	}
	else if (port == SPDIFRX_PORT_ARC){
		afe_msk_write(AFE_SPDIFIN_INT_EXT,MULTI_INPUT_SEL_OPT,MULTI_INPUT_SEL_MASK);
		afe_msk_write(AFE_SPDIFIN_BR,0,AFE_SPDIFIN_ARC_FROM_OPTICAL);
	}
...
{
	u32 regval1, regval2, regval3, regval4, fsval, fsvalold,chsintflag;
	int i, j;
	bool err;
	...
	if (err) {
		...
		if (fsval != SPDIFIN_OUT_RANGE) {
			/*if (spdifrx_state.rate == 0) {*/
				/*pr_err("%s Spdif Rx Lock!\n", __func__);*/
#if 0 /* BENTENSOFT-853 HGRM15BG-945 */
...
#endif
				afe_write(AFE_SPDIFIN_INT_EXT2,(afe_read(AFE_SPDIFIN_INT_EXT2)&(~SPDIFIN_LRCK_CHG_INT_EN)&(~SPDIFIN_LRC_MASK))|SPDIFIN_LRCK_CHG_INT_EN|_u4LRCKCmp594M[fsval-SPDIFIN_32K]);
				fsvalold = spdifrx_state.rate;
				spdifrx_state.rate = fsval;
				pr_debug("%s spdifrx_state.rate =0x%x.\n", __func__, spdifrx_state.rate);
				if ((spdifrx_callback)&&(fsvalold != fsval))
					spdifrx_callback();
			/*}*/
		}
...
static void spdifrx_irq_enable(int en)
{
	if (en) {
		...
		afe_msk_write(AFE_SPDIFIN_CFG0, SPDIFIN_DIS | SPDIFRX_INT_DIS | AFE_SPDIFIN_SEL_SPDIFIN_DIS | SPDIFIN_FLIP_DIS,
...
static void spdifrx_init(enum afe_spdifrx_port port)
{
	...
	/*afe_msk_write(AFE_SPDIFIN_INT_EXT2, 0x10E | SPDIFIN_594MODE_EN, SPDIFIN_LRC_MASK | SPDIFIN_594MODE_EN); *//*10E: 432M mode 186:624M Mode*/
	afe_write(AFE_SPDIFIN_INT_EXT2,(afe_read(AFE_SPDIFIN_INT_EXT2)&(~SPDIFIN_LRCK_CHG_INT_EN)&(~SPDIFIN_LRC_MASK))|SPDIFIN_LRCK_CHG_INT_EN | SPDIFIN_LRC_COMPARE_594M);
	...
	/*remove the edge int , fix alps0261822*/
...
/*board:spdif_in0(201):arc
             spdif_in1(202):opt*/
void afe_spdifrx_start(enum afe_spdifrx_port port, void (*callback)(void))
{
	switch (port) {
	case SPDIFRX_PORT_OPT:
		mt_set_gpio_mode(GPIO202, GPIO_MODE_01); /* spdif in1*/
		mt_set_gpio_mode(GPIO201, GPIO_MODE_00); /* spdif in0*/
		break;
	case SPDIFRX_PORT_ARC:
		mt_set_gpio_mode(GPIO201, GPIO_MODE_01); /* spdif in0*/
		mt_set_gpio_mode(GPIO202, GPIO_MODE_00); /* spdif in1*/
		break;
	default:
		pr_err("%s() invalid port: %d\n", __func__, port);
		return;
	}
	/*enable_clock(MT_CG_AUDIO_INTDIR, "AUDIO");*/
