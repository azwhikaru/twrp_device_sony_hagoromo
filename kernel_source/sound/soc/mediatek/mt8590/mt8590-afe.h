...
#ifndef _MT8590_AFE_H_
#define _MT8590_AFE_H_
...
/*
 * a1sys_start:
 * start/stop for working_pll of a1sys, MT_CG_AUDIO_A1SYS, MT_CG_AUDIO_AFE, itrcon
 */
...
static inline int rate_to_pll(enum afe_sampling_rate fs)
{
	return (fs >= FS_7350HZ && fs <= FS_352800HZ) ? AUD2PLL : AUD1PLL;
}
...
void afe_merge_set_sync_dly(uint);
void afe_merge_set_clk_edge_dly(uint);
void afe_merge_set_clk_dly(uint);
...
struct lp_mode {
	...
	unsigned int count; /* count of enter ULP */
#endif
};
