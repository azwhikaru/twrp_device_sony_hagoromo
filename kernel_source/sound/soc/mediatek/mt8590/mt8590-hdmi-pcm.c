...
static void mtk8590_hdmi_set_interconnection(unsigned int connection_state, unsigned int channels);
...
static void mtk8590_hdmi_set_interconnection(HDMI_INTERCON_STATUS connection_state,
...
static int mtk8590_hdmi_pcm_open(struct snd_pcm_substream *substream)
{
	int ret = 0;
	...
	enable_clock(MT_CG_AUDIO_HDMI_CK, "AUDIO");
...
static int mtk8590_hdmi_pcm_close(struct snd_pcm_substream *substream)
{
	pr_debug("%s() substream->pcm->device = %d\n", __func__, substream->pcm->device);	
...
static int mtk8590_hdmi_pcm_params(struct snd_pcm_substream *substream,
				   struct snd_pcm_hw_params *hw_params)
{
	...
	pr_debug("%s() runtime_periods = %u, period_size = %lu, buffer_size = %lu\n", __func__,
		 runtime->periods, runtime->period_size, runtime->buffer_size);
	return ret;
}

static int mtk8590_hdmi_pcm_free(struct snd_pcm_substream *substream)
{
	pr_debug("%s()\n", __func__);
	return snd_pcm_lib_free_pages(substream);
}

static int mtk8590_hdmi_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	pr_debug("%s() rate = %u  channels = %u period_size = %lu,periods = %u,buffer_size = %lu\n",
...
static int mtk8590_hdmi_pcm_start(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	pr_debug("%s() dma_bytes = %zu, dma_area = %p, dma_addr = 0x%llx\n", __func__,
		 runtime->dma_bytes, runtime->dma_area, (unsigned long long)runtime->dma_addr);
	vAudioClockSetting(sample_rate_idx, 128, APLL_HDMI_CK, PCM_OUTPUT_32BIT, 0);
	...
	mtk8590_hdmi_set_interconnection(HDMI_Connection, runtime->channels);
...
static int mtk8590_hdmi_pcm_stop(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct mt_stream *s = runtime->private_data;
	enum afe_sampling_rate sample_rate_idx = fs_enum(runtime->rate);
	pr_debug("%s()\n", __func__);
	mtk8590_hdmi_set_interconnection(HDMI_DisConnect, runtime->channels);
...
static int mtk8590_hdmi_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	pr_debug("%s() cmd = %d\n", __func__, cmd);
	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		return mtk8590_hdmi_pcm_start(substream);
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		return mtk8590_hdmi_pcm_stop(substream);
	default:
		pr_err("%s() command %d not handled error\n", __func__, cmd);
		break;
	}
	return -EINVAL;
}

static snd_pcm_uframes_t mtk8590_hdmi_pcm_pointer(struct snd_pcm_substream *substream)
{
	int offset = 0;
...
static struct snd_pcm_ops hdmi_pcm_ops = {
	.open = mtk8590_hdmi_pcm_open,
	.close = mtk8590_hdmi_pcm_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = mtk8590_hdmi_pcm_params,
	.hw_free = mtk8590_hdmi_pcm_free,
	.prepare = mtk8590_hdmi_pcm_prepare,
	.trigger = mtk8590_hdmi_pcm_trigger,
	.pointer = mtk8590_hdmi_pcm_pointer,
};
