...
static void mrg_power(int on)
{
	if (on) {
		...
		enable_clock(MT_CG_AUDIO_AFE_MRGIF, "AUDIO");
	} else {
...
static int mt8590_btmrg_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (priv->daibt.occupied[substream->stream]) {
		pr_err(
		       "%s() error: can't open MT8590_DAI_MRGIF_BT_ID(dir %d) because it has been occupied\n",
		       __func__, substream->stream);
		return -EINVAL;
	}
	if (priv->btpcm.occupied[substream->stream]) {
		pr_err("%s() error: can't open MT8590_DAI_MRGIF_BT_ID(dir %d) because MT8590_DAI_BTPCM_ID(dir %d) has been occupied\n",
...
static int mt8590_btmrg_hw_params(struct snd_pcm_substream *substream,
...
static void mt8590_btmrg_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	priv = dev_get_drvdata(dai->dev);
	pr_debug("%s() cpu_dai id %d\n", __func__, dai->id);
...
static struct snd_soc_dai_ops mt8590_btmrg_ops = {
	.startup = mt8590_btmrg_startup,
	.shutdown = mt8590_btmrg_shutdown,
	.hw_params = mt8590_btmrg_hw_params,
};

static int mt8590_i2smrg_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	...
	if (i2s_all->i2s_out[AFE_I2S_OUT_6].occupied) {
		pr_err(
		       "%s() error: can't open MT8590_DAI_MRGIF_I2S_ID because MT8590_DAI_I2S6_ID has been occupied\n",
		       __func__);
		return -EINVAL;
	}
	if (priv->i2smrg.occupied) {
		pr_err(
		       "%s() error: can't open MT8590_DAI_MRGIF_I2S_ID because it has been occupied\n",
		       __func__);
		return -EINVAL;
	}
	mrg_power(1);
...
static int mt8590_i2smrg_hw_params(struct snd_pcm_substream *substream,
				   struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	int stream_fs;
...
static void mt8590_i2smrg_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	pr_debug("%s() cpu_dai id %d\n", __func__, dai->id);
	priv = dev_get_drvdata(dai->dev);
...
static struct snd_soc_dai_ops mt8590_i2smrg_ops = {
	.startup = mt8590_i2smrg_startup,
	.hw_params = mt8590_i2smrg_hw_params,
	.shutdown = mt8590_i2smrg_shutdown,
};
